...
public class MultiplexersLuceneTest {
    ...
    public void suggestDir() throws Exception {
        assertEquals(LuceneIndexConstants.SUGGEST_DATA_CHILD_NAME,
            MultiplexersLucene.getSuggestDirName(defaultMount));
        assertTrue(MultiplexersLucene.isSuggestIndexDirName(
            MultiplexersLucene.getSuggestDirName(defaultMount)));
        assertFalse(MultiplexersLucene.isSuggestIndexDirName(
            MultiplexersLucene.getIndexDirName(defaultMount)));
    }
    ...
    public void suggestDirWithMount() throws Exception {
        ...
        assertTrue(MultiplexersLucene.isSuggestIndexDirName(
            MultiplexersLucene.getSuggestDirName(fooMount)));
    }

    @Test
    public void indexDir() throws Exception {
        assertEquals(FulltextIndexConstants.INDEX_DATA_CHILD_NAME,
            MultiplexersLucene.getIndexDirName(defaultMount));
        assertTrue(
            MultiplexersLucene.isIndexDirName(MultiplexersLucene.getIndexDirName(defaultMount)));
        assertFalse(
            MultiplexersLucene.isIndexDirName(MultiplexersLucene.getSuggestDirName(defaultMount)));
    }
