...
public class IndexFieldProviderImpl implements IndexFieldProvider {
    ...
    private static final String PREDICTED_TAGS_REL_PATH =
        JcrConstants.JCR_CONTENT + "/" + METADATA_FOLDER + "/" +
            PREDICTED_TAGS + "/";
    ...
    @Override
    public @NotNull Iterable<Field> getAugmentedFields(String path, NodeState nodeState,
        NodeState indexDefinition) {
        Set<Field> fields = newHashSet();
        NodeState dynaTags = nodeState.getChildNode(JcrConstants.JCR_CONTENT)
                                      .getChildNode(METADATA_FOLDER).getChildNode(PREDICTED_TAGS);
        for (String nodeName : dynaTags.getChildNodeNames()) {
            ...
            Double dynaTagConfidence = dynaTag.getProperty(PREDICTED_TAG_CONFIDENCE)
                                              .getValue(Type.DOUBLE);
            ...
            for (String token : tokens) {
                if (token.length() > 0) {
                    fields.add(new AugmentedField(PREDICTED_TAGS_REL_PATH + token.toLowerCase(),
                        dynaTagConfidence));
                }
            }
            LOG.trace(
