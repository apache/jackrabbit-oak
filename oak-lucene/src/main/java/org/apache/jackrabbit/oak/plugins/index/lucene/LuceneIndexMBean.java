...
public interface LuceneIndexMBean extends IndexMBean {
    ...
    @Description(
        "Determines the set of index paths up to given maxLevel. This can be used to determine the value for"
            +
    ...
    @Description(
        "Returns the diff of index definition for index at given path from the stored index definition in "
            +
            "string form")
    ...
    @Description(
        "Performs consistency check for all Lucene indexes and reports true if all indexes are found "
            +
