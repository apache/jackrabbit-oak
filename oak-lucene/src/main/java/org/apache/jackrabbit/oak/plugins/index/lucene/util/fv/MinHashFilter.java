 ...
 * be 5 word shingles.
 * <p>
 * The number of hashes used and the number of minimum values for each hash can be set. You could
 ...
 * <p>
 * A 128-bit hash is used internally. 5 word shingles from 10e5 words generate 10e25 combinations So
 * a 64 bit hash would have collisions (1.8e19)
 * <p>
 * When using different hashes 32 bits are used for the hash position leaving scope for 8e28 unique
 * hashes. A single hash will use all 128 bits.
 */
class MinHashFilter extends TokenFilter {
    ...
    private final PositionLengthAttribute posLenAttribute = addAttribute(
     ...
     */
    public MinHashFilter(TokenStream input, int hashCount, int bucketCount, int hashSetSize,
        ...
        if (requiresInitialisation) {
            ...
            while (input.incrementToken()) {
                ...
                for (int i = 0; i < hashCount; i++) {
                    ...
                    minHashSets.get(i).get((int) ((rehashed.val2 >>> 32) / bucketSize))
                               .add(rehashed);
                }
                    ...
                    for (int bucketLoop = 0; bucketLoop < bucketCount; bucketLoop++) {
                        if (minHashSets.get(hashLoop).get(bucketLoop).size() == 0) {
                            for (int bucketOffset = 1; bucketOffset < bucketCount; bucketOffset++) {
                                if (minHashSets.get(hashLoop)
                                               .get((bucketLoop + bucketOffset) % bucketCount)
                                               .size() > 0) {
                                    LongPair replacementHash = minHashSets.get(hashLoop).get(
            ...
            if (hashPosition == -1) {
                ...
                while (bucketPosition < bucketCount) {
                    if (bucketPosition == -1) {
                        ...
                        LongPair hash = minHashSets.get(hashPosition).get(bucketPosition)
                                                   .pollFirst();
                        if (hash != null) {
