...
public class NRTIndexFactory implements Closeable {
    ...
    private static final int REFRESH_DELTA_IN_SECS = Integer.getInteger(
        "oak.lucene.refreshDeltaSecs", 1);
    ...
    private boolean assertAllResourcesClosed = Boolean.getBoolean(
        "oak.lucene.assertAllResourcesClosed");
    ...
    /**
     * Test mode upon which enables assertions to confirm that all readers are closed by the time
     * NRTIndex is closed
     */
    ...
    private NRTIndex getPrevious(String indexPath) {
        ...
        checkArgument(existing.size() <= MAX_INDEX_COUNT, "Found [%s] more than 3 index",
            existing.size());
    ...
    private enum DefaultNRTDirFactory implements NRTDirectoryFactory {
        ...
        @Override
        public Directory createNRTDir(IndexDefinition definition, File indexDir)
            throws IOException {
