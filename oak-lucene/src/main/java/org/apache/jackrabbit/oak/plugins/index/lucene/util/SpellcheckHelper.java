 ...
 * {@link DirectSpellChecker} under the hood.
 */
public class SpellcheckHelper {
    ...
    public static SpellcheckQuery getSpellcheckQuery(String spellcheckQueryString,
        IndexReader reader) {
        String text = null;
        for (String param : spellcheckQueryString.split("&")) {
            ...
            if (keyValuePair.length != 2 || keyValuePair[0] == null || keyValuePair[1] == null) {
                throw new RuntimeException(
                    "Unparsable native Lucene Spellcheck query: " + spellcheckQueryString);
            } else {
        ...
        return new SpellcheckHelper.SpellcheckQuery(new Term(FieldNames.SPELLCHECK, text), 10,
            reader);
    }
