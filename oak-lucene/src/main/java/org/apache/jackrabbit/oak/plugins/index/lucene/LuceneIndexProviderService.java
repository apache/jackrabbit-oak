...
public class LuceneIndexProviderService {
    ...
    @Property(
        ...
        description =
            "Local file system path where Lucene indexes would be copied when CopyOnRead is enabled. "
                +
    ...
    @Property(
        ...
        label = "Queue timeout",
        description =
            "Maximum time to wait for adding entries to the queue used for storing Lucene Documents which need to be "
                +
                "added to local index"
    )
    ...
    @Property(
        ...
        description =
            "By default index definitions would be stored at time of reindexing to ensure that future "
                +
    ...
    @Property(
        ...
        description =
            "Index blobs are explicitly unique and don't require mark-sweep type collection." +
    ...
    @Property(
        ...
        description =
            "Cleaner interval time (in seconds) for synchronous property indexes configured as " +
    ...
    @Property(
        ...
        description =
            "Index files can be written as single blobs as chunked into smaller blobs. Enable" +
    ...
    private void activate(BundleContext bundleContext, Map<String, ?> config) throws IOException {
        ...
        hybridIndex = PropertiesUtil.toBoolean(config.get(PROP_HYBRID_INDEXING),
            PROP_DISABLED_DEFAULT);
        ...
        if (System.getProperty(BufferedOakDirectory.ENABLE_WRITING_SINGLE_BLOB_INDEX_FILE_PARAM)
            == null) {
            ...
            log.info(
                "Not setting config for single blob for an index file as it's set by command line!");
        }

        whiteboard = new OsgiWhiteboard(bundleContext);
        threadPoolSize = PropertiesUtil.toInteger(config.get(PROP_THREAD_POOL_SIZE),
        ...
        regs.add(
            bundleContext.registerService(QueryIndexProvider.class.getName(), indexProvider, null));
        ...
        LuceneIndexMBeanImpl mBean = new LuceneIndexMBeanImpl(tracker, nodeStore, indexPathService,
            getIndexCheckDir(), cleaner);
        ...
        LuceneIndexFileSystemStatistics luceneIndexFSStats = new LuceneIndexFileSystemStatistics(
            statisticsProvider, indexCopier);
        registerLuceneFileSystemStats(luceneIndexFSStats,
            PropertiesUtil.toLong(config.get(PROP_INDEX_FILESYSTEM_STATS_INTERVAL),
                PROP_INDEX_FILESYSTEM_STATS_INTERVAL_DEFAULT));
    }
    ...
    void initializeIndexDir(BundleContext bundleContext, Map<String, ?> config) {
        ...
        checkNotNull(indexDirPath, "Index directory cannot be determined as neither index " +
                "directory path [%s] nor repository home [%s] defined", PROP_LOCAL_INDEX_DIR,
            REPOSITORY_HOME);
    ...
    private void registerIndexEditor(BundleContext bundleContext, IndexTracker tracker,
        LuceneIndexMBean mBean, Map<String, ?> config) throws IOException {
        boolean enableCopyOnWrite = PropertiesUtil.toBoolean(config.get(PROP_COPY_ON_WRITE),
        ...
        if (enableCopyOnWrite) {
            ...
            editorProvider = new LuceneIndexEditorProvider(indexCopier, tracker, extractedTextCache,
                augmentorFactory, mountInfoProvider, activeDeletedBlobCollector, mBean,
                statisticsProvider);
            log.info("Enabling CopyOnWrite support. Index files would be copied under {}",
                indexDir.getAbsolutePath());
        } else {
            editorProvider = new LuceneIndexEditorProvider(null, tracker, extractedTextCache,
                augmentorFactory,
    ...
    private IndexTracker createTracker(BundleContext bundleContext, Map<String, ?> config)
        ...
        if (enableCopyOnRead) {
            initializeIndexCopier(bundleContext, config);
            log.info("Enabling CopyOnRead support. Index files would be copied under {}",
                indexDir.getAbsolutePath());
            if (hybridIndex) {
                nrtIndexFactory = new NRTIndexFactory(indexCopier, statisticsProvider);
            }
            tracker = new IndexTracker(
                new DefaultIndexReaderFactory(mountInfoProvider, indexCopier), nrtIndexFactory);
        } else {
    ...
    private void initializeIndexCopier(BundleContext bundleContext, Map<String, ?> config)
        throws IOException {
        if (indexCopier != null) {
            return;
        }
    ...
    private ExecutorService createExecutor() {
        ThreadPoolExecutor executor = new ThreadPoolExecutor(5, 5, 60L, TimeUnit.SECONDS,
            ...
            private final Thread.UncaughtExceptionHandler handler = (t, e) -> log.warn(
                "Error occurred in asynchronous processing ", e);
    ...
    private void registerObserver(BundleContext bundleContext, Map<String, ?> config) {
        boolean enableAsyncIndexOpen = PropertiesUtil.toBoolean(config.get(PROP_ASYNC_INDEX_OPEN),
            true);
    ...
    private void registerLocalIndexObserver(BundleContext bundleContext, IndexTracker tracker,
        Map<String, ?> config) {
        if (!hybridIndex) {
        ...
        int queueSize = PropertiesUtil.toInteger(config.get(PROP_HYBRID_QUEUE_SIZE),
            PROP_HYBRID_QUEUE_SIZE_DEFAULT);
        long queueOfferTimeoutMillis = PropertiesUtil.toLong(config.get(PROP_HYBRID_QUEUE_TIMEOUT),
            PROP_HYBRID_QUEUE_TIMEOUT_DEFAULT);
        documentQueue = new DocumentQueue(queueSize, queueOfferTimeoutMillis, tracker,
            getExecutorService(), statisticsProvider);
        LocalIndexObserver localIndexObserver = new LocalIndexObserver(documentQueue,
            statisticsProvider);
        ...
        ExternalObserverBuilder builder = new ExternalObserverBuilder(documentQueue, tracker,
            statisticsProvider,
            getExecutorService(), observerQueueSize);
        log.info(
            "Configured JournalPropertyBuilder with max size {} and backed by BackgroundObserver " +
    ...
    private void initializeExtractedTextCache(BundleContext bundleContext, Map<String, ?> config,
        ...
        int cacheExpiryInSecs = PropertiesUtil.toInteger(
            config.get(PROP_EXTRACTED_TEXT_CACHE_EXPIRY),
            PROP_EXTRACTED_TEXT_CACHE_EXPIRY_DEFAULT);
        boolean alwaysUsePreExtractedCache = PropertiesUtil.toBoolean(
            config.get(PROP_PRE_EXTRACTED_TEXT_ALWAYS_USE),
    ...
    private void configureIndexDefinitionStorage(Map<String, ?> config) {
        boolean disableStorage = PropertiesUtil.toBoolean(
            config.get(PROP_DISABLE_STORED_INDEX_DEFINITION),
            PROP_DISABLE_DEFN_STORAGE_DEFAULT);
        if (disableStorage) {
            log.info(
                "Feature to ensure that index definition matches the index state is disabled. Change in "
                    +
    ...
    private void registerIndexInfoProvider(BundleContext bundleContext) {
        IndexInfoProvider infoProvider = new LuceneIndexInfoProvider(nodeStore,
            asyncIndexInfoService, getIndexCheckDir());
        regs.add(
            bundleContext.registerService(IndexInfoProvider.class.getName(), infoProvider, null));
    }
    ...
    private void registerIndexImporterProvider(BundleContext bundleContext) {
        ...
        regs.add(
            bundleContext.registerService(IndexImporterProvider.class.getName(), importer, null));
    }
    ...
    private void initializeActiveBlobCollector(Whiteboard whiteboard, Map<String, ?> config) {
        ...
        if (activeDeletionEnabled && blobStore != null) {
            ...
            activeDeletedBlobCollector = ActiveDeletedBlobCollectorFactory.newInstance(
                blobCollectorWorkingDir,
                getExecutorService());
            ActiveDeletedBlobCollectorMBean bean =
                new ActiveDeletedBlobCollectorMBeanImpl(activeDeletedBlobCollector, whiteboard,
                    nodeStore,
    ...
    private void registerLuceneFileSystemStats(LuceneIndexFileSystemStatistics luceneIndexFSStats,
        long delayInSeconds) {
        Map<String, Object> config = ImmutableMap.of(
            "scheduler.name", LuceneIndexFileSystemStatistics.class.getName()
        );
        oakRegs.add(
            scheduleWithFixedDelay(whiteboard, luceneIndexFSStats, config, delayInSeconds, false,
                true));
        log.info("Lucene FileSystem Statistics calculator configured to run every [{}] seconds",
            delayInSeconds);
    }
