 ...
 * correspond to the different file formats used by the {@link Codec}. When using the Compound File
 * format these files are collapsed into a single <tt>.cfs</tt> file (except for the
 * {@link LiveDocsFormat}, with a corresponding <tt>.cfe</tt> file indexing its sub-files.
 * <p>
 * Files:
 * <ul>
...
public final class CompoundFileDirectory extends BaseDirectory {
     ...
     */
    public CompoundFileDirectory(Directory directory, String fileName, IOContext context,
    ...
    private static Map<String, FileEntry> readLegacyEntries(IndexInput stream,
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (offset < 0 || offset > streamLength) {
                throw new CorruptIndexException(
                    "Invalid CFS entry offset: " + offset + " (resource: " + stream + ")");
            }
            ...
            if (previous != null) {
                throw new CorruptIndexException(
                    "Duplicate cfs entry id=" + id + " in CFS: " + stream);
            }
        }
    ...
    public synchronized IndexInput openInput(String name, IOContext context) throws IOException {
        ...
        if (entry == null) {
            throw new FileNotFoundException(
                "No sub-file with id " + id + " found (fileName=" + name + " files: "
                    + entries.keySet() + ")");
        }
