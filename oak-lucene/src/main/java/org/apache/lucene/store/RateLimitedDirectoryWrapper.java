...
public final class RateLimitedDirectoryWrapper extends FilterDirectory {
    ...
    /**
     * Sets the maximum (approx) MB/sec allowed by all write IO performed by {@link IndexOutput}
     * created with the given {@link IOContext.Context}. Pass
     * <code>null</code> to have no limit.
     *
     * <p>
     * <b>NOTE</b>: For already created {@link IndexOutput} instances there is no
     * guarantee this new rate will apply to them; it will only be guaranteed to apply for new
     * created {@link IndexOutput} instances.
     * <p>
     * <b>NOTE</b>: this is an optional operation and might not be respected by
     * all Directory implementations. Currently only {@link FSDirectory buffered} Directory
     * implementations use rate-limiting.
     *
     * @throws IllegalArgumentException if context is <code>null</code>
    ...
    /**
     * Sets the rate limiter to be used to limit (approx) MB/sec allowed by all IO performed with
     * the given {@link IOContext.Context context}. Pass <code>null</code> to have no limit.
     *
     * <p>
     * Passing an instance of rate limiter compared to setting it using
     * {@link #setMaxWriteMBPerSec(Double, IOContext.Context)} allows to use the same limiter
