...
public abstract class FSDirectory extends BaseDirectory {
    ...
    /**
     * Default read chunk size: 8192 bytes (this is the size up to which the JDK does not allocate
     * additional arrays while reading/writing)
     *
    ...
    protected final Set<String> staleFiles = synchronizedSet(
        new HashSet<String>()); // Files written, but not yet sync'ed
    ...
    protected FSDirectory(File path, LockFactory lockFactory) throws IOException {
        ...
        if (directory.exists() && !directory.isDirectory()) {
            throw new NoSuchDirectoryException(
                "file '" + directory + "' exists but is not a directory");
        }
     ...
     * environment. The directory returned uses the {@link NativeFSLockFactory}.
     *
     * <p>Currently this returns {@link MMapDirectory} for most Solaris
     * and Windows 64-bit JREs, {@link NIOFSDirectory} for other non-Windows JREs, and
     * {@link SimpleFSDirectory} for other JREs on Windows. It is highly recommended that you
     * consult the implementation's documentation for your platform before using this method.
     *
     * <p><b>NOTE</b>: this method may suddenly change which
     * implementation is returned from release to release, in the event that higher performance
     * defaults become possible; if the precise implementation is important to your application,
     * please instantiate it directly, instead. For optimal performance you should consider using
    ...
    /**
     * Lists all files (not subdirectories) in the directory.  This method never returns null
     * (throws {@link IOException} instead).
     *
     * @throws NoSuchDirectoryException if the directory does not exist, or does exist but is not a
     *                                  directory.
    ...
    public String toString() {
        return this.getClass().getSimpleName() + "@" + directory + " lockFactory="
            + getLockFactory();
    }
    ...
    protected abstract static class FSIndexInput extends BufferedIndexInput {
         ...
         */
        protected FSIndexInput(String resourceDesc, File path, IOContext context)
            throws IOException {
         ...
         */
        protected FSIndexInput(String resourceDesc, RandomAccessFile file, long off, long length,
            int bufferSize) {
    ...
    protected static class FSIndexOutput extends BufferedIndexOutput {
         ...
         * buffer outside of stack if the write buffer size is larger.
         */
