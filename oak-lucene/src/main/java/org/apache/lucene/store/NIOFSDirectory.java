 ...
 * how FileChannel.read is implemented in Sun's JRE. Inside of the implementation the position is
 ...
 * indirectly from a thread while it's interrupted can close the underlying file descriptor
 * immediately if at the same time the thread is blocked on IO. The file descriptor will remain
 * closed and subsequent access to {@link NIOFSDirectory} will throw a
 * {@link ClosedChannelException}. If your application uses either {@link Thread#interrupt()} or
 * {@link Future#cancel(boolean)} you should use {@link SimpleFSDirectory} in favor of
 * {@link NIOFSDirectory}.</font>
 * </p>
 */
public class NIOFSDirectory extends FSDirectory {
    ...
    public IndexInputSlicer createSlicer(final String name,
        ...
        return new Directory.IndexInputSlicer() {
            ...
            public IndexInput openSlice(String sliceDescription, long offset, long length) {
                return new NIOFSIndexInput(sliceDescription, path, descriptor,
                    descriptor.getChannel(), offset,
    ...
    protected static class NIOFSIndexInput extends FSIndexInput {
        ...
        public NIOFSIndexInput(String sliceDescription, File path, RandomAccessFile file,
            ...
            super("NIOFSIndexInput(" + sliceDescription + " in path=\"" + path + "\" slice=" + off
                + ":" + (off + length) + ")", file, off, length, bufferSize);
            channel = fc;
            isClone = true;
        }
        ...
        protected void readInternal(byte[] b, int offset, int len) throws IOException {
            ...
            try {
                ...
                while (readLength > 0) {
                    ...
                    if (i
                        < 0) { // be defensive here, even though we checked before hand, something could have changed
                        throw new EOFException(
                            "read past EOF: " + this + " off: " + offset + " len: " + len + " pos: "
                                + pos + " chunkLen: " + toRead + " end: " + end);
                    }
                    assert i
                        > 0 : "FileChannel.read with non zero-length bb.remaining() must always read at least one byte (FileChannel is in blocking mode, see spec of ReadableByteChannel)";
                    pos += i;
                    readLength -= i;
                }
                assert readLength == 0;
