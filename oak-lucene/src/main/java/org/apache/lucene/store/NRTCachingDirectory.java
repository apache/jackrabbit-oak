...
/**
 * Wraps a {@link RAMDirectory} around any provided delegate directory, to be used during NRT
 * search.
 *
 * <p>This class is likely only useful in a near-real-time
 * context, where indexing rate is lowish but reopen rate is highish, resulting in many tiny files
 * being written.  This directory keeps such segments (as well as the segments produced by merging
 * them, as long as they are small enough), in RAM.</p>
 *
 ...
 * whose expected segment size is <= 5 MB, unless the net cached bytes exceeds 60 MB at which point
 * all writes will not be cached (until the net bytes falls below 60 MB).</p>
 *
 * @lucene.experimental
 */
...
public class NRTCachingDirectory extends Directory {
     ...
     * the merged segment is <= maxMergeSizeMB, and 2) the total cached bytes is <= maxCachedMB
     */
    ...
    public String toString() {
        return "NRTCachingDirectory(" + delegate + "; maxCacheMB=" + (maxCachedBytes / 1024 / 1024.)
            + " maxMergeSizeMB=" + (maxMergeSizeBytes / 1024 / 1024.) + ")";
    }
    ...
    @Override
    public synchronized IndexInputSlicer createSlicer(final String name, final IOContext context)
        throws IOException {
        ensureOpen();
        if (VERBOSE) {
            System.out.println("nrtdir.openInput name=" + name);
    ...
    protected boolean doCacheWrite(String name, IOContext context) {
        ...
        return !name.equals(IndexFileNames.SEGMENTS_GEN) && (bytes <= maxMergeSizeBytes)
            && (bytes + cache.sizeInBytes()) <= maxCachedBytes;
    }
