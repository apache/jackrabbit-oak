...
public class SimpleFSDirectory extends FSDirectory {
    ...
    public IndexInput openInput(String name, IOContext context) throws IOException {
        ...
        return new SimpleFSIndexInput("SimpleFSIndexInput(path=\"" + path.getPath() + "\")", path,
            context);
    }
    ...
    public IndexInputSlicer createSlicer(final String name,
        ...
        return new IndexInputSlicer() {
            ...
            public IndexInput openSlice(String sliceDescription, long offset, long length) {
                return new SimpleFSIndexInput(
                    "SimpleFSIndexInput(" + sliceDescription + " in path=\"" + file.getPath()
                        + "\" slice=" + offset + ":" + (offset + length) + ")", descriptor, offset,
    ...
    protected static class SimpleFSIndexInput extends FSIndexInput {
        ...
        public SimpleFSIndexInput(String resourceDesc, File path, IOContext context)
            throws IOException {
            super(resourceDesc, path, context);
        }

        public SimpleFSIndexInput(String resourceDesc, RandomAccessFile file, long off, long length,
            int bufferSize) {
                    ...
                    while (total < len) {
                        ...
                        if (i
                            ...
                            throw new EOFException(
                                "read past EOF: " + this + " off: " + offset + " len: " + len
                                    + " total: " + total + " chunkLen: " + toRead + " end: " + end);
                        }
                        assert i
                            > 0 : "RandomAccessFile.read with non zero-length toRead must always read at least one byte";
                        total += i;
                    }
                    assert total == len;
