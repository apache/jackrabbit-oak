...
public abstract class DataOutput {
     ...
     * high-order bit of each byte indicates whether more bytes remain to be read. The low-order
     * seven bits are appended as increasingly more significant bits in the resulting integer value.
     * Thus values from zero to 127 may be stored in a single byte, values from 128 to 16,383 may be
     * stored in two bytes, and so on.</p>
    ...
    /**
     * Writes an long in a variable-length format.  Writes between one and nine bytes.  Smaller
     ...
     * Writes a string.
     * <p>
     * Writes strings as UTF-8 encoded bytes. First the length, in bytes, is written as a
     * {@link #writeVInt VInt}, followed by the bytes.
     *
     ...
     * Writes a String map.
     * <p>
     * First the size is written as an {@link #writeInt(int) Int32}, followed by each key-value pair
     * written as two consecutive {@link #writeString(String) String}s.
     *
     ...
     * Writes a String set.
     * <p>
     * First the size is written as an {@link #writeInt(int) Int32}, followed by each value written
