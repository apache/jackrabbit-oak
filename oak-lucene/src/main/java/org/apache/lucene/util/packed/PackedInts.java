...
/**
 * Simplistic compression for array of unsigned long values. Each value is >= 0 and <= a specified
 * maximum value.  The values are stored as packed ints, with each value consuming a fixed number of
 * bits.
 *
 * @lucene.internal
 */
public class PackedInts {
    ...
    public enum Format {
         ...
         * rather use {@link PackedInts#fastestFormatAndBits(int, int, float)} to find the format
        ...
        /**
         * Computes how many byte blocks are needed to store <code>values</code> values of size
         * <code>bitsPerValue</code>.
         */
        public long byteCount(int packedIntsVersion, int valueCount, int bitsPerValue) {
        ...
        /**
         * Computes how many long blocks are needed to store <code>values</code> values of size
         * <code>bitsPerValue</code>.
         */
        public int longCount(int packedIntsVersion, int valueCount, int bitsPerValue) {
            assert bitsPerValue >= 0 && bitsPerValue <= 64 : bitsPerValue;
     ...
     * fastest reader whose overhead is less than
     * <code>acceptableOverheadRatio</code>.
     * </p><p>
     * The <code>acceptableOverheadRatio</code> parameter makes sense for random-access
     * {@link Reader}s. In case you only plan to perform sequential access on this stream later on,
     * you should probably use {@link PackedInts#COMPACT}.
     * </p><p>
    ...
    public static FormatAndBits fastestFormatAndBits(int valueCount, int bitsPerValue,
        ...
        } else if (valueCount <= Packed16ThreeBlocks.MAX_SIZE && bitsPerValue <= 48
            && maxBitsPerValue >= 48) {
    ...
    public static interface Decoder {
        ...
        /**
         * The minimum number of long blocks to encode in a single iteration, when using long
         * encoding.
         */
        int longBlockCount();
        ...
        /**
         * The minimum number of byte blocks to encode in a single iteration, when using byte
         * encoding.
         */
        int byteBlockCount();
        ...
        /**
         * Read <code>iterations * blockCount()</code> blocks from <code>blocks</code>, decode them
         * and write <code>iterations * valueCount()</code> values into
         * <code>values</code>.
         *
         ...
         */
        void decode(long[] blocks, int blocksOffset, long[] values, int valuesOffset,
            int iterations);

        /**
         * Read <code>8 * iterations * blockCount()</code> blocks from <code>blocks</code>, decode
         * them and write <code>iterations * valueCount()</code> values into
         * <code>values</code>.
         *
         * @param blocks       the long blocks that hold packed integer values
         * @param blocksOffset the offset where to start reading blocks
         ...
         * @param iterations   controls how much data to decode
         */
        void decode(byte[] blocks, int blocksOffset, long[] values, int valuesOffset,
            int iterations);
         ...
         * @param iterations   controls how much data to decode
         */
        void decode(long[] blocks, int blocksOffset, int[] values, int valuesOffset,
            int iterations);
         ...
         * @param iterations   controls how much data to decode
         */
        void decode(byte[] blocks, int blocksOffset, int[] values, int valuesOffset,
            int iterations);
    ...
    public static interface Encoder {
        ...
        /**
         * Read <code>iterations * valueCount()</code> values from <code>values</code>, encode them
         * and write <code>iterations * blockCount()</code> blocks into
         * <code>blocks</code>.
         *
         * @param blocks       the long blocks that hold packed integer values
         ...
         */
        void encode(long[] values, int valuesOffset, long[] blocks, int blocksOffset,
            int iterations);

        /**
         * Read <code>iterations * valueCount()</code> values from <code>values</code>, encode them
         * and write <code>8 * iterations * blockCount()</code> blocks into
         * <code>blocks</code>.
         *
         * @param blocks       the long blocks that hold packed integer values
         * @param blocksOffset the offset where to start writing blocks
         ...
         * @param iterations   controls how much data to encode
         */
        void encode(long[] values, int valuesOffset, byte[] blocks, int blocksOffset,
            int iterations);
         ...
         * @param iterations   controls how much data to encode
         */
        void encode(int[] values, int valuesOffset, long[] blocks, int blocksOffset,
            int iterations);
         ...
         * @param iterations   controls how much data to encode
         */
        void encode(int[] values, int valuesOffset, byte[] blocks, int blocksOffset,
            int iterations);
    ...
    public static abstract class Reader extends NumericDocValues {
        ...
        /**
         * Bulk get: read at least one and at most <code>len</code> longs starting from
         * <code>index</code> into <code>arr[off:off+len]</code> and return the actual number of
        ...
        /**
         * Expert: if the bit-width of this reader matches one of java's native types, returns the
         * underlying array (ie, byte[], short[], int[], long[]); else, returns null.  Note that
         * when accessing the array you must upgrade the type (bitwise AND with all ones), to
         * interpret the full value as unsigned.  Ie, bytes[idx]&0xFF, shorts[idx]&0xFFFF, etc.
         */
    ...
    public static interface ReaderIterator {
        ...
        /**
         * Returns at least 1 and at most <code>count</code> next values, the returned ref MUST NOT
         * be modified
         */
    ...
    public static abstract class Mutable extends Reader {
        ...
        /**
         * Bulk set: set at least one and at most <code>len</code> longs starting at
         * <code>off</code> in <code>arr</code> into this mutable, starting at
        ...
        /**
         * Save this mutable into <code>out</code>. Instantiating a reader from the generated data
         * will return a reader with the same number of bits per value.
         */
        public void save(DataOutput out) throws IOException {
            Writer writer = getWriterNoHeader(out, getFormat(), size(), getBitsPerValue(),
                DEFAULT_BUFFER_SIZE);
            writer.writeHeader();
    ...
    public static abstract class Writer {
        ...
        /**
         * Returns the current ord in the stream (number of values that have been written so far
         * minus one).
         */
    ...
    /**
     * Expert: Restore a {@link Reader} from a stream without reading metadata at the beginning of
     * the stream. This method is useful to restore data from streams which have been created using
    ...
    /**
     * Expert: Restore a {@link Reader} from a stream without reading metadata at the beginning of
     * the stream. This method is useful to restore data when metadata has been previously read
     * using {@link #readHeader(DataInput)}.
     *
    ...
    public static Reader getReaderNoHeader(DataInput in, Header header) throws IOException {
        return getReaderNoHeader(in, header.format, header.version, header.valueCount,
            header.bitsPerValue);
    }
     ...
     * {@link Reader#get(int)} is likely to perform a disk seek.
     *
     * @param in           the stream to read data from
     * @param format       the format used to serialize
     * @param version      the version used to serialize the data
    ...
    /**
     * Expert: Construct a direct {@link Reader} from an {@link IndexInput} without reading metadata
     * at the beginning of the stream. This method is useful to restore data when metadata has been
     * previously read using {@link #readHeader(DataInput)}.
     *
     * @param in     the stream to read data from, positioned at the beginning of the packed values
     * @param header metadata result from <code>readHeader()</code>
     * @return a Reader
    ...
    public static Reader getDirectReaderNoHeader(IndexInput in, Header header) throws IOException {
        return getDirectReaderNoHeader(in, header.format, header.version, header.valueCount,
            header.bitsPerValue);
    }

    /**
     * Construct a direct {@link Reader} from an {@link IndexInput}. This method is useful to
    ...
    /**
     * Create a packed integer array with the given amount of values initialized to 0. the
     * valueCount and the bitsPerValue cannot be changed after creation. All Mutables known by this
     * factory are kept fully in RAM.
     * </p><p>
     * Positive values of <code>acceptableOverheadRatio</code> will trade space for speed by
     * selecting a faster but potentially less memory-efficient implementation. An
     * <code>acceptableOverheadRatio</code> of {@link PackedInts#COMPACT} will make sure that the
     * most memory-efficient implementation is selected whereas {@link PackedInts#FASTEST} will make
     * sure that the fastest implementation is selected.
     *
    ...
    public static Mutable getMutable(int valueCount,
        ...
        final FormatAndBits formatAndBits = fastestFormatAndBits(valueCount, bitsPerValue,
     ...
     * used, up to 63 bits will be wasted. An easy way to make sure that no space is lost is to
     * always use a <code>valueCount</code> that is a multiple of 64.
     * </p><p>
     ...
     * restore a {@link Reader} from it. You don't need to track
     ...
     * </p><p>
     * The <code>acceptableOverheadRatio</code> parameter controls how readers that will be restored
     * from this stream trade space for speed by selecting a faster but potentially less
     * memory-efficient implementation. An <code>acceptableOverheadRatio</code> of
     * {@link PackedInts#COMPACT} will make sure that the most memory-efficient implementation is
     * selected whereas {@link PackedInts#FASTEST} will make sure that the fastest implementation is
     * selected. In case you are only interested in reading this stream sequentially later on, you
    ...
    public static Writer getWriter(DataOutput out,
        ...
        final FormatAndBits formatAndBits = fastestFormatAndBits(valueCount, bitsPerValue,
            acceptableOverheadRatio);
        final Writer writer = getWriterNoHeader(out, formatAndBits.format, valueCount,
    ...
    public static int bitsRequired(long maxValue) {
        if (maxValue < 0) {
            throw new IllegalArgumentException(
                "maxValue must be non-negative (got: " + maxValue + ")");
        }
    ...
    /**
     * Expert: reads only the metadata from a stream. This is useful to later restore a stream or
     * open a direct reader via {@link #getReaderNoHeader(DataInput, Header)} or
     * {@link #getDirectReaderNoHeader(IndexInput, Header)}.
     *
    ...
    /**
     * Check that the block size is a power of 2, in the right bounds, and return its log in base
     * 2.
     */
    static int checkBlockSize(int blockSize, int minBlockSize, int maxBlockSize) {
        if (blockSize < minBlockSize || blockSize > maxBlockSize) {
            throw new IllegalArgumentException(
                "blockSize must be >= " + minBlockSize + " and <= " + maxBlockSize + ", got "
                    + blockSize);
        }
