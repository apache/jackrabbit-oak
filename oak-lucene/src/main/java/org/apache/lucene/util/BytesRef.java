 ...
 * represent terms that are encoded as <b>UTF8</b> bytes in the index. To convert them to a Java
 * {@link String} (which is UTF16), use {@link #utf8ToString}. Using code like
 * {@code new String(bytes, offset, length)} to do this is <b>wrong</b>, as it does not respect the
 * correct character set and may return wrong results (depending on the platform's defaults)!
 */
public final class BytesRef implements Comparable<BytesRef>, Cloneable {
    ...
    /**
     * Create a BytesRef pointing to a new array of size <code>capacity</code>. Offset and length
     * will both be zero.
     */
     ...
     *
     * @param text Must be well-formed unicode text, with no unpaired surrogates or invalid UTF16
     *             code units.
     */
    ...
    public boolean isValid() {
        ...
        if (offset + length > bytes.length) {
            throw new IllegalStateException(
                "offset+length out of bounds: offset=" + offset + ",length=" + length
