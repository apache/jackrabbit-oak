...
public abstract class AttributeImpl implements Cloneable, Attribute {
     ...
     * non-static fields from the implementing class, using the field name as key and the field
     * value as value. The Attribute class is also determined by reflection. Please note that the
     * default implementation can only handle single-Attribute implementations.
     *
     ...
     * </pre>
     *
     * <p>If you implement this method, make sure that for each invocation, the same set of
    ...
    public void reflectWith(AttributeReflector reflector) {
        ...
        final LinkedList<WeakReference<Class<? extends Attribute>>> interfaces = AttributeSource.getAttributeInterfaces(
            clazz);
