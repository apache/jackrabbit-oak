...
/**
 * Provides support for converting byte sequences to Strings and back again. The resulting Strings
 * preserve the original byte sequences' sort order.
 * <p/>
 * The Strings are constructed using a Base 8000h encoding of the original binary data - each char
 * of an encoded String represents a 15-bit chunk from the byte sequence.  Base 8000h was chosen
 * because it allows for all lower 15 bits of char to be used without restriction; the surrogate
 * range [U+D8000-U+DFFF] does not represent valid chars, and would require complicated handling to
 * avoid them and allow use of char's high bit.
 * <p/>
 * Although unset bits are used as padding in the final char, the original byte sequence could
 * contain trailing bytes with no set bits (null bytes): padding is indistinguishable from valid
 * information.  To overcome this problem, a char is appended, indicating the number of encoded
 * bytes in the final content char.
 * <p/>
 *
 * @lucene.experimental
...
public final class IndexableBinaryStringTools {
    ...
    /**
     * Encodes the input byte sequence into the output char sequence.  Before calling this method,
     * ensure that the output array has sufficient capacity by calling
     * {@link #getEncodedLength(byte[], int, int)}.
     *
     * @param inputArray   byte sequence to be encoded
     * @param inputOffset  initial offset into inputArray
     * @param inputLength  number of bytes in inputArray
    ...
    /**
     * Decodes the input char sequence into the output byte sequence. Before calling this method,
     * ensure that the output array has sufficient capacity by calling
     * {@link #getDecodedLength(char[], int, int)}.
     *
     ...
     * @param outputOffset initial offset into outputArray
     * @param outputLength length of output, must be getDecodedLength(inputArray, inputOffset,
     *                     inputLength)
     */
            ...
            for (; inputCharNum < numInputChars - 1; ++inputCharNum) {
                ...
                if (2 == codingCase.numBytes) {
                    if (0 == caseNum) {
                        ...
                        outputArray[outputByteNum] += (byte) (inputChar
