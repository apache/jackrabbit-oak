 ...
 * length <code>maxSize+1</code> if instantiated via the {@link #PriorityQueue(int, boolean)}
 * constructor with
...
public abstract class PriorityQueue<T> {
     ...
     * used by the {@link PriorityQueue#PriorityQueue(int, boolean)} constructor to fill the queue,
     * so that the code which uses that queue can always assume it's full and only change the top
     * without attempting to insert any new object.<br>
     * <p>
     * Those sentinel values should always compare worse than any non-sentinel value (i.e.,
     * {@link #lessThan} should always favor the non-sentinel values).<br>
     * <p>
     ...
     * the {@link PriorityQueue#PriorityQueue(int, boolean)} constructor {@link #size()} times,
     * relying on a new object to be returned and will not check if it's null again. Therefore you
     * should ensure any call to this method creates a new instance and behaves consistently, e.g.,
     * it cannot return null if it previously returned non-null.
     *
