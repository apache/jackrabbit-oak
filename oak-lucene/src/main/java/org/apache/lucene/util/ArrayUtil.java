...
public final class ArrayUtil {
    ...
    /**
     * Maximum length for an array; we set this to "a bit" below Integer.MAX_VALUE because the exact
     * max allowed byte[] is JVM dependent, so we want to avoid a case where a large value worked
     * during indexing on one JVM but failed later at search time with a different JVM.
     */
  ...
  */
    ...
    public static short[] grow(short[] array, int minSize) {
        assert
            minSize >= 0 : "size must be positive (got " + minSize + "): likely integer overflow?";
  ...
  */
    ...
    private static class NaturalComparator<T extends Comparable<? super T>> implements
        Comparator<T> {
     ...
     * @param toIndex   end index (exclusive)
     */
    public static <T extends Comparable<? super T>> void introSort(T[] a, int fromIndex,
        int toIndex) {
        if (toIndex - fromIndex <= 1) {
            return;
        }
     ...
     * @param toIndex   end index (exclusive)
     */
    public static <T extends Comparable<? super T>> void timSort(T[] a, int fromIndex,
        int toIndex) {
        if (toIndex - fromIndex <= 1) {
            return;
        }
    ...
    /**
     * Sorts the given array in natural order. This method uses the Tim sort algorithm, but falls
     * back to binary sort for small arrays.
     */
