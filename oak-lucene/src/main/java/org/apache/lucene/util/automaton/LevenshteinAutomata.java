 ...
 * <p>
 * Implements the algorithm described in: Schulz and Mihov: Fast String Correction with Levenshtein
 * Automata
 * <p>
 *
 * @lucene.experimental
...
public class LevenshteinAutomata {
    ...
    public LevenshteinAutomata(int[] word, int alphaMax, boolean withTranspositions) {
        ...
        descriptions = new ParametricDescription[]{
            ...
            withTranspositions ? new Lev1TParametricDescription(word.length)
                : new Lev1ParametricDescription(word.length),
            withTranspositions ? new Lev2TParametricDescription(word.length)
        ...
        for (int k = 0; k < states.length; k++) {
            ...
            if (dest >= 0) {
                for (int r = 0; r < numRanges; r++) {
                    states[k].addTransition(
                        new Transition(rangeLower[r], rangeUpper[r], states[dest]));
                }
            }
        }
    ...
    /**
     * Get the characteristic vector <code>X(x, V)</code> where V is <code>substring(pos,
     * end)</code>
     */
     ...
     * <p>
     * There are four components of a parametric description, all parameterized on the length of the
     * word <code>w</code>:
     * <ol>
    ...
    static abstract class ParametricDescription {
        ...
        /**
         * Returns the position in the input word for a given <code>state</code>. This is the
         * minimal boundary for the state.
         */
        ...
        /**
         * Returns the state number for a transition from the given <code>state</code>, assuming
         * <code>position</code> and characteristic vector <code>vector</code>
         */
