...
public class QueryBuilder {
     ...
     * <p>
     * When set, result phrase and multi-phrase queries will be aware of position increments. Useful
     * when e.g. a StopFilter increases the position increment of the token that follows an omitted
     * token.
     * <p>
     * Default: true.
     */
     ...
     * directly, just use {@link #createBooleanQuery(String, String)} and
     * {@link #createPhraseQuery(String, String)}
     *
     * @param analyzer   analyzer used for this query
     * @param operator   default boolean operator used for this query
     * @param field      field to create queries against
     * @param queryText  text to be passed to the analysis chain
     * @param quoted     true if phrases should be generated when terms occur at more than one
     *                   position
     * @param phraseSlop slop factor for phrase/multiphrase queries
     */
    protected final Query createFieldQuery(Analyzer analyzer, BooleanClause.Occur operator,
        String field, String queryText, boolean quoted, int phraseSlop) {
                ...
                try {
                    ...
                    while (hasMoreTokens) {
                        ...
                        int positionIncrement =
                            (posIncrAtt != null) ? posIncrAtt.getPositionIncrement() : 1;
                        ...
                        for (int i = 0; i < numTokens; i++) {
                            ...
                            if (posIncrAtt != null && posIncrAtt.getPositionIncrement() == 0) {
                                if (!(currentQuery instanceof BooleanQuery)) {
                                    ...
                                    ((BooleanQuery) currentQuery).add(t,
                                        BooleanClause.Occur.SHOULD);
                                }
                                ((BooleanQuery) currentQuery).add(
                                    newTermQuery(new Term(field, BytesRef.deepCopyOf(bytes))),
                                    BooleanClause.Occur.SHOULD);
                            } else {
                                if (currentQuery != null) {
                                    q.add(currentQuery, operator);
                                }
                                currentQuery = newTermQuery(
                                    new Term(field, BytesRef.deepCopyOf(bytes)));
                            }
                        }
                        q.add(currentQuery, operator);
