...
public class EliasFanoDecoder {
    ...
    /**
     * Construct a decoder for a given {@link EliasFanoEncoder}. The decoding index is set to just
     * before the first encoded value.
     */
    ...
    /**
     * The current decoding index. The first value encoded by {@link EliasFanoEncoder#encodeNext}
     * has index 0. Only valid directly after {@link #nextValue}, {@link #advanceToValue},
     * {@link #previousValue}, or {@link #backToValue} returned another value than
     * {@link #NO_MORE_VALUES}, or {@link #advanceToIndex} returned true.
     *
     * @return The decoding index of the last decoded value, or as last set by
     * {@link #advanceToIndex}.
     */
    ...
    /**
     * The value at the current decoding index. Only valid when {@link #currentIndex} would return a
     * valid result.
    ...
    private long currentLowValue() {
        ...
        return unPackValue(efEncoder.lowerLongs, efEncoder.numLowBits, efIndex,
            efEncoder.lowerBitsMask);
    }

    /**
     * @return The given highValue shifted left by the number of low bits from by the
     * EliasFanoSequence, logically OR-ed with the given lowValue.
     */
    ...
    /**
     * Increment efIndex and setBitForIndex and shift curHighLong so that it does not contain the
     * high bits before setBitForIndex.
     *
    ...
    /**
     * setBitForIndex and efIndex have just been incremented, scan to the next high set bit by
     * incrementing setBitForIndex, and by setting curHighLong accordingly.
     */
    ...
    /**
     * If another value is available after the current decoding index, return this value and and
     * increase the decoding index by 1. Otherwise return {@link #NO_MORE_VALUES}.
     */
    ...
    /**
     * Advance the decoding index to a given index. and return <code>true</code> iff it is
     * available.
    ...
    public boolean advanceToIndex(long index) {
        ...
        while ((efIndex + curSetBits)
            < index) { // curHighLong has not enough set bits to reach index
            efIndex += curSetBits;
            toNextHighLong();
    ...
    /**
     * Given a target value, advance the decoding index to the first bigger or equal value and
     * return it if it is available. Otherwise return {@link #NO_MORE_VALUES}.
    ...
    public long advanceToValue(long target) {
        ...
        curHighLong = upperLong >>> ((int) (setBitForIndex & (Long.SIZE
            - 1))); // may contain the unary 1 bit for efIndex
        ...
        if (indexEntryIndex >= 0) { // not before first index entry
            ...
            if (indexHighValue > (setBitForIndex
                - efIndex)) { // advance to just after zero bit position of index entry.
                setBitForIndex = unPackValue(efEncoder.upperZeroBitPositionIndex,
                    efEncoder.nIndexEntryBits, indexEntryIndex, indexMask);
                efIndex = setBitForIndex
                    - indexHighValue; // the high bit at setBitForIndex belongs to the unary code for efIndex
        ...
        int curClearBits = Long.SIZE - curSetBits - ((int) (setBitForIndex & (Long.SIZE
            - 1))); // subtract right shift, may be more than encoded
        ...
        int rank = (int) (highTarget - (setBitForIndex
            - efIndex)); // the rank of the zero bit for highValue.
        ...
        if (rank >= 1) {
            ...
            setBitForIndex +=
                clearBitForValue + 1; // the high bit just before setBitForIndex is zero
    ...
    /**
     * Decrement efindex and setBitForIndex and shift curHighLong so that it does not contain the
     * high bits after setBitForIndex.
     *
    ...
    /**
     * setBitForIndex and efIndex have just been decremented, scan to the previous high set bit by
     * decrementing setBitForIndex and by setting curHighLong accordingly.
     *
     ...
     * decrease the decoding index by 1. Otherwise return {@link #NO_MORE_VALUES}.
     */
    ...
    /**
     * setBitForIndex and efIndex have just been decremented, scan backward to the high set bit of
     * at most a given high value by decrementing setBitForIndex and by setting curHighLong
     * accordingly.
    ...
    /**
     * Given a target value, go back to the first smaller or equal value and return it if it is
     * available. Otherwise return {@link #NO_MORE_VALUES}.
     * <br>The current implementation does not use the index on the upper zero bit positions.
     */
