 ...
 * <p>
 * Each slice is filled with 0's initially, and we mark the end with a non-zero byte.  This way the
 * methods that are writing into the slice don't need to record its length and instead allocate a
...
public final class ByteBlockPool {
    ...
    /**
     * Resets the pool to its initial state reusing the first buffer and fills all buffers with
     * <tt>0</tt> bytes before they reused or passed to
     ...
     *                        block pool was used before ie. {@link ByteBlockPool#nextBuffer()} was
     *                        called before.
     */
    ...
    /**
     * An array holding the offset into the {@link ByteBlockPool#LEVEL_SIZE_ARRAY} to quickly
    ...
    /**
     * Reads bytes bytes out of the pool starting at the given offset with the given length into the
     * given byte array at offset <tt>off</tt>.
