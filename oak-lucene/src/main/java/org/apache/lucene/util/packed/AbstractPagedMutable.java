...
abstract class AbstractPagedMutable<T extends AbstractPagedMutable<T>> extends LongValues {
     ...
     * method is much more efficient than creating a new instance and copying values one by one.
     */
    public final T resize(long newSize) {
        ...
        for (int i = 0; i < copy.subMutables.length; ++i) {
            final int valueCount =
                i == copy.subMutables.length - 1 ? lastPageSize(newSize) : pageSize();
            final int bpv =
                i < numCommonPages ? subMutables[i].getBitsPerValue() : this.bitsPerValue;
