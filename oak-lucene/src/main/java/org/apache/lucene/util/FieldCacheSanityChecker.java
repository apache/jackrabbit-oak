...
/**
 * Provides methods for sanity checking that entries in the FieldCache are not wasteful or
 * inconsistent.
 * </p>
 * <p>
 * Lucene 2.9 Introduced numerous enhancements into how the FieldCache is used by the low levels of
 * Lucene searching (for Sorting and ValueSourceQueries) to improve both the speed for Sorting, as
 * well as reopening of IndexReaders.  But these changes have shifted the usage of FieldCache from
 * "top level" IndexReaders (frequently a MultiReader or DirectoryReader) down to the leaf level
 * SegmentReaders. As a result, existing applications that directly access the FieldCache may find
 * RAM usage increase significantly when upgrading to 2.9 or Later.  This class provides an API for
 * these applications (or their Unit tests) to check at run time if the FieldCache contains "insane"
...
public final class FieldCacheSanityChecker {
    ...
    /**
     * Quick and dirty convenience method that instantiates an instance with "good defaults" and
     * uses it to test the CacheEntrys
     *
     * @see #check
     */
    ...
    /**
     * Internal helper method used by check that iterates over valMismatchKeys and generates a
     * Collection of Insanity instances accordingly.  The MapOfSets are used to populate the
     * Insanity objects.
     *
    ...
    /**
     * Internal helper method used by check that iterates over the keys of readerFieldToValIds and
     * generates a Collection of Insanity instances whenever two (or more) ReaderField instances are
    ...
    /**
     * Checks if the seed is an IndexReader, and if so will walk the hierarchy of subReaders
     * building up a list of the objects returned by {@code seed.getCoreCacheKey()}
     */
    ...
    /**
     * Simple container for a collection of related CacheEntry objects that in conjunction with each
     * other represent some "insane" usage of the FieldCache.
     */
    public final static class Insanity {
         ...
         * followed by each CacheEntry.toString() on it's own line prefaced by a tab character
         */
        @Override
        public String toString() {
    ...
    public final static class InsanityType {
        ...
        /**
         * <p>
         * Indicates entries have the same reader+fieldname but different cached values.  This can
         * happen if different datatypes, or parsers are used -- and while it's not necessarily a
         * bug it's typically an indication of a possible problem.
         * </p>
         * <p>
         * <b>NOTE:</b> Only the reader, fieldname, and cached value are actually
         * tested -- if two cache entries have different parsers or datatypes but the cached values
         * are the same Object (== not just equal()) this method does not consider that a red flag.
         * This allows for subtle variations in the way a Parser is specified (null vs
         * DEFAULT_LONG_PARSER, etc...)
         * </p>
         */
        ...
        /**
         * Indicates an expected bit of "insanity".  This may be useful for clients that wish to
         * preserve/log information about insane usage but indicate that it was expected.
         */
