...
public final class PagedBytes {
    ...
    public final static class Reader {
        ...
        /**
         * Gets a slice out of {@link PagedBytes} starting at <i>start</i> with a given length. Iff
         * the slice spans across a block border this method will allocate sufficient resources and
         * copy the paged data.
         * <p>
    ...
    public PagedBytes(int blockBits) {
        ...
        upto = blockSize;
        bytesUsedPerBlock = blockSize + RamUsageEstimator.NUM_BYTES_ARRAY_HEADER
        ...
        if (upto + bytes.length + 2 > blockSize) {
            if (bytes.length + 2 > blockSize) {
                throw new IllegalArgumentException(
                    "block size " + blockSize + " is too small to store length " + bytes.length
                        + " bytes");
            }
            if (currentBlock != null) {
                blocks.add(currentBlock);
    ...
    /**
     * Returns a DataOutput that you may use to write into this PagedBytes instance.  If you do
     * this, you should not call the other writing methods (eg, copy); results are undefined.
     */
