 ...
 * be converted to bytes. For that integer values (32 bit or 64 bit) are made unsigned and the bits
 * are converted to ASCII chars with each 7 bit. The resulting byte[] is sortable like the original
 ...
 * {@link #floatToSortableInt}. You will have no precision loss by converting floating point numbers
 * to integers and back (only that the integer form is not usable). Other data types like dates can
 ...
 * {@link NumericRangeQuery} and {@link NumericRangeFilter} implement the query part for the same
 * data types.
 *
 * <p>This class can also be used, to generate lexicographically sortable (according to
 * {@link BytesRef#getUTF8SortedAsUTF16Comparator()}) representations of numeric data types for
 * other usages (e.g. sorting).
 *
 * @lucene.internal
...
public final class NumericUtils {
    ...
    /**
     * The default precision step used by {@link IntField}, {@link FloatField}, {@link LongField},
     * {@link DoubleField}, {@link NumericTokenStream}, {@link NumericRangeQuery}, and
    ...
    /**
     * The maximum term length (used for <code>byte[]</code> buffer size) for encoding
     ...
     * method is used by {@link NumericTokenStream}. After encoding, {@code bytes.offset} will
     * always be 0.
     *
    ...
    public static void longToPrefixCodedBytes(final long val, final int shift,
        ...
        int nChars =
            (((63 - shift) * 37) >> 8) + 1;    // i/7 is the same as (i*37)>>8 for i in 0..63
        bytes.offset = 0;
    ...
    public static void intToPrefixCodedBytes(final int val, final int shift, final BytesRef bytes) {
        ...
        int nChars =
            (((31 - shift) * 37) >> 8) + 1;    // i/7 is the same as (i*37)>>8 for i in 0..63
        bytes.offset = 0;
        bytes.length = nChars + 1;   // one extra for the byte that contains the shift info
        if (bytes.bytes.length < bytes.length) {
            bytes.bytes = new byte[NumericUtils.BUF_SIZE_LONG];  // use the max
     ...
     *
     * @throws NumberFormatException if the supplied {@link BytesRef} is not correctly prefix
     *                               encoded.
     */
     ...
     * then some bits are swapped, to be able to compare the result as long. By this the precision
     * is not reduced, but the value can easily used as a long. The sort order (including
     * {@link Double#NaN}) is defined by {@link Double#compareTo}; {@code NaN} is greater than
     * positive infinity.
     *
     ...
     * converted by getting their IEEE 754 floating-point &quot;float format&quot; bit layout and
     * then some bits are swapped, to be able to compare the result as int. By this the precision is
     * not reduced, but the value can easily used as an int. The sort order (including
     * {@link Float#NaN}) is defined by {@link Float#compareTo}; {@code NaN} is greater than
     * positive infinity.
     *
