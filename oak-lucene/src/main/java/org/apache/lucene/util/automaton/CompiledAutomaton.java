...
public class CompiledAutomaton {
    ...
    /**
     * Automata are compiled into different internal forms for the most efficient execution
     * depending upon the language they accept.
     */
    ...
    /**
     * For {@link AUTOMATON_TYPE#PREFIX}, this is the prefix term; for {@link AUTOMATON_TYPE#SINGLE}
     ...
     * numbering is consistent with {@link #runAutomaton}. Only valid for
     * {@link AUTOMATON_TYPE#NORMAL}.
     */
        ...
        if (simplify) {
            ...
            if (BasicOperations.isEmpty(automaton)) {
                ...
                if (automaton.getSingleton() == null) {
                    ...
                    if (commonPrefix.length() > 0 && BasicOperations.sameLanguage(automaton,
                        BasicAutomata.makeString(commonPrefix))) {
     ...
     * is placed in output; it's fine for output and input to point to the same BytesRef.  The
     * returned result is either the provided output, or null if there is no floor term (ie, the
     * provided input term is before the first term accepted by this Automaton).
     */
