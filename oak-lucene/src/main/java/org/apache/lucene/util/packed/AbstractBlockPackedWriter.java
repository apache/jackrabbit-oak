...
abstract class AbstractBlockPackedWriter {
    ...
    /**
     * Flush all buffered data to disk. This instance is not usable anymore after this method has
     * been called until {@link #reset(DataOutput)} has been called.
     */
    ...
    protected final void writeValues(int bitsRequired) throws IOException {
        final PackedInts.Encoder encoder = PackedInts.getEncoder(PackedInts.Format.PACKED,
            PackedInts.VERSION_CURRENT, bitsRequired);
        ...
        final int blockCount = (int) PackedInts.Format.PACKED.byteCount(PackedInts.VERSION_CURRENT,
            off, bitsRequired);
