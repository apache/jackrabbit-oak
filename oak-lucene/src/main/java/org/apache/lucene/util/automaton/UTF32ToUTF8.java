...
public final class UTF32ToUTF8 {
    ...
    private void build(State start, State end, UTF8Sequence startUTF8, UTF8Sequence endUTF8,
        int upto) {
        ...
        if (startUTF8.byteAt(upto) == endUTF8.byteAt(upto)) {
            ...
            if (upto == startUTF8.len - 1 && upto == endUTF8.len - 1) {
                ...
                start.addTransition(
                    new Transition(startUTF8.byteAt(upto), endUTF8.byteAt(upto), end));
                return;
            } else {
            ...
            if (upto == startUTF8.len - 1) {
                start.addTransition(new Transition(startUTF8.byteAt(upto), endUTF8.byteAt(upto),
                    end));        // type=startend
            } else {
    ...
    private void start(State start, State end, UTF8Sequence utf8, int upto, boolean doAll) {
        if (upto == utf8.len - 1) {
            // Done recursing
            start.addTransition(
                new Transition(utf8.byteAt(upto), utf8.byteAt(upto) | MASKS[utf8.numBits(upto) - 1],
                    end));  // type=start
        } else {
            State n = newUTF8State();
    ...
    private void end(State start, State end, UTF8Sequence utf8, int upto, boolean doAll) {
        if (upto == utf8.len - 1) {
            // Done recursing
            start.addTransition(new Transition(utf8.byteAt(upto) & (~MASKS[utf8.numBits(upto) - 1]),
                utf8.byteAt(upto), end));   // type=end
        } else {
            final int startCode;
    ...
    /**
     * Converts an incoming utf32 automaton to an equivalent utf8 one.  The incoming automaton need
     * not be deterministic.  Note that the returned automaton will not in general be deterministic,
     * so you must determinize it if that's needed.
     */
    ...
    private State newUTF8State() {
        ...
        if (utf8StateCount == utf8States.length) {
            final State[] newArray = new State[ArrayUtil.oversize(1 + utf8StateCount,
