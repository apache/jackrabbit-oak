...
final class Packed16ThreeBlocks extends PackedInts.MutableImpl {
    ...
    Packed16ThreeBlocks(int packedIntsVersion, DataInput in, int valueCount) throws IOException {
        ...
        final int remaining = (int) (
            PackedInts.Format.PACKED.byteCount(packedIntsVersion, valueCount, 48)
                - 3L * valueCount * 2);
    ...
    public long get(int index) {
        final int o = index * 3;
        return (blocks[o] & 0xFFFFL) << 32 | (blocks[o + 1] & 0xFFFFL) << 16 | (blocks[o + 2]
            & 0xFFFFL);
    }
    ...
    public int get(int index, long[] arr, int off, int len) {
        ...
        for (int i = index * 3, end = (index + gets) * 3; i < end; i += 3) {
            arr[off++] =
                (blocks[i] & 0xFFFFL) << 32 | (blocks[i + 1] & 0xFFFFL) << 16 | (blocks[i + 2]
                    & 0xFFFFL);
        }
        return gets;
    }
