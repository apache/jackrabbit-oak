...
/**
 * {@link BytesRefHash} is a special purpose hash-map like data-structure optimized for
 * {@link BytesRef} instances. BytesRefHash maintains mappings of byte arrays to ids
 ...
 *
 * <p>
 * Note: The maximum capacity {@link BytesRef} instance passed to {@link #add(BytesRef)} must not be
 * longer than {@link ByteBlockPool#BYTE_BLOCK_SIZE}-2. The internal storage is limited to 2GB total
 * byte storage.
 * </p>
 *
 * @lucene.internal
...
public final class BytesRefHash {
    ...
    public BytesRefHash(ByteBlockPool pool, int capacity,
        ...
        bytesUsed = bytesStartArray.bytesUsed() == null ? Counter.newCounter()
     ...
     * <p>
     * Note: the given bytesID must be a positive integer less than the current size
     * ({@link #size()})
     *
    ...
    /**
     * Returns the ids array in arbitrary order. Valid ids start at offset of 0 and end at a limit
     * of {@link #size()} - 1
     * <p>
     * Note: This is a destructive operation. {@link #clear()} must be called in order to reuse this
     * {@link BytesRefHash} instance.
     * </p>
     */
    int[] compact() {
        assert bytesStart != null : "bytesStart is null - not initialized";
     ...
     * @throws MaxBytesLengthExceededException if the given bytes are > 2 +
    ...
    /**
     * Adds a "arbitrary" int offset instead of a BytesRef term.  This is used in the indexer to
     * hold the hash for term vectors, because they do not redundantly store the byte[] term
     * directly and instead reference the byte[] term already stored by the postings BytesRefHash.
     * See add(int textStart) in TermsHashPerField.
     */
    ...
    /**
     * reinitializes the {@link BytesRefHash} after a previous {@link #clear()} call. If
     * {@link #clear()} has not been called previously this method has no effect.
     */
    public void reinit() {
        if (bytesStart == null) {
            bytesStart = bytesStartArray.init();
    ...
    public abstract static class BytesStartArray {
         ...
         * {@link BytesStartArray}. The {@link BytesRefHash} uses this reference to track it memory
         * usage
         *
         * @return a {@link AtomicLong} reference holding the number of bytes used by this
         * {@link BytesStartArray}.
         */
        public abstract Counter bytesUsed();
    }

    /**
     * A simple {@link BytesStartArray} that tracks memory allocation using a private
     * {@link Counter} instance.
     */
