...
/**
 * Utility class to buffer signed longs in memory, which is optimized for the case where the
 * sequence is monotonic, although it can encode any sequence of arbitrary longs. It only supports
 * appending.
 *
 * @lucene.internal
 */
public final class MonotonicAppendingLongBuffer extends AbstractAppendingLongBuffer {
    ...
    /**
     * Create an {@link MonotonicAppendingLongBuffer} with initialPageCount=16, pageSize=1024 and
    ...
    /**
     * Create an {@link AppendingDeltaPackedLongBuffer} with initialPageCount=16, pageSize=1024
     */
        ...
        if (block == valuesOff) {
            ...
            if (values[block] == null) {
                ...
                for (int r = 0; r < read; r++, off++, element++) {
                    arr[off] =
                        minValues[block] + (long) (averages[block] * (long) element) + zigZagDecode(
                            arr[off]);
                }
                return read;
            }
    ...
    void packPendingValues() {
        ...
        averages[valuesOff] =
            pendingOff == 1 ? 0 : (float) (pending[pendingOff - 1] - pending[0]) / (pendingOff - 1);

        for (int i = 0; i < pendingOff; ++i) {
            pending[i] = zigZagEncode(
                pending[i] - minValues[valuesOff] - (long) (averages[valuesOff] * (long) i));
        }
        long maxDelta = 0;
        for (int i = 0; i < pendingOff; ++i) {
            if (pending[i] < 0) {
