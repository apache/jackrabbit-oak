...
public final class PForDeltaDocIdSet extends DocIdSet {
    ...
    static final MonotonicAppendingLongBuffer SINGLE_ZERO_BUFFER = new MonotonicAppendingLongBuffer(
        0, 64, PackedInts.COMPACT);
    static final PForDeltaDocIdSet EMPTY = new PForDeltaDocIdSet(null, 0, Integer.MAX_VALUE,
    ...
    static {
        ...
        for (int i = 1; i < ITERATIONS.length; ++i) {
            DECODERS[i] = PackedInts.getDecoder(PackedInts.Format.PACKED,
                PackedInts.VERSION_CURRENT, i);
    ...
    public static class Builder {
        ...
        /**
         * Set the index interval. Every <code>indexInterval</code>-th block will be stored in the
         * index. Set to {@link Integer#MAX_VALUE} to disable indexing.
         */
        ...
        public Builder add(int doc) {
            if (doc <= previousDoc) {
                throw new IllegalArgumentException(
                    "Doc IDs must be provided in order, but previousDoc=" + previousDoc
                        + " and doc=" + doc);
            }
        ...
        int computeOptimalNumberOfBits() {
            ...
            for (int bitsPerValue = this.bitsPerValue - 1, numExceptions = freqs[this.bitsPerValue];
                bitsPerValue >= 0 && numExceptions <= MAX_EXCEPTIONS;
                numExceptions += freqs[bitsPerValue--]) {
                final int newBlockSize = pforBlockSize(bitsPerValue, numExceptions,
        ...
        void pforEncode() {
            ...
            if (bitsPerValue > 0) {
                final PackedInts.Encoder encoder = PackedInts.getEncoder(PackedInts.Format.PACKED,
                    PackedInts.VERSION_CURRENT, bitsPerValue);
            ...
            if (numExceptions > 0) {
                ...
                final PackedInts.Encoder encoder = PackedInts.getEncoder(PackedInts.Format.PACKED,
                    PackedInts.VERSION_CURRENT, bitsPerException);
                final int numIterations =
                    (numExceptions + encoder.byteValueCount() - 1) / encoder.byteValueCount();
                encoder.encode(exceptions, 0, data.bytes, data.length, numIterations);
                data.length += PackedInts.Format.PACKED.byteCount(PackedInts.VERSION_CURRENT,
                    numExceptions, bitsPerException);
        ...
        void encodeBlock() {
            ...
            if (pforBlockSize <= unaryBlockSize) {
                ...
                data.bytes = ArrayUtil.grow(data.bytes,
                    data.length + blockSize + MAX_BYTE_BLOCK_COUNT);
            ...
            assert
                data.length - originalLength == blockSize :
                (data.length - originalLength) + " <> " + blockSize;
        }
        ...
        public PForDeltaDocIdSet build() {
            ...
            if (indexSize <= 1) {
                ...
                docIDs = new MonotonicAppendingLongBuffer(initialPageCount, pageSize,
                    PackedInts.COMPACT);
                offsets = new MonotonicAppendingLongBuffer(initialPageCount, pageSize,
                    PackedInts.COMPACT);
                // Now build the index
                final Iterator it = new Iterator(dataArr, cardinality, Integer.MAX_VALUE,
                    SINGLE_ZERO_BUFFER, SINGLE_ZERO_BUFFER);
                index:
    ...
    PForDeltaDocIdSet(byte[] data, int cardinality, int indexInterval,
        MonotonicAppendingLongBuffer docIDs, MonotonicAppendingLongBuffer offsets) {
        this.data = data;
    ...
    static class Iterator extends DocIdSetIterator {
        ...
        void pforDecompress(byte token) {
            ...
            if ((token & HAS_EXCEPTIONS) != 0) {
                ...
                final int numIterations =
                    (numExceptions + DECODERS[bitsPerException].byteValueCount() - 1)
                        / DECODERS[bitsPerException].byteValueCount();
                DECODERS[bitsPerException].decode(data, offset, nextExceptions, 0, numIterations);
                offset += PackedInts.Format.PACKED.byteCount(PackedInts.VERSION_CURRENT,
                    numExceptions, bitsPerException);
                for (int i = 0; i < numExceptions; ++i) {
