...
/**
 * Encode a non decreasing sequence of non negative whole numbers in the Elias-Fano encoding that
 * was introduced in the 1970's by Peter Elias and Robert Fano.
 * <p>
 * The Elias-Fano encoding is a high bits / low bits representation of a monotonically increasing
 * sequence of <code>numValues > 0</code> natural numbers <code>x[i]</code>
 * <p>
 ...
 * <br>
 * The Elias-Fano encoding uses less than half a bit per encoded number more than the smallest
 * representation that can encode any monotone sequence with the same bounds.
 * <p>
 * The lower <code>L</code> bits of each <code>x[i]</code> are stored explicitly and contiguously in
 * the lower-bits array, with <code>L</code> chosen as (<code>log()</code> base 2):
 * <p>
 * <code>L = max(0, floor(log(upperBound/numValues)))</code>
 * <p>
 * The upper bits are stored in the upper-bits array as a sequence of unary-coded gaps (<code>x[-1]
 * = 0</code>):
 * <p>
 * <code>(x[i]/2**L) - (x[i-1]/2**L)</code>
 * <p>
 * The unary code encodes a natural number <code>n</code> by <code>n</code> 0 bits followed by a 1
 * bit:
 * <code>0...01</code>. <br>
 * In the upper bits the total the number of 1 bits is <code>numValues</code> and the total number
 * of 0 bits is:<p>
 * <code>floor(x[numValues-1]/2**L) <= upperBound/(2**max(0, floor(log(upperBound/numValues)))) <=
 * 2*numValues</code>
 * <p>
 ...
 * <p>
 * bits per encoded number. With <code>upperBound</code> in these bounds (<code>p</code> is an
 * integer):
 * <p>
...
public class EliasFanoEncoder {
    ...
    /**
     * upperZeroBitPositionIndex[i] (filled using packValue) will contain the bit position just
     * after the zero bit ((i+1) * indexInterval) in the upper bits.
     */
    ...
    /**
     * Construct an Elias-Fano encoder. After construction, call {@link #encodeNext}
     * <code>numValues</code> times to encode a non decreasing sequence of non negative numbers.
     *
     * @param numValues     The number of values that is to be encoded.
     * @param upperBound    At least the highest value that will be encoded. For space efficiency
     *                      this should not exceed the power of two that equals or is the first
     *                      higher than the actual maximum.
     ...
     *                      The index will have at most <code>2 * numValues / indexInterval</code>
     *                      entries and each index entry will use at most <code>ceil(log2(3 *
     *                      numValues))</code> bits, see {@link EliasFanoEncoder}.
    ...
    public EliasFanoEncoder(long numValues, long upperBound, long indexInterval) {
        ...
        if ((numValues > 0L) && (upperBound < 0L)) {
            throw new IllegalArgumentException(
                "upperBound should not be negative: " + upperBound + " when numValues > 0");
        }
        this.upperBound =
            numValues > 0 ? upperBound : -1L; // if there is no value, -1 is the best upper bound
        int nLowBits = 0;
        if (this.numValues > 0) { // nLowBits = max(0; floor(2log(upperBound/numValues)))
            long lowBitsFac = this.upperBound / this.numValues;
            if (lowBitsFac > 0) {
                nLowBits = 63 - Long.numberOfLeadingZeros(
                    lowBitsFac); // see Long.numberOfLeadingZeros javadocs
            }
        }
        ...
        if (numLongsForLowBits > Integer.MAX_VALUE) {
            throw new IllegalArgumentException(
                "numLongsForLowBits too large to index a long array: " + numLongsForLowBits);
        }
        ...
        if (numLongsForHighBits > Integer.MAX_VALUE) {
            throw new IllegalArgumentException(
                "numLongsForHighBits too large to index a long array: " + numLongsForHighBits);
        }
        ...
        long maxIndexEntry =
            maxHighValue + numValues - 1; // clear upper bits, set upper bits, start at zero
        ...
        if (numLongsForIndexBits > Integer.MAX_VALUE) {
            throw new IllegalArgumentException(
                "numLongsForIndexBits too large to index a long array: " + numLongsForIndexBits);
        }
    ...
    private static long numLongsForBits(
    ...
    /**
     * Call at most <code>numValues</code> times to encode a non decreasing sequence of non negative
     * numbers.
     *
    ...
    public void encodeNext(long x) {
        ...
        while (indexValue <= highValue) {
            ...
            packValue(afterZeroBitPosition, upperZeroBitPositionIndex, nIndexEntryBits,
                currentEntryIndex);
            currentEntryIndex += 1;
    ...
    private void encodeUpperBits(long highValue) {
        ...
        upperLongs[(int) (nextHighBitNum >>> LOG2_LONG_SIZE)] |= (1L << (nextHighBitNum & (Long.SIZE
            - 1)));
    }
    ...
    /**
     * Provide an indication that it is better to use an {@link EliasFanoEncoder} than a
     * {@link FixedBitSet} to encode document identifiers. This indication is not precise and may
     ...
     * this is the same as comparing estimates of the number of bits accessed by a pair of
     * FixedBitSets and by a pair of non indexed EliasFanoDocIdSets when determining the
     * intersections of the pairs.
     * <br>A bit set is preferred when <code>upperbound <= 256</code>.
     * <br>It is assumed that {@link #DEFAULT_INDEX_INTERVAL} is used.
     *
     * @param numValues  The number of document identifiers that is to be encoded. Should be non
     *                   negative.
     * @param upperBound The maximum possible value for a document identifier. Should be at least
     *                   <code>numValues</code>.
     */
    ...
    /**
     * Returns an {@link EliasFanoDecoder} to access the encoded values. Perform all calls to
     * {@link #encodeNext} before calling {@link #getDecoder}.
     */
