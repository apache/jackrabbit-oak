...
public class SloppyMath {
    ...
    /**
     * Returns the distance in kilometers between two points specified in decimal degrees
     * (latitude/longitude).
     *
    ...
    public static double asin(double a) {
        ...
        if (a <= ASIN_MAX_VALUE_FOR_TABS) {
            ...
            double result =
                asinTab[index] + delta * (asinDer1DivF1Tab[index] + delta * (asinDer2DivF2Tab[index]
            ...
            if (a < 1.0) {
                double t = (1.0 - a) * 0.5;
                double p =
                    t * (ASIN_PS0 + t * (ASIN_PS1 + t * (ASIN_PS2 + t * (ASIN_PS3 + t * (ASIN_PS4
                        + t * ASIN_PS5)))));
    ...
    /**
     * Return an approximate value of the diameter of the earth at the given latitude, in
     * kilometers.
     */
    public static double earthDiameter(double latitude) {
        final int index =
            (int) (Math.abs(latitude) * RADIUS_INDEXER + 0.5) % earthDiameterPerLatitude.length;
