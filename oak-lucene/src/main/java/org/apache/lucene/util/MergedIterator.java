 ...
 * <p>
 * If built with <code>removeDuplicates</code> set to true and an element appears in multiple
 * iterators then it is deduplicated, that is this iterator returns the sorted union of elements.
 * <p>
 * If built with <code>removeDuplicates</code> set to false then all elements in all iterators are
 * returned.
 * <p>
 * Caveats:
 * <ul>
...
public final class MergedIterator<T extends Comparable<T>> implements Iterator<T> {
    ...
    private static class TermMergeQueue<C extends Comparable<C>> extends
        PriorityQueue<SubIterator<C>> {
