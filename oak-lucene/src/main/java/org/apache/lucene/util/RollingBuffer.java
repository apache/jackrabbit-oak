...
public abstract class RollingBuffer<T extends RollingBuffer.Resettable> {
    ...
    public T get(int pos) {
        ...
        while (pos >= nextPos) {
            if (count == buffer.length) {
                @SuppressWarnings("unchecked") T[] newBuffer = (T[]) new Resettable[ArrayUtil.oversize(
    ...
    /**
     * Returns the maximum position looked up, or -1 if no position has been looked up sinc
     * reset/init.
     */
