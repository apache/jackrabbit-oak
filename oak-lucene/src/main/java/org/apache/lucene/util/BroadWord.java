...
/**
 * Methods and constants inspired by the article "Broadword Implementation of Rank/Select Queries"
 * by Sebastiano Vigna, January 30, 2012:
 * <ul>
...
public final class BroadWord {
    ...
    /**
     * Bit count of a long. Only here to compare the implementation with {@link #select(long, int)},
    ...
    public static int select(long x, int r) {
        ...
        s = (s & 0x3333333333333333L) + ((s >>> 2)
            & 0x3333333333333333L); // Step 1, nibblewise bitsums

        s = ((s + (s >>> 4)) & 0x0F0F0F0F0F0F0F0FL) * L8_L; // Step 2, bytewise bitsums

        long b = ((smallerUpTo7_8(s, (r * L8_L)) >>> 7) * L8_L)
            >>> 53; // & (~7L); // Step 3, side ways addition for byte number times 8

        long l = r - (((s << 8) >>> b)
            & 0xFFL); // Step 4, byte wise rank, subtract the rank with byte at b-8, or zero for b=0;
        assert 0L <= l : l;
        //assert l < 8 : l; //fails when bit r is not available.

        // Select bit l from byte (x >>> b):
        long spr = (((x >>> b) & 0xFFL) * L8_L)
            & L9_L; // spread the 8 bits of the byte at b over the long at L9 positions
        ...
        s = (spr_bigger8_zero >>> 7)
            * L8_L; // Step 5, sideways byte add the 8 bits towards the high byte
    ...
    /**
     * A signed bytewise smaller &lt;<sub><small>8</small></sub> operator, for operands 0L<= x, y
     * <=0x7L. This uses the following numbers of basic long operations: 1 or, 2 and, 2 xor, 1
     * minus, 1 not.
     *
     * @return A long with bits set in the {@link #H8_L} positions corresponding to each input
     * signed byte pair that compares smaller.
     */
    ...
    /**
     * An unsigned bytewise smaller &lt;<sub><small>8</small></sub> operator. This uses the
     * following numbers of basic long operations: 3 or, 2 and, 2 xor, 1 minus, 1 not.
     *
     * @return A long with bits set in the {@link #H8_L} positions corresponding to each input
     * unsigned byte pair that compares smaller.
     */
    ...
    /**
     * An unsigned bytewise not equals 0 operator. This uses the following numbers of basic long
     * operations: 2 or, 1 and, 1 minus.
     *
     * @return A long with bits set in the {@link #H8_L} positions corresponding to each unsigned
     * byte that does not equal 0.
     */
    ...
    /**
     * A bytewise smaller &lt;<sub><small>16</small></sub> operator. This uses the following numbers
     * of basic long operations: 1 or, 2 and, 2 xor, 1 minus, 1 not.
     *
     * @return A long with bits set in the {@link #H16_L} positions corresponding to each input
     * signed short pair that compares smaller.
     */
    ...
    /**
     * Lk denotes the constant whose ones are in position 0, k, 2k, . . . These contain the low bit
     * of each group of k bits. The suffix _L indicates the long implementation.
     */
    ...
    /**
     * Hk = Lk << (k-1) . These contain the high bit of each group of k bits. The suffix _L
     * indicates the long implementation.
     */
