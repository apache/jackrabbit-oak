 ...
 * subclasses: {@link IntField}, {@link LongField}, {@link FloatField}, {@link DoubleField},
...
public class Field implements IndexableField {
     ...
     * @param type   field type
     * @throws IllegalArgumentException if either the name or type is null, or if the field's type
     *                                  is stored(), or if tokenized() is false.
     ...
     * @param type        field type
     * @throws IllegalArgumentException if either the name or type is null, or if the field's type
     *                                  is stored(), or if tokenized() is false, or if indexed() is
     *                                  false.
     ...
     * @param type  field type
     * @throws IllegalArgumentException if the field name is null, or the field's type is indexed()
     ...
     * @param type  field type
     * @throws IllegalArgumentException if either the name or value is null, or if the field's type
     *                                  is neither indexed() nor stored(), or if indexed() is false
     *                                  but storeTermVectors() is true.
     * @throws NullPointerException     if the type is null
     */
     ...
     * used. Exactly one of stringValue(), readerValue(), and getBinaryValue() must be set.
     */
    @Override
    ...
    /**
     * The value of the field as a Reader, or null. If null, the String value or binary value is
     * used. Exactly one of stringValue(), readerValue(), and getBinaryValue() must be set.
     */
    @Override
    ...
    /**
     * The TokenStream for this field to be used when indexing, or null. If null, the Reader value
     ...
     * instances. Typically a single {@link Document} instance is re-used as well. This helps most
     * on small documents.
     * </p>
     *
     * <p>
     * Each Field instance should only be used once within a single {@link Document} instance. See
     * <a href="http://wiki.apache.org/lucene-java/ImproveIndexingSpeed" >ImproveIndexingSpeed</a>
     * for details.
     * </p>
     */
    public void setStringValue(String value) {
        if (!(fieldsData instanceof String)) {
            throw new IllegalArgumentException(
                "cannot change value type from " + fieldsData.getClass().getSimpleName()
                    + " to String");
        }
        fieldsData = value;
    }
    ...
    /**
     * Expert: sets the token stream to be used for indexing and causes isIndexed() and
     * isTokenized() to return true. May be combined with stored values from stringValue() or
     * getBinaryValue()
     */
    ...
    public static enum Store {
        ...
        /**
         * Store the original field value in the index. This is useful for short texts like a
         * document's title which should be displayed with the results. The value is stored in its
    ...
    public static enum Index {
         ...
         * access its contents provided it is {@link Field.Store stored}.
         */
        NO {
            @Override
        ...
        /**
         * Index the tokens produced by running the field's value through an Analyzer.  This is
        ...
        /**
         * Index the field's value without using an Analyzer, so it can be searched. As no analyzer
         * is used the value will be stored as a single term. This is useful for unique Ids like
         * product numbers.
         */
        NOT_ANALYZED {
            @Override
        ...
        /**
         * Expert: Index the field's value without an Analyzer, and also disable the indexing of
         * norms.  Note that you can also separately enable/disable norms by calling
         * {@link FieldType#setOmitNorms}.  No norms means that index-time field and document
         * boosting and field length normalization are disabled.  The benefit is less memory usage
         * as norms take up one byte of RAM per indexed field for every document in the index,
         * during searching.  Note that once you index a given field <i>with</i> norms enabled,
         * disabling norms will have no effect.  In other words, for this to have the above
         * described effect on a field, all instances of that field must be indexed with
        ...
        /**
         * Expert: Index the tokens produced by running the field's value through an Analyzer, and
         * also separately disable the storing of norms.  See {@link #NOT_ANALYZED_NO_NORMS} for
         * what norms are and why you may want to disable them.
         */
        ANALYZED_NO_NORMS {
            @Override
            public boolean isIndexed() {
    ...
    public static enum TermVector {
        ...
        /**
         * Store the term vectors of each document. A term vector is a list of the document's terms
         * and their number of occurrences in that document.
         */
        YES {
            @Override
            public boolean isStored() {
         ...
         */
        public static TermVector toTermVector(boolean stored, boolean withOffsets,
    ...
    /**
     * Translates the pre-4.0 enums for specifying how a field should be indexed into the 4.0
     * {@link FieldType} approach.
     *
     * @deprecated This is here only to ease transition from the pre-4.0 APIs.
     */
    @Deprecated
    public static final FieldType translateFieldType(Store store, Index index,
    ...
    /**
     * Create a field by specifying its name, value and how it will be saved in the index. Term
     * vectors will not be stored in the index.
     *
     * @param name  The name of the field
     * @param value The string to process
     * @param store Whether <code>value</code> should be stored in the index
     * @param index Whether the field should be indexed, and if so, if it should be tokenized before
     *              indexing
     ...
     * @param value      The string to process
     * @param store      Whether <code>value</code> should be stored in the index
     * @param index      Whether the field should be indexed, and if so, if it should be tokenized
     *                   before indexing
    ...
    /**
     * Create a tokenized and indexed field that is not stored. Term vectors will not be stored. The
     * Reader is read only when the Document is added to the index, i.e. you may not close the
     * Reader until {@link IndexWriter#addDocument} has been called.
     *
     * @param name   The name of the field
    ...
    /**
     * Create a tokenized and indexed field that is not stored, optionally with storing term
     * vectors.  The Reader is read only when the Document is added to the index, i.e. you may not
     * close the Reader until {@link IndexWriter#addDocument} has been called.
     *
     * @param name       The name of the field
     * @param reader     The reader with the content
     * @param termVector Whether term vector should be stored
     ...
     * This is useful for pre-analyzed fields. The TokenStream is read only when the Document is
     * added to the index, i.e. you may not close the TokenStream until
     * {@link IndexWriter#addDocument} has been called.
     *
     * @param name        The name of the field
    ...
    /**
     * Create a tokenized and indexed field that is not stored, optionally with storing term
     * vectors.  This is useful for pre-analyzed fields. The TokenStream is read only when the
     * Document is added to the index, i.e. you may not close the TokenStream until
     * {@link IndexWriter#addDocument} has been called.
     *
     * @param name        The name of the field
     * @param tokenStream The TokenStream with the content
