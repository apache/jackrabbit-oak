...
public abstract class TokenStream extends AttributeSource implements Closeable {
    ...
    /**
     * A TokenStream using the supplied AttributeFactory for creating new {@link Attribute}
     * instances.
     */
    ...
    /**
     * Consumers (i.e., {@link IndexWriter}) use this method to advance the stream to the next
     * token. Implementing classes must implement this method and update the appropriate
     ...
     * returned: the caller may arbitrarily change it. If the producer needs to preserve the state
     * for subsequent calls, it can use {@link #captureState} to create a copy of the current
     * attribute state.
     * <p>
     * This method is called for every token of a document, so an efficient implementation is
     * crucial for good performance. To avoid calls to {@link #addAttribute(Class)} and
     ...
     * be added during instantiation. Filters and consumers are not required to check for
     ...
     * offset of a stream. The final offset of a stream might differ from the offset of the last
     * token eg in case one or more whitespaces followed after the last token, but a
     * WhitespaceTokenizer was used.
     * <p>
     * Additionally any skipped positions (such as those removed by a stopfilter) can be applied to
     * the position increment, or any adjustment of other attributes where the end-of-stream value
     * may be important.
     * <p>
     ...
     * <p>
     * If you override this method, always call {@code super.reset()}, otherwise some internal state
     * will not be correctly reset (e.g., {@link Tokenizer} will throw {@link IllegalStateException}
     * on further usage).
     */
     ...
     * <p>
     * If you override this method, always call {@code super.close()}, otherwise some internal state
     * will not be correctly reset (e.g., {@link Tokenizer} will throw {@link IllegalStateException}
     * on reuse).
     */
    @Override
