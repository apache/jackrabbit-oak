 ...
 * <p/>
 * {@link #getWrappedAnalyzer(String)} allows the Analyzer to wrap multiple Analyzers which are
 * selected on a per field basis.
 * <p/>
 * {@link #wrapComponents(String, Analyzer.TokenStreamComponents)} allows the TokenStreamComponents
 * of the wrapped Analyzer to then be wrapped (such as adding a new {@link TokenFilter} to form new
 * TokenStreamComponents.
 */
public abstract class AnalyzerWrapper extends Analyzer {

    /**
     * Creates a new AnalyzerWrapper.  Since the {@link Analyzer.ReuseStrategy} of the wrapped
     * Analyzers are unknown, {@link #PER_FIELD_REUSE_STRATEGY} is assumed.
     *
     * @deprecated Use {@link #AnalyzerWrapper(Analyzer.ReuseStrategy)} and specify a valid
     * {@link Analyzer.ReuseStrategy}, probably retrieved from the wrapped analyzer using
     * {@link #getReuseStrategy()}.
     */
    @Deprecated
     ...
     * {@link #initReader(String, Reader)}. By default, the given reader is returned.
     *
     * @param fieldName name of the field which is to be analyzed
    ...
    protected final TokenStreamComponents createComponents(String fieldName, Reader aReader) {
        return wrapComponents(fieldName,
            getWrappedAnalyzer(fieldName).createComponents(fieldName, aReader));
    }
