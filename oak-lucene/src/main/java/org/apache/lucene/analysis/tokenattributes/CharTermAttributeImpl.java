...
public class CharTermAttributeImpl extends AttributeImpl implements CharTermAttribute,
    ...
    public final CharTermAttribute append(CharSequence csq, int start, int end) {
        ...
        if (len > 4) { // only use instanceof check series for longer CSQs, else simply iterate
            if (csq instanceof String) {
                ...
                System.arraycopy(((CharTermAttribute) csq).buffer(), start, termBuffer, termLength,
                    len);
     ...
     * before it returned a String representation of the whole term with all attributes. This
     * affects especially the {@link org.apache.lucene.analysis.Token} subclass.
     */
    @Override
