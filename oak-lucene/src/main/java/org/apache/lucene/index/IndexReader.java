...
/**
 * IndexReader is an abstract class, providing an interface for accessing an index.  Search of an
...
public abstract class IndexReader implements Closeable {
    ...
    /**
     * Expert: adds a {@link ReaderClosedListener}.  The provided listener will be invoked when this
     * reader is closed.
     *
     * @lucene.experimental
     */
    ...
    /**
     * Expert: This method is called by {@code IndexReader}s which wrap other readers (e.g.
     * {@link CompositeReader} or {@link FilterAtomicReader}) to register the parent at the child
     * (this reader) on construction of the parent. When this reader is closed, it will mark all
     * registered parents as closed, too. The references to parent readers are weak only, so they
     * can be GCed once they are no longer in use.
     *
     * @lucene.experimental
     */
    ...
    /**
     * Expert: increments the refCount of this IndexReader instance.  RefCounts are used to
     * determine when a reader can be closed safely, i.e. as soon as there are no more references.
     * Be sure to always call a corresponding {@link #decRef}, in a finally clause; otherwise the
     * reader may never be closed.  Note that {@link #close} simply calls decRef(), which means that
     * the IndexReader will not really be closed until {@link #decRef} has been called for all
     ...
     * already closed or is currently being closed. Either way this reader instance shouldn't be
     * used by an application unless
     * <code>true</code> is returned.
     * <p>
     * RefCounts are used to determine when a reader can be closed safely, i.e. as soon as there are
     * no more references.  Be sure to always call a corresponding {@link #decRef}, in a finally
     * clause; otherwise the reader may never be closed.  Note that {@link #close} simply calls
     * decRef(), which means that the IndexReader will not really be closed until {@link #decRef}
    ...
    /**
     * Expert: decreases the refCount of this IndexReader instance.  If the refCount drops to 0,
     * then this reader is closed.  If an exception is hit, the refCount is unchanged.
     *
     ...
     * to implement equals/hashCode, so methods are declared final. To lookup instances from caches
     * use {@link #getCoreCacheKey} and {@link #getCombinedCoreAndDeletesKey}.
     */
    @Override
    ...
    /**
     * Retrieve term vectors for this document, or null if term vectors were not indexed.  The
     ...
     * determine how big to allocate an array which will have an element for every document number
     * in an index.
     */
     ...
     * requested document is deleted, and therefore asking for a deleted document may yield
     * unspecified results. Usually this is not required, however you can test if the doc is deleted
     * by checking the {@link Bits} returned from {@link MultiFields#getLiveDocs}.
     *
     * <b>NOTE:</b> only the content of a field is returned,
     * if that field was stored during indexing.  Metadata like boost, omitNorm, IndexOptions,
     ...
     * method returns a {@link CompositeReaderContext} holding the reader's direct children as well
     * as a view of the reader tree's atomic leaf contexts. All sub- {@link IndexReaderContext}
     * instances referenced from this readers top-level context are private to this reader and are
     * not shared with another context tree. For example, IndexSearcher uses this API to drive
     * searching by one atomic leaf reader at a time. If this reader is not composed of child
     * readers, this method returns an {@link AtomicReaderContext}.
     * <p>
     * Note: Any of the sub-{@link CompositeReaderContext} instances referenced from this top-level
    ...
    /**
     * Expert: Returns a key for this IndexReader, so FieldCache/CachingWrapperFilter can find it
     * again. This key must not have equals()/hashCode() methods, so &quot;equals&quot; means
     * &quot;identical&quot;.
     */
     ...
     * FieldCache/CachingWrapperFilter can find it again. This key must not have equals()/hashCode()
     * methods, so &quot;equals&quot; means &quot;identical&quot;.
     */
     ...
     * field does not exists.  This method does not take into account deleted documents that have
     * not yet been merged away.
     *
     ...
     * freq() for each doc that has this term). This will be -1 if the codec doesn't support this
     * measure. Note that, like other term measures, this measure does not take deleted documents
     * into account.
     */
    public abstract long totalTermFreq(Term term) throws IOException;

    /**
     * Returns the sum of {@link TermsEnum#docFreq()} for all terms in this field, or -1 if this
     * measure isn't stored by the codec. Note that, just like other term measures, this measure
    ...
    /**
     * Returns the number of documents that have at least one term for this field, or -1 if this
     * measure isn't stored by the codec. Note that, just like other term measures, this measure
     * does not take deleted documents into account.
     *
    ...
    /**
     * Returns the sum of {@link TermsEnum#totalTermFreq} for all terms in this field, or -1 if this
     * measure isn't stored by the codec (or if this fields omits term freq and positions). Note
     * that, just like other term measures, this measure does not take deleted documents into
     * account.
     *
