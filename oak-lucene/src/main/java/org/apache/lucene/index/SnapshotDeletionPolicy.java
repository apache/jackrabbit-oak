 ...
 * {@link IndexWriter} will not remove any files associated with it even if the index is otherwise
 * being actively, arbitrarily changed. Because we wrap another arbitrary
 * {@link IndexDeletionPolicy}, this gives you the freedom to continue using whatever
 * {@link IndexDeletionPolicy} you would normally want to use with your index.
 *
 * <p>
 ...
 * {@link PersistentSnapshotDeletionPolicy}.
 *
 * @lucene.experimental
 */
public class SnapshotDeletionPolicy extends IndexDeletionPolicy {
     ...
     * from deletion (as long as this {@link IndexDeletionPolicy} is used). The snapshot can be
     * removed by calling {@link #release(IndexCommit)} followed by a call to
     * {@link IndexWriter#deleteUnusedFiles()}.
     *
     * <p>
     * <b>NOTE:</b> while the snapshot is held, the files it references will not
     * be deleted, which will consume additional disk space in your index. If you take a snapshot at
     * a particularly bad time (say just before you call forceMerge) then in the worst case this
     * could consume an extra 1X of your total index size, until you release the snapshot.
     *
