 ...
 * AtomicReaders, but it is not possible to directly retrieve postings. To do that, get the
 * {@link AtomicReaderContext} for all sub-readers via {@link #leaves()}. Alternatively, you can
 * mimic an {@link AtomicReader} (with a serious slowdown), by wrapping composite readers with
 * {@link SlowCompositeReaderWrapper}.
 *
 * <p>IndexReader instances for indexes on disk are usually constructed
 * with a call to one of the static <code>DirectoryReader.open()</code> methods, e.g.
 * {@link DirectoryReader#open(Directory)}. {@link DirectoryReader} implements the
 * {@code CompositeReader} interface, it is not possible to directly get postings.
 * <p> Concrete subclasses of IndexReader are usually constructed with a call to
 * one of the static <code>open()</code> methods, e.g. {@link DirectoryReader#open(Directory)}.
 *
...
public abstract class CompositeReader extends IndexReader {
     ...
     * is no longer public, code that wants to get all {@link AtomicReader}s this composite is
     * composed of should use {@link IndexReader#leaves()}.
     *
