 ...
 * {@link UpgradeIndexMergePolicy} and triggers the upgrade via an forceMerge request to
 * {@link IndexWriter}.
 * <p>This tool keeps only the last commit in an index; for this
 * reason, if the incoming index has more than one commit, the tool refuses to run by default.
 * Specify {@code -delete-prior-commits} to override this, allowing the tool to delete all but the
 * last commit. From Java code this can be enabled by passing {@code true} to
 ...
 * upgraded before execution (e.g., documents were added). If your application relies on
 * &quot;monotonicity&quot; of doc IDs (which means that the order in which the documents were added
 * to the index is preserved), do a full forceMerge instead. The {@link MergePolicy} set by
...
public final class IndexUpgrader {
     ...
     * by removing all older ones. If {@code infoStream} is not {@code null}, all logging output
     * will be sent to this stream.
     */
    public IndexUpgrader(Directory dir, Version matchVersion, PrintStream infoStream,
    ...
    public void upgrade() throws IOException {
        ...
        try {
            ...
            if (infoStream.isEnabled("IndexUpgrader")) {
                infoStream.message("IndexUpgrader",
                    "Upgrading all pre-" + Constants.LUCENE_MAIN_VERSION
            ...
            if (infoStream.isEnabled("IndexUpgrader")) {
                infoStream.message("IndexUpgrader",
                    "All segments upgraded to version " + Constants.LUCENE_MAIN_VERSION);
            }
        } finally {
            w.close();
        }
