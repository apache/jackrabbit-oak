...
class DocumentsWriterFlushQueue {
    ...
    static abstract class FlushTicket {
         ...
         */
        protected final void publishFlushedSegment(IndexWriter indexWriter,
            FlushedSegment newSegment, FrozenBufferedUpdates globalPacket)
            throws IOException {
            ...
            if (indexWriter.infoStream.isEnabled("DW")) {
                indexWriter.infoStream.message("DW",
                    "publishFlushedSegment seg-private updates=" + segmentUpdates);
            }

            if (segmentUpdates != null && indexWriter.infoStream.isEnabled("DW")) {
                indexWriter.infoStream.message("DW",
                    "flush: push buffered seg private updates: " + segmentUpdates);
            }
            // now publish!
        ...
        protected final void finishFlush(IndexWriter indexWriter, FlushedSegment newSegment,
            ...
            if (newSegment == null) {
                ...
                if (bufferedUpdates != null && bufferedUpdates.any()) {
                    ...
                    if (indexWriter.infoStream.isEnabled("DW")) {
                        indexWriter.infoStream.message("DW",
                            "flush: push buffered updates: " + bufferedUpdates);
                    }
                }
            } else {
