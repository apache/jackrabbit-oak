...
public class LogByteSizeMergePolicy extends LogMergePolicy {
     ...
     * byte size of the segment's files, in MB) that may be merged with other segments.  Small
     * values (e.g., less than 50 MB) are best for interactive indexing, as this limits the length
     * of pauses while indexing to a few seconds.  Larger values are best for batched indexing and
     * speedier searches.</p>
     *
    ...
    /**
     * <p>Determines the largest segment (measured by total
     * byte size of the segment's files, in MB) that may be merged with other segments during
     * forceMerge. Setting it low will leave the index with more than 1 segment, even if
     * {@link IndexWriter#forceMerge} is called.
     */
