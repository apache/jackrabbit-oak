...
/**
 * A {@link DocumentsWriterPerThreadPool} implementation that tries to assign an indexing thread to
 * the same {@link ThreadState} each time the thread tries to obtain a {@link ThreadState}. Once a
 * new {@link ThreadState} is created it is associated with the creating thread. Subsequently, if
 * the threads associated {@link ThreadState} is not in use it will be associated with the
 * requesting thread. Otherwise, if the {@link ThreadState} is used by another thread
 * {@link ThreadAffinityDocumentsWriterThreadPool} tries to find the currently minimal contended
 * {@link ThreadState}.
 */
class ThreadAffinityDocumentsWriterThreadPool extends DocumentsWriterPerThreadPool {
    ...
    /**
     * Creates a new {@link ThreadAffinityDocumentsWriterThreadPool} with a given maximum of
     * {@link ThreadState}s.
     */
