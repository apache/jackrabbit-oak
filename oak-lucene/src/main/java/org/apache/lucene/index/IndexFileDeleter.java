...
final class IndexFileDeleter implements Closeable {
     ...
     * reference, call the policy to let it delete commits.  This will remove any files not
    ...
    public IndexFileDeleter(Directory directory, IndexDeletionPolicy policy,
        ...
        if (infoStream.isEnabled("IFD")) {
            infoStream.message("IFD",
                "init: current segments file is \"" + currentSegmentsFile + "\"; deletionPolicy="
                    + policy);
        }
        ...
        if (currentSegmentsFile != null) {
            ...
            for (String fileName : files) {
                m.reset(fileName);
                if (!fileName.endsWith("write.lock") && !fileName.equals(
                    ...
                    if (fileName.startsWith(IndexFileNames.SEGMENTS)) {
                        ...
                        try {
                            ...
                            if (SegmentInfos.generationFromSegmentsFileName(fileName) <= currentGen
                                && directory.fileLength(fileName) > 0) {
                                throw e;
                            } else {
        ...
        if (currentCommitPoint == null && currentSegmentsFile != null && initialIndexExists) {
            ...
            try {
                ...
                throw new CorruptIndexException(
                    "failed to locate current segments_N file \"" + currentSegmentsFile + "\"");
            }
            if (infoStream.isEnabled("IFD")) {
                infoStream.message("IFD",
                    "forced open of current segments file " + segmentInfos.getSegmentsFileName());
            }
    ...
    private void deleteCommits() throws IOException {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (infoStream.isEnabled("IFD")) {
                    infoStream.message("IFD",
                        "deleteCommits: now decRef commit \"" + commit.getSegmentsFileName()
                            + "\"");
                }
    ...
    /**
     * Writer calls this when it has hit an error and had to roll back, to tell us that there may
     * now be unreferenced files in the filesystem.  So we re-list the filesystem and delete such
     * files.  If segmentName is non-null, we will only delete files corresponding to that segment.
     */
    public void refresh(String segmentName) throws IOException {
        ...
        for (int i = 0; i < files.length; i++) {
            ...
            m.reset(fileName);
            if ((segmentName == null || fileName.startsWith(segmentPrefix1) || fileName.startsWith(
                segmentPrefix2)) &&
     ...
     * {@link IndexDeletionPolicy#onCommit(List)} again with the known commits. This is useful in
     * cases where a deletion policy which holds onto index commits is used. The application may
     * know that some commits are not held by the deletion policy anymore and call
     * {@link IndexWriter#deleteUnusedFiles()}, which will attempt to delete the unused commits
     * again.
     */
     ...
     * written to the index and the in-memory SegmentInfos have been modified to point to those
     * files.
     * <p>
    ...
    public void checkpoint(SegmentInfos segmentInfos, boolean isCommit) throws IOException {
        ...
        if (infoStream.isEnabled("IFD")) {
            t0 = System.nanoTime();
            infoStream.message("IFD",
                "now checkpoint \"" + writer.segString(writer.toLiveInfos(segmentInfos)) + "\" ["
    ...
    void incRef(String fileName) {
        ...
        if (infoStream.isEnabled("IFD")) {
            if (VERBOSE_REF_COUNTS) {
                infoStream.message("IFD",
                    "  IncRef \"" + fileName + "\": pre-incr count is " + rc.count);
            }
        }
        rc.IncRef();
    }
    ...
    void decRef(String fileName) throws IOException {
        ...
        if (infoStream.isEnabled("IFD")) {
            if (VERBOSE_REF_COUNTS) {
                infoStream.message("IFD",
                    "  DecRef \"" + fileName + "\": pre-decr count is " + rc.count);
            }
        }
        if (0 == rc.DecRef()) {
    ...
    void deleteFile(String fileName)
        ...
        try {
            ...
            if (directory.fileExists(fileName)) {
                ...
                if (infoStream.isEnabled("IFD")) {
                    infoStream.message("IFD",
                        "unable to remove file \"" + fileName + "\": " + e.toString()
                            + "; Will re-try later.");
                }
                if (deletable == null) {
    ...
    final private static class RefCount {
        ...
        public int IncRef() {
            if (!initDone) {
                ...
                assert
                    count > 0 :
                    Thread.currentThread().getName() + ": RefCount is 0 pre-increment for file \""
                        + fileName + "\"";
            }
            return ++count;
        }
        ...
        public int DecRef() {
            assert
                count > 0 :
                Thread.currentThread().getName() + ": RefCount is 0 pre-decrement for file \""
                    + fileName + "\"";
            return --count;
        }
    }
