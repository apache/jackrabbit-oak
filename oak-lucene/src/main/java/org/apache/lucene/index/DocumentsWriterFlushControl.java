 ...
 * {@link DocumentsWriterPerThread} as flush pending iff a {@link DocumentsWriterPerThread} exceeds
 * the {@link IndexWriterConfig#getRAMPerThreadHardLimitMB()} to prevent address space exhaustion.
 */
final class DocumentsWriterFlushControl {
    ...
    private long stallLimitBytes() {
        ...
        return maxRamMB != IndexWriterConfig.DISABLE_AUTO_FLUSH ? (long) (2 * (maxRamMB * 1024
            * 1024)) : Long.MAX_VALUE;
    }
    ...
    private boolean assertMemory() {
        ...
        if (maxRamMB != IndexWriterConfig.DISABLE_AUTO_FLUSH) {
            ...
            final long expected =
                (2 * (ramBufferBytes)) + ((numPending + numFlushingDWPT() + numBlockedFlushes())
                    * peakDelta) + (numDocsSinceStalled * peakDelta);
    ...
    /**
     * Sets flush pending state on the given {@link ThreadState}. The {@link ThreadState} must have
     * indexed at least on Document and must not be already pending.
     */
    ...
    public int getNumGlobalTermDeletes() {
        return documentsWriter.deleteQueue.numGlobalTermDeletes()
            + bufferedUpdatesStream.numTerms();
    }
    ...
    void markForFullFlush() {
        ...
        synchronized (this) {
            ...
            assert fullFlushBuffer.isEmpty() :
                "full flush buffer should be empty: " + fullFlushBuffer;
            fullFlush = true;
    ...
    void addFlushableState(ThreadState perThread) {
        ...
        if (dwpt.getNumDocsInRAM() > 0) {
            synchronized (this) {
                ...
                final DocumentsWriterPerThread flushingDWPT = internalTryCheckOutForFlush(
                    perThread);
                assert flushingDWPT
                    != null : "DWPT must never be null here since we hold the lock and it holds documents";
