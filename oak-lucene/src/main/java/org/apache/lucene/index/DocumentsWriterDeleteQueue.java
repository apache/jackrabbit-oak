 ...
 * need to maintain their 'own' head of the queue (as a DeleteSlice instance per DWPT). The
 * difference between the DWPT and the global pool is that the DWPT starts maintaining a head once
 * it has added its first document since for its segments private deletes only the deletes after
 * that document are relevant. The global pool instead starts maintaining the head once this
 * instance is created by taking the sentinel instance as its initial head.
 * <p>
 * Since each {@link DeleteSlice} maintains its own head and the list is only single linked the
 ...
 * {@link DeleteSlice} or by the global {@link BufferedUpdates} slice.
 * <p>
 * Each DWPT as well as the global delete pool maintain their private DeleteSlice instance. In the
 * DWPT case updating a slice is equivalent to atomically finishing the document. The slice update
 * guarantees a "happens before" relationship to all other updates in the same indexing session.
 * When a DWPT updates a document it:
 *
 * <ol>
    ...
    static class DeleteSlice {
        ...
        void apply(BufferedUpdates del, int docIDUpto) {
            ...
            do {
                ...
                assert current
                    != null : "slice property violated between the head on the tail must not be a null node";
