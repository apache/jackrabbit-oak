...
public final class CompositeReaderContext extends IndexReaderContext {
    ...
    /**
     * Creates a {@link CompositeReaderContext} for top-level readers with parent set to
     * <code>null</code>
     */
    CompositeReaderContext(CompositeReader reader, List<IndexReaderContext> children,
    ...
    private static final class Builder {
        ...
        private IndexReaderContext build(CompositeReaderContext parent, IndexReader reader, int ord,
            int docBase) {
            if (reader instanceof AtomicReader) {
                final AtomicReader ar = (AtomicReader) reader;
                final AtomicReaderContext atomic = new AtomicReaderContext(parent, ar, ord, docBase,
                    leaves.size(), leafDocBase);
                leaves.add(atomic);
                ...
                final List<IndexReaderContext> children = Arrays.asList(
                    new IndexReaderContext[sequentialSubReaders.size()]);
