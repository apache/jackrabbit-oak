...
public final class TwoPhaseCommitTool {
    ...
    /**
     * Executes a 2-phase commit algorithm by first {@link TwoPhaseCommit#prepareCommit()} all
     * objects and only if all succeed, it proceeds with {@link TwoPhaseCommit#commit()}. If any of
     * the objects fail on either the preparation or actual commit, it terminates and
     * {@link TwoPhaseCommit#rollback()} all of them.
     * <p>
     * <b>NOTE:</b> it may happen that an object fails to commit, after few have
     * already successfully committed. This tool will still issue a rollback instruction on them as
     * well, but depending on the implementation, it may not have any effect.
     * <p>
     ...
     * @throws CommitFailException        if any of the objects fail to
     *                                    {@link TwoPhaseCommit#commit()}
     */
