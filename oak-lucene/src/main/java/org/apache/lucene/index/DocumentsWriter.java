 ...
 * <p>
 * Each added document is passed to the {@link DocConsumer}, which in turn processes the document
 * and interacts with other consumers in the indexing chain.  Certain consumers, like
 * {@link StoredFieldsConsumer} and {@link TermVectorsConsumer}, digest a document and immediately
 * write bytes to the "doc store" files (ie, they do not consume RAM per document, except while they
 * are processing the document).
 * <p>
 * Other consumers, eg {@link FreqProxTermsWriter} and {@link NormsConsumer}, buffer bytes in RAM
 * and flush only when a new segment is produced.
 * <p>
 ...
 * getThreadState which allocates a ThreadState for this thread.  The same thread will get the same
 * ThreadState over time (thread affinity) so that if there are consistent patterns (for example
 * each thread is indexing a different content source) then we make better use of RAM.  Then
 * processDocument is called on that ThreadState without synchronization (most of the "heavy
 * lifting" is in this call).  Finally the synchronized "finishDocument" is called to flush changes
 * to the directory.
 * <p>
 * When flush is called by IndexWriter we forcefully idle all threads and flush only once they are
 * all idle.  This means you can call flush with a given thread even while other threads are
 * actively adding/deleting documents.
 * <p>
 * <p>
 * Exceptions:
 * <p>
 * Because this class directly updates in-memory posting lists, and flushes stored fields and term
 * vectors directly to files in the directory, there are certain limited times when an exception can
 * corrupt this state. For example, a disk full while flushing stored fields leaves this file in a
 * corrupt state.  Or, an OOM exception while appending to the in-memory posting lists can corrupt
 * that posting list.  We call such exceptions "aborting exceptions".  In these cases we must call
...
final class DocumentsWriter implements Closeable {
    ...
    synchronized boolean updateNumericDocValue(Term term, String field, Long value)
        throws IOException {
        final DocumentsWriterDeleteQueue deleteQueue = this.deleteQueue;
    ...
    private final boolean applyAllDeletes(DocumentsWriterDeleteQueue deleteQueue)
        throws IOException {
     ...
     * all currently buffered docs.  This resets our state, discarding any docs added since last
     * flush.
     */
        ...
        try {
            ...
            if (infoStream.isEnabled("DW")) {
                infoStream.message("DW",
                    "done abort; abortedFiles=" + newFilesSet + " success=" + success);
            }
        }
    }
    ...
    final synchronized void unlockAllAfterAbortAll(IndexWriter indexWriter) {
        ...
        for (int i = 0; i < limit; i++) {
            try {
                ...
                if (infoStream.isEnabled("DW")) {
                    infoStream.message("DW",
                        "unlockAll: could not unlock state: " + i + " msg:" + e.getMessage());
                }
    ...
    private boolean doFlush(DocumentsWriterPerThread flushingDWPT) throws IOException {
        ...
        if (ramBufferSizeMB != IndexWriterConfig.DISABLE_AUTO_FLUSH &&
            ...
            if (infoStream.isEnabled("DW")) {
                infoStream.message("DW",
                    "force apply deletes bytesUsed=" + flushControl.getDeleteBytesUsed()
                        + " vs ramBuffer=" + (1024 * 1024 * ramBufferSizeMB));
            }
            hasEvents = true;
    ...
    final boolean flushAllThreads(final IndexWriter indexWriter)
        ...
        try {
            ...
            if (!anythingFlushed
                ...
                if (infoStream.isEnabled("DW")) {
                    infoStream.message("DW",
                        Thread.currentThread().getName() + ": flush naked frozen global deletes");
                }
    ...
    final void finishFullFlush(boolean success) {
        try {
            if (infoStream.isEnabled("DW")) {
                infoStream.message("DW",
                    Thread.currentThread().getName() + " finishFullFlush success=" + success);
            }
    ...
    static final class ApplyDeletesEvent implements Event {
        ...
        @Override
        public void process(IndexWriter writer, boolean triggerMerge, boolean forcePurge)
            throws IOException {
