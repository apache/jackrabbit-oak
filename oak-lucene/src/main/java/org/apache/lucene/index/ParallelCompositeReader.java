...
/**
 * An {@link CompositeReader} which reads multiple, parallel indexes.  Each index added must have
 * the same number of documents, and exactly the same hierarchical subreader structure, but
 ...
 * result in undefined behavior</em>. A good strategy to create suitable indexes with
 * {@link IndexWriter} is to use {@link LogDocMergePolicy}, as this one does not reorder documents
 * during merging (like {@code TieredMergePolicy}) and triggers merges by number of documents per
 * segment. If you use different {@link MergePolicy}s it might happen that the segment structure of
 * your index is no longer predictable.
 */
public class ParallelCompositeReader extends BaseCompositeReader<IndexReader> {
    ...
    /**
     * Expert: create a ParallelCompositeReader based on the provided readers and
     * storedFieldReaders; when a document is loaded, only storedFieldsReaders will be used.
     */
    public ParallelCompositeReader(boolean closeSubReaders, CompositeReader[] readers,
    ...
    private static IndexReader[] prepareSubReaders(CompositeReader[] readers,
        CompositeReader[] storedFieldsReaders) throws IOException {
        if (readers.length == 0) {
            ...
            for (int i = 0; i < subReaders.length; i++) {
                if (firstSubReaders.get(i) instanceof AtomicReader) {
                    ...
                    for (int j = 0; j < storedFieldsReaders.length; j++) {
                        storedSubs[j] = (AtomicReader) storedFieldsReaders[j].getSequentialSubReaders()
                                                                             .get(i);
                    }
                    ...
                    for (int j = 0; j < readers.length; j++) {
                        compositeSubs[j] = (CompositeReader) readers[j].getSequentialSubReaders()
                                                                       .get(i);
                    }
                    final CompositeReader[] storedSubs = new CompositeReader[storedFieldsReaders.length];
                    for (int j = 0; j < storedFieldsReaders.length; j++) {
                        storedSubs[j] = (CompositeReader) storedFieldsReaders[j].getSequentialSubReaders()
                                                                                .get(i);
                    }
                    // We pass true for closeSubs and we prevent closing of subreaders in doClose():
                    // By this the synthetic throw-away readers used here are completely invisible to ref-counting
    ...
    private static void validate(CompositeReader[] readers, int maxDoc, int[] childMaxDoc,
        ...
        for (int i = 0; i < readers.length; i++) {
            ...
            if (noSubs != childMaxDoc.length) {
                throw new IllegalArgumentException(
                    "All readers must have same number of subReaders");
            }
            for (int subIDX = 0; subIDX < noSubs; subIDX++) {
                final IndexReader r = subs.get(subIDX);
                if (r.maxDoc() != childMaxDoc[subIDX]) {
                    throw new IllegalArgumentException(
                        "All readers must have same corresponding subReader maxDoc");
                }
                if (!(childAtomic[subIDX] ? (r instanceof AtomicReader)
                    : (r instanceof CompositeReader))) {
                    throw new IllegalArgumentException(
                        "All readers must have same corresponding subReader types (atomic or composite)");
                }
            }
        }
    }
