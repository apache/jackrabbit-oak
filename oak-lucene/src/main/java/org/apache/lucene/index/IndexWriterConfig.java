...
/**
 * Holds all the configuration that is used to create an {@link IndexWriter}. Once
 * {@link IndexWriter} has been created with this object, changes to this object will not affect the
 * {@link IndexWriter} instance. For that, use {@link LiveIndexWriterConfig} that is returned from
 * {@link IndexWriter#getConfig()}.
 *
 * <p>
 * All setter methods return {@link IndexWriterConfig} to allow chaining settings conveniently, for
 * example:
 *
...
public final class IndexWriterConfig extends LiveIndexWriterConfig implements Cloneable {
    ...
    public static enum OpenMode {
        ...
        /**
         * Creates a new index if one does not exist, otherwise it opens the index and documents
         * will be appended.
         */
        CREATE_OR_APPEND
    }
    ...
    /**
     * Default value is 16 MB (which means flush when buffered docs consume approximately 16 MB
     * RAM).
     */
    ...
    /**
     * The maximum number of simultaneous threads that may be indexing documents at once in
     * IndexWriter; if more than this many threads arrive they will wait for others to finish.
     * Default value is 8.
     */
    ...
    /**
     * Creates a new config that with defaults that match the specified {@link Version} as well as
     * the default {@link Analyzer}. If matchVersion is >= {@link Version#LUCENE_32},
     * {@link TieredMergePolicy} is used for merging; else {@link LogByteSizeMergePolicy}. Note that
     * {@link TieredMergePolicy} is free to select non-contiguous merges, which means docIDs may not
     * remain monotonic over time.  If this is a problem you should switch to
     * {@link LogByteSizeMergePolicy} or {@link LogDocMergePolicy}.
     */
    ...
    /**
     * Expert: allows an optional {@link IndexDeletionPolicy} implementation to be specified. You
     * can use this to control when prior commits are deleted from the index. The default policy is
     * {@link KeepOnlyLastCommitDeletionPolicy} which removes all prior commits as soon as a new
     * commit is done (this matches behavior before 2.2). Creating your own policy can allow you to
     * explicitly keep previous "point in time" commits alive in the index for some time, to allow
     * readers to refresh to the new commit without having the old commit deleted out from under
     * them. This is necessary on filesystems like NFS that do not support "delete on last close"
    ...
    /**
     * Sets the maximum time to wait for a write lock (in milliseconds) for this instance. You can
     * change the default value for all instances by calling
    ...
    /**
     * Expert: {@link MergePolicy} is invoked whenever there are changes to the segments in the
     * index. Its role is to select which merges to do, if any, and return a
     * {@link MergePolicy.MergeSpecification} describing the merges. It also selects merges to do
     * for forceMerge.
     *
     * <p>Only takes effect when IndexWriter is first created.
     */
    ...
    /**
     * Expert: Sets the {@link DocumentsWriterPerThreadPool} instance used by the IndexWriter to
     * assign thread-states to incoming indexing threads. If no {@link DocumentsWriterPerThreadPool}
     * is set {@link IndexWriter} will use {@link ThreadAffinityDocumentsWriterThreadPool} with max
     * number of thread-states set to {@link #DEFAULT_MAX_THREAD_STATES} (see
     * {@link #DEFAULT_MAX_THREAD_STATES}).
     * </p>
     * <p>
     * NOTE: The given {@link DocumentsWriterPerThreadPool} instance must not be used with other
     * {@link IndexWriter} instances once it has been initialized / associated with an
     * {@link IndexWriter}.
     * </p>
     * <p>
    ...
    /**
     * Sets the max number of simultaneous threads that may be indexing documents at once in
     * IndexWriter. Values &lt; 1 are invalid and if passed
     ...
     * merging, unless a near-real-time reader has been obtained by calling
     * {@link DirectoryReader#open(IndexWriter, boolean)}. This method lets you enable pooling
     * without getting a near-real-time reader.  NOTE: if you set this to false, IndexWriter will
     * still pool readers once {@link DirectoryReader#open(IndexWriter, boolean)} is called.
     *
     * <p>Only takes effect when IndexWriter is first created.
     */
     ...
     * initialized during {@link IndexWriter} instantiation and once initialized the given instance
     * is bound to this {@link IndexWriter} and should not be used with another writer.
     *
     ...
     * A {@link DocumentsWriterPerThread} is forcefully flushed once it exceeds this limit even if
     * the {@link #getRAMBufferSizeMB()} has not been exceeded. This is a safety limit to prevent a
     * {@link DocumentsWriterPerThread} from address space exhaustion due to its internal 32 bit
     * signed integer based memory addressing. The given value must be less that 2GB (2048MB)
     *
    ...
    /**
     * Information about merges, deletes and a message when maxFieldLength is reached will be
     * printed to this. Must not be null, but {@link InfoStream#NO_OUTPUT} may be used to supress
     * output.
     */
