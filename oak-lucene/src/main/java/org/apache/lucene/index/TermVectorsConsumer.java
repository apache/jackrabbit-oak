...
final class TermVectorsConsumer extends TermsHashConsumer {
    ...
    @Override
    void flush(Map<String, TermsHashConsumerPerField> fieldsToFlush, final SegmentWriteState state)
        throws IOException {
        if (writer != null) {
    ...
    /**
     * Fills in no-term-vectors for all docs we haven't seen since the last doc that had term
     * vectors.
     */
    ...
    private final void initTermVectorsWriter() throws IOException {
        if (writer == null) {
            IOContext context = new IOContext(
                new FlushInfo(docWriter.getNumDocsInRAM(), docWriter.bytesUsed()));
            writer = docWriter.codec.termVectorsFormat()
                                    .vectorsWriter(docWriter.directory, docWriter.getSegmentInfo(),
                                        context);
            lastDocID = 0;
        }
    }
    ...
    void finishDocument(TermsHash termsHash) throws IOException {
        ...
        assert
            ...
            "lastDocID=" + lastDocID + " docState.docID=" + docState.docID;
