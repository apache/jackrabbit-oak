...
public class MultiDocValues {
     ...
     * {@link AtomicReader#getNormValues(String)}
     * </p>
     */
     ...
     * <p>
     * This is a slow way to access numeric values. Instead, access them per-segment with
     ...
     * <p>
     * This is a slow way to access this bitset. Instead, access them per-segment with
     * {@link AtomicReader#getDocsWithField(String)}
     * </p>
     */
    public static Bits getDocsWithField(final IndexReader r, final String field)
        throws IOException {
        final List<AtomicReaderContext> leaves = r.leaves();
        final int size = leaves.size();
        if (size == 0) {
     ...
     * <p>
     * This is a slow way to access binary values. Instead, access them per-segment with
     ...
     * {@link AtomicReader#getSortedDocValues(String)}
     * </p>
     */
     ...
     * <p>
     * This is an extremely slow way to access sorted values. Instead, access them per-segment with
     * {@link AtomicReader#getSortedSetDocValues(String)}
     * </p>
     */
    ...
    public static class OrdinalMap {
        ...
        /**
         * Creates an ordinal map that allows mapping ords to/from a merged space from
         * <code>subs</code>.
         *
         * @param owner a cache key
         * @param subs  TermsEnums that support {@link TermsEnum#ord()}. They need not be dense
         *              (e.g. can be FilteredTermsEnums}.
        ...
        /**
         * Given global ordinal, returns the ordinal of the first segment which contains this
         * ordinal (the corresponding to the segment return {@link #getFirstSegmentNumber}).
         */
    ...
    public static class MultiSortedDocValues extends SortedDocValues {
         ...
         */
        MultiSortedDocValues(SortedDocValues values[], int docStarts[], OrdinalMap mapping)
            throws IOException {
    ...
    public static class MultiSortedSetDocValues extends SortedSetDocValues {
         ...
         */
        MultiSortedSetDocValues(SortedSetDocValues values[], int docStarts[], OrdinalMap mapping)
            throws IOException {
            assert values.length == mapping.ordDeltas.length;
            assert docStarts.length == values.length + 1;
            this.values = values;
