 ...
 * <p>
 * Like FieldCache, it uninverts the index and holds a packed data structure in RAM to enable fast
 * access. Unlike FieldCache, it can handle multi-valued fields, and, it does not hold the term
 ...
 * <p>
 * While normally term ords are type long, in this API they are int as the internal representation
 * here cannot address more than MAX_INT unique terms.  Also, typically this class is used on fields
 * with relatively few unique terms vs the number of documents.  In addition, there is an internal
 * limit (16 MB) on how many bytes each chunk of documents may consume.  If you trip this limit
 ...
 * <p>
 * The returned per-document ords do not retain their original order in the document.  Instead they
 * are returned in sorted (by ord, ie term's BytesRef comparator) order.  They are also de-dup'd (ie
 * if doc has same term more than once in this field, you'll only get that ord back once).
 * <p>
 * This class tests whether the provided reader is able to retrieve terms by ord (ie, it's single
 * segment, and it uses an ord-capable terms index).  If not, this class will create its own term
 * index internally, allowing to create a wrapped TermsEnum that can handle ord.  The
 * {@link #getOrdTermsEnum} method then provides this wrapped enum, if necessary.
 * <p>
...
public class DocTermOrds {
    ...
    /**
     * Inverts only terms starting w/ prefix, and only terms whose docFreq (not taking deletions
     * into account) is <=  maxTermDocFreq
     */
    public DocTermOrds(AtomicReader reader, Bits liveDocs, String field, BytesRef termPrefix,
    ...
    /**
     * Inverts only terms starting w/ prefix, and only terms whose docFreq (not taking deletions
     * into account) is <=  maxTermDocFreq, with a custom indexing interval (default is every 128nd
     * term).
     */
    public DocTermOrds(AtomicReader reader, Bits liveDocs, String field, BytesRef termPrefix,
    ...
    /**
     * Returns a TermsEnum that implements ord.  If the provided reader supports ord, we just return
     * its TermsEnum; if it does not, we build a "private" terms index internally (WARNING: consumes
     * RAM) and use that index to implement ord.  This also enables ord on top of a composite
     * reader.  The returned TermsEnum is unpositioned.  This returns null if there are no terms.
     *
    ...
    /**
     * Invoked during {@link #uninvert(AtomicReader, Bits, BytesRef)} to record the document
     * frequency for each uninverted term.
     */
     ...
     */
    protected void uninvert(final AtomicReader reader, Bits liveDocs, final BytesRef termPrefix)
        throws IOException {
        ...
        if (termInstances == 0) {
            ...
            for (int pass = 0; pass < 256; pass++) {
              ...
              */
                            ...
                            pos +=
                                len + 1;  // skip single byte at end and leave it 0 for terminator
                        }
                    }
                }
    ...
    private class Iterator extends SortedSetDocValues {
        ...
        /**
         * Buffer must be at least 5 ints long.  Returns number of term ords placed into buffer; if
         * this count is less than buffer.length then that is the end.
         */
        int read(int[] buffer) {
            int bufferUpto = 0;
            if (arr == null) {
