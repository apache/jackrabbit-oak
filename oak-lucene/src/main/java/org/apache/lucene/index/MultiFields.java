...
/**
 * Exposes flex API, merged from flex API of sub-segments. This is useful when you're interacting
 * with an {@link IndexReader} implementation that consists of sequential sub-readers (eg
 * {@link DirectoryReader} or {@link MultiReader}).
 *
 * <p><b>NOTE</b>: for composite readers, you'll get better
 * performance by gathering the sub readers using {@link IndexReader#getContext()} to get the atomic
 * leaves and then operate per-AtomicReader, instead of using this class.
 *
 * @lucene.experimental
 */
...
public final class MultiFields extends Fields {
    ...
    /**
     * Returns a single {@link Fields} instance for this reader, merging fields/terms/docs/positions
     * on the fly.  This method will return null if the reader has no postings.
     *
    ...
    /**
     * Returns a single {@link Bits} instance for this reader, merging live Documents on the fly.
     * This method will return null if the reader has no deletions.
     *
     * <p><b>NOTE</b>: this is a very slow way to access live docs.
     * For example, each Bits access will require a binary search. It's better to get the
    ...
    /**
     * Returns {@link DocsEnum} for the specified field & term.  This will return null if the field
     * or term does not exist.
     */
    public static DocsEnum getTermDocsEnum(IndexReader r, Bits liveDocs, String field,
        BytesRef term) throws IOException {
        return getTermDocsEnum(r, liveDocs, field, term, DocsEnum.FLAG_FREQS);
    }

    /**
     * Returns {@link DocsEnum} for the specified field & term, with control over whether freqs are
     * required. Some codecs may be able to optimize their implementation when freqs are not
     * required.  This will return null if the field or term does not exist.  See
     * {@link TermsEnum#docs(Bits, DocsEnum, int)}.
     */
    public static DocsEnum getTermDocsEnum(IndexReader r, Bits liveDocs, String field,
        BytesRef term, int flags) throws IOException {
        assert field != null;
        assert term != null;
    ...
    /**
     * Returns {@link DocsAndPositionsEnum} for the specified field & term.  This will return null
     * if the field or term does not exist or positions were not indexed.
     *
     * @see #getTermPositionsEnum(IndexReader, Bits, String, BytesRef, int)
     */
    public static DocsAndPositionsEnum getTermPositionsEnum(IndexReader r, Bits liveDocs,
    ...
    /**
     * Returns {@link DocsAndPositionsEnum} for the specified field & term, with control over
     * whether offsets and payloads are required.  Some codecs may be able to optimize their
     * implementation when offsets and/or payloads are not required. This will return null if the
     * field or term does not exist or positions were not indexed. See
    ...
    /**
     * Call this to get the (merged) FieldInfos representing the set of indexed fields <b>only</b>
     * for a composite reader.
     * <p>
     * NOTE: the returned field numbers will likely not correspond to the actual field numbers in
     * the underlying readers, and codec metadata ({@link FieldInfo#getAttribute(String)} will be
     * unavailable.
