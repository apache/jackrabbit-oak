 ...
 * call {@code seek()}, it is forward only; it throws {@link UnsupportedOperationException} when a
...
public abstract class FilteredTermsEnum extends TermsEnum {
    ...
    /**
     * Return value, if term should be accepted or the iteration should {@code END}. The
     * {@code *_SEEK} values denote, that after handling the current term the enum should call
     * {@link #nextSeekTerm} and step forward.
     *
     * @see #accept(BytesRef)
     */
    protected static enum AcceptStatus {
        ...
        YES,
        /**
         * Accept the term and advance ({@link FilteredTermsEnum#nextSeekTerm(BytesRef)}) to the
         * next term.
         */
        YES_AND_SEEK,
        /**
        ...
        NO,
        /**
         * Reject the term and advance ({@link FilteredTermsEnum#nextSeekTerm(BytesRef)}) to the
         * next term.
         */
        NO_AND_SEEK,
        /**
     ...
     * {@link AcceptStatus#YES_AND_SEEK} or {@link AcceptStatus#NO_AND_SEEK}, this method will be
     * called to eventually seek the underlying TermsEnum to a new position. On the first call,
     * {@code currentTerm} will be {@code null}, later calls will provide the term the underlying
     * enum is positioned at. This method returns per default only one time the initial seek term
     * and then {@code null}, so no repositioning is ever done.
     * <p>Override this method, if you want a more sophisticated TermsEnum,
     * that repositions the iterator during enumeration. If this method always returns {@code null}
     * the enum is empty.
     * <p><em>Please note:</em> This method should always provide a greater term
     * than the last enumerated term, else the behaviour of this enum violates the contract for
     * TermsEnums.
     */
    ...
    public BytesRef next() throws IOException {
        ...
        for (; ; ) {
            ...
            if (doSeek) {
                ...
                assert
                    actualTerm == null || t == null || getComparator().compare(t, actualTerm) > 0 :
