...
final class StandardDirectoryReader extends DirectoryReader {
        ...
        return (DirectoryReader) new SegmentInfos.FindSegmentsFile(directory) {
            ...
            protected Object doBody(String segmentFileName) throws IOException {
                ...
                for (int i = sis.size() - 1; i >= 0; i--) {
                    ...
                    try {
                        readers[i] = new SegmentReader(sis.info(i), termInfosIndexDivisor,
                            IOContext.READ);
                        success = true;
                ...
                return new StandardDirectoryReader(directory, readers, null, sis,
                    termInfosIndexDivisor, false);
            }
        }.run(commit);
    }
     ...
     */
    private static DirectoryReader open(Directory directory, SegmentInfos infos,
        List<? extends AtomicReader> oldReaders,
        ...
        for (int i = infos.size() - 1; i >= 0; i--) {
            ...
            try {
                ...
                if (newReaders[i] == null || infos.info(i).info.getUseCompoundFile()
                    != newReaders[i].getSegmentInfo().info.getUseCompoundFile()) {

                    // this is a new reader; in case we hit an exception we can close it safely
                    newReader = new SegmentReader(infos.info(i), termInfosIndexDivisor,
                        IOContext.READ);
                    readerShared[i] = false;
                    newReaders[i] = newReader;
                } else {
                    if (newReaders[i].getSegmentInfo().getDelGen() == infos.info(i).getDelGen()
                        && newReaders[i].getSegmentInfo().getFieldInfosGen() == infos.info(i)
                                                                                     .getFieldInfosGen()) {
                        ...
                        if (newReaders[i].getSegmentInfo().getDelGen() == infos.info(i)
                                                                               .getDelGen()) {
                            // only DV updates
                            newReaders[i] = new SegmentReader(infos.info(i), newReaders[i],
                                newReaders[i].getLiveDocs(), newReaders[i].numDocs());
                        } else {
        ...
        return new StandardDirectoryReader(directory, newReaders, null, infos,
            termInfosIndexDivisor, false);
    }
    ...
    private DirectoryReader doOpenNoWriter(IndexCommit commit) throws IOException {
        ...
        if (commit == null) {
            ...
            if (segmentInfos != null && commit.getSegmentsFileName()
                                              .equals(segmentInfos.getSegmentsFileName())) {
                return null;
            }
        }
