...
/**
 * A <code>FilterAtomicReader</code> contains another AtomicReader, which it uses as its basic
 ...
 * of <code>IndexReader</code> with versions that pass all requests to the contained index reader.
 * Subclasses of <code>FilterAtomicReader</code> may further override some of these methods and may
 ...
 * content the contained reader, you could consider overriding {@link #getCoreCacheKey()} so that
 * {@link FieldCache} and {@link CachingWrapperFilter} share the same entries for this atomic reader
 * and the wrapped one. {@link #getCombinedCoreAndDeletesKey()} could be overridden as well if the
 * {@link #getLiveDocs() live docs} are not changed either.
 */
public class FilterAtomicReader extends AtomicReader {
     ...
     * these terms are going to be intersected with automata, you could consider overriding
     * {@link #intersect} for better performance.
     */
