...
public final class FieldInfo {
    ...
    public static enum IndexOptions {
         ...
         * positional queries on the field will throw an exception, and scoring will behave as if
         * any term in the document appears only once.
         */
        // TODO: maybe rename to just DOCS?
        DOCS_ONLY,
        /**
         * Only documents and term frequencies are indexed: positions are omitted. This enables
         * normal scoring, except Phrase and other positional queries will throw an exception.
         */
        DOCS_AND_FREQS,
        /**
        ...
        /**
         * Indexes documents, frequencies, positions and offsets. Character offsets are encoded
    ...
    /**
     * DocValues types. Note that DocValues is strongly typed, so a field cannot have different
     * types across different documents.
     */
    public static enum DocValuesType {
        ...
        NUMERIC,
        /**
         * A per-document byte[].  Values may be larger than 32766 bytes, but different codecs may
         * enforce their own limits.
         */
        BINARY,
        /**
         * A pre-sorted byte[]. Fields with this type only store distinct byte values and store an
         * additional offset pointer per document to dereference the shared byte[]. The stored
         * byte[] is presorted and allows access via document id, ordinal and by-value.  Values must
         * be <= 32766 bytes.
         */
        SORTED,
        /**
         * A pre-sorted Set&lt;byte[]&gt;. Fields with this type only store distinct byte values and
         * store additional offset pointers per document to dereference the shared byte[]s. The
         * stored byte[] is presorted and allows access via document id, ordinal and by-value.
        ...
        if (indexed) { // if updated field data is not for indexing, leave the updates out
            ...
            if (this.indexOptions != indexOptions) {
                if (this.indexOptions == null) {
                    ...
                    // downgrade
                    this.indexOptions =
                        this.indexOptions.compareTo(indexOptions) < 0 ? this.indexOptions
                            : indexOptions;
                }
    ...
    void setDocValuesType(DocValuesType type) {
        if (docValueType != null && docValueType != type) {
            throw new IllegalArgumentException(
                "cannot change DocValues type from " + docValueType + " to " + type
                    + " for field \"" + name + "\"");
        }
        docValueType = type;
        assert checkConsistency();
    ...
    /**
     * Returns {@link DocValuesType} of the docValues. this may be null if the field has no
     * docvalues.
     */
    ...
    void setNormValueType(DocValuesType type) {
        if (normType != null && normType != type) {
            throw new IllegalArgumentException(
                "cannot change Norm type from " + normType + " to " + type + " for field \"" + name
                    + "\"");
        }
        normType = type;
        assert checkConsistency();
