 ...
 * once, and the maximum number of simultaneous merges with {@link #setMaxMergesAndThreads}.</p>
 *
 * <p>If the number of merges exceeds the max number of threads
 * then the largest merges are paused until one of the smaller merges completes.</p>
 *
 * <p>If more than {@link #getMaxMergeCount} merges are
 * requested then this class will forcefully throttle the incoming threads by pausing until one more
 * more merges complete.</p>
 */
public class ConcurrentMergeScheduler extends MergeScheduler {
    ...
    /**
     * Default {@code maxThreadCount}. We default to 1: tests on spinning-magnet drives showed
     * slower indexing performance if more than one merge thread runs at once (though on an SSD it
     * was faster)
     */
     ...
     *                       yet we already have this many threads running, the incoming thread
     *                       (that is calling add/updateDocument) will block until a merge thread
     *                       has completed.  Note that we will only run the smallest
     *                       <code>maxThreadCount</code> merges at a time.
     * @param maxThreadCount the max # simultaneous merge threads that should be running at once.
     *                       This must be &lt;= <code>maxMergeCount</code>
     */
    ...
    /**
     * Set the base priority that merge threads run at. Note that CMS may increase priority of some
     * merge threads beyond this base priority.  It's best not to set this any higher than
     * Thread.MAX_PRIORITY-maxThreadCount, so that CMS has room to set relative priority among
     * threads.
     */
    public synchronized void setMergeThreadPriority(int pri) {
        if (pri > Thread.MAX_PRIORITY || pri < Thread.MIN_PRIORITY) {
            throw new IllegalArgumentException(
                "priority must be in range " + Thread.MIN_PRIORITY + " .. " + Thread.MAX_PRIORITY
                    + " inclusive");
        }
    ...
    /**
     * Called whenever the running merges have changed, to pause & unpause threads. This method
     * sorts the merge threads by their merge size in descending order and then pauses/unpauses
     * threads from first to last -- that way, smaller merges are guaranteed to run before larger
     * ones.
     */
    ...
    /**
     * Returns the number of merge threads that are alive. Note that this number is &le;
    ...
    public synchronized void merge(IndexWriter writer) throws IOException {
        ...
        while (true) {
            ...
            if (verbose()) {
                if (startStallTime != 0) {
                    message(
                        "  stalled for " + (System.currentTimeMillis() - startStallTime) + " msec");
                }
            }
    ...
    protected class MergeThread extends Thread {
        ...
        /**
         * Return the current merge, or null if this {@code MergeThread} is done.
         */
            ...
            try {
                ...
                while (true) {
                    ...
                    if (merge != null) {
                        updateMergeThreads();
                        if (verbose()) {
                            message("  merge thread: do another merge " + tWriter.segString(
                                merge.segments));
                        }
                    } else {
                        break;
