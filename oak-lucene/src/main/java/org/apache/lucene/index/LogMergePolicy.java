 ...
 * to merge segments into levels of exponentially increasing size, where each level has fewer
 * segments than the value of the merge factor. Whenever extra segments (beyond the merge factor
 * upper bound) are encountered, all segments within the level are merged. You can get or set the
 * merge factor using {@link #getMergeFactor()} and {@link #setMergeFactor(int)} respectively.</p>
 *
 * <p>This class is abstract and requires a subclass to
 * define the {@link #size} method which specifies how a segment's size is determined.
 * {@link LogDocMergePolicy} is one subclass that measures size by document count in the segment.
 * {@link LogByteSizeMergePolicy} is another subclass that measures size as the total byte size of
 * the file(s) for the segment.</p>
 */
...
public abstract class LogMergePolicy extends MergePolicy {
     ...
     * once and also controls the total number of segments allowed to accumulate in the index.</p>
     */
     ...
     * RAM is used while indexing, and searches are faster, but indexing speed is slower.  With
     * larger values, more RAM is used during indexing, and while searches is slower, indexing is
     * faster.  Thus larger values (> 10) are best for batch index creation, and smaller values (<
            ...
            if (size(info) > maxMergeSizeForForcedMerge || sizeDocs(info) > maxMergeDocs) {
                if (verbose()) {
                    message("findForcedMergesSizeLimit: skip segment=" + info
                        + ": size is > maxMergeSize (" + maxMergeSizeForForcedMerge
                        + ") or sizeDocs is > maxMergeDocs (" + maxMergeDocs + ")");
                }
                // need to skip that segment + add a merge for the 'right' segments,
                // unless there is only 1 which is merged.
                if (last - start - 1 > 1 || (start != last - 1 && !isMerged(infos,
                    infos.info(start + 1)))) {
     ...
     * merges only by the {@code maxNumSegments} parameter, and guaranteed that exactly that number
     * of segments will remain in the index.
     */
    private MergeSpecification findForcedMergesMaxNumSegments(SegmentInfos infos,
     ...
     * respects the {@link #maxMergeSizeForForcedMerge} setting. By default, and assuming
     * {@code maxNumSegments=1}, only one segment will be left in the index, where that segment has
     * no deletions pending nor separate norms, and it is in compound file format if the current
     * useCompoundFile setting is true.  This method returns multiple merges (mergeFactor at a time)
     * so the {@link MergeScheduler} in use may make use of concurrency.
     */
    @Override
        ...
        for (int i = 0; i < numSegments; i++) {
            ...
            if (delCount > 0) {
                ...
                if (firstSegmentWithDeletions == -1) {
                    ...
                    if (verbose()) {
                        message("  add merge " + firstSegmentWithDeletions + " to " + (i - 1)
                            + " inclusive");
                    }
    ...
    /**
     * Checks if any merges are now necessary and returns a {@link MergePolicy.MergeSpecification}
     * if so.  A merge is necessary when there are more than {@link #setMergeFactor} segments at a
     * given level.  When multiple levels have too many segments, this method will return multiple
     * merges, allowing the {@link MergeScheduler} to use concurrency.
     */
    @Override
    public MergeSpecification findMerges(MergeTrigger mergeTrigger, SegmentInfos infos)
        ...
        for (int i = 0; i < numSegments; i++) {
            ...
            if (verbose()) {
                ...
                message(
                    "seg=" + writer.get().segString(info) + " level=" + infoLevel.level + " size="
                        + String.format(Locale.ROOT, "%.3f MB", segBytes / 1024 / 1024.) + extra);
            }
        }
        ...
        while (start < numMergeableSegments) {
            ...
            while (end <= 1 + upto) {
                ...
                if (anyMerging) {
                    ...
                    if (verbose()) {
                        message(
                            "  add merge=" + writer.get().segString(mergeInfos) + " start=" + start
                                + " end=" + end);
                    }
     ...
     * document count) that may be merged with other segments. Small values (e.g., less than 10,000)
     * are best for interactive indexing, as this limits the length of pauses while indexing to a
     * few seconds.  Larger values are best for batched indexing and speedier searches.</p>
     *
     * <p>The default value is {@link Integer#MAX_VALUE}.</p>
     *
     * <p>The default merge policy ({@link
     * LogByteSizeMergePolicy}) also allows you to set this limit by net size (in MB) of the
     * segment, using {@link LogByteSizeMergePolicy#setMaxMergeMB}.</p>
     */
