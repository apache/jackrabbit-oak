...
/**
 * Holds numeric values updates of documents, of a single {@link NumericDocValuesField}.
 *
 * @lucene.experimental
 */
interface NumericFieldUpdates {
    ...
    static abstract class UpdatesIterator {
        ...
        /**
         * Returns the next document which has an update, or {@link DocIdSetIterator#NO_MORE_DOCS}
         * if there are no more documents to return.
         */
        ...
        /**
         * Returns the value of the document returned from {@link #nextDoc()}. A {@code null} value
         * means that it was unset for this document.
         */
        abstract Long value();

        /**
         * Reset the iterator's state. Should be called before {@link #nextDoc()} and
         * {@link #value()}.
         */
        abstract void reset();
    ...
    /**
     * A {@link NumericFieldUpdates} which holds the updated documents and values in packed
     * structures. Only supports up to 2B entries (docs and values) since we need to sort the
     * docs/values and the Sorter interfaces currently only take integer indexes.
     */
    static final class PackedNumericFieldUpdates implements NumericFieldUpdates {
        ...
        public PackedNumericFieldUpdates(int maxDoc) {
            ...
            docs = new PagedMutable(1, 1024, PackedInts.bitsRequired(maxDoc - 1),
                PackedInts.COMPACT);
    ...
    /**
     * Returns an {@link UpdatesIterator} over the updated documents and their values.
     */
    public UpdatesIterator getUpdates();

    /**
     * Merge with another {@link NumericFieldUpdates}. This is called for a segment which received
     * updates while it was being merged. The given updates should override whatever numeric updates
     * are in that instance.
     */
    public void merge(NumericFieldUpdates other);
