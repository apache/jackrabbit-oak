...
/**
 * Writes norms.  Each thread X field accumulates the norms for the doc/fields it saw, then the
 * flush method below merges all of these together into a single _X.nrm file.
 */
...
final class NormsConsumer extends InvertedDocEndConsumer {
    ...
    @Override
    public void flush(Map<String, InvertedDocEndConsumerPerField> fieldsToFlush,
        ...
        try {
            if (state.fieldInfos.hasNorms()) {
                ...
                for (FieldInfo fi : state.fieldInfos) {
                    final NormsConsumerPerField toWrite = (NormsConsumerPerField) fieldsToFlush.get(
                        fi.name);
                    ...
                    if (!fi.omitsNorms()) {
                        if (toWrite != null && !toWrite.isEmpty()) {
                            ...
                            assert
                                ...
                                "got " + fi.getNormType() + "; field=" + fi.name;
                        }
                    }
                }
            }
