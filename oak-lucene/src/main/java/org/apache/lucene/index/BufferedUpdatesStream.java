...
class BufferedUpdatesStream {
    ...
    public synchronized long push(FrozenBufferedUpdates packet) {
        ...
        assert updates.isEmpty() || updates.get(updates.size() - 1).delGen()
            < packet.delGen() : "Delete packets must be in order";
        updates.add(packet);
        numTerms.addAndGet(packet.numTermDeletes);
        bytesUsed.addAndGet(packet.bytesUsed);
        if (infoStream.isEnabled("BD")) {
            infoStream.message("BD",
                "push deletes " + packet + " delGen=" + packet.delGen() + " packetCount="
                    + updates.size() + " totBytesUsed=" + bytesUsed.get());
        }
        assert checkDeleteStats();
     ...
     */
    public synchronized ApplyDeletesResult applyDeletesAndUpdates(IndexWriter.ReaderPool readerPool,
        List<SegmentCommitInfo> infos) throws IOException {
        ...
        if (infoStream.isEnabled("BD")) {
            infoStream.message("BD",
                "applyDeletes: infos=" + infos + " packetCount=" + updates.size());
        }
...
//        System.out.println("  coalesce");
                ...
                try {
                    ...
                    if (coalescedUpdates != null) {
                        ...
                        delCount += applyQueryDeletes(coalescedUpdates.queriesIterable(), rld,
                            reader);
                        fieldUpdates = applyNumericDocValuesUpdates(
                    ...
                    fieldUpdates = applyNumericDocValuesUpdates(Arrays.asList(packet.updates), rld,
                        reader, fieldUpdates);
                ...
                if (infoStream.isEnabled("BD")) {
                    infoStream.message("BD",
                        "seg=" + info + " segGen=" + segGen + " segDeletes=[" + packet
                            + "]; coalesced deletes=[" + (coalescedUpdates == null ? "null"
                            : coalescedUpdates) + "] newDelCount=" + delCount + (segAllDeletes
                            ? " 100% deleted" : ""));
                }
                ...
                if (coalescedUpdates != null) {
                    ...
                    try {
                        ...
                        delCount += applyQueryDeletes(coalescedUpdates.queriesIterable(), rld,
                            reader);
                        Map<String, NumericFieldUpdates> fieldUpdates = applyNumericDocValuesUpdates(
                            coalescedUpdates.numericDVUpdates, rld, reader, null);
                        if (!fieldUpdates.isEmpty()) {
                            rld.writeFieldUpdates(info.info.dir, fieldUpdates);
                        }
                    ...
                    if (infoStream.isEnabled("BD")) {
                        infoStream.message("BD",
                            "seg=" + info + " segGen=" + segGen + " coalesced deletes=["
                                + coalescedUpdates + "] newDelCount=" + delCount + (segAllDeletes
                                ? " 100% deleted" : ""));
                    }
                }
    ...
    public synchronized void prune(SegmentInfos segmentInfos) {
        ...
        if (infoStream.isEnabled("BD")) {
            infoStream.message("BD",
                "prune sis=" + segmentInfos + " minGen=" + minGen + " packetCount="
                    + updates.size());
        }
    ...
    private synchronized void prune(int count) {
        if (count > 0) {
            if (infoStream.isEnabled("BD")) {
                infoStream.message("BD",
                    "pruneDeletes: prune " + count + " packets; " + (updates.size() - count)
                        + " packets remain");
            }
    ...
    // Delete by Term
    private synchronized long applyTermDeletes(Iterable<Term> termsIter, ReadersAndUpdates rld,
    ...
    private synchronized Map<String, NumericFieldUpdates> applyNumericDocValuesUpdates(
        ...
        ReadersAndUpdates rld, SegmentReader reader,
        Map<String, NumericFieldUpdates> otherFieldUpdates) throws IOException {
        Fields fields = reader.fields();
        if (fields == null) {
            // This reader has no postings
        ...
        final Map<String, NumericFieldUpdates> result =
            otherFieldUpdates == null ? new HashMap<String, NumericFieldUpdates>()
                : otherFieldUpdates;
...
//        assert currentField == null || currentField.compareTo(term.field()) < 0;
    ...
    private static long applyQueryDeletes(Iterable<QueryAndLimit> queriesIter,
        ReadersAndUpdates rld, final SegmentReader reader) throws IOException {
        long delCount = 0;
    ...
    private boolean checkDeleteTerm(Term term) {
        if (term != null) {
            assert
                lastDeleteTerm == null || term.compareTo(lastDeleteTerm) > 0 :
        ...
        lastDeleteTerm =
            term == null ? null : new Term(term.field(), BytesRef.deepCopyOf(term.bytes));
        return true;
    }
