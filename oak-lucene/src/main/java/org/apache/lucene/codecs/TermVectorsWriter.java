...
public abstract class TermVectorsWriter implements Closeable {
     ...
     * {@link #startField(FieldInfo, int, boolean, boolean, boolean)} will be called
     * <code>numVectorFields</code> times. Note that if term vectors are enabled, this is called
     * even if the document has no vector fields, in this case <code>numVectorFields</code> will be
     * zero.
     */
    ...
    /**
     * Called before writing the terms of the field. {@link #startTerm(BytesRef, int)} will be
     * called <code>numTerms</code> times.
     */
    public abstract void startField(FieldInfo info, int numTerms, boolean positions,
     ...
     * offsets enabled, then {@link #addPosition(int, int, int, BytesRef)} will be called
    ...
    /**
     * Called before {@link #close()}, passing in the number of documents that were written. Note
     * that this is intentionally redundant (equivalent to the number of calls to
     * {@link #startDocument(int)}, but a Codec should check that this is the case to detect the JRE
     * bug described in LUCENE-1282.
     */
     ...
     * the indexer.
     * <p>
     * The default implementation calls {@link #addPosition(int, int, int, BytesRef)}, but
     * subclasses can override this if they want to efficiently write all the positions, then all
     * the offsets, for example.
     * <p>
     ...
     * {@link #startTerm(BytesRef, int)}, {@link #addPosition(int, int, int, BytesRef)}, and
     * {@link #finish(FieldInfos, int)}, returning the number of documents that were written.
    ...
    protected final void addAllDocVectors(Fields vectors, MergeState mergeState)
        ...
        for (String fieldName : vectors) {
            ...
            assert
                lastFieldName == null || fieldName.compareTo(lastFieldName) > 0 :
