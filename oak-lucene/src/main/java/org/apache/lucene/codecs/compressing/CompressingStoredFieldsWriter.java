    ...
    private static void saveInts(int[] values, int length, DataOutput out) throws IOException {
        ...
        if (length == 1) {
            ...
            if (allEqual) {
                ...
                final PackedInts.Writer w = PackedInts.getWriterNoHeader(out,
                    PackedInts.Format.PACKED, length, bitsRequired, 1);
                for (int i = 0; i < length; ++i) {
                    w.add(values[i]);
                }
                w.finish();
        ...
        if (number != null) {
            if (number instanceof Byte || number instanceof Short || number instanceof Integer) {
                ...
                throw new IllegalArgumentException(
                    "cannot store numeric type " + number.getClass());
            }
            string = null;
            bytes = null;
        } else {
    ...
    public int merge(MergeState mergeState) throws IOException {
        ...
        for (AtomicReader reader : mergeState.readers) {
            ...
            if (matchingFieldsReader == null
                ...
                || matchingFieldsReader.getChunkSize()
                != chunkSize) { // the way data is decompressed depends on the chunk size
                // naive merge...
                        ...
                        if (numBufferedDocs == 0 // starting a new chunk
                            ...
                            if (startOffsets[it.chunkDocs - 1] + it.lengths[it.chunkDocs - 1]
                                ...
                                throw new CorruptIndexException(
                                    "Corrupted: expected chunk size=" + startOffsets[it.chunkDocs
                                        - 1] + it.lengths[it.chunkDocs - 1] + ", got "
                                        + it.bytes.length);
                            }
                            // copy non-deleted docs
                            for (; docID < it.docBase + it.chunkDocs;
                                docID = nextLiveDoc(docID + 1, liveDocs, maxDoc)) {
                                final int diff = docID - it.docBase;
                                startDocument(it.numStoredFields[diff]);
                                bufferedDocs.writeBytes(it.bytes.bytes,
                                    it.bytes.offset + startOffsets[diff], it.lengths[diff]);
                                finishDocument();
                                ++docCount;
                                mergeState.checkAbort.work(300);
                            }
