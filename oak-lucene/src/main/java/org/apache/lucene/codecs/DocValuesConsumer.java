...
/**
 * Abstract API that consumes numeric, binary and sorted docvalues.  Concrete implementations of
 * this actually do "something" with the docvalues (write it into the index in a specific format).
 * <p>
 * The lifecycle is:
 * <ol>
...
public abstract class DocValuesConsumer implements Closeable {
     ...
     * @param values Iterable of numeric values (one for each document). {@code null} indicates a
     *               missing value.
     * @throws IOException if an I/O error occurred.
     */
    public abstract void addNumericField(FieldInfo field, Iterable<Number> values)
        throws IOException;
     ...
     *
     * @param field  field information
     * @param values Iterable of binary values (one for each document). {@code null} indicates a
     *               missing value.
     * @throws IOException if an I/O error occurred.
     */
    public abstract void addBinaryField(FieldInfo field, Iterable<BytesRef> values)
        throws IOException;
     ...
     * @param docToOrd Iterable of ordinals (one for each document). {@code -1} indicates a missing
     *                 value.
     * @throws IOException if an I/O error occurred.
     */
    public abstract void addSortedField(FieldInfo field, Iterable<BytesRef> values,
     ...
     * @param field         field information
     * @param values        Iterable of binary values in sorted order (deduplicated).
     * @param docToOrdCount Iterable of the number of values for each document. A zero ordinal count
     *                      indicates a missing value.
     * @param ords          Iterable of ordinal occurrences (docToOrdCount*maxDoc total).
     * @throws IOException if an I/O error occurred.
     */
    public abstract void addSortedSetField(FieldInfo field, Iterable<BytesRef> values,
        Iterable<Number> docToOrdCount, Iterable<Number> ords) throws IOException;

    /**
     * Merges the numeric docvalues from <code>toMerge</code>.
     * <p>
     * The default implementation calls {@link #addNumericField}, passing an Iterable that merges
     * and filters deleted documents on the fly.
     */
    public void mergeNumericField(final FieldInfo fieldInfo, final MergeState mergeState,
        final List<NumericDocValues> toMerge, final List<Bits> docsWithField) throws IOException {
     ...
     * <p>
     * The default implementation calls {@link #addBinaryField}, passing an Iterable that merges and
     * filters deleted documents on the fly.
     */
    public void mergeBinaryField(FieldInfo fieldInfo, final MergeState mergeState,
        final List<BinaryDocValues> toMerge, final List<Bits> docsWithField) throws IOException {
     ...
     * <p>
     * The default implementation calls {@link #addSortedField}, passing an Iterable that merges
    ...
    public void mergeSortedField(FieldInfo fieldInfo, final MergeState mergeState,
        List<SortedDocValues> toMerge) throws IOException {
                        ...
                        private boolean setNext() {
                            while (true) {
                                ...
                                if (currentLiveDocs == null || currentLiveDocs.get(docIDUpto)) {
                                    ...
                                    nextValue = segOrd == -1 ? -1
                                        : (int) map.getGlobalOrd(readerUpto, segOrd);
                                    docIDUpto++;
                                    return true;
                                }
     ...
     * <p>
     * The default implementation calls {@link #addSortedSetField}, passing an Iterable that merges
     * ordinals and values and filters deleted documents .
     */
    public void mergeSortedSetField(FieldInfo fieldInfo, final MergeState mergeState,
        List<SortedSetDocValues> toMerge) throws IOException {
        final AtomicReader readers[] = mergeState.readers.toArray(new AtomicReader[toMerge.size()]);
    ...
    static class BitsFilteredTermsEnum extends FilteredTermsEnum {
        ...
        BitsFilteredTermsEnum(TermsEnum in, LongBitSet liveTerms) {
            super(in,
                false); // <-- not passing false here wasted about 3 hours of my time!!!!!!!!!!!!!
