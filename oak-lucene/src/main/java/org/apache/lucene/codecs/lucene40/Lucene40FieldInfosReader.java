...
class Lucene40FieldInfosReader extends FieldInfosReader {
    ...
    public FieldInfos read(Directory directory, String segmentName, String segmentSuffix,
        ...
        final String fileName = IndexFileNames.segmentFileName(segmentName, "",
            Lucene40FieldInfosFormat.FIELD_INFOS_EXTENSION);
        ...
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                final LegacyDocValuesType oldNormsType = getDocValuesType(
                    (byte) ((val >>> 4) & 0x0F));
                ...
                if (oldNormsType.mapping != null) {
                    if (oldNormsType.mapping != DocValuesType.NUMERIC) {
                        throw new CorruptIndexException(
                            "invalid norm type: " + oldNormsType + " (resource=" + input + ")");
                    }
                    attributes.put(LEGACY_NORM_TYPE_KEY, oldNormsType.name());
                }
                infos[i] = new FieldInfo(name, isIndexed, fieldNumber, storeTermVector,
                    omitNorms, storePayloads, indexOptions, oldValuesType.mapping,
                    oldNormsType.mapping, Collections.unmodifiableMap(attributes));
            }
    ...
    static final String LEGACY_DV_TYPE_KEY =
        Lucene40FieldInfosReader.class.getSimpleName() + ".dvtype";
    static final String LEGACY_NORM_TYPE_KEY =
        Lucene40FieldInfosReader.class.getSimpleName() + ".normtype";
