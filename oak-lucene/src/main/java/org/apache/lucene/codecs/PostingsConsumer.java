...
public abstract class PostingsConsumer {
    ...
    /**
     * Add a new position & payload, and start/end offset.  A null payload means no payload; a
     * non-null payload with zero length also means no payload.  Caller may reuse the
     * {@link BytesRef} for the payload between calls (method must fully consume the payload).
     * <code>startOffset</code> and <code>endOffset</code> will be -1 when offsets are not indexed.
     */
     ...
     */
    public TermStats merge(final MergeState mergeState, IndexOptions indexOptions,
        final DocsEnum postings, final FixedBitSet visitedDocs) throws IOException {
        ...
        if (indexOptions == IndexOptions.DOCS_ONLY) {
            ...
            while (true) {
                ...
                for (int i = 0; i < freq; i++) {
                    final int position = postingsEnum.nextPosition();
                    final BytesRef payload = postingsEnum.getPayload();
                    this.addPosition(position, payload, postingsEnum.startOffset(),
