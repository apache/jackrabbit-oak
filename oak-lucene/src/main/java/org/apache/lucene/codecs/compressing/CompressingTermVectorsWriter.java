...
public final class CompressingTermVectorsWriter extends TermVectorsWriter {
    ...
    private class DocData {
        ...
        FieldData addField(int fieldNum, int numTerms, boolean positions, boolean offsets,
            boolean payloads) {
            final FieldData field;
            if (fields.isEmpty()) {
                field = new FieldData(fieldNum, numTerms, positions, offsets, payloads, posStart,
                    offStart, payStart);
            } else {
     ...
     * Sole constructor.
     */
    public CompressingTermVectorsWriter(Directory directory, SegmentInfo si, String segmentSuffix,
        IOContext context,
        ...
        IndexOutput indexStream = directory.createOutput(
            ...
            context);
        try {
            vectorsStream = directory.createOutput(
    ...
    private int[] flushFieldNums() throws IOException {
        ...
        final PackedInts.Writer writer = PackedInts.getWriterNoHeader(vectorsStream,
            PackedInts.Format.PACKED, fieldNums.size(), bitsRequired, 1);
    ...
    private void flushFields(int totalFields, int[] fieldNums) throws IOException {
        final PackedInts.Writer writer = PackedInts.getWriterNoHeader(vectorsStream,
            PackedInts.Format.PACKED, totalFields, PackedInts.bitsRequired(fieldNums.length - 1),
            1);
        for (DocData dd : pendingDocs) {
            for (FieldData fd : dd.fields) {
    ...
    private void flushFlags(int totalFields, int[] fieldNums) throws IOException {
        ...
        if (nonChangingFlags) {
            ...
            final PackedInts.Writer writer = PackedInts.getWriterNoHeader(vectorsStream,
                PackedInts.Format.PACKED, fieldFlags.length, FLAGS_BITS, 1);
            ...
            final PackedInts.Writer writer = PackedInts.getWriterNoHeader(vectorsStream,
                PackedInts.Format.PACKED, totalFields, FLAGS_BITS, 1);
            for (DocData dd : pendingDocs) {
                for (FieldData fd : dd.fields) {
                    writer.add(fd.flags);
                }
    ...
    private void flushOffsets(int[] fieldNums) throws IOException {
        ...
        for (int i = 0; i < fieldNums.length; ++i) {
            charsPerTerm[i] = (sumPos[i] <= 0 || sumOffsets[i] <= 0) ? 0
                : (float) ((double) sumOffsets[i] / sumPos[i]);
        }
                ...
                if ((fd.flags & OFFSETS) != 0) {
                    ...
                    for (int i = 0; i < fd.numTerms; ++i) {
                        ...
                        for (int j = 0; j < fd.freqs[i]; ++j) {
                            ...
                            final int startOffset = startOffsetsBuf[fd.offStart + pos];
                            writer.add(
                                startOffset - previousOff - (int) (cpt * (position - previousPos)));
                            previousPos = position;
                            previousOff = startOffset;
                            ++pos;
                        }
        ...
        for (DocData dd : pendingDocs) {
            for (FieldData fd : dd.fields) {
                if ((fd.flags & OFFSETS) != 0) {
                    ...
                    for (int i = 0; i < fd.numTerms; ++i) {
                        for (int j = 0; j < fd.freqs[i]; ++j) {
                            writer.add(lengthsBuf[fd.offStart + pos++] - fd.prefixLengths[i]
                                - fd.suffixLengths[i]);
                        }
                    }
                    assert pos == fd.totalPositions;
        ...
        for (AtomicReader reader : mergeState.readers) {
            ...
            if (matchingSegmentReader != null) {
                ...
                if (vectorsReader != null
                    && vectorsReader instanceof CompressingTermVectorsReader) {
            ...
            if (matchingVectorsReader == null
                ...
                for (int i = nextLiveDoc(0, liveDocs, maxDoc); i < maxDoc; ) {
                    if (pendingDocs.isEmpty()
                        && (i == 0 || index.getStartPointer(i - 1) < index.getStartPointer(
                        i))) { // start of a chunk
                        ...
                        if (docBase + chunkDocs < matchingSegmentReader.maxDoc()
                            && nextDeletedDoc(docBase, liveDocs, docBase + chunkDocs)
                            == docBase + chunkDocs) {
