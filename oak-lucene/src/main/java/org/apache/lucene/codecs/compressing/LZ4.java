...
final class LZ4 {
     ...
     * enough to be able to hold <b>all</b> decompressed data (meaning that you need to know the
    ...
    public static int decompress(DataInput compressed, int decompressedLen, byte[] dest, int dOff)
        ...
        do {
            ...
            // matchs
            final int matchDec =
                (compressed.readByte() & 0xFF) | ((compressed.readByte() & 0xFF) << 8);
            assert matchDec > 0;
    ...
    private static void encodeLiterals(byte[] bytes, int token, int anchor, int literalLen,
    ...
    private static void encodeSequence(byte[] bytes, int anchor, int matchRef, int matchOff,
        int matchLen, DataOutput out) throws IOException {
    ...
    /**
     * Compress <code>bytes[off:off+len]</code> into <code>out</code> using at most 16KB of memory.
     * <code>ht</code> shouldn't be shared across threads but can safely be reused.
     */
    public static void compress(byte[] bytes, int off, int len, DataOutput out, HashTable ht)
        ...
        if (len > LAST_LITERALS + MIN_MATCH) {
            ...
            while (off <= limit) {
                ...
                final int matchLen =
                    MIN_MATCH + commonBytes(bytes, ref + MIN_MATCH, off + MIN_MATCH, limit);
    ...
    static final class HCHashTable {
        ...
        boolean insertAndFindBestMatch(byte[] buf, int off, int matchLimit, Match match) {
            ...
            if (ref >= off - 4 && ref <= off && ref >= base) { // potential repetition
                if (readIntEquals(buf, ref, off)) { // confirmed
                    delta = off - ref;
                    repl = match.len =
                        MIN_MATCH + commonBytes(buf, ref + MIN_MATCH, off + MIN_MATCH, matchLimit);
                    match.ref = ref;
                }
                ref = next(ref);
            }
        ...
        boolean insertAndFindWiderMatch(byte[] buf, int off, int startLimit, int matchLimit,
            int minLen, Match match) {
            match.len = minLen;
            ...
            for (int i = 0; i < MAX_ATTEMPTS; ++i) {
                ...
                if (buf[ref - delta + match.len] == buf[startLimit + match.len]
                    ...
                    final int matchLenForward =
                        MIN_MATCH + commonBytes(buf, ref + MIN_MATCH, off + MIN_MATCH, matchLimit);
                    final int matchLenBackward = commonBytesBackward(buf, ref, off, base,
                        startLimit);
     ...
     * {@link LZ4#compress(byte[], int, int, DataOutput, HashTable)}, this method is slower and uses
     * more memory (~ 256KB per thread) but should provide better compression ratios (especially on
     * large inputs) because it chooses the best match among up to 256 candidates and then performs
     * trade-offs to fix overlapping matches. <code>ht</code> shouldn't be shared across threads but
     * can safely be reused.
     */
    public static void compressHC(byte[] src, int srcOff, int srcLen, DataOutput out,
        HCHashTable ht) throws IOException {
            ...
            while (true) {
                ...
                while (true) {
                    ...
                    if (match3.start
                        ...
                        if (match3.start
                            >= match1.end()) { // // can write Seq1 immediately ==> Seq2 is removed, so Seq3 becomes Seq1
                            if (match2.start < match1.end()) {
