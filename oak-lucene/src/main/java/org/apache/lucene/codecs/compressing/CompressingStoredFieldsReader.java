...
public final class CompressingStoredFieldsReader extends StoredFieldsReader {
     ...
     * Sole constructor.
     */
    public CompressingStoredFieldsReader(Directory d, SegmentInfo si, String segmentSuffix,
        FieldInfos fn,
        ...
        try {
            ...
            version = CodecUtil.checkHeader(indexStream, codecNameIdx, VERSION_START,
                VERSION_CURRENT);
            ...
            final String fieldsStreamFN = IndexFileNames.segmentFileName(segment, segmentSuffix,
                FIELDS_EXTENSION);
            fieldsStream = d.openInput(fieldsStreamFN, context);
            final String codecNameDat = formatName + CODEC_SFX_DAT;
            final int fieldsVersion = CodecUtil.checkHeader(fieldsStream, codecNameDat,
                VERSION_START, VERSION_CURRENT);
            if (version != fieldsVersion) {
                throw new CorruptIndexException(
                    "Version mismatch between stored fields index and data: " + version + " != "
                        + fieldsVersion);
            }
    ...
    private static void readField(DataInput in, StoredFieldVisitor visitor, FieldInfo info,
    ...
    public void visitDocument(int docID, StoredFieldVisitor visitor)
        ...
        if (chunkDocs == 1) {
            ...
            if (bitsPerStoredFields == 0) {
                ...
                fieldsStream.seek(
                    filePointer + PackedInts.Format.PACKED.byteCount(packedIntsVersion, chunkDocs,
                        bitsPerStoredFields));
            }
            ...
            if (bitsPerLength == 0) {
                ...
                final PackedInts.ReaderIterator it = PackedInts.getReaderIteratorNoHeader(
                    fieldsStream, PackedInts.Format.PACKED, packedIntsVersion, chunkDocs,
                    bitsPerLength, 1);
                int off = 0;
        ...
        if ((length == 0) != (numStoredFields == 0)) {
            throw new CorruptIndexException(
                "length=" + length + ", numStoredFields=" + numStoredFields + " (resource="
                    + fieldsStream + ")");
        }
        ...
        if (version >= VERSION_BIG_CHUNKS && totalLength >= 2 * chunkSize) {
            ...
            final BytesRef bytes =
                totalLength <= BUFFER_REUSE_THRESHOLD ? this.bytes : new BytesRef();
        ...
        void decompress() throws IOException {
            ...
            if (version >= VERSION_BIG_CHUNKS
                ...
                for (int decompressed = 0; decompressed < chunkSize; ) {
                    ...
                    System.arraycopy(spare.bytes, spare.offset, bytes.bytes, bytes.length,
                        spare.length);
            ...
            if (bytes.length != chunkSize) {
                throw new CorruptIndexException(
                    "Corrupted: expected chunk size = " + chunkSize() + ", got " + bytes.length
