...
/**
 * Optimized implementation of a vector of bits.  This is more-or-less like java.util.BitSet, but
...
final class BitVector implements Cloneable, MutableBits {
    ...
    /**
     * Sets the value of <code>bit</code> to true, and returns true if bit was already set
     */
    ...
    /**
     * Returns the number of bits in this vector.  This is also one greater than the number of the
     * largest valid bit number.
     */
    ...
    /**
     * Returns the total number of one bits in this vector.  This is efficiently computed and
     * cached, so that, if the vector is not changed, no recomputation is done for repeated calls.
     */
    ...
    private void writeClearedDgaps(IndexOutput output) throws IOException {
        ...
        for (int i = 0; i < bits.length && numCleared > 0; i++) {
            if (bits[i] != (byte) 0xff) {
                ...
                assert
                    numCleared >= 0 || (i == (bits.length - 1) && numCleared == -(8 - (size & 7)));
            }
        }
    }

    /**
     * Indicates if the bit vector is sparse and should be saved as a d-gaps list, or dense, and
     * should be saved as a bit set.
     */
    ...
    private boolean verifyCount() {
        ...
        assert
            countSav == count() :
            "saved count was " + countSav + " but recomputed count is " + count;
        return true;
    }
    ...
    private void readClearedDgaps(IndexInput input) throws IOException {
        ...
        while (numCleared > 0) {
            ...
            assert
                numCleared >= 0 || (last == (bits.length - 1) && numCleared == -(8 - (size & 7)));
        }
    }
}
