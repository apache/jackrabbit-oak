...
class SegmentTermDocs {
    ...
    void seek(TermInfo ti, Term term) throws IOException {
        ...
        this.indexOptions =
            (fi != null) ? fi.getIndexOptions() : IndexOptions.DOCS_AND_FREQS_AND_POSITIONS;
    ...
    private final int readNoTf(final int[] docs, final int[] freqs, final int length)
        throws IOException {
        int i = 0;
        while (i < length && count < df) {
            // manually inlined call to next() for speed
    ...
    public boolean skipTo(int target) throws IOException {
        ...
        if ((target - skipInterval) >= doc
            && df >= skipInterval) {                      // optimized case
            if (skipListReader == null) {
                skipListReader = new Lucene3xSkipListReader(freqStream.clone(), maxSkipLevels,
                    skipInterval); // lazily clone
            }

            if (!haveSkipped) {                          // lazily initialize skip stream
                skipListReader.init(skipPointer, freqBasePointer, proxBasePointer, df,
