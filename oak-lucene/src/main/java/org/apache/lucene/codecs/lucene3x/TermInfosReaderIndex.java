...
/**
 * This stores a monotonically increasing set of <Term, TermInfo> pairs in an index segment. Pairs
 * are accessed either by Term or by ordinal position the set. The Terms and TermInfo are actually
 * serialized and stored into a byte array and pointers to the position of each are stored in a int
 * array.
 *
...
class TermInfosReaderIndex {
     ...
     * @param tiiFileLength      the size of the tii file, used to approximate the size of the
     *                           buffer.
     * @param totalIndexInterval the total index interval.
     */
    TermInfosReaderIndex(SegmentTermEnum indexEnum, int indexDivisor, long tiiFileLength,
        int totalIndexInterval) throws IOException {
        ...
        ramBytesUsed = fields.length * (RamUsageEstimator.NUM_BYTES_OBJECT_REF
    ...
    /**
     * The compares the given term against the term in the index specified by the term index. ie It
     * returns negative N when term is less than index term;
     *
    ...
    /**
     * Compare the fields of the terms first, and if not equals return from compare. If equal
     * compare terms.
     *
