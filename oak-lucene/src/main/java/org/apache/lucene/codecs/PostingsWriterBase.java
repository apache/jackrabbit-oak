 ...
 * <p>
 * This class contains additional hooks to interact with the provided term dictionaries such as
 * {@link BlockTreeTermsWriter}. If you want to re-use an existing implementation and are only
 * interested in customizing the format of the postings list, extend this class instead.
 *
 * @lucene.experimental
...
public abstract class PostingsWriterBase extends PostingsConsumer implements Closeable {
    ...
    /**
     * Called once after startup, before any terms have been added.  Implementations typically write
     * a header to the provided {@code termsOut}.
     */
     ...
     * encoded according to latest term. Usually elements in {@code longs} are file pointers, so
     * each one always increases when a new term is consumed. {@code out} is used to write generic
     * bytes, which are not monotonic.
     * <p>
     * NOTE: sometimes long[] might contain "don't care" values that are unused, e.g. the pointer to
     * postings list may not be defined for some terms but is defined for others, if it is designed
     * to inline  some postings data in term dictionary. In this case, the postings writer should
     * always use the last value, so that each element in metadata long[] remains monotonic.
     */
