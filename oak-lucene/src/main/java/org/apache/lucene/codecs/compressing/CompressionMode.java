...
public abstract class CompressionMode {
     ...
     * might remain high, compression and decompression are very fast. Use this mode with indices
     * that have a high update rate but should be able to load documents from disk quickly.
     */
    ...
    /**
     * This compression mode is similar to {@link #FAST} but it spends more time compressing in
     * order to improve the compression ratio. This compression mode is best used with indices that
     * have a low update rate but should be able to load documents from disk quickly.
     */
        ...
        public void decompress(DataInput in, int originalLength, int offset, int length,
            ...
            if (decompressedLength > originalLength) {
                throw new CorruptIndexException(
                    "Corrupted: lengths mismatch: " + decompressedLength + " > " + originalLength
                        + " (resource=" + in + ")");
            }
            bytes.offset = offset;
            bytes.length = length;
    ...
    private static final class DeflateDecompressor extends Decompressor {
        ...
        public void decompress(DataInput in, int originalLength, int offset, int length,
            ...
            if (bytes.length != originalLength) {
                throw new CorruptIndexException(
                    "Lengths mismatch: " + bytes.length + " != " + originalLength + " (resource="
                        + in + ")");
            }
            bytes.offset = offset;
            bytes.length = length;
    ...
    private static class DeflateCompressor extends Compressor {
        ...
        public void compress(byte[] bytes, int off, int len, DataOutput out) throws IOException {
            ...
            int totalCount = 0;
            for (; ; ) {
                final int count = compressor.deflate(compressed, totalCount,
                    compressed.length - totalCount);
                totalCount += count;
