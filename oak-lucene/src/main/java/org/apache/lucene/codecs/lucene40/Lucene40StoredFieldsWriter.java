...
public final class Lucene40StoredFieldsWriter extends StoredFieldsWriter {
    ...
    public Lucene40StoredFieldsWriter(Directory directory, String segment, IOContext context)
        ...
        try {
            ...
            indexStream = directory.createOutput(
                IndexFileNames.segmentFileName(segment, "", FIELDS_INDEX_EXTENSION), context);
    ...
    /**
     * Bulk write a contiguous series of documents.  The lengths array is the length (in bytes) of
     * each raw document.  The stream IndexInput is the fieldsStream from which we should bulk-copy
     * all bytes.
     */
    ...
    public void finish(FieldInfos fis, int numDocs) {
        ...
        // details.
        {
            throw new RuntimeException(
                "fdx size mismatch: docCount is " + numDocs + " but fdx file size is "
                    + indexStream.getFilePointer() + " file=" + indexStream.toString()
