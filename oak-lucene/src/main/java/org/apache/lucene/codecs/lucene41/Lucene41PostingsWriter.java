...
public final class Lucene41PostingsWriter extends PostingsWriterBase {
        ...
        try {
            ...
            if (state.fieldInfos.hasProx()) {
                ...
                if (state.fieldInfos.hasPayloads() || state.fieldInfos.hasOffsets()) {
                    payOut = state.directory.createOutput(
                        IndexFileNames.segmentFileName(state.segmentInfo.name, state.segmentSuffix,
    ...
    final static class IntBlockTermState extends BlockTermState {
        ...
        public String toString() {
            return super.toString() + " docStartFP=" + docStartFP + " posStartFP=" + posStartFP
                + " payStartFP=" + payStartFP + " lastPosBlockOffset=" + lastPosBlockOffset
                + " singletonDocID=" + singletonDocID;
        }
    }
    ...
    public void startDoc(int docID, int termDocFreq) throws IOException {
        ...
        if (docID < 0 || (docCount > 0 && docDelta <= 0)) {
            throw new CorruptIndexException(
                "docs out of order (" + docID + " <= " + lastDocID + " ) (docOut: " + docOut + ")");
        }
    ...
    public void addPosition(int position, BytesRef payload, int startOffset, int endOffset)
        ...
        if (fieldHasPayloads) {
            if (payload == null || payload.length == 0) {
                ...
                System.arraycopy(payload.bytes, payload.offset, payloadBytes, payloadByteUpto,
                    payload.length);
