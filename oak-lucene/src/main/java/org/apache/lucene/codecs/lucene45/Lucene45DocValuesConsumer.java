...
public class Lucene45DocValuesConsumer extends DocValuesConsumer implements Closeable {
    ...
    /**
     * Single-valued sorted set values, encoded as sorted values, so no level of indirection: docId
     * -> ord.
     */
     ...
     */
    public Lucene45DocValuesConsumer(SegmentWriteState state, String dataCodec,
    ...
    void addNumericField(FieldInfo field, Iterable<Number> values, boolean optimizeStorage)
        throws IOException {
        long count = 0;
        ...
        if (uniqueValues != null
            && (delta < 0L
            || PackedInts.bitsRequired(uniqueValues.size() - 1) < PackedInts.bitsRequired(delta))
        ...
        switch (format) {
            ...
            case TABLE_COMPRESSED:
                ...
                final PackedInts.Writer ordsWriter = PackedInts.getWriterNoHeader(data,
                    PackedInts.Format.PACKED, (int) count, bitsRequired,
    ...
    public void addBinaryField(FieldInfo field, Iterable<BytesRef> values) throws IOException {
        ...
        meta.writeVInt(
            minLength == maxLength ? BINARY_FIXED_UNCOMPRESSED : BINARY_VARIABLE_UNCOMPRESSED);
        if (missing) {
            meta.writeLong(data.getFilePointer());
            writeMissingBitset(values);
        } else {
        ...
        if (minLength != maxLength) {
            ...
            final MonotonicBlockPackedWriter writer = new MonotonicBlockPackedWriter(data,
                BLOCK_SIZE);
            long addr = 0;
            for (BytesRef v : values) {
                if (v != null) {
     ...
     */
    protected void addTermsDict(FieldInfo field, final Iterable<BytesRef> values)
        throws IOException {
