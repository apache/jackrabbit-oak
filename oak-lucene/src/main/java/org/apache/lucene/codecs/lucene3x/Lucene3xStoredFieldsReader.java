...
final class Lucene3xStoredFieldsReader extends StoredFieldsReader implements Cloneable, Closeable {
    ...
    /**
     * Returns a cloned FieldsReader that shares open IndexInputs with the original one.  It is the
     * caller's job not to close the original FieldsReader until all clones are called (eg,
     * currently SegmentReader manages this logic).
     */
    @Override
    public Lucene3xStoredFieldsReader clone() {
        ensureOpen();
        return new Lucene3xStoredFieldsReader(fieldInfos, numTotalDocs, size, format,
            docStoreOffset, fieldsStream.clone(), indexStream.clone());
    }
    ...
    public static void checkCodeVersion(Directory dir, String segment) throws IOException {
        final String indexStreamFN = IndexFileNames.segmentFileName(segment, "",
        ...
        try {
            ...
            if (format < FORMAT_MINIMUM) {
                throw new IndexFormatTooOldException(idxStream, format, FORMAT_MINIMUM,
                    FORMAT_CURRENT);
            }
            if (format > FORMAT_CURRENT) {
                throw new IndexFormatTooNewException(idxStream, format, FORMAT_MINIMUM,
                    FORMAT_CURRENT);
            }
        } finally {
            idxStream.close();
    ...
    // Used only by clone
    private Lucene3xStoredFieldsReader(FieldInfos fieldInfos, int numTotalDocs, int size,
        int format, int docStoreOffset,
    ...
    public Lucene3xStoredFieldsReader(Directory d, SegmentInfo si, FieldInfos fn, IOContext context)
        ...
        try {
            ...
            if (format < FORMAT_MINIMUM) {
                throw new IndexFormatTooOldException(indexStream, format, FORMAT_MINIMUM,
                    FORMAT_CURRENT);
            }
            if (format > FORMAT_CURRENT) {
                throw new IndexFormatTooNewException(indexStream, format, FORMAT_MINIMUM,
                    FORMAT_CURRENT);
            }
            ...
            if (docStoreOffset != -1) {
                ...
                assert
                    ...
                    "indexSize=" + indexSize + " size=" + size + " docStoreOffset="
                        + docStoreOffset;
            } else {
                ...
                if (this.size != si.getDocCount()) {
                    throw new CorruptIndexException(
                        "doc counts differ for segment " + segment + ": fieldsReader shows "
                            + this.size + " but segmentInfo shows " + si.getDocCount());
                }
            }
    ...
    /**
     * Closes the underlying {@link org.apache.lucene.store.IndexInput} streams. This means that the
     * Fields values will not be accessible.
     *
    ...
    public final void visitDocument(int n, StoredFieldVisitor visitor)
        ...
        for (int fieldIDX = 0; fieldIDX < numFields; fieldIDX++) {
            ...
            assert
                bits <= (FIELD_IS_NUMERIC_MASK | FIELD_IS_BINARY) :
    ...
    private void readField(StoredFieldVisitor visitor, FieldInfo info, int bits)
        ...
        if (numeric != 0) {
            switch (numeric) {
                ...
                default:
                    throw new CorruptIndexException(
                        "Invalid numeric type: " + Integer.toHexString(numeric));
            }
        } else {
            ...
            if ((bits & FIELD_IS_BINARY) != 0) {
                ...
                visitor.stringField(info,
                    new String(bytes, 0, bytes.length, IOUtils.CHARSET_UTF_8));
            }
        }
    }
