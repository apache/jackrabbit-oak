...
public final class Lucene40TermVectorsWriter extends TermVectorsWriter {
    ...
    public Lucene40TermVectorsWriter(Directory directory, String segment, IOContext context)
        ...
        try {
            ...
            tvx = directory.createOutput(IndexFileNames.segmentFileName(segment, "",
                Lucene40TermVectorsReader.VECTORS_INDEX_EXTENSION), context);
            CodecUtil.writeHeader(tvx, CODEC_NAME_INDEX, VERSION_CURRENT);
            tvd = directory.createOutput(IndexFileNames.segmentFileName(segment, "",
                Lucene40TermVectorsReader.VECTORS_DOCUMENTS_EXTENSION), context);
            CodecUtil.writeHeader(tvd, CODEC_NAME_DOCS, VERSION_CURRENT);
            tvf = directory.createOutput(IndexFileNames.segmentFileName(segment, "",
                Lucene40TermVectorsReader.VECTORS_FIELDS_EXTENSION), context);
    ...
    public void startField(FieldInfo info, int numTerms, boolean positions, boolean offsets,
        ...
        assert
            lastFieldName == null || info.name.compareTo(lastFieldName) > 0 :
            "fieldName=" + info.name + " lastFieldName=" + lastFieldName;
            ...
            if (payloadLength > 0) {
                if (payloadLength + payloadData.length < 0) {
                    ...
                    throw new UnsupportedOperationException(
                        "A term cannot have more than Integer.MAX_VALUE bytes of payload data in a single document");
                }
    ...
    public void abort() {
        ...
        IOUtils.deleteFilesIgnoringExceptions(directory, IndexFileNames.segmentFileName(segment, "",
            ...
            IndexFileNames.segmentFileName(segment, "",
                Lucene40TermVectorsReader.VECTORS_FIELDS_EXTENSION));
    }

    /**
     * Do a bulk copy of numDocs documents from reader to our streams.  This is used to expedite
     * merging, if the field numbers are congruent.
     */
    private void addRawDocuments(Lucene40TermVectorsReader reader, int[] tvdLengths,
    ...
    public final int merge(MergeState mergeState) throws IOException {
        ...
        for (int i = 0; i < mergeState.readers.size(); i++) {
            ...
            if (reader.getLiveDocs() != null) {
                numDocs += copyVectorsWithDeletions(mergeState, matchingVectorsReader, reader,
                    rawDocLengths, rawDocLengths2);
            } else {
                numDocs += copyVectorsNoDeletions(mergeState, matchingVectorsReader, reader,
                    rawDocLengths, rawDocLengths2);
            }
        }
    ...
    public void finish(FieldInfos fis, int numDocs) {
        ...
        // details.
        {
            throw new RuntimeException(
                "tvx size mismatch: mergedDocs is " + numDocs + " but tvx size is "
                    + tvx.getFilePointer() + " file=" + tvx.toString()
