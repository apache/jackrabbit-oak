...
public class Lucene40PostingsReader extends PostingsReaderBase {
     ...
     * Sole constructor.
     */
    public Lucene40PostingsReader(Directory dir, FieldInfos fieldInfos, SegmentInfo segmentInfo,
        IOContext ioContext, String segmentSuffix) throws IOException {
        boolean success = false;
        IndexInput freqIn = null;
        IndexInput proxIn = null;
        try {
            freqIn = dir.openInput(IndexFileNames.segmentFileName(segmentInfo.name, segmentSuffix,
    ...
    private final static class StandardTermState extends BlockTermState {
        ...
        public String toString() {
            return super.toString() + " freqFP=" + freqOffset + " proxFP=" + proxOffset
    ...
    */
    ...
    private abstract class SegmentDocsEnumBase extends DocsEnum {
        ...
        DocsEnum reset(FieldInfo fieldInfo, StandardTermState termState) throws IOException {
            ...
            storeOffsets = fieldInfo.getIndexOptions().compareTo(
                IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS) >= 0;
        ...
        public final int advance(int target) throws IOException {
            ...
            if (++start < count && maxBufferedDocId >= target) {
                if ((count - start)
                    > 32) { // 32 seemed to be a sweetspot here so use binsearch if the pending results are a lot
        ...
        private final int skipTo(int target) throws IOException {
            if ((target - skipInterval) >= accum && limit >= skipMinimum) {
                ...
                if (skipper == null) {
                    ...
                    skipper = new Lucene40SkipListReader(freqIn.clone(), maxSkipLevels,
                        skipInterval);
                }
    ...
    private final class SegmentDocsAndPositionsEnum extends DocsAndPositionsEnum {
        ...
        public SegmentDocsAndPositionsEnum reset(FieldInfo fieldInfo, StandardTermState termState,
        ...
        public int nextPosition() throws IOException {
            ...
            assert posPendingCount >= 0 :
                "nextPosition() was called too many times (more than freq() times) posPendingCount="
                    + posPendingCount;
    ...
    private class SegmentFullPositionsEnum extends DocsAndPositionsEnum {
        ...
        public SegmentFullPositionsEnum reset(FieldInfo fieldInfo, StandardTermState termState,
            Bits liveDocs) throws IOException {
            storeOffsets = fieldInfo.getIndexOptions().compareTo(
                IndexOptions.DOCS_AND_FREQS_AND_POSITIONS_AND_OFFSETS) >= 0;
            storePayloads = fieldInfo.hasPayloads();
            assert fieldInfo.getIndexOptions().compareTo(IndexOptions.DOCS_AND_FREQS_AND_POSITIONS)
                >= 0;
