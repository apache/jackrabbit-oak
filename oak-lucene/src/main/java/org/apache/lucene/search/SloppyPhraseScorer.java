...
final class SloppyPhraseScorer extends Scorer {
     ...
     * traversing/hopping the PhrasePositions.
     ...
     * we always propagate the least PhrasePosition. This allows to base on PriorityQueue and move
     * forward faster. As result, for example, document "a b c b a" would score differently for
     * queries "a b c"~4 and "c b a"~4, although they really are equivalent. Similarly, for doc "a b
     * c b a f g", query "c b"~2 would get same score as "g f"~2, although "c b"~2 could be matched
     * twice. We may want to fix this in the future (currently not, for performance reasons).
     */
    ...
    private boolean advanceRpts(PhrasePositions pp) throws IOException {
        ...
        FixedBitSet bits = new FixedBitSet(
            rg.length); // for re-queuing after collisions are resolved
        int k0 = pp.rptInd;
        int k;
    ...
    private void initSimple() throws IOException {
        ...
        for (PhrasePositions pp = min, prev = null; prev != max;
            pp = (prev = pp).next) {  // iterate cyclic list: done once handled max
            pp.firstPosition();
            if (pp.position > end) {
                end = pp.position;
            }
    ...
    /**
     * At initialization (each doc), each repetition group is sorted by (query) offset. This
     * provides the start condition: no collisions.
     * <p>Case 1: no multi-term repeats<br>
     * It is sufficient to advance each pp in the group by one less than its group index. So lesser
     * pp is not advanced, 2nd one advance once, 3rd one advanced twice, etc.
            ...
            if (hasMultiTermRpts) {
                ...
                for (int i = 0; i < rg.length; i += incr) {
                    ...
                    while ((k = collide(pp)) >= 0) {
                        ...
                        if (!advancePP(
                            pp2)) {  // at initialization always advance pp with higher offset
                            return false; // exhausted
                        }
     ...
     * doc.<p> If there are repetitions, check if multi-term postings (MTP) are involved.<p> Without
     * MTP, once PPs are placed in the first candidate doc, repeats (and groups) are visible.<br>
     * With MTP, a more complex check is needed, up-front, as there may be "hidden collisions".<br>
     * For example P1 has {A,B}, P1 has {B,C}, and the first doc is: "A C B". At start, P1 would
     * point to "A", p2 to "C", and it will not be identified that P1 and P2 are repetitions of each
     * other.<p> The more complex initialization has two parts:<br> (1) identification of repetition
     * groups.<br> (2) advancing repeat groups at the start of the doc.<br> For (1), a possible
     * solution is to just create a single repetition group, made of all repeating pps. But this
     * would slow down the check for collisions, as all pps would need to be checked. Instead, we
     * compute "connected regions" on the bipartite graph of postings and terms.
     */
                ...
                for (int j = i + 1; j < rpp.length; j++) {
                    ...
                    if (
                        pp2.rptGroup >= 0        // already marked as a repetition
                            || pp2.offset == pp.offset
                            // not a repetition: two PPs are originally in same offset in the query!
     ...
     * set
     */
    private ArrayList<FixedBitSet> ppTermsBitSets(PhrasePositions[] rpp,
        HashMap<Term, Integer> tord) {
    ...
    private HashMap<Term, Integer> termGroups(LinkedHashMap<Term, Integer> tord,
        ArrayList<FixedBitSet> bb) throws IOException {
