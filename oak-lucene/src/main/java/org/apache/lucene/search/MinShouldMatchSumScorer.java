...
/**
 * A Scorer for OR like queries, counterpart of <code>ConjunctionScorer</code>. This Scorer
 * implements {@link Scorer#advance(int)} and uses advance() on the given Scorers.
 * <p>
 * This implementation uses the minimumMatch constraint actively to efficiently prune the number of
 * candidates, it is hence a mixture between a pure DisjunctionScorer and a ConjunctionScorer.
 */
class MinShouldMatchSumScorer extends Scorer {
    ...
    /**
     * A monotonically increasing index into the array pointing to the next subscorer that is to be
     * excluded
     */
    ...
    /**
     * mmStack is supposed to contain the most costly subScorers that still did not run out of docs,
     * sorted by increasing sparsity of docs returned by that subScorer. For now, the cost of
     * subscorers is assumed to be inversely correlated with sparsity.
     */
     ...
     * @param minimumNrMatchers The positive minimum number of subscorers that should match to match
     *                          this query.
     ...
     */
    public MinShouldMatchSumScorer(Weight weight, List<Scorer> subScorers, int minimumNrMatchers)
        throws IOException {
        super(weight);
    ...
    /**
     * Construct a <code>DisjunctionScorer</code>, using one as the minimum number of matching
     * subscorers.
     */
    ...
    private void evaluateSmallestDocInHeap() throws IOException {
        ...
        for (int i = mm - 2; i >= 0; i--) { // first advance sparsest subScorer
            if (mmStack[i].docID() >= doc || mmStack[i].advance(doc) != NO_MORE_DOCS) {
                if (mmStack[i].docID()
                    == doc) { // either it was already on doc, or got there via advance()
                    nrMatchers++;
     ...
     * to a given target. <br> The implementation uses the advance() method on the subscorers.
     *
     * @param target the target document number.
     * @return the document whose number is greater than or equal to the given target, or -1 if none
     * exist.
     */
    @Override
    ...
    /**
     * Removes a given Scorer from the heap by placing end of heap at that position and bubbling it
     * either up or down
     */
