...
public abstract class ScoringRewrite<Q extends Query> extends TermCollectingRewrite<Q> {
     ...
     * almost always better to use {@link MultiTermQuery#CONSTANT_SCORE_AUTO_REWRITE_DEFAULT}
     * instead.
     *
    ...
    @Override
    public final Q rewrite(final IndexReader reader, final MultiTermQuery query)
        throws IOException {
        ...
        if (size > 0) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                addClause(result, term, termStates[pos].docFreq(), query.getBoost() * boost[pos],
                    termStates[pos]);
            }
        }
        return result;
    ...
    final class ParallelArraysTermCollector extends TermCollector {
        ...
        public boolean collect(BytesRef bytes) throws IOException {
            ...
            if (e < 0) {
                ...
                assert array.boost[pos]
                    == boostAtt.getBoost() : "boost should be equal in all segment TermsEnums";
            } else {
