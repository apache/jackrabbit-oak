 ...
 * <p/>
 * See the {@link #create(org.apache.lucene.search.Sort, int, boolean, boolean, boolean, boolean)}
...
public abstract class TopFieldCollector extends TopDocsCollector<Entry> {
     ...
     * @param fillFields        specifies whether the actual field values should be returned on the
     *                          results (FieldDoc).
     * @param trackDocScores    specifies whether document scores should be tracked and set on the
     *                          results. Note that if set to false, then the results' scores will be
     *                          set to Float.NaN. Setting this to true affects performance, as it
     *                          incurs the score computation on each competitive result. Therefore
     *                          if document scores are not required by the application, it is
     *                          recommended to set it to false.
     * @param trackMaxScore     specifies whether the query's maxScore should be tracked and set on
     *                          the resulting {@link TopDocs}. Note that if set to false,
     ...
     *                          result. Also, setting this true automatically sets
     *                          <code>trackDocScores</code> to true as well.
     * @param docsScoredInOrder specifies whether documents are scored in doc Id order or not by the
     *                          given {@link Scorer} in {@link #setScorer(Scorer)}.
     * @return a {@link TopFieldCollector} instance which will sort the results by the sort
     * criteria.
        ...
        if (after == null) {
            if (queue.getComparators().length == 1) {
                if (docsScoredInOrder) {
                    if (trackMaxScore) {
                        return new OneComparatorScoringMaxScoreCollector(queue, numHits,
                            fillFields);
                    } else if (trackDocScores) {
                        return new OneComparatorScoringNoMaxScoreCollector(queue, numHits,
                            fillFields);
                    } else {
                    ...
                    if (trackMaxScore) {
                        return new OutOfOrderOneComparatorScoringMaxScoreCollector(queue, numHits,
                            fillFields);
                    } else if (trackDocScores) {
                        return new OutOfOrderOneComparatorScoringNoMaxScoreCollector(queue, numHits,
                            fillFields);
                    } else {
                        return new OutOfOrderOneComparatorNonScoringCollector(queue, numHits,
                            fillFields);
                    }
                }
            }
            ...
            if (docsScoredInOrder) {
                if (trackMaxScore) {
                    ...
                    return new MultiComparatorScoringNoMaxScoreCollector(queue, numHits,
                        fillFields);
                } else {
                ...
                if (trackMaxScore) {
                    return new OutOfOrderMultiComparatorScoringMaxScoreCollector(queue, numHits,
                        fillFields);
                } else if (trackDocScores) {
                    return new OutOfOrderMultiComparatorScoringNoMaxScoreCollector(queue, numHits,
                        fillFields);
                } else {
                    return new OutOfOrderMultiComparatorNonScoringCollector(queue, numHits,
                        fillFields);
                }
            }
        } else {
            ...
            if (after.fields.length != sort.getSort().length) {
                throw new IllegalArgumentException(
                    "after.fields has " + after.fields.length + " values but sort has "
                        + sort.getSort().length);
            }

            return new PagingFieldCollector(queue, after, numHits, fillFields, trackDocScores,
                trackMaxScore);
        }
    }
