 ...
 * BooleanQuery and DisjunctionMaxQuery:  for each term a DisjunctionMaxQuery searches for it in
 * each field, while the set of these DisjunctionMaxQuery's is combined into a BooleanQuery. The tie
 * breaker capability allows results that include the same term in multiple fields to be judged
 * better than results that include this term in only the best of those multiple fields, without
...
public class DisjunctionMaxQuery extends Query implements Iterable<Query> {
     ...
     *
     * @param tieBreakerMultiplier the score of each non-maximum disjunct for a document is
     ...
     *                             the lower scored field (i.e., one that is not in any higher
     *                             scored field.
     */
    ...
    /**
     * Add a collection of disjuncts to this disjunction via {@code Iterable<Query>}
     *
    ...
    /**
     * Expert: the Weight for DisjunctionMaxQuery, used to normalize, score and explain these
     * queries.
     *
    ...
    protected class DisjunctionMaxWeight extends Weight {
        ...
        /**
         * Compute the sub of squared weights of us applied to our subqueries.  Used for
         * normalization.
         */
        @Override
        public float getValueForNormalization() throws IOException {
            ...
            return (((sum - max) * tieBreakerMultiplier * tieBreakerMultiplier) + max) * boost
                * boost;
        }
        ...
        public Scorer scorer(AtomicReaderContext context, boolean scoreDocsInOrder,
            ...
            DisjunctionMaxScorer result = new DisjunctionMaxScorer(this, tieBreakerMultiplier,
                scorers.toArray(new Scorer[scorers.size()]));
            return result;
        }
        ...
        public Explanation explain(AtomicReaderContext context, int doc) throws IOException {
            if (disjuncts.size() == 1) {
                return weights.get(0).explain(context, doc);
            }
     ...
     * @return true iff o is a DisjunctionMaxQuery with the same boost and the same subqueries, in
