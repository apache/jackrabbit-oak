...
abstract class TermCollectingRewrite<Q extends Query> extends MultiTermQuery.RewriteMethod {
     ...
     */
    protected final void addClause(Q topLevel, Term term, int docCount, float boost)
        throws IOException {
        addClause(topLevel, term, docCount, boost, null);
    }

    protected abstract void addClause(Q topLevel, Term term, int docCount, float boost,
        TermContext states) throws IOException;
        ...
        for (AtomicReaderContext context : topReaderContext.leaves()) {
            ...
            if (lastTermComp != null && newTermComp != null && newTermComp != lastTermComp) {
                throw new RuntimeException(
                    "term comparator should not change between segments: " + lastTermComp + " != "
                        + newTermComp);
            }
    ...
    static abstract class TermCollector {
        ...
        public void setReaderContext(IndexReaderContext topReaderContext,
