...
/**
 * An alternative to BooleanScorer that also allows a minimum number of optional scorers that should
 * match.
...
class BooleanScorer2 extends Scorer {
    ...
    /**
     * The scorer to which all scoring will be delegated, except for computing and using the
     * coordination factor.
     */
    ...
    /**
     * Creates a {@link Scorer} with the given similarity and lists of required, prohibited and
     * optional scorers. In no required scorers are added, at least one of the optional scorers will
     * have to match during the search.
     *
     ...
     * @param minNrShouldMatch The minimum number of optional added scorers that should match during
     *                         the search. In case no required scorers are added, at least one of
    ...
    public BooleanScorer2(BooleanWeight weight, boolean disableCoord, int minNrShouldMatch,
        List<Scorer> required, List<Scorer> prohibited, List<Scorer> optional, int maxCoord)
        throws IOException {
        super(weight);
    ...
    private Scorer countingDisjunctionSumScorer(final List<Scorer> scorers,
        ...
        if (minNrShouldMatch > 1) {
            ...
            return new DisjunctionSumScorer(weight, scorers.toArray(new Scorer[scorers.size()]),
                null) {
                @Override
                public float score() throws IOException {
                    coordinator.nrMatchers += super.nrMatchers;
    ...
    private Scorer countingConjunctionSumScorer(boolean disableCoord,
        ...
        return new ConjunctionScorer(weight,
            requiredScorers.toArray(new Scorer[requiredScorers.size()])) {
            private int lastScoredDoc = -1;
            // Save the score of lastScoredDoc, so that we don't compute it more than
            // once in score().
            private float lastDocScore = Float.NaN;
     ...
     * optionalScorers and prohibitedScorers.
     */
    ...
    private Scorer makeCountingSumScorerNoReq(boolean disableCoord)
        ...
        if (optionalScorers.size() > nrOptRequired) {
            requiredCountingSumScorer = countingDisjunctionSumScorer(optionalScorers,
                nrOptRequired);
    ...
    private Scorer makeCountingSumScorerSomeReq(boolean disableCoord)
        ...
        if (optionalScorers.size() == minNrShouldMatch) { // all optional scorers also required.
            ...
            if (minNrShouldMatch
                > 0) { // use a required disjunction scorer over the optional scorers
    ...
    /**
     * Returns the scorer to be used for match counting and score summing. Uses the given required
     * scorer and the prohibitedScorers.
     *
