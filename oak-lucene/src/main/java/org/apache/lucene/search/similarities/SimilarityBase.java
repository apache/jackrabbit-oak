 ...
 * {@link #explain(Explanation, BasicStats, int, float, float)} is optional, inasmuch as
 * SimilarityBase already provides a basic explanation of the score and the term frequency. However,
 * implementers of a subclass are encouraged to include as much detail about the scoring method as
 * possible.
 * <p>
...
public abstract class SimilarityBase extends Similarity {
    ...
    /**
     * Fills all member fields defined in {@code BasicStats} in {@code stats}. Subclasses can
     * override this method to fill additional stats.
     */
    protected void fillBasicStats(BasicStats stats, CollectionStatistics collectionStats,
        TermStatistics termStats) {
    ...
    /**
     * Subclasses should implement this method to explain the score. {@code expl} already contains
     * the score, the name of the class and the doc id, as well as the term frequency and its
     * explanation; subclasses can add additional clauses to explain details of their scoring
     * formulae.
     ...
     * attaches the score (computed via the {@link #score(BasicStats, float, float)} method) and the
     * explanation for the term frequency. Subclasses content with this format may add additional
     * details in {@link #explain(Explanation, BasicStats, int, float, float)}.
     *
     * @param stats  the corpus level statistics.
     * @param doc    the document id.
    ...
    public SimScorer simScorer(SimWeight stats, AtomicReaderContext context) throws IOException {
        if (stats instanceof MultiSimilarity.MultiStats) {
            ...
            for (int i = 0; i < subScorers.length; i++) {
                ...
                subScorers[i] = new BasicSimScorer(basicstats,
    ...
    /**
     * Delegates the {@link #score(int, float)} and {@link #explain(int, Explanation)} methods to
     * {@link SimilarityBase#score(BasicStats, float, float)} and
     * {@link SimilarityBase#explain(BasicStats, int, Explanation, float)}, respectively.
     */
