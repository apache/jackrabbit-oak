 ...
 * gather raw results from a search, and implement sorting or custom result filtering, collation,
 * etc. </p>
 *
...
public abstract class Collector {
    ...
    /**
     * Called before successive calls to {@link #collect(int)}. Implementations that need the score
     * of the current document (passed-in to {@link #collect(int)}), should save the passed-in
     * Scorer and call scorer.score() when needed.
     */
     ...
     * a {@link CollectionTerminatedException}. In this case, the last docs of the current
     * {@link AtomicReaderContext} will be skipped and {@link IndexSearcher} will swallow the
     * exception and continue collection with the next leaf.
     * <p>
     * Note: This is called in an inner search loop. For good search performance, implementations of
     * this method should not call {@link IndexSearcher#doc(int)} or
     * {@link org.apache.lucene.index.IndexReader#document(int)} on every hit. Doing so can slow
    ...
    /**
     * Return <code>true</code> if this collector does not require the matching docIDs to be
     * delivered in int sort order (smallest to largest) to {@link #collect}.
     *
     * <p> Most Lucene Query implementations will visit
     * matching docIDs in order.  However, some queries (currently limited to certain cases of
     * {@link BooleanQuery}) can achieve faster searching if the
     * <code>Collector</code> allows them to deliver the
     * docIDs out of order.</p>
     *
     * <p> Many collectors don't mind getting docIDs out of
     * order, so it's important to return <code>true</code> here.
     */
