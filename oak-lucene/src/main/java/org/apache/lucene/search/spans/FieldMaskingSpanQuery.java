 ...
 * single-field SpanQueries by 'lying' about their search field. That is, the masked SpanQuery will
 * function as normal, but {@link SpanQuery#getField()} simply hands back the value supplied in this
 * class's constructor.</p>
 *
 * <p>This can be used to support Queries like {@link SpanNearQuery} or
 * {@link SpanOrQuery} across different fields, which is not ordinarily permitted.</p>
 *
 ...
 * teacherid 2 (which has a 'james' in position 0 and 'jones' in position 1). </p>
 *
 * <p>Note: as {@link #getField()} returns the masked field, scoring will be
 * done using the Similarity and collection statistics of the field name supplied, but with the term
 * statistics of the real field. This may lead to exceptions, poor performance, and unexpected
 * scoring behaviour.</p>
 */
