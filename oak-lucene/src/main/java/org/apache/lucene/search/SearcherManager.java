 ...
 *
 * <p>
 * Use {@link #acquire} to obtain the current searcher, and {@link #release} to release it, like
 * this:
 *
 ...
 *
 * <p>
 * In addition you should periodically call {@link #maybeRefresh}. While it's possible to call this
 * just before running each query, this is discouraged since it penalizes the unlucky queries that
 * do the reopen. It's better to use a separate background thread, that periodically calls
 * maybeReopen. Finally, be sure to call {@link #close} once you are done.
 *
 * @lucene.experimental
 * @see SearcherFactory
 */
public final class SearcherManager extends ReferenceManager<IndexSearcher> {
     ...
     *                        visible) in the {@link IndexSearcher} / {@link DirectoryReader}. If
     *                        <code>false</code>, the deletes may or may not be applied, but remain
    ...
    /**
     * Returns <code>true</code> if no changes have occured since this searcher ie. reader was
    ...
    /**
     * Expert: creates a searcher from the provided {@link IndexReader} using the provided
     * {@link SearcherFactory}.  NOTE: this decRefs incoming reader on throwing an exception.
     */
