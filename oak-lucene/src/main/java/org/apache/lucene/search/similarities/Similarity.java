 ...
 * Expert: Scoring API.
 * <p>
 * This is a low-level API, you should only extend this API if you want to implement an information
 * retrieval <i>model</i>.  If you are instead looking for a convenient way to alter Lucene's
 * scoring, consider extending a higher-level implementation such as {@link TFIDFSimilarity}, which
 * implements the vector space model with this API, or just tweaking the default implementation:
 * {@link DefaultSimilarity}.
 * <p>
 * Similarity determines how Lucene weights terms, and Lucene interacts with this class at both <a
 * href="#indextime">index-time</a> and
 * <a href="#querytime">query-time</a>.
 * <p>
 * <a name="indextime"/>
 * At indexing time, the indexer calls {@link #computeNorm(FieldInvertState)}, allowing the
 * Similarity implementation to set a per-document value for the field that will be later accessible
 * via {@link AtomicReader#getNormValues(String)}.  Lucene makes no assumption about what is in this
 ...
 * classical {@link TFIDFSimilarity} encodes a combination of index-time boost and length
 * normalization information with {@link SmallFloat} into a single byte, this might not be suitable
 * for all purposes.
 * <p>
 * Many formulas require the use of average document length, which can be computed via a combination
 * of {@link CollectionStatistics#sumTotalTermFreq()} and {@link CollectionStatistics#maxDoc()} or
 * {@link CollectionStatistics#docCount()}, depending upon whether the average should reflect field
 * sparsity.
 * <p>
...
public abstract class Similarity {
     ...
     * <p>
     * By default this is disabled (returns <code>1</code>), as with most modern models this will
     * only skew performance, but some implementations such as {@link TFIDFSimilarity} override
     * this.
     *
     ...
     * {@link SimWeight#getValueForNormalization()} of each of the query terms.  This value is
     * passed back to the weight ({@link SimWeight#normalize(float, float)} of each query term, to
     * provide a hook to attempt to make scores from different queries comparable.
     * <p>
     * By default this is disabled (returns <code>1</code>), but some implementations such as
     * {@link TFIDFSimilarity} override this.
     *
     ...
     * method usually set smaller values when <code>state.getLength()</code> is large, and larger
     * values when <code>state.getLength()</code> is small.
     *
     ...
     *                        collection.
     * @param termStats       term-level statistics, such as the document frequency of a term across
     *                        the collection.
    ...
    /**
     * Creates a new {@link Similarity.SimScorer} to score matching documents from a segment of the
     * inverted index.
     *
    ...
    /**
     * API for scoring "sloppy" queries such as {@link TermQuery}, {@link SpanQuery}, and
     * {@link PhraseQuery}.
     * <p>
     * Frequencies are floating-point values: an approximate within-document frequency adjusted for
     * "sloppiness" by {@link SimScorer#computeSlopFactor(int)}.
     */
    ...
    public static abstract class SimWeight {
         ...
         * <p>
         * NOTE: a Similarity implementation might not use any query normalization at all, its not
         * required. However, if it wants to participate in query normalization, it can return a
         * value here.
         */
         ...
         * <p>
         * NOTE: a Similarity implementation might not use this normalized value at all, its not
         * required. However, its usually a good idea to at least incorporate the topLevelBoost
