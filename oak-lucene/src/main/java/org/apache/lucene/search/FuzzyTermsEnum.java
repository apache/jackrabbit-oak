...
public class FuzzyTermsEnum extends TermsEnum {
     ...
     *                      {@link MultiTermQuery} thats contains information about competitive
     *                      boosts during rewrite. It is also used to cache DFAs between segment
     *                      transitions.
     * @param term          Pattern term.
     * @param minSimilarity Minimum required similarity for terms from the reader. Pass an integer
     *                      value representing edit distance. Passing a fraction is deprecated.
     * @param prefixLength  Length of required common prefix. Default value is 0.
     * @throws IOException if there is a low-level IO error
     */
    public FuzzyTermsEnum(Terms terms, AttributeSource atts, Term term,
        final float minSimilarity, final int prefixLength, boolean transpositions)
        throws IOException {
    ...
    protected TermsEnum getAutomatonEnum(int editDistance, BytesRef lastTerm)
        ...
        if (editDistance < runAutomata.size()) {
            ...
            return new AutomatonFuzzyTermsEnum(terms.intersect(compiled,
                lastTerm == null ? null : compiled.floor(lastTerm, new BytesRef())),
                runAutomata.subList(0, editDistance + 1)
                           .toArray(new CompiledAutomaton[editDistance + 1]));
        } else {
            return null;
        }
    }
    ...
    private List<CompiledAutomaton> initAutomata(int maxDistance) {
        ...
        if (runAutomata.size() <= maxDistance &&
            ...
            LevenshteinAutomata builder =
                new LevenshteinAutomata(UnicodeUtil.newString(termText, realPrefixLength,
    ...
    private void bottomChanged(BytesRef lastTerm, boolean init)
        ...
        boolean termAfter = bottomTerm == null || (lastTerm != null
            && termComparator.compare(lastTerm, bottomTerm) >= 0);
     ...
     * <p>
     * This is the fastest method as opposed to LinearFuzzyTermsEnum: as enumeration is logarithmic
     * to the number of terms (instead of linear) and comparison is linear to length of the term
    ...
    private class AutomatonFuzzyTermsEnum extends FilteredTermsEnum {
        ...
        final boolean matches(BytesRef term, int k) {
            return k == 0 ? term.equals(termRef)
                : matchers[k].run(term.bytes, term.offset, term.length);
        }
    }
    ...
    /**
     * reuses compiled automata across different segments, because they are independent of the
     * index
     *
     * @lucene.internal
     */
     ...
     * @lucene.internal
     */
    public static final class LevenshteinAutomataAttributeImpl extends AttributeImpl implements
