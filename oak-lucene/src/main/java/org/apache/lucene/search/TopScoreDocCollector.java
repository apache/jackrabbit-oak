...
/**
 * A {@link Collector} implementation that collects the top-scoring hits, returning them as a
 * {@link TopDocs}. This is used by {@link IndexSearcher} to implement {@link TopDocs}-based search.
 * Hits are sorted by score descending and then (when the scores are tied) docID ascending. When you
 ...
 * {@link Float#NEGATIVE_INFINITY} are not valid scores.  This collector will not properly collect
 * hits with such scores.
 */
public abstract class TopScoreDocCollector extends TopDocsCollector<ScoreDoc> {
    ...
    /**
     * Creates a new {@link TopScoreDocCollector} given the number of hits to collect and whether
     * documents are scored in order by the input {@link Scorer} to {@link #setScorer(Scorer)}.
     *
     ...
     * the previous page, and whether documents are scored in order by the input {@link Scorer} to
     * {@link #setScorer(Scorer)}.
     *
     * <p><b>NOTE</b>: The instances returned by this method
     * pre-allocate a full array of length
     * <code>numHits</code>, and fill the array with sentinel
