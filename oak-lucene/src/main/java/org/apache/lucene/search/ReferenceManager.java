 ...
 * have finished using it. It is recommended to consult the documentation of
 * {@link ReferenceManager} implementations for their {@link #maybeRefresh()} semantics.
 *
...
public abstract class ReferenceManager<G> implements Closeable {
    ...
    /**
     * Refresh the given reference if needed. Returns {@code null} if no refresh was needed,
     ...
     * {@link #release}; it's best to do so in a finally clause, and set the reference to
     * {@code null} to prevent accidental usage after it has been released.
     *
     * @throws AlreadyClosedException if the reference manager has been {@link #close() closed}.
     */
     ...
     * application using the {@link ReferenceManager} is shutting down. The managed resource might
     * not be released immediately, if the {@link ReferenceManager} user is holding on to a
     * previously {@link #acquire() acquired} reference. The resource will be released once when the
     * last reference is {@link #release(Object) released}. Those references can still be used as if
     * the manager was still active.
     * </p>
     * <p>
     * Applications should not {@link #acquire() acquire} new references from this manager once this
     * method has been called. {@link #acquire() Acquiring} a resource on a closed
     * {@link ReferenceManager} will throw an {@link AlreadyClosedException}.
     * </p>
     *
        ...
        try {
            ...
            try {
                ...
                if (newReference != null) {
                    assert newReference
                        != reference : "refreshIfNeeded should return null if refresh wasn't needed";
                    try {
    ...
    /**
     * You must call this (or {@link #maybeRefreshBlocking()}), periodically, if you want that
     * {@link #acquire()} will return refreshed instances.
     *
     * <p>
     * <b>Threads</b>: it's fine for more than one thread to call this at once.
     * Only the first thread will attempt the refresh; subsequent threads will see that another
     * thread is already handling refresh and will return immediately. Note that this means if
     * another thread is already refreshing then subsequent threads will return right away without
    ...
    /**
     * You must call this (or {@link #maybeRefresh()}), periodically, if you want that
     * {@link #acquire()} will return refreshed instances.
     *
     * <p>
     * <b>Threads</b>: unlike {@link #maybeRefresh()}, if another thread is
     * currently refreshing, this method blocks until that thread completes. It is useful if you
     * want to guarantee that the next call to {@link #acquire()} will return a refreshed instance.
     * Otherwise, consider using the non-blocking {@link #maybeRefresh()}.
     *
     * @throws IOException            if refreshing the resource causes an {@link IOException}
     * @throws AlreadyClosedException if the reference manager has been {@link #close() closed}.
     */
     ...
     *
     * @throws IOException if the release operation on the given resource throws an
     *                     {@link IOException}
     */
    ...
    public interface RefreshListener {
        ...
        /**
         * Called after the attempted refresh; if the refresh did open a new reference then
         * didRefresh will be true and {@link #acquire()} is guaranteed to return the new
         * reference.
         */
