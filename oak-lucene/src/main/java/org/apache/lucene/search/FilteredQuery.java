 ...
 * query is used in a search - use a CachingWrapperFilter to avoid regenerating the bits every
 * time.
 *
...
public class FilteredQuery extends Query {
    ...
    /**
     * Returns a Weight that applies the filter to the enclosed query's Weight. This is accomplished
     * by overriding the Scorer returned by the Weight.
     */
    @Override
    public Weight createWeight(final IndexSearcher searcher) throws IOException {
        ...
        return new Weight() {
            ...
            public float getValueForNormalization() throws IOException {
                return weight.getValueForNormalization() * getBoost()
                    * getBoost(); // boost sub-weight
            }
            ...
            public Scorer scorer(AtomicReaderContext context, boolean scoreDocsInOrder,
                ...
                return strategy.filteredScorer(context, scoreDocsInOrder, topScorer, weight,
                    filterDocIdSet);
            }
        };
    }
    ...
    /**
     * A scorer that consults the filter iff a document was matched by the delegate scorer. This is
     * useful if the filter computation is more expensive than document scoring or if the filter has
     * a linear running time to compute the next matching doc like exact geo distances.
     */
    ...
    /**
     * A Scorer that uses a "leap-frog" approach (also called "zig-zag join"). The scorer and the
     * filter take turns trying to advance to each other's next matching document, often jumping
     * past the target document. When both land on the same document, it's collected.
     */
    private static class LeapFrogScorer extends Scorer {
        ...
        protected LeapFrogScorer(Weight weight, DocIdSetIterator primary,
            DocIdSetIterator secondary, Scorer scorer) {
            super(weight);
    ...
    private static final class PrimaryAdvancedLeapFrogScorer extends LeapFrogScorer {
        ...
        protected PrimaryAdvancedLeapFrogScorer(Weight weight, int firstFilteredDoc,
            DocIdSetIterator filterIter, Scorer other) {
    ...
    /**
     * Rewrites the query. If the wrapped is an instance of {@link MatchAllDocsQuery} it returns a
     * {@link ConstantScoreQuery}. Otherwise it returns a new {@code FilteredQuery} wrapping the
     * rewritten query.
     */
    @Override
    ...
    public boolean equals(Object o) {
        ...
        return fq.query.equals(this.query) && fq.filter.equals(this.filter) && fq.strategy.equals(
            this.strategy);
    }
    ...
    /**
     * A {@link FilterStrategy} that conditionally uses a random access filter if the given
     * {@link DocIdSet} supports random access (returns a non-null value from
     * {@link DocIdSet#bits()}) and {@link RandomAccessFilterStrategy#useRandomAccess(Bits, int)}
     * returns
    ...
    /**
     * A filter strategy that uses a "leap-frog" approach (also called "zig-zag join"). The scorer
     * and the filter take turns trying to advance to each other's next matching document, often
     ...
     * </p>
     */
    public static final FilterStrategy LEAP_FROG_FILTER_FIRST_STRATEGY = new LeapFrogFilterStrategy(
        false);
     ...
     * </p>
     */
    public static final FilterStrategy LEAP_FROG_QUERY_FIRST_STRATEGY = new LeapFrogFilterStrategy(
        true);
     ...
     * </p>
     * <p>
     * Use this strategy if the filter computation is more expensive than document scoring or if the
     * filter has a linear running time to compute the next matching doc like exact geo distances.
     * </p>
     */
    public static final FilterStrategy QUERY_FIRST_FILTER_STRATEGY = new QueryFirstFilterStrategy();

    /**
     * Abstract class that defines how the filter ({@link DocIdSet}) applied during document
     * collection.
     */
    public static abstract class FilterStrategy {
         ...
         *
         * @param context          the {@link AtomicReaderContext} for which to return the
         *                         {@link Scorer}.
         * @param scoreDocsInOrder specifies whether in-order scoring of documents is required. Note
         *                         that if set to false (i.e., out-of-order scoring is required),
         *                         this method can return whatever scoring mode it supports, as
         *                         every in-order scorer is also an out-of-order one. However, an
         *                         out-of-order scorer may not support {@link Scorer#nextDoc()}
         *                         and/or {@link Scorer#advance(int)}, therefore it is recommended
         *                         to request an in-order scorer if use of these methods is
         *                         required.
         * @param topScorer        if true, {@link Scorer#score(Collector)} will be called; if
         *                         false, {@link Scorer#nextDoc()} and/or
         *                         {@link Scorer#advance(int)} will be called.
         * @param weight           the {@link FilteredQuery} {@link Weight} to create the filtered
         *                         scorer.
    ...
    public static class RandomAccessFilterStrategy extends FilterStrategy {
        ...
        @Override
        public Scorer filteredScorer(AtomicReaderContext context, boolean scoreDocsInOrder,
            boolean topScorer, Weight weight, DocIdSet docIdSet) throws IOException {
            ...
            final boolean useRandomAccess =
                filterAcceptDocs != null && useRandomAccess(filterAcceptDocs, firstFilterDoc);
            if (useRandomAccess) {
                ...
                return (scorer == null) ? null
                    : new PrimaryAdvancedLeapFrogScorer(weight, firstFilterDoc, filterIter, scorer);
            }
        }

        /**
         * Expert: decides if a filter should be executed as "random-access" or not. random-access
         * means the filter "filters" in a similar way as deleted docs are filtered in Lucene. This
         * is faster when the filter accepts many documents. However, when the filter is very
         * sparse, it can be faster to execute the query+filter as a conjunction in some cases.
         * <p>
         * The default implementation returns <code>true</code> if the first document accepted by
         * the filter is < 100.
         *
         * @lucene.internal
         */
    ...
    private static final class LeapFrogFilterStrategy extends FilterStrategy {
        ...
        public Scorer filteredScorer(AtomicReaderContext context,
            ...
            if (scorerFirst) {
                return (scorer == null) ? null
                    : new LeapFrogScorer(weight, scorer, filterIter, scorer);
            } else {
                return (scorer == null) ? null
                    : new LeapFrogScorer(weight, filterIter, scorer, scorer);
            }
        }
    ...
    /**
     * A filter strategy that advances the {@link Scorer} first and consults the {@link DocIdSet}
    ...
    private static final class QueryFirstFilterStrategy extends FilterStrategy {
        ...
        public Scorer filteredScorer(final AtomicReaderContext context,
            ...
            if (filterAcceptDocs == null) {
                return LEAP_FROG_QUERY_FIRST_STRATEGY.filteredScorer(context, scoreDocsInOrder,
                    topScorer, weight, docIdSet);
            }
            final Scorer scorer = weight.scorer(context, true, false, null);
