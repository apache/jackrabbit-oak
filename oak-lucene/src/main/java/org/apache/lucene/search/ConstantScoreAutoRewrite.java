...
class ConstantScoreAutoRewrite extends TermCollectingRewrite<BooleanQuery> {
    ...
    /**
     * If the number of terms in this query is equal to or larger than this setting then
     * {@link MultiTermQuery#CONSTANT_SCORE_FILTER_REWRITE} is used.
     */
    ...
    /**
     * If the number of documents to be visited in the postings exceeds this specified percentage of
     * the maxDoc() for the index, then {@link MultiTermQuery#CONSTANT_SCORE_FILTER_REWRITE} is
     * used.
     *
        ...
        if (col.hasCutOff) {
            ...
            if (size > 0) {
                ...
                for (int i = 0; i < size; i++) {
                    ...
                    addClause(bq, new Term(query.field, pendingTerms.get(pos, new BytesRef())), 1,
                        1.0f, col.array.termState[pos]);
                }
            }
            // Strip scores
    ...
    static final class CutOffTermCollector extends TermCollector {
        ...
        public boolean collect(BytesRef bytes) throws IOException {
            ...
            if (pos < 0) {
                ...
                array.termState[pos] = new TermContext(topReaderContext, termState,
                    readerContext.ord, termsEnum.docFreq(), termsEnum.totalTermFreq());
            }
            return true;
        }
        ...
        final BytesRefHash pendingTerms = new BytesRefHash(
            new ByteBlockPool(new ByteBlockPool.DirectAllocator()), 16, array);
    }
    ...
    public boolean equals(Object obj) {
        ...
        if (Double.doubleToLongBits(other.docCountPercent) != Double.doubleToLongBits(
            docCountPercent)) {
            return false;
        }
    ...
    static final class TermStateByteStart extends DirectBytesStartArray {
        ...
        public int[] grow() {
            ...
            if (termState.length < ord.length) {
                TermContext[] tmpTermState = new TermContext[ArrayUtil.oversize(ord.length,
                    RamUsageEstimator.NUM_BYTES_OBJECT_REF)];
