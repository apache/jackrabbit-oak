 ...
 * specified range.  To use this, you must first index the numeric values using {@link IntField},
 * {@link FloatField}, {@link LongField} or {@link DoubleField} (expert:
 * {@link NumericTokenStream}).  If your terms are instead textual, you should use
 * {@link TermRangeQuery}.  {@link NumericRangeFilter} is the filter equivalent of this query.</p>
 *
 ...
 * than the corresponding {@link TermRangeQuery} because the number of terms that must be searched
 * is usually far fewer, thanks to trie indexing, described below.</p>
 *
 * <p>You can optionally specify a <a
 * href="#precisionStepDesc"><code>precisionStep</code></a> when creating this query.  This is
 * necessary if you've changed this configuration from its default (4) during indexing.  Lower
 * values consume more disk space but speed up searching.  Suitable values are between <b>1</b> and
 * <b>8</b>. A good starting point to test is <b>4</b>,
 * which is the default value for all <code>Numeric*</code> classes.  See <a
 * href="#precisionStepDesc">below</a> for details.
 *
 * <p>This query defaults to {@linkplain
 * MultiTermQuery#CONSTANT_SCORE_AUTO_REWRITE_DEFAULT}. With precision steps of &le;4, this query
 * can be run with one of the BooleanQuery rewrite methods without changing BooleanQuery's default
 * max clause count.
 *
 ...
 * Computers &amp; Geosciences 34 (12), 1947-1955. <a
 ...
 * search engine and not a conventional database, it cannot handle numerical ranges (e.g., field
 * value is inside user defined bounds, even dates are numerical values). We have developed an
 * extension to Apache Lucene that stores the numerical values in a special string-encoded format
 * with variable precision (all numerical values like doubles, longs, floats, and ints are converted
 * to lexicographic sortable string representations and stored with different precisions (for a more
 * detailed description of how the values are stored, see {@link NumericUtils}). A range is then
 * divided recursively into multiple intervals for searching: The center of the range is searched
 * only with the lowest possible precision in the <em>trie</em>, while the boundaries are matched
 ...
 * <code>7*255*2 + 255 = 3825</code> distinct terms (when there is a term for every distinct value
 * of an 8-byte-number in the index and the range covers almost all of them; a maximum of 255
 * distinct values is used because it would always be possible to reduce the full 256 values to one
 * term with degraded precision). In practice, we have seen up to 300 terms in most cases (index
 * with 500,000 metadata records and a uniform value distribution).</p>
 *
 * <a name="precisionStepDesc"><h3>Precision Step</h3>
 * <p>You can choose any <code>precisionStep</code> when encoding values.
 * Lower step values mean more precisions and so more terms in index (and index gets larger). The
 * number of indexed terms per value is (those are generated by {@link NumericTokenStream}):
 * <p style="font-family:serif">
 * &nbsp;&nbsp;indexedTermsPerValue = <b>ceil</b><big>(</big>bitsPerValue /
 * precisionStep<big>)</big>
 * </p>
 * As the lower precision terms are shared by many values, the additional terms only slightly grow
 * the term dictionary (approx. 7% for <code>precisionStep=4</code>), but have a larger impact on
 * the postings (the postings file will have  more entries, as every document is linked to
 ...
 * <p>
 * <!-- the formula in the alt attribute was transformed from latex to PNG with
 * http://1.618034.com/latex.php (with 110 dpi): --> &nbsp;&nbsp;<img
 * src="doc-files/nrq-formula-1.png" alt="\mathrm{termDictOverhead} =
 ...
 * <!-- the formula in the alt attribute was transformed from latex to PNG with
 * http://1.618034.com/latex.php (with 110 dpi): --> &nbsp;&nbsp;<img
 * src="doc-files/nrq-formula-2.png" alt="\mathrm{maxQueryTerms} = \left[ \left(
 ...
 * </p>
 * <p>For longs stored using a precision step of 4, <code>maxQueryTerms = 15*15*2 + 15 =
 * 465</code>,
 * and for a precision step of 2, <code>maxQueryTerms = 31*3*2 + 3 = 189</code>. But the faster
 * search speed is reduced by more seeking in the term enum of the index. Because of this, the
 * ideal
...
public final class NumericRangeQuery<T extends Number> extends MultiTermQuery {
    ...
    /**
     * Factory that creates a <code>NumericRangeQuery</code>, that queries a <code>long</code> range
     * using the given <a href="#precisionStepDesc"><code>precisionStep</code></a>. You can have
    ...
    public static NumericRangeQuery<Long> newLongRange(final String field, final int precisionStep,
        ...
        return new NumericRangeQuery<Long>(field, precisionStep, NumericType.LONG, min, max,
     ...
     * You can have half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting
     * the min or max value to <code>null</code>. By setting inclusive to false, it will match all
    ...
    public static NumericRangeQuery<Long> newLongRange(final String field,
        ...
        return new NumericRangeQuery<Long>(field, NumericUtils.PRECISION_STEP_DEFAULT,
            NumericType.LONG, min, max, minInclusive, maxInclusive);
    }

    /**
     * Factory that creates a <code>NumericRangeQuery</code>, that queries a <code>int</code> range
     * using the given <a href="#precisionStepDesc"><code>precisionStep</code></a>. You can have
     * half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting the min or max
     * value to <code>null</code>. By setting inclusive to false, it will match all documents
     * excluding the bounds, with inclusive on, the boundaries are hits, too.
    ...
    public static NumericRangeQuery<Integer> newIntRange(final String field,
        ...
        return new NumericRangeQuery<Integer>(field, precisionStep, NumericType.INT, min, max,
            minInclusive, maxInclusive);
    }
     ...
     * range using the given <a href="#precisionStepDesc"><code>precisionStep</code></a>. You can
     * have half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting the min
     * or max value to <code>null</code>. {@link Double#NaN} will never match a half-open range, to
     * hit {@code NaN} use a query with {@code min == max == Double.NaN}.  By setting inclusive to
     * false, it will match all documents excluding the bounds, with inclusive on, the boundaries
     * are hits, too.
     */
    public static NumericRangeQuery<Double> newDoubleRange(final String field,
        ...
        return new NumericRangeQuery<Double>(field, precisionStep, NumericType.DOUBLE, min, max,
            minInclusive, maxInclusive);
    }

    /**
     * Factory that creates a <code>NumericRangeQuery</code>, that queries a <code>double</code>
     * range using the default <code>precisionStep</code>
     * {@link NumericUtils#PRECISION_STEP_DEFAULT} (4). You can have half-open ranges (which are in
     * fact &lt;/&le; or &gt;/&ge; queries) by setting the min or max value to <code>null</code>.
     * {@link Double#NaN} will never match a half-open range, to hit {@code NaN} use a query with
     * {@code min == max == Double.NaN}.  By setting inclusive to false, it will match all documents
     * excluding the bounds, with inclusive on, the boundaries are hits, too.
     */
    public static NumericRangeQuery<Double> newDoubleRange(final String field,
        Double min, Double max, final boolean minInclusive, final boolean maxInclusive
    ) {
        return new NumericRangeQuery<Double>(field, NumericUtils.PRECISION_STEP_DEFAULT,
     ...
     * range using the given <a href="#precisionStepDesc"><code>precisionStep</code></a>. You can
     * have half-open ranges (which are in fact &lt;/&le; or &gt;/&ge; queries) by setting the min
     * or max value to <code>null</code>. {@link Float#NaN} will never match a half-open range, to
    ...
    public static NumericRangeQuery<Float> newFloatRange(final String field,
        ...
        return new NumericRangeQuery<Float>(field, precisionStep, NumericType.FLOAT, min, max,
            minInclusive, maxInclusive);
    }
     ...
     * {@link NumericUtils#PRECISION_STEP_DEFAULT} (4). You can have half-open ranges (which are in
     * fact &lt;/&le; or &gt;/&ge; queries) by setting the min or max value to <code>null</code>.
     * {@link Float#NaN} will never match a half-open range, to hit {@code NaN} use a query with
     * {@code min == max == Float.NaN}.  By setting inclusive to false, it will match all documents
     * excluding the bounds, with inclusive on, the boundaries are hits, too.
     */
    public static NumericRangeQuery<Float> newFloatRange(final String field,
        Float min, Float max, final boolean minInclusive, final boolean maxInclusive
    ) {
        return new NumericRangeQuery<Float>(field, NumericUtils.PRECISION_STEP_DEFAULT,
     ...
     * {@link Term#compareTo}. The ordering depends on how {@link NumericUtils#splitLongRange} and
     * {@link NumericUtils#splitIntRange} generates the sub-ranges. For {@link MultiTermQuery}
    ...
    private final class NumericRangeTermsEnum extends FilteredTermsEnum {
            ...
            switch (dataType) {
                ...
                case DOUBLE: {
                    ...
                    NumericUtils.splitLongRange(new NumericUtils.LongRangeBuilder() {
                        @Override
                        public final void addRange(BytesRef minPrefixCoded,
        ...
        private void nextRange() {
            ...
            assert currentUpperBound == null
                || termComp.compare(currentUpperBound, currentLowerBound) <= 0 :
