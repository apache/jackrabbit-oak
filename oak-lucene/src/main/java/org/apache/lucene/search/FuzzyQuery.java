 ...
 * as default. So terms will be collected and scored according to their edit distance. Only the top
 * terms are used for building the {@link BooleanQuery}. It is not recommended to change the rewrite
 * mode for fuzzy queries.
 *
 ...
 * Higher distances (especially with transpositions enabled), are generally not useful and will
 * match a significant amount of the term dictionary. If you really want this, consider using an
 * n-gram indexing technique (such as the SpellChecker in the
 ...
 * distance between two terms is computed.  For a term to match, the edit distance between the terms
 * must be less than the minimum length term (either the input term, or the candidate term).  For
 * example, FuzzyQuery on term "abcd" with maxEdits=2 will not match an indexed term "ab", and
...
public class FuzzyQuery extends MultiTermQuery {
     ...
     *                       {@link BooleanQuery#getMaxClauseCount} when the query is rewritten,
    ...
    /**
     * Calls
     * {@link #FuzzyQuery(Term, int, int, int, boolean) FuzzyQuery(term, maxEdits, prefixLength,
    ...
    protected TermsEnum getTermsEnum(Terms terms, AttributeSource atts) throws IOException {
        if (maxEdits == 0 || prefixLength >= term.text()
                                                 .length()) {  // can only match if it's exact
    ...
    /**
     * Helper function to convert from deprecated "minimumSimilarity" fractions to raw edit
     * distances.
     *
