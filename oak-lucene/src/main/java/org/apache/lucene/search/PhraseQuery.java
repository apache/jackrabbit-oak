...
public class PhraseQuery extends Query {
    ...
    /**
     * Sets the number of other words permitted between words in query phrase. If zero, then this is
     * an exact phrase search.  For larger values this works like a <code>WITHIN</code> or
     * <code>NEAR</code> operator.
     *
     * <p>The slop is in fact an edit-distance, where the units correspond to
     * moves of terms in the query phrase out of position.  For example, to switch the order of two
     * words requires two moves (the first move places the words atop one another), so to permit
     * re-orderings of phrases, the slop must be at least two.
     *
     ...
     * phrase is specified explicitly. This allows e.g. phrases with more than one term at the same
    ...
    private class PhraseWeight extends Weight {
        ...
        public PhraseWeight(IndexSearcher searcher)
            ...
            stats = similarity.computeWeight(getBoost(), searcher.collectionStatistics(field),
                termStats);
        }
        ...
        public Scorer scorer(AtomicReaderContext context, boolean scoreDocsInOrder,
            ...
            for (int i = 0; i < terms.size(); i++) {
                ...
                if (state == null) { /* term doesnt exist in this segment */
                    assert termNotInReader(reader,
                        t) : "no termstate found but term exists in reader";
                    return null;
                }
                ...
                postingsFreqs[i] = new PostingsAndFreq(postingsEnum, te.docFreq(),
            ...
            if (slop == 0) {  // optimize exact case
                ...
                return
                    new SloppyPhraseScorer(this, postingsFreqs, slop,
                        similarity.simScorer(stats, context));
            }
        }
        ...
        public Explanation explain(AtomicReaderContext context, int doc) throws IOException {
            ...
            if (scorer != null) {
                ...
                if (newDoc == doc) {
                    float freq =
                        slop == 0 ? scorer.freq() : ((SloppyPhraseScorer) scorer).sloppyFreq();
                    SimScorer docScorer = similarity.simScorer(stats, context);
                    ComplexExplanation result = new ComplexExplanation();
                    result.setDescription(
                        "weight(" + getQuery() + " in " + doc + ") [" + similarity.getClass()
                                                                                  .getSimpleName()
                            + "], result of:");
