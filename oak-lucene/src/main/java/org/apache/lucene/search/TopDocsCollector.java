 ...
 * avoid the use of the priority queue entirely by passing null to
 * {@link #TopDocsCollector(PriorityQueue)}. In that case however, you might want to consider
...
public abstract class TopDocsCollector<T extends ScoreDoc> extends Collector {
     ...
     * either because there were 0 calls to collect() or because the arguments to topDocs were
     * invalid.
     */
     ...
     * Note that if start >= pq.size(), an empty TopDocs is returned.<br> This method is convenient
     * to call if the application always asks for the last results, starting from the last
     * 'page'.<br>
     * <b>NOTE:</b> you cannot call this method more than once for each search
     * execution. If you need to call it more than once, passing each time a different
     * <code>start</code>, you should call {@link #topDocs()} and work with the returned
     * {@link TopDocs} object, which will contain all the results this search execution collected.
     */
    ...
    /**
     * Returns the documents in the rage [start .. start+howMany) that were collected by this
     * collector. Note that if start >= pq.size(), an empty TopDocs is returned, and if pq.size() -
     * start &lt; howMany, then only the available documents in [start .. pq.size()) are
     * returned.<br> This method is useful to call in case pagination of search results is allowed
     * by the search application, as well as it attempts to optimize the memory used by allocating
     * only as much as requested by howMany.<br>
     * <b>NOTE:</b> you cannot call this method more than once for each search
     * execution. If you need to call it more than once, passing each time a different range, you
     * should call {@link #topDocs()} and work with the returned {@link TopDocs} object, which will
