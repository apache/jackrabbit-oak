 ...
 * {@link TermQuery}s, {@link PhraseQuery}s or other BooleanQuerys.
 */
public class BooleanQuery extends Query implements Iterable<BooleanClause> {
     ...
     * typically happens if a PrefixQuery, FuzzyQuery, WildcardQuery, or TermRangeQuery is expanded
     * to many terms during search.
     */
    ...
    /**
     * Return the maximum number of clauses permitted, 1024 by default. Attempts to add more than
     * the permitted number of clauses cause {@link TooManyClauses} to be thrown.
     *
     ...
     * Constructs an empty boolean query.
     * <p>
     * {@link Similarity#coord(int, int)} may be disabled in scoring, as appropriate. For example,
     * this score factor does not make sense for most automatically generated queries, like
    ...
    /**
     * Returns true iff {@link Similarity#coord(int, int)} is disabled in scoring for this query
     * instance.
     *
    ...
    protected class BooleanWeight extends Weight {
        ...
        public Scorer scorer(AtomicReaderContext context, boolean scoreDocsInOrder,
            ...
            if (optional.size() == 0 && prohibited.size() == 0) {
                ...
                return new ConjunctionScorer(this, required.toArray(new Scorer[required.size()]),
                    coord);
            }

            // simple disjunction
            if (required.size() == 0 && prohibited.size() == 0 && minNrShouldMatch <= 1
                && optional.size() > 1) {
                float coord[] = new float[optional.size() + 1];
                for (int i = 0; i < coord.length; i++) {
                    coord[i] = disableCoord ? 1.0f : coord(i, maxCoord);
                }
                return new DisjunctionSumScorer(this, optional.toArray(new Scorer[optional.size()]),
                    coord);
            }

            // Return a BooleanScorer2
            return new BooleanScorer2(this, disableCoord, minNrShouldMatch, required, prohibited,
                optional, maxCoord);
        }
    ...
    public Query rewrite(IndexReader reader) throws IOException {
        if (minNrShouldMatch == 0
            && clauses.size() == 1) {                    // optimize 1-clause queries
