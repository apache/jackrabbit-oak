 ...
 * sorting, by exposing a tight interaction with {@link FieldValueHitQueue} as it visits hits.
 * Whenever a hit is competitive, it's enrolled into a virtual slot, which is an int ranging from 0
 * to numHits-1.  The {@link FieldComparator} is made aware of segment transitions during searching
 * in case any internal state it's tracking needs to be recomputed during these transitions.</p>
 *
...
public abstract class FieldComparator<T> {
     ...
     * @return any N < 0 if slot2's value is sorted after slot1, any N > 0 if the slot2's value is
     * sorted before slot1 and 0 if they are equal
     */
    public abstract int compare(int slot1, int slot2);

    /**
     * Set the bottom slot, ie the "weakest" (sorted last) entry in the queue.  When
     * {@link #compareBottom} is called, you should compare against this slot.  This will always be
     * called before {@link #compareBottom}.
     *
    ...
    /**
     * Record the top value, for future calls to {@link #compareTop}.  This is only called for
     * searches that use searchAfter (deep paging), and is called before any calls to
     * {@link #setNextReader}.
     */
    public abstract void setTopValue(T value);

    /**
     * Compare the bottom of the queue with this doc.  This will only invoked after setBottom has
     * been called.  This should return the same result as {@link #compare(int, int)}} as if bottom
     * were slot1 and the new document were slot 2.
     *
     * <p>For a search that hits many results, this method
     * will be the hotspot (invoked by far the most frequently).</p>
     *
     * @param doc that was hit
     * @return any N < 0 if the doc's value is sorted after the bottom entry (not competitive), any
     * N > 0 if the doc's value is sorted before the bottom entry and 0 if they are equal.
     */
    public abstract int compareBottom(int doc) throws IOException;

    /**
     * Compare the top value with this doc.  This will only invoked after setTopValue has been
     * called.  This should return the same result as {@link #compare(int, int)}} as if topValue
    ...
    /**
     * This method is called when a new hit is competitive. You should copy any state associated
     * with this document that will be required for future comparisons, into the specified slot.
     *
    ...
    /**
     * Set a new {@link AtomicReaderContext}. All subsequent docIDs are relative to the current
     * reader (you must add docBase if you need to map it to a top-level docID).
     *
     * @param context current reader context
     * @return the comparator to use for this segment; most comparators can just return "this" to
     * reuse the same comparator across segments
    ...
    /**
     * Returns -1 if first is less than second.  Default impl to assume the type implements
     * Comparable and invoke .compareTo; be sure to override this method if your FieldComparator's
     * type isn't a Comparable or if your values may sometimes be null
     */
    ...
    /**
     * Parses field's values as byte (using {@link FieldCache#getBytes} and sorts by ascending
     * value
     */
    @Deprecated
    ...
    /**
     * Parses field's values as double (using {@link FieldCache#getDoubles} and sorts by ascending
     * value
     */
    ...
    /**
     * Parses field's values as float (using {@link FieldCache#getFloats} and sorts by ascending
     * value
     */
    ...
    /**
     * Parses field's values as short (using {@link FieldCache#getShorts} and sorts by ascending
     * value
     */
    @Deprecated
    public static final class ShortComparator extends NumericComparator<Short> {
        ...
        @Override
        public FieldComparator<Short> setNextReader(AtomicReaderContext context)
            throws IOException {
            // NOTE: must do this before calling super otherwise
            // we compute the docsWithField Bits twice!
    ...
    /**
     * Sorts by descending relevance.  NOTE: if you are sorting only by descending relevance and
     * then secondarily by ascending docID, performance is faster using {@link TopScoreDocCollector}
     ...
     * string to their relative ordinal positions (using the index returned by
     * {@link FieldCache#getTermsIndex}), and does most comparisons using the ordinals.  For medium
     * to large results, this comparator will be much faster than
     * {@link org.apache.lucene.search.FieldComparator.TermValComparator}.  For very small result
     * sets it may be slower.
     */
    public static final class TermOrdValComparator extends FieldComparator<BytesRef> {
        ...
        /**
         * Creates this, with control over how missing values are sorted.  Pass sortMissingLast=true
         * to put missing values at the end.
         */
