 ...
 * query boost for every document that matches the filter or query. For queries it therefore simply
 * strips of all scores and returns a constant one.
 */
public class ConstantScoreQuery extends Query {
     ...
     * of this query. If you simply want to strip off scores from a Query, no longer use
     * {@code new ConstantScoreQuery(new QueryWrapperFilter(query))}, instead use
     * {@link #ConstantScoreQuery(Query)}!
     */
        ...
        public Explanation explain(AtomicReaderContext context, int doc) throws IOException {
            ...
            if (exists) {
                ...
                result.setDescription(
                    ConstantScoreQuery.this.toString() + " doesn't match id " + doc);
                result.setValue(0);
        ...
        public Collection<ChildScorer> getChildren() {
            if (query != null) {
                return Collections.singletonList(
                    new ChildScorer((Scorer) docIdSetIterator, "constant"));
            } else {
