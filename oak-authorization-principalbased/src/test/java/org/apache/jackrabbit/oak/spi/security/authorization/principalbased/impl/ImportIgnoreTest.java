...
public class ImportIgnoreTest extends ImportBaseTest {
    ...
    public void testTransientPrincipal() throws Exception {
        ...
        assertTrue(
            getSession().getNode(transientSystemUser.getPath()).hasNode(REP_PRINCIPAL_POLICY));
        Node policy = getSession().getNode(transientSystemUser.getPath())
                                  .getNode(REP_PRINCIPAL_POLICY);
        assertTrue(policy.hasProperty(REP_PRINCIPAL_NAME));

        // but looking up policy doesn't work because of transient principal.
        AccessControlPolicy[] policies = getAccessControlManager().getPolicies(
            transientSystemUser.getPrincipal());
