...
2. `async` - This determines if the index is to be updated synchronously or asynchronously.
    ...
    * `sync` - The default value. It indicates that index is meant to be updated as part of each
      commit.
...
Depending on the type of the index, one can create these index definitions under the root
path ('/'),
or non-root paths.
...
Changes to both the main content, as well as the index content, are done atomically in a single
commit.
...
6. The diff traverses in a depth-first manner,
   ...
   Depending on the index implementation, the index data can be either stored in the NodeStore
   itself
...
The term "indexing lane" refers to a set of indexes which are to be updated by a given async
indexer.
...
and may become stale, that means new content added will only show up in query results after a longer
time.
...
Async indexers can be configured via the OSGi config
for `org.apache.jackrabbit.oak.plugins.index.AsyncIndexerService`.
...
* FailingIndexStats - Stats around indexes which
  are [failing and marked as corrupt](#corrupt-index-handling).
...
This feature can be disabled by setting `failingIndexTimeoutSeconds` to 0 in the `AsyncIndexService`
config.
...
Any query making use of such an index will automatically make use of both the persisted and the
local indexes.
With this, new content added in the repository after the last async index run will also show up
quickly.
...
In cluster setup, each cluster node maintains its own local index for changes happening in that
cluster node.
...
NRT indexing expose a few configuration options as part of
the [LuceneIndexProviderService](lucene.html#osgi-config):
...
We add the properties `supersedes = ["/oak:index/sampleIndex1"]` and `reindex=true`
to `sampleIndex2` and save those. As a
result, the superseded index `sampleIndex1` would be disabled (by setting type=disabled property)
during the next indexing cycle.
...
To reindex an _existing_ index (when needed), set the `reindex` property to `true` in the respective
index definition:
...
see the [reindeinxing property indexes](property-index.html#reindexing) section for more details on
that.
...
If the index being reindexed has full text extraction configured then reindexing can take long time
as most of the
time is spent in text extraction.
For such cases it's recommended to use text [pre-extraction support](pre-extract-text.html).
The text pre-extraction can be done before starting the actual reindexing. This would then ensure
that during reindexing
time is not spent in performing text extraction and hence the actual time taken for reindexing such
an index gets reduced
considerably.
...
For synchronous indexes, it can be stopped if it was started using
