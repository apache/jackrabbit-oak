...
/**
 * This type is a main entry point for the segment node store persistence. It's used every time the
 * access to the underlying storage (eg. tar files) is required.
 */
public interface SegmentNodeStorePersistence {
     ...
     *
     * @param memoryMapping    whether the memory mapping should be used (if the given persistence
     *                         supports it)
     * @param offHeapAccess    whether off heap access for segments should be used
     * @param ioMonitor        object used to monitor segment-related IO access. The implementation
     *                         should call the appropriate methods when accessing segments.
     * @param fileStoreMonitor object used to monitor the general IO usage.
     * @return segment archive manager
     * @throws IOException
     */
    SegmentArchiveManager createArchiveManager(boolean memoryMapping, boolean offHeapAccess,
        IOMonitor ioMonitor,
    ...
    /**
     * Acquire the lock on the repository. During the lock lifetime it shouldn't be possible to
     * acquire it again, either by a local or by a remote process.
     * <p>
     * The lock can be released manually by calling {@link RepositoryLock#unlock()}. If the segment
     * node store is shut down uncleanly (eg. the process crashes), it should be released
     * automatically, so no extra maintenance tasks are required to run the process again.
     *
