...
public class RequestDecoder extends MessageToMessageDecoder<String> {
    ...
    protected void decode(ChannelHandlerContext ctx, String msg, List<Object> out)
        ...
        if (request == null) {
            ...
            out.add(new GetBlobRequest(Messages.extractClientFrom(msg),
                request.substring(Messages.GET_BLOB.length())));
            ...
            out.add(new GetSegmentRequest(Messages.extractClientFrom(msg),
                request.substring(Messages.GET_SEGMENT.length())));
        } else if (request.startsWith(Messages.GET_REFERENCES)) {
            log.debug("Parsed 'get references' message");
            out.add(new GetReferencesRequest(Messages.extractClientFrom(msg),
                request.substring(Messages.GET_REFERENCES.length())));
        } else {
