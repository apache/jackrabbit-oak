...
public class SegmentNodeStoreFactory {
    ...
    @interface Configuration {
        ...
        @AttributeDefinition(
            name = "Role",
            description =
                "As multiple SegmentNodeStores can be configured, this parameter defines the role "
                    +
    ...
    public void activate(ComponentContext context, Configuration configuration) throws IOException {
        ...
        registrations.register(
            asCloseable(whiteboard.register(NodeStoreProvider.class, () -> store, props)));
        log.info("Registered NodeStoreProvider backed by SegmentNodeStore of type '{}'",
            configuration.role());
    }
        ...
        return SegmentNodeStoreRegistrar.registerSegmentNodeStore(
            new SegmentNodeStoreRegistrar.Configuration() {
                ...
                public int getStringDeduplicationCacheSize() {
                    return getCacheSize("stringDeduplicationCache.size",
                        configuration.stringDeduplicationCache_size());
                }

                @Override
                public int getTemplateDeduplicationCacheSize() {
                    return getCacheSize("templateDeduplicationCache.size",
                        configuration.templateDeduplicationCache_size());
                }

                @Override
                public int getNodeDeduplicationCacheSize() {
                    return roundToNextPowerOfTwo(getCacheSize("nodeDeduplicationCache.size",
                        configuration.nodeDeduplicationCache_size()));
                }
    ...
    private static StatisticsProvider getRoleStatisticsProvider(StatisticsProvider delegate,
        String role) {
