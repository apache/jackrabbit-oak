...
abstract class AbstractCompactionStrategy implements CompactionStrategy {
    ...
    private CompactionResult compactionSucceeded(
        ...
        return CompactionResult.succeeded(getCompactionType(), generation, context.getGCOptions(),
            compactedRootId, context.getGCCount());
    }
    ...
    private CompactionResult compactionPartiallySucceeded(
        ...
        context.getGCListener().compactionSucceeded(generation);
        return CompactionResult.partiallySucceeded(generation, compactedRootId,
            context.getGCCount());
    }
    ...
    private boolean setHead(Context context, SegmentNodeState previous, SegmentNodeState head) {
        return context.getRevisions()
                      .setHead(previous.getRecordId(), head.getRecordId(), EXPEDITE_OPTION);
    }

    final CompactionResult compact(Context context, NodeState base) {
        context.getGCListener()
               .info("running {} compaction", formatCompactionType(getCompactionType()));
        ...
        GCIncrement gcIncrement = new GCIncrement(baseGeneration, partialGeneration,
            targetGeneration);
        ...
        try {
            ...
            context.getCompactionMonitor()
                   .init(gcEntry.getRepoSize(), gcEntry.getNodes(), initialSize);
            ...
            do {
                ...
                Canceller stateSaveTrigger = context.getStateSaveTriggerSupplier().get()
                                                    .withShortCircuit();
                ...
                if (stateSaveTrigger.isCancelable()) {
                    ...
                    Canceller saveStateCanceller = softCanceller.withCondition(
                        "save intermediate compaction state",
                        () -> stateSaveTrigger.check().isCancelled());
                    compacted = compactor.compactDown(base, after, hardCanceller,
                        saveStateCanceller);
                ...
                context.getGCListener()
                       .info("compaction cycle 0 completed in {}. Compacted {} to {}",
                ...
                while (!(success = setHead(context, head, compacted)) && cycles < retryCount) {
                    ...
                    cycles++;
                    context.getGCListener()
                           .info("compaction detected concurrent commits while compacting. " +
                    ...
                    context.getGCListener().info(
                        "compaction cycle {} completed in {}. Compacted {} against {} to {}",
                        cycles, cycleWatch, head.getRecordId(), newHead.getRecordId(),
            ...
            if (!success) {
                context.getGCListener()
                       .info("compaction gave up compacting concurrent commits after {} cycles.",
                           cycles);
                int forceTimeout = context.getGCOptions().getForceTimeout();
                if (forceTimeout > 0) {
                    context.getGCListener()
                           .info("trying to force compact remaining commits for {} seconds. " +
                    ...
                    Canceller forcedCompactionCanceller = hardCanceller
                        ...
                        .withShortCircuit();
                    compacted = forceCompact(context, head, compacted, compactor,
                        forcedCompactionCanceller);
                    if (compacted != null) {
                        success = true;
                        flusher.flush();
                        context.getGCListener().info(
                            "compaction succeeded to force compact remaining commits after {}.",
                            forceWatch);
                    } else {
                        Cancellation cancellation = forcedCompactionCanceller.check();
                        if (cancellation.isCancelled()) {
                            context.getGCListener()
                                   .warn("compaction failed to force compact remaining commits " +
                                           "after {}. Compaction was cancelled: {}.",
                                       forceWatch,
                                       cancellation.getReason().orElse("unknown reason"));
                        } else {
                            context.getGCListener()
                                   .warn("compaction failed to force compact remaining commits. " +
            ...
            if (success) {
                ...
                context.getSuccessfulCompactionListener()
                       .onSuccessfulCompaction(getCompactionType());
                context.getCompactionMonitor().finished();

                if (compacted.isComplete()) {
                    context.getGCListener()
                           .info("compaction succeeded in {}, after {} cycles", watch, cycles);
                    ...
                    return compactionPartiallySucceeded(context, partialGeneration,
                        compacted.getRecordId());
                }
            } else {
                context.getGCListener()
                       .info("compaction failed after {}, and {} cycles", watch, cycles);
                return compactionAborted(context, targetGeneration);
            }
            ...
            context.getGCListener().error("compaction encountered an error",
    ...
    private Compactor newCompactor(Context context, CompactionWriter writer) {
        ...
        switch (compactorType) {
            ...
            case CHECKPOINT_COMPACTOR:
                return new CheckpointCompactor(context.getGCListener(), writer,
                    context.getCompactionMonitor());
