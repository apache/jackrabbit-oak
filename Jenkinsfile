        ...
        node(label: 'ubuntu') {
            ...
            timeout(60) {
                ...
                withEnv(["Path+JDK=$JAVA_JDK_8/bin","Path+MAVEN=$MAVEN_3_LATEST/bin","JAVA_HOME=$JAVA_JDK_8"]) {
                    // clean all modules
                    sh "${MAVEN_CMD} -Dfoo1=bara -T 1C clean"
                    // build and install up to desired module
                    sh "${MAVEN_CMD} -Dfoo2=barb -Dbaseline.skip=true -Prat -T 1C install -DskipTests -pl :${moduleName} -am"
                    try {
                        sh "${MAVEN_CMD} ${testOptions} -Dfoo3=barc -DtrimStackTrace=false -Dnsfixtures=SEGMENT_TAR,DOCUMENT_NS -Dmongo.db=MongoMKDB-${MONGODB_SUFFIX} clean verify -pl :${moduleName}"
                    } finally {
