...
public class ForkJoinUtilsTest {
    ...
    @Test
    public void executeCallableInCustomPool() {
        ...
        Map<Integer, Boolean> results = ForkJoinUtils
                .executeInCustomPool("custom-pool", 4, callable)
                .join();
        assertThat(results)
                .hasSize(10)
                .allSatisfy((key, value) -> {
    ...
    public void reentryInCustomPool() {
        Map<Integer, String> results = ForkJoinUtils
                .executeInCustomPool("outer-pool", 4, getReentryTestCallable())
                .join();
    ...
    private static @NotNull Callable<Map<Integer, String>> getReentryTestCallable() {
        return () -> IntStream.range(0, 4)
                .parallel()
                .mapToObj(i -> ForkJoinUtils.executeInCustomPool("inner-pool-" + i, 4, () -> IntStream.range(0, 4)
                                .parallel()
                                .map(j -> i * 4 + j)
    ...
    public void executeInCustomPoolWithInvalidParallelism() {
        ForkJoinUtils.executeInCustomPool("custom-pool", 0, () -> {});
    }
