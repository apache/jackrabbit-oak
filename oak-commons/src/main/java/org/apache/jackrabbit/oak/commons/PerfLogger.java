...
/**
 * PerfLogger is a simpler wrapper around a slf4j Logger which comes with the capability to issue
 * log statements containing the measurement between start() and end() methods.
 * <p>
 * Usage:
 * <ul>
...
public final class PerfLogger {
    ...
    /**
     * Returns quickly if log level is not DEBUG or TRACE - if it is DEBUG, then just returns the
     * current time in millis, if it is TRACE, then log the given message and also return the
     * current time in millis.
     *
     * @param traceMsgOrNull the message to log if log level is TRACE - or null if no message should
     *                       be logged (even on TRACE level)
    ...
    /**
     * See {@link #end(long, long, long, String, Object...)} Note that this method exists for
     * performance optimization only (compared to the other end() method with a vararg.
     */
    ...
    /**
     * Shortcut to {@link #end(long, long, long, String, Object...)} for {@code logMessagePrefix} =
     * {@link Long#MAX_VALUE}
     */
    ...
    /**
     * Returns quickly if start is negative (which is the case according to log level at the time of
     * {@link #start()} or {@link #startForInfoLog()}). If log level is set to TRACE, log.trace is
     * always emitted. If log level is set to DEBUG, then log.debug is emitted if 'now' is bigger
     * (slower) than {@code start} by at least {@code logAtDebugIfSlowerThanMs}. If log level is set
     * to INFO, then long.info is emitted if 'now' is bigger (slower) than {@code start} by at least
     * {@code logAtInfoIfSlowerThanMs}.
     *
     * @param start                    the start time with which 'now' should be compared
     * @param logAtDebugIfSlowerThanMs the number of milliseconds that must be surpassed to issue a
     *                                 log.debug (if log level is DEBUG)
     * @param logAtInfoIfSlowerThanMs  the number of milliseconds that must be surpassed to issue a
     *                                 log.info (if log level is DEBUG)
     * @param logMessagePrefix         the log message 'prefix' - to which the given arguments will
     *                                 be passed, plus the measured time difference in the format
     *                                 '[took x ms']
     ...
     * @return {@code true} if log level not even at INFO or log level is not at least DEBUG as well
     * as {@code logAtInfoToo} is false too ; false otherwise
     */
