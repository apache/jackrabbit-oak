...
public interface HttpDataRecordUpload {
     ...
     * uploadPartURIs.  In other words, clients wishing to perform a multi-part upload
     ...
     * multi-part upload without requiring more URIs that those provided, SO LONG AS the file
     ...
     * API is that using parts of maxPartSize will work without using more URIs than those
     * available in the collection of uploadPartURIs.
     *
     * If a client calls {@link HttpDataRecordProvider#initiateHttpUpload(long, int)}
     * with a value of maxUploadSizeInBytes that ends up being smaller than the actual size
     * of the binary to be uploaded, it may not be possible to complete the upload with the
     * URIs provided.  The client should initialize the transaction again with the correct size.
     *
    ...
    /**
     * Returns a collection of direct-writable upload URIs for uploading a file, or file part in the case
     * of multi-part uploading.  This collection may contain only a single URI in the following cases:
     ...
     * single-put upload and write the entire binary to the single URI.  Otherwise the client
     * may choose to consume up to the entire collection of URIs provided.
     *
