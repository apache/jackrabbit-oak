...
public final class AwsContext {
    ...
    private AwsContext(AmazonS3 s3, String bucketName, String rootDirectory, AmazonDynamoDB ddb,
        ...
        this(s3, bucketName, rootDirectory, ddb, journalTableName, lockTableName,
            DynamoDBProvisioningData.DEFAULT);
    }
    ...
    private AwsContext(Configuration configuration) {
        ...
        this.dynamoDBClient = new DynamoDBClient(dynamoDBClientBuilder.build(),
            configuration.journalTableName(),
     ...
     * @param rootDirectory    The root directory under which the segment store is setup.
     * @param ddb              Client for accessing Amazon DynamoDB.
     * @param journalTableName Name of table used for storing log entries for journal and gc. The
     *                         table will be created if it doesn't already exist. It should have a
     *                         partition key on "{@link DynamoDBClient#TABLE_ATTR_FILENAME}" and
     *                         sort key on "{@link DynamoDBClient#TABLE_ATTR_TIMESTAMP}".
     * @param lockTableName    Name of table used for managing the distributed lock. The table will
     *                         be created if it doesn't already exist. It should have a partition
     *                         key on "{@link DynamoDBClient#LOCKTABLE_KEY}".
     * @return The context.
     * @throws IOException
     */
    public static AwsContext create(AmazonS3 s3, String bucketName, String rootDirectory,
        AmazonDynamoDB ddb,
    ...
    public static AwsContext create(AmazonS3 s3, String bucketName, String rootDirectory,
        ...
        throws IOException {
        AwsContext awsContext = new AwsContext(s3, bucketName, rootDirectory, ddb, journalTableName,
            lockTableName, provisioningData);
        awsContext.directory.ensureBucket();
        awsContext.dynamoDBClient.ensureTables();
        return awsContext;
    }
