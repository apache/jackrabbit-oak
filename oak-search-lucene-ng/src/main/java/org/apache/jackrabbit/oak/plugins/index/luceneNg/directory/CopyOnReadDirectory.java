 ...
 * <p>
 * Port of {@code oak-lucene}'s {@code CopyOnReadDirectory} for Lucene 9's {@link Directory}
 * API. {@code remote} is narrowed to {@link OakDirectory} (this module's only remote
 * implementation, see {@link LuceneNgIndexCopier#wrapForRead}) and a {@code localDir}
 * {@link File} is threaded through so local-side existence checks can use direct
 * {@link File} calls (via {@link LuceneNgIndexCopier#existsLocally(File, String)}) instead of
 * the removed {@code Directory.fileExists}. Lucene 9 also removed the source-invoked
 * {@code Directory.copy(...)} in favour of a destination-invoked {@code copyFrom(...)}.
 */
public class CopyOnReadDirectory extends FilterDirectory {
    ...
    private final LuceneNgIndexCopier indexCopier;
    ...
    public CopyOnReadDirectory(LuceneNgIndexCopier indexCopier, OakDirectory remote, Directory local, File localDir,
                               boolean prefetch, String indexPath, Executor executor) throws IOException {
        super(remote);
    ...
    public IndexInput openInput(String name, IOContext context) throws IOException {
        if (Objects.nonNull(name) && LuceneNgIndexCopier.REMOTE_ONLY.contains(name)) {
    ...
    private void removeDeletedFiles() throws IOException {
        Set<String> remoteFiles = stream(remote.listAll())
                .filter(name -> !LuceneNgIndexCopier.REMOTE_ONLY.contains(name))
                .collect(Collectors.toSet());

        long maxTS = LuceneNgIndexCopier.getNewestLocalFSTimestampFor(remoteFiles, local);
        if (maxTS == -1) {
        ...
        Set<String> filesToBeDeleted =
                ...
                .filter(name -> LuceneNgIndexCopier.isFileModifiedBefore(name, local, deleteBeforeTS))
                // can be deleted
                .collect(Collectors.toSet())
        ;
        ...
        for (String fileName : filesToBeDeleted) {
            boolean deleted = indexCopier.deleteFile(local, localDir, fileName, true);
            if (!deleted){
