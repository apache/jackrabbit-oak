...
public class SegmentAzureFactoryTest {
    ...
    public void testConnectionWithConnectionString_accessKey() throws IOException {
        ...
        String connectionString = String.format(connectionStringWithPlaceholder,
            AzuriteDockerRule.ACCOUNT_NAME, AzuriteDockerRule.ACCOUNT_KEY, azurite.getMappedPort(),
            AzuriteDockerRule.ACCOUNT_NAME);
        SegmentAzureFactory segmentAzureFactory = new SegmentAzureFactory.Builder("respository",
            256,
            false)
        ...
        FileStoreUtils.NodeStoreWithFileStore nodeStore = (FileStoreUtils.NodeStoreWithFileStore) segmentAzureFactory.create(
            null, closer);
    ...
    public void testConnectionWithConnectionString_sas() throws IOException {
        ...
        String connectionString = String.format(connectionStringWithPlaceholder,
            AzuriteDockerRule.ACCOUNT_NAME, sasToken, azurite.getMappedPort(),
            AzuriteDockerRule.ACCOUNT_NAME);
    ...
    public void testConnectionWithUri_accessKey() throws IOException {
        ...
        assumeNotNull(ENVIRONMENT.getVariable(AZURE_ACCOUNT_NAME),
            ENVIRONMENT.getVariable(AZURE_SECRET_KEY));

        final String CONTAINER_NAME = "oak-migration-test";

        String uri = String.format(CONNECTION_URI, ENVIRONMENT.getVariable(AZURE_ACCOUNT_NAME),
            CONTAINER_NAME);
        Closer closer = Closer.create();
        try {
            SegmentAzureFactory segmentAzureFactory = new SegmentAzureFactory.Builder(DIR, 256,
    ...
    public void testConnectionWithUri_servicePrincipal() throws IOException, InterruptedException {
        assumeNotNull(ENVIRONMENT.getVariable(AZURE_ACCOUNT_NAME),
            ENVIRONMENT.getVariable(AZURE_TENANT_ID),
    ...
    private void cleanup(String uri) {
        uri = uri + "/" + DIR;
        try {
            CloudBlobDirectory cloudBlobDirectory = ToolUtils.createCloudBlobDirectory(uri,
                ENVIRONMENT);
    ...
    private SharedAccessAccountPolicy getPolicy() {
        ...
        EnumSet<SharedAccessAccountPermissions> sharedAccessAccountPermissions = EnumSet.of(
            SharedAccessAccountPermissions.CREATE,
            SharedAccessAccountPermissions.DELETE, SharedAccessAccountPermissions.READ,
            SharedAccessAccountPermissions.UPDATE,
            SharedAccessAccountPermissions.WRITE, SharedAccessAccountPermissions.LIST);
        EnumSet<SharedAccessAccountService> sharedAccessAccountServices = EnumSet.of(
