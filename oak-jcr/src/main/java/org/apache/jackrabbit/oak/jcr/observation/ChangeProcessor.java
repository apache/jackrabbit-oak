...
/**
 * A {@code ChangeProcessor} generates observation {@link javax.jcr.observation.Event}s based on a
 * {@link FilterProvider filter} and delivers them to an {@link EventListener}.
 * <p>
 * After instantiation a {@code ChangeProcessor} must be started in order to start delivering
 * observation events and stopped to stop doing so.
 */
class ChangeProcessor implements FilteringAwareObserver {
    ...
    private enum FilterResult {
        /**
         * marks a commit as to be included, ie delivered. It's okay to falsely mark a commit as
         * included, since filtering (as part of converting to events) will be applied at a later
         * stage again.
         */
        INCLUDE,
        /**
         * mark a commit as not of interest to this ChangeProcessor. Exclusion is definite, ie it's
         * not okay to falsely mark a commit as excluded
         */
        EXCLUDE,
        /**
         * mark a commit as included but indicate that this is not a result of prefiltering but that
         * prefiltering was skipped/not applicable for some reason
         */
        PREFILTERING_SKIPPED
    }

    /**
     * Fill ratio of the revision queue at which commits should be delayed (conditional of
     * {@code commitRateLimiter} being non {@code null}).
     */
    public static final double DELAY_THRESHOLD;

    /**
     * Maximal number of milli seconds a commit is delayed once {@code DELAY_THRESHOLD} kicks in.
     */
    ...
    /**
     * Number of milliseconds to wait before issuing consecutive queue full warn messages Controlled
     * by command line property "oak.observation.full-queue.warn.interval". Note, the command line
     * parameter is wait interval in minutes.
     */
    ...
    static {
        ...
        try {
            if (delayThresholdStr != null && delayThresholdStr.length() != 0) {
                ...
                LOG.info(
                    "<clinit> using oak.commitRateLimiter.delayThreshold of " + delayThreshold);
            }
        } catch (RuntimeException e) {
            LOG.warn("<clinit> could not parse oak.commitRateLimiter.delayThreshold, using default("
                + delayThreshold + "): " + e, e);
        }
        try {
            ...
            LOG.warn(
                "<clinit> could not parse oak.commitRateLimiter.maxDelay, using default(" + maxDelay
                    + "): " + e, e);
        }
    ...
    /**
     * JMX ObjectName property storing the listenerId which allows to correlate various mbeans
     */
    ...
    /**
     * for statistics: tracks how many times prefiltering was ignored (not evaluated at all), either
     * because it was disabled, queue too small, CommitInfo null or CommitContext null
     */
     ...
     *
     * @param whiteboard the whiteboard instance to used for scheduling individual runs of this
     *                   change processor.
    ...
    public synchronized void start(Whiteboard whiteboard) {
        ...
        registration = new CompositeRegistration(
            ...
            registerMBean(whiteboard, BackgroundObserverMBean.class,
                filteringObserver.getBackgroundObserver().getMBean(), BackgroundObserverMBean.TYPE,
                name, attrs),
            ...
            private void queueSizeChanged(int newQueueSize) {
                ...
                if (newQueueSize >= queueLength) {
                    if (commitRateLimiter != null) {
                        if (!blocking) {
                            logQueueFullWarning(
                                "Revision queue is full. Further commits will be blocked.");
                        }
                        commitRateLimiter.blockCommits();
                    } else if (!blocking) {
                        logQueueFullWarning(
                            "Revision queue is full. Further revisions will be compacted.");
                    }
                    blocking = true;
                } else {
                    ...
                    if (fillRatio > DELAY_THRESHOLD) {
                        if (commitRateLimiter != null) {
                            if (delay == 0) {
                                LOG.warn(
                                    "Revision queue is becoming full. Further commits will be delayed.");
                            }
                            ...
                            int newDelay =
                                1 + (int) ((fillRatio - DELAY_THRESHOLD) / (1 - DELAY_THRESHOLD)
                                    * MAX_DELAY);
                            if (newDelay > delay) {
                                delay = newDelay;
    ...
    /**
     * Try to stop this change processor if running. This method will wait the specified time for a
     * pending event listener to complete. If no timeout occurred no further events will be
     * delivered after this method returns.
     * <p>
     * Does nothing if stopped already.
     *
     * @param timeOut time this method will wait for an executing event listener to complete.
     * @param unit    time unit for {@code timeOut}
     * @return {@code true} if no time out occurred and this change processor could be stopped,
    ...
    /**
     * Stop this change processor after all pending events have been delivered. In contrast to
     * {@link #stopAndWait(int, java.util.concurrent.TimeUnit)} this method returns immediately
     * without waiting for pending listeners to complete.
     */
    ...
    private static class CountingIterator implements EventIterator {
        ...
        public Event next() {
            if (eventCount == -1) {
                LOG.warn(
                    "Access to EventIterator outside the onEvent callback detected. This will " +
    ...
    private static class RunningGuard extends Guard {
        ...
        /**
         * @return {@code true} if this call set this guard to stopped, {@code false} if another
         * call set this guard to stopped before.
         */
        public boolean stop() {
     ...
     *
     * @param changeSet
     * @return a FilterResult indicating either inclusion, exclusion or inclusion-due-to-skipping.
     * The latter is used to reflect prefilter evaluation better in statistics (as it could also
     * have been reported just as include)
     */
