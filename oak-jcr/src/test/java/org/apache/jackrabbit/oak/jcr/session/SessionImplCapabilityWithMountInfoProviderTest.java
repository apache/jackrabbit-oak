...
public class SessionImplCapabilityWithMountInfoProviderTest {
    ...
    public void prepare() throws Exception {
        ...
        MemoryNodeStore roStore = new MemoryNodeStore();
        {
            ...
            builder
                .child("private")
                .setProperty(JcrConstants.JCR_PRIMARYTYPE, JcrConstants.NT_UNSTRUCTURED)
    ...
    public void addNode() throws Exception {
        ...
        assertFalse("Must not be able to add a child not under the private mount root",
            adminSession.hasCapability("addNode", adminSession.getNode(MOUNT_PATH),
                new String[]{"foo"}));
        assertFalse("Must not be able to add a child not under the private mount",
            adminSession.hasCapability("addNode", adminSession.getNode(MOUNT_PATH_FOO),
                new String[]{"bar"}));
        // able to add nodes outside the read-only mount
        assertTrue("Must be able to add a child node under the root",
            adminSession.hasCapability("addNode", adminSession.getNode(ROOT_PATH),
                new String[]{"not-private"}));
    ...
    public void orderBefore() throws Exception {
        ...
        assertTrue(
            adminSession.hasCapability("orderBefore", adminSession.getNode(MOUNT_PATH), null));
        assertFalse(
            adminSession.hasCapability("orderBefore", adminSession.getNode(MOUNT_PATH_FOO), null));
        // root node can never be reordered
        assertFalse(
            adminSession.hasCapability("orderBefore", adminSession.getNode(ROOT_PATH), null));
    }

    @Test
    public void simpleNodeOperations() throws Exception {
        for (String operation : new String[]{"setPrimaryType", "addMixin", "removeMixin",
            "setProperty", "remove"}) {
            for (String privateMountNode : new String[]{MOUNT_PATH, MOUNT_PATH_FOO}) {
                assertFalse("Unexpected return value for hasCapability(" + operation + ") on node '"
                        + privateMountNode + "' from the private mount",
                    adminSession.hasCapability(operation, adminSession.getNode(privateMountNode),
                        null));
            }
            assertTrue("Unexpected return value for hasCapability(" + operation + ") on node '"
                    + GLOBAL_PATH_FOO + "' from the global mount",
    ...
    public void simpleNodeOperationsMissingPermission() throws Exception {
        ...
        for (String operation : new String[]{"setPrimaryType", "addMixin", "removeMixin",
            "setProperty", "remove"}) {
            assertFalse(
                "Unexpected return value for hasCapability(" + operation + ") on node '" + ROOT_PATH
                    + "' from the global mount",
    ...
    public void uncoveredNodeOperation() throws Exception {
        ...
        assertFalse(
            "Unexpected return value for hasCapability('+unknownOperation+') on node '/private' from the private mount.",
            adminSession.hasCapability(unknownOperation, adminSession.getNode(MOUNT_PATH), null));
        assertTrue(
            "Unexpected return value for hasCapability('+unknownOperation+') on node '/foo' from the global mount.",
            adminSession.hasCapability(unknownOperation, adminSession.getNode(GLOBAL_PATH_FOO),
                null));
    }
    ...
    public void itemOperations() throws Exception {
        for (String operation : new String[]{"setValue", "remove", "unknown"}) {
            ...
            assertFalse("Unexpected return value for hasCapability(" + operation + ") on item '"
                    + privateMountProp + "' from the private mount",
                adminSession.hasCapability(operation, adminSession.getItem(privateMountProp),
                    null));
            assertTrue("Unexpected return value for hasCapability(" + operation + ") on item '"
                    + globalMountProp + "' from the global mount",
    ...
    public void policyWriteOperation() throws Exception {
        ...
        for (String operation : new String[]{"setPolicy", "removePolicy"}) {
            assertFalse(
                "Unexpected return value for hasCapability(" + operation + ") on the private mount",
                adminSession.hasCapability(operation, acMgr, new Object[]{MOUNT_PATH_FOO, policy}));
            assertTrue(
                "Unexpected return value for hasCapability(" + operation + ") on the global mount",
                adminSession.hasCapability(operation, acMgr,
                    new Object[]{GLOBAL_PATH_FOO, policy}));
        }
    ...
    public void policyWriteOperationMissingPermission() throws Exception {
        ...
        for (String operation : new String[]{"setPolicy", "removePolicy"}) {
            ...
            assertFalse(
                "Unexpected return value for hasCapability(" + operation + ") on the global mount",
                guestSession.hasCapability(operation, acMgr,
                    new Object[]{GLOBAL_PATH_FOO, policy}));
        }
        verifyNoInteractions(policy);
    }
    ...
    public void policyWriteOperationMissingArguments() throws Exception {
        ...
        for (String operation : new String[]{"setPolicy", "removePolicy"}) {
            ...
            assertFalse("Unexpected return value for hasCapability(" + operation
                    + ") with insufficient arguments.",
                guestSession.hasCapability(operation, acMgr, new Object[0]));
            assertFalse(
                "Unexpected return value for hasCapability(" + operation + ") with null arguments.",
    ...
    public void uncoveredAccessControlMethod() throws Exception {
        AccessControlManager acMgr = guestSession.getAccessControlManager();
        AccessControlPolicy policy = mock(AccessControlPolicy.class);
        for (String operation : new String[]{"getApplicablePolicies", "getPolicies",
