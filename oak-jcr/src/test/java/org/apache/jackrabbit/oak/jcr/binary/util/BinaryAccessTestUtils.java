...
public class BinaryAccessTestUtils {
    ...
    /**
     * Creates an nt:file based on the content, saves the session and retrieves the Binary value
     * again.
     */
    public static Binary storeBinaryAndRetrieve(Session session, String path, Content content)
     ...
     */
    public static void storeBinary(Session session, String path, Binary binary)
        throws RepositoryException {
     ...
     */
    public static void putBinary(Session session, String ntFilePath, Binary binary)
        throws RepositoryException {
        Node ntResource;
        if (session.nodeExists(ntFilePath + "/" + JcrConstants.JCR_CONTENT)) {
            ntResource = session.getNode(ntFilePath + "/" + JcrConstants.JCR_CONTENT);
        } else {
            Node file = session.getRootNode()
                               .addNode(ntFilePath.substring(1), JcrConstants.NT_FILE);
     ...
     * @return HTTP response code from the upload request.  Note that a successful response for S3
     * is 200 - OK whereas for Azure it is 201 - Created.
     * @throws IOException
     */
    public static int httpPut(@Nullable URI uri, long contentLength, InputStream in)
        throws IOException {
