...
public abstract class IndexImproperUsageCommonTest extends AbstractQueryTest {
    ...
    private static final String PATH_RESTRICTION_WARN_MESSAGE =
    ...
    public void pathIncludeWithPathRestrictionsWarn() throws Exception {
        ...
        assertEventually(() -> {
            ...
            assertTrue(explain("select [jcr:path] from [nt:base] where [propa] = 10").contains(
    ...
    public void queryFilterRegexRestrictionsWarn() throws Exception {
        ...
        assertEventually(() -> {
            assertTrue(explain("select [jcr:path] from [nt:base] where [propa] = \"oak\"").contains(
                indexOptions.getIndexType() + ":" + indexName));
            ...
            assertFalse(isWarnMessagePresent(listAppender,
                String.format(QUERY_FILTER_WARN_MESSAGE, indexName, regex, "oak")));

            assertTrue(explain("select [jcr:path] from [nt:base] where [propa] = \"ack\"").contains(
                indexOptions.getIndexType() + ":" + indexName));
            ...
            assertTrue(explain(
                "select [jcr:path] from [nt:base] where [propa] = \"oak\" option (index tag testtag)").contains(
                indexOptions.getIndexType() + ":" + indexName));
            ...
            assertFalse(isWarnMessagePresent(listAppender,
                String.format(QUERY_FILTER_WARN_MESSAGE, indexName, regex, "testtag")));
