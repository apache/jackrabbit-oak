...
public class IndexDefinitionFacetConfigTest {
    ...
    public void systemPropSecureFacetStatisticalSampleSize() {
        ...
        sampleSize = SecureFacetConfiguration.getInstance(RANDOM_SEED, root)
                                             .getStatisticalFacetSampleSize();
    ...
    public void invalidSecureFacetSampleSize() {
        int sampleSize;
        NodeBuilder configBuilder = builder.child("config").setProperty(PROP_SECURE_FACETS,
            PROP_SECURE_FACETS_VALUE_STATISTICAL);
        NodeState nodeState = configBuilder.getNodeState();

        configBuilder.setProperty(PROP_STATISTICAL_FACET_SAMPLE_SIZE, -10);
        sampleSize = SecureFacetConfiguration.getInstance(RANDOM_SEED, nodeState)
                                             .getStatisticalFacetSampleSize();
        assertEquals(STATISTICAL_FACET_SAMPLE_SIZE_DEFAULT, sampleSize);
    ...
    public void orderingOfOverrides() {
        ...
        nodeState = builder.child("config1").setProperty(PROP_SECURE_FACETS, "secure")
                           .getNodeState();
    ...
    public void randomSeedWithoutOneInDef() {
        long seed1 = new IndexDefinition(root, root, "/foo").getSecureFacetConfiguration()
                                                            .getRandomSeed();
        long seed2 = new IndexDefinition(root, root, "/foo").getSecureFacetConfiguration()
                                                            .getRandomSeed();
