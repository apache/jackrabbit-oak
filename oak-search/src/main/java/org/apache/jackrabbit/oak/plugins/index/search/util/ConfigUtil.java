...
public class ConfigUtil {
    ...
    public static boolean getOptionalValue(NodeState definition, String propName,
        boolean defaultVal) {
        try {
            ...
            throw new IllegalStateException(
                String.format(ILLEGAL_STATE_EXCEPTION_ERROR_MESSAGE, propName), e);
        }
    }
    ...
    public static String getOptionalValue(NodeState definition, String propName,
        String defaultVal) {
        try {
            PropertyState ps = definition.getProperty(propName);
    ...
    public static double getOptionalValue(NodeState definition, String propName,
        double defaultVal) {
        try {
            PropertyState ps = definition.getProperty(propName);
    ...
    public static String getPrimaryTypeName(NodeState nodeState) {
        try {
            ...
            throw new IllegalStateException(
                String.format(ILLEGAL_STATE_EXCEPTION_ERROR_MESSAGE, JcrConstants.JCR_PRIMARYTYPE),
                e);
        }
    }
    ...
    /**
     * Assumes that given state is of type nt:file and then reads the jcr:content/@jcr:data property
     * to get the binary content
     */
    @Nullable
    public static Blob getBlob(NodeState state, String resourceName) {
        ...
        checkArgument(contentNode.exists(),
            "Was expecting to find jcr:content node to read resource %s", resourceName);
     ...
     */
    public static <T> T[] getOptionalValues(NodeState definition, String propName,
        Type<Iterable<T>> type, Class<T> typeParam) {
        return getOptionalValues(definition, propName, type, typeParam, null);
    }

    /**
     * Returns an array of optional values for the a given property if present, otherwise returns
     * the specified default
     */
    public static <T> T[] getOptionalValues(NodeState definition, String propName,
        Type<Iterable<T>> type, Class<T> typeParam, T[] defaultValues) {
        PropertyState ps = definition.getProperty(propName);
        if (ps != null) {
