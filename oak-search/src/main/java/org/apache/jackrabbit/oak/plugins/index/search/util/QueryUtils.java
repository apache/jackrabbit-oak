...
public class QueryUtils {
    ...
    private static void replaceWildcard(StringBuilder sb, int position, char oldWildcard,
        char newWildcard) {
        if (sb.charAt(position) == oldWildcard) {
            int escapeCount = 0;
            for (int m = position - 1; m >= 0 && sb.charAt(m) == WILDCARD_ESCAPE;
                m--, escapeCount++)
                ;
            if (escapeCount % 2 == 0) {
    ...
    /**
     * Converts an SQL2 like pattern to a Lucene/Elastic pattern for wildcard queries. In
     * particular, this method converts any wildcard in the SQL2 like pattern (% and _) to Lucene
     * wildcards (* and ?), handling escaped characters (as in \% and \_).
     *
