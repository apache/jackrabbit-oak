...
public abstract class FulltextIndexEditorContext<D> {
    ...
    private static final PerfLogger PERF_LOGGER =
        new PerfLogger(
            LoggerFactory.getLogger(FulltextIndexEditorContext.class.getName() + ".perf"));
    ...
    public abstract DocumentMaker<D> newDocumentMaker(IndexDefinition.IndexingRule rule,
        String path);

    protected FulltextBinaryTextExtractor createBinaryTextExtractor(
        ExtractedTextCache extractedTextCache,
    ...
    public void closeWriter() throws IOException {
        ...
        if (indexUpdated) {
            ...
            status.setProperty(IndexDefinition.STATUS_LAST_UPDATED, getUpdatedTime(currentTime),
                Type.DATE);
            ...
            if (reindex) {
                ...
                log.info(
                    IndexDefinition.REINDEX_COMPLETION_TIMESTAMP + " set to current time for index:"
    ...
    private String getUpdatedTime(Calendar currentTime) {
        ...
        String checkpointTime = (String) info.getInfo()
    ...
    public void enableReindexMode() {
        ...
        ReindexOperations reindexOps =
            new ReindexOperations(root, definitionBuilder, definition.getIndexPath(),
                newDefinitionBuilder(), storedIndexDefinitionEnabled());
    ...
    private IndexDefinition createIndexDefinition(NodeState root, NodeBuilder definition,
        IndexingContext
                ...
                if (definition.getBoolean(PROP_REFRESH_DEFN)) {
                    ...
                    definition.getChildNode(INDEX_DEFINITION_NODE)
                              .setProperty(IndexDefinition.CREATION_TIMESTAMP,
                                  ISO8601.format(Calendar.getInstance()), Type.DATE);
                    log.info(
                        "Stored the cloned index definition for [{}]. Changes in index definition would now only be "
                            +
                    ...
                    Long clonedRandom = getLongPropertyOrNull(
                        clonedState.getProperty(PROP_RANDOM_SEED));

                    if (clonedRandom == null || clonedRandom != defRandom) {
                        definition.getChildNode(INDEX_DEFINITION_NODE)
                                  .setProperty(PROP_RANDOM_SEED, defRandom);
                    }
                }
            }
        }
