    ...
    private String parseStringValue0(Blob v, Metadata metadata, String path) {
        ...
        try {
            CountingInputStream stream = new CountingInputStream(
                new LazyInputStream(() -> v.getNewStream()));
            try {
        ...
        extractedTextCache.put(v,
            new ExtractedText(ExtractedText.ExtractionResult.SUCCESS, result));
        return result;
    }
    ...
    private static TikaConfigHolder initializeTikaConfig(@Nullable IndexDefinition definition) {
        ...
        try {
            Thread.currentThread()
                  .setContextClassLoader(FulltextIndexEditorContext.class.getClassLoader());
