...
public class ElasticTestServer implements AutoCloseable {
        ...
        if (CONTAINER == null || !CONTAINER.isRunning()) {
            ...
            if (CONTAINER == null || !CONTAINER.isRunning()) {
                ...
                throw new RuntimeException(
                    "Unable to start ES container after retries. Any further tests will fail");
            }
    ...
    private synchronized void setup() {
        ...
        CONTAINER = new ElasticsearchContainer(
            "docker.elastic.co/elasticsearch/elasticsearch:" + esDockerImageVersion)
    ...
    private void downloadSimilaritySearchPluginIfNotExists(String localPluginPath,
        ...
        if (!pluginFile.exists()) {
            ...
            if (PLUGIN_OFFICIAL_RELEASES_DIGEST_MAP.containsKey(pluginVersion)) {
                ...
                pluginUri =
                    "https://github.com/alexklibisz/elastiknn/releases/download/" + pluginVersion
                        + "/elastiknn-" + pluginVersion + ".zip";
            } else {
                pluginDigest = null; // Skip validation
                pluginUri = ElasticTestUtils.ELASTIC_KNN_PLUGIN_URI;
                if (pluginUri == null) {
                    throw new RuntimeException("Elastiknn " + pluginVersion
                        + " is not a known official release, so it cannot be downloaded from the official GitHub repo. Please provide the download URI in system property \""
                        + ElasticTestUtils.ELASTIC_KNN_PLUGIN_URI_KEY + "\".");
                }
            }
                ...
                try (InputStream inputStream = new URL(pluginUri).openStream();
                    ...
                    if (pluginDigest != null) {
                        ...
                        if (!pluginDigest.equals(result.toString())) {
                            ...
                            throw new RuntimeException(
                                "Plugin digest unequal. Found " + result + ". Expected "
                                    + pluginDigest + ". " + deleteString);
                        }
                    } else {
    ...
    private void checkIfDockerClientAvailable() {
        ...
        try {
            ...
            LOG.warn(
                "Docker is not available and elasticConnectionDetails sys prop not specified or incorrect"
                    +
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println(
            "Docker container with Elasticsearch launched at \"" + esContainer.getHttpHostAddress()
                +
