...
public class ElasticIndexNameHelper {
    ...
    private static final Pattern INVALID_START_CHARS_REGEX = Pattern.compile(
        INVALID_NAME_START_CHARS
            .stream()
            .map(c -> "\\" + c)
            .collect(Collectors.joining("", "^[", "]+")));
    ...
    public static String getRemoteIndexName(String indexPrefix, String indexName,
        NodeBuilder definitionBuilder) {
        PropertyState seedProp = definitionBuilder.getProperty(
            ElasticIndexDefinition.PROP_INDEX_NAME_SEED);
        if (seedProp == null) {
     ...
     *
     * @param indexPrefix the prefix of the index. This value does not get validated at this stage
     *                    since it comes from outside and the validation already happened in
     *                    {@link ElasticConnection.Builder} when the entire module gets
     *                    initialized.
     * @param indexPath   the path of the index that will be checked, eventually transformed and
    ...
    public static String getElasticSafeIndexName(String indexPrefix, String indexPath) {
        String name = indexPrefix + "." + StreamSupport
            ...
            .limit(
                3) //Max 3 nodeNames including oak:index which is the immediate parent for any indexPath
    ...
    /**
     * Convert {@code e} to Elasticsearch safe index name. Ref:
    ...
    static boolean isValidPrefix(@NotNull String prefix) {
        if (!prefix.equals("")) {
            return prefix.equals(prefix.toLowerCase()) && // it has to be lowercase
                !INVALID_START_CHARS_REGEX.matcher(prefix).find() &&
                // not start with specific chars
