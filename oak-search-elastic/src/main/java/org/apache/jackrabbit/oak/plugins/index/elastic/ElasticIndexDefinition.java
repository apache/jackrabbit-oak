...
public class ElasticIndexDefinition extends IndexDefinition {
    ...
    /**
     * When 0, the index name gets dynamically generated by adding a random suffix to the index
     * name.
     */
    ...
    /**
     * Dynamic properties are fields that are not explicitly defined in the index mapping and are
     * added on the fly when a document is indexed. Examples: aggregations with relative nodes,
     * regex properties (to be supported), etc.
     */
    ...
    static {
        int[] NOT_ANALYZED_TYPES = new int[]{
            Type.BINARY.tag(), Type.LONG.tag(), Type.DOUBLE.tag(), Type.DECIMAL.tag(),
            Type.DATE.tag(), Type.BOOLEAN.tag()
        };
    ...
    public ElasticIndexDefinition(NodeState root, NodeState defn, String indexPath,
        String indexPrefix) {
        super(root, defn, determineIndexFormatVersion(defn), determineUniqueId(defn), indexPath);
        this.indexPrefix = indexPrefix;
        this.indexAlias = ElasticIndexNameHelper.getElasticSafeIndexName(indexPrefix,
            getIndexPath());
        ...
        this.bulkFlushIntervalMs = getOptionalValue(defn, BULK_FLUSH_INTERVAL_MS,
        ...
        this.similarityTagsEnabled = getOptionalValue(defn, SIMILARITY_TAGS_ENABLED,
            SIMILARITY_TAGS_ENABLED_DEFAULT);
        this.similarityTagsBoost = getOptionalValue(defn, SIMILARITY_TAGS_BOOST,
            SIMILARITY_TAGS_BOOST_DEFAULT);
        this.queryFetchSizes = Arrays.stream(
                                         getOptionalValues(defn, QUERY_FETCH_SIZES, Type.LONGS, Long.class,
                                             QUERY_FETCH_SIZES_DEFAULT))
        ...
        this.failOnError = getOptionalValue(defn, FAIL_ON_ERROR,
            Boolean.parseBoolean(System.getProperty(TYPE_ELASTICSEARCH + "." + FAIL_ON_ERROR,
                Boolean.toString(FAIL_ON_ERROR_DEFAULT)))
        );
        this.indexNameSeed = getOptionalValue(defn, INDEX_NAME_SEED, INDEX_NAME_SEED_DEFAULT);
        this.similarityTagsFields = getOptionalValues(defn, SIMILARITY_TAGS_FIELDS, Type.STRINGS,
            String.class, SIMILARITY_TAGS_FIELDS_DEFAULT);
    ...
    /**
     * Returns the index alias on the Elasticsearch cluster. This alias should be used for any query
     * related operations. The actual index name is used only when a reindex is in progress.
     *
    ...
    /**
     * Returns the mapping version for this index definition. If the version is not specified, the
     * default value is {@code 1.0.0}.
     */
    ...
    @Override
    protected PropertyDefinition createPropertyDefinition(IndexDefinition.IndexingRule rule,
        String name, NodeState nodeState) {
        return new ElasticPropertyDefinition(rule, name, nodeState);
    }

    /**
     * Class to help with {@link ElasticIndexDefinition} creation. The built object represents the
     * index definition only without the node structure.
     */
