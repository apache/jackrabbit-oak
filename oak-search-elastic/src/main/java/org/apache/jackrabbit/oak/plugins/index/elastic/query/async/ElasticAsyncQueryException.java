...
public class ElasticAsyncQueryException extends RuntimeException {
    ...
    public String getMessage() {
        ...
        if (getCause() instanceof ElasticsearchException) {
            ...
            sb.append("\n:: Elasticsearch response status: ").append(esException.status())
                    .append(", message: ").append(esException.getMessage());
            ErrorCause error = esException.error();
            if (error == null) {
                sb.append(", error information unavailable.");
            } else {
                List<ErrorCause> rootCause = error.rootCause();
                if (rootCause == null) {
                    sb.append(", root cause unavailable.");
                } else {
                    rootCause.forEach(ec -> sb.append("\n\t* caused by ").append(ec.type()).append(": ").append(ec.reason()));
                }
            }
        }
        return sb.toString();
