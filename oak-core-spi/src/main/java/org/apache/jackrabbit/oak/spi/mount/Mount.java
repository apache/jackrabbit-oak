...
/**
 * Refers to a set of paths from a {@code ContentRepository}x that are possibly stored in a separate
 * physical persistent store.
 *
...
public interface Mount {
    ...
    /**
     * Name of the mount. If this <code>@Mount</code> is the default mount, an empty string is
     * returned
     */
    String getName();
     ...
     *
     * @return true if this mount represents the default mount
     */
    boolean isDefault();

    /**
     * Returns fragment name which can be used to construct node name used for storing meta content
     * belonging to path under this
     * <code>Mount</code>.
     * Such a node name would be used by NodeStore to determine the storage for nodes under those
     * paths.
     *
     ...
     * node name like <code>oak:mount-private-index</code> and then any derived content for path
     * under this mount would be stored as child node under
     * <i>oak:mount-private-index</i> like <code>/fooIndex/oak:mount-private-index/foo</code>.
     * Such paths would then be stored in a separate store which would only be storing paths
     * belonging to that mount
     *
    ...
    /**
     * Checks if this mount supports mounting nodes containing the fragment (see
     * {@link #getPathFragmentName()}) under the given path.
     *
     ...
     *
     * @param path ancestor path
     * @return true if the path fragment mounts are supported under some descendants of the
     * specified path
     */
