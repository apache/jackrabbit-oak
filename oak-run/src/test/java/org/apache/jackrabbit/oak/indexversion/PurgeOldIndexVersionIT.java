...
public class PurgeOldIndexVersionIT {
    ...
    public void setup() {
        purgeOldIndexVersionLogger = LogCustomizer.forLogger(PurgeOldIndexVersion.class.getName())
                                                  .enable(Level.INFO).contains(
                "Found some index need to be purged over base").create();
    }
    ...
    public void test() throws Exception {
        ...
        Assert.assertTrue(
            IndexUtils.isIndexEnabledAndHiddenNodesPresent(n, "/oak:index/test-2-custom-1"));
        p.close();
    }
