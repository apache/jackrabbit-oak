...
var oak = (function (global) {
  ...
  api.changesForRevisions = function (revs) {
    revs = revs || {};
    db.nodes.find({}, {_id: 1, _revisions: 1, _commitRoot: 1}).forEach(
        function (doc) {
          for (var r in revs) {
  ...
  api.indexStats = function () {
    ...
    var totalCount = 0;
    var totalSize = 0;
    db.nodes.find({'_id': /^2\:\/oak\:index\//}, {_id: 1}).forEach(
        function (doc) {
    ...
    result.push({
      ...
      count: totalCount,
      size: totalSize,
      "simple": humanFileSize(totalSize)
    });
    return result;
  };
  ...
  api.blobStats = function () {
    ...
    var bsonSize = 0;
    db.blobs.find().forEach(function (doc) {
      bsonSize += Object.bsonsize(doc)
    });
  ...
  api.removeUnmergedBranches = function (path, clusterId, limit) {
    ...
    if (num > 0) {
      ...
      update["$unset"] = unset;
      print(
          "Removing " + num + " unmerged branches for clusterId " + clusterId);
      return db.nodes.update({_id: pathDepth(path) + ":" + path}, update);
    } else {
  ...
  api.listChildren = function (path) {
    ...
    db.nodes.find({_id: pathFilter(pathDepth(path) + 1, prefix)}).forEach(
        function (doc) {
  ...
  api.printMongoExportCommand = function (path, options) {
    return createExportCommand(JSON.stringify(getDocAndHierarchyQuery(path)),
        options);
  };
  ...
  api.printOplogSliceCommand = function (revStr, options) {
    ...
    var query = '{"ns" : "' + db + '.nodes", "ts": {"$gte": Timestamp('
        + startOplogTime
  ...
  var checkOrFixDeepHistory = function (path, fix, prepare, verbose) {
    ...
    api.forEachChild(path, function (aChild) {
      ...
      if (result) {
        if (prepare) {
          ...
          if (numDangling != 0) {
            print(" - path: " + p + " removed " + numDangling
                + " dangling previous revisions");
            affected++;
          }
        } else {
          var numDangling = result.numPrevLinksDangling;
          if (numDangling != 0) {
            print(" - path: " + p + " has " + numDangling
                + " dangling previous revisions");
            affected++;
          }
        }
        if (!prepare && (++count % 10000 == 0)) {
          print("[checked " + count + " so far (" + affected + " affected, "
              + ignored + " ignored) ...]");
        }
      } else {
        if (!prepare) {
    ...
    if (!prepare) {
      print(
          "Total: " + count + " handled, " + affected + " affected, " + ignored
          + " ignored (path too long)");
      print("done.");
    }
  };
        ...
        if (revStr) {
          ...
          if (lastRev && lastRev.isNewerThan(rev)) {
            if (dryRun) {
              result.push({
                _id: doc._id,
                _lastRev: rev.toString(),
                needsFix: lastRev.toString()
              });
            } else {
              var update = {$set: {}};
  ...
  var Revision = function (rev) {
    ...
    this.counter = parseInt(
        rev.substring(dashIdx + 1, rev.indexOf("-", dashIdx + 1)), 16);
