...
public class MetricsExporterFixtureProvider {
    ...
    @Nullable
    public static MetricsExporterFixture create(DataStoreOptions options, Whiteboard wb)
        throws Exception {
    ...
    /**
     * Metrics exporter arguments 1. Exporter Type 2. Exporter URI 3. Metadata map e.g.
     * pushgateway;uri;key1=value1,key2=value2
     */
    static class ExportMetricsArgs {
        ...
        ExportMetricsArgs(String args) {
            List<String> split = Splitter.on(";").limit(3).omitEmptyStrings().trimResults()
                                         .splitToList(args);
            ...
            if (split.size() > 2) {
                this.pushMap = Splitter.on(",").omitEmptyStrings().trimResults()
                                       .withKeyValueSeparator("=").split(split.get(2));
            } else {
