...
public class IndexDefinitionUpdater {
    ...
    public static void main(String[] args) throws Exception {
        ...
        parser.accepts("initializer",
                  "implementation of RepositoryInitializer to update index definitions").withRequiredArg()
              .required();
        ...
        Preconditions.checkArgument(optionSet.has("initializer"),
            "initializer class must be provided");
        ...
        Preconditions.checkArgument(obj instanceof RepositoryInitializer,
            repoInitClazz + " is not a RepositoryInitializer");
    ...
    private void process(String inFilePath, String outFilePath, RepositoryInitializer initializer)
        throws Exception {
    ...
    private NodeState setupBaseState(String inFilePath) throws Exception {
        org.apache.jackrabbit.oak.plugins.index.importer.IndexDefinitionUpdater indexDefinitionUpdater = new org.apache.jackrabbit.oak.plugins.index.importer.IndexDefinitionUpdater(
            new File(inFilePath));
        ...
        for (String indexPath : indexDefinitionUpdater.getIndexPaths()) {
            oiBuilder.setChildNode(PathUtils.getName(indexPath),
    ...
    private NodeState applyInitializer(NodeBuilder builder, RepositoryInitializer initializer)
        throws CommitFailedException {
        initializer.initialize(builder);
        return merge(builder);
    }

    private void writeReindexingLuceneDefs(String outFilePath,
        List<String> reindexingLuceneDefPaths) throws IOException {
    ...
    private List<String> getReindexIndexPaths() {
        return stream(store.getRoot().getChildNode(INDEX_DEFINITIONS_NAME).getChildNodeEntries()
                           .spliterator(), false)
            .filter(
                cne -> !cne.getNodeState().hasProperty(REINDEX_PROPERTY_NAME) || cne.getNodeState()
                                                                                    .getBoolean(
            ...
            .map(cne -> "/" + INDEX_DEFINITIONS_NAME + "/" + cne.getName())
    ...
    private List<String> getLuceneIndexPaths(List<String> indexDefPaths) {
        ...
        return indexDefPaths.stream()
                            .filter(path -> "lucene".equals(
                                NodeStateUtils.getNode(root, path).getString(TYPE_PROPERTY_NAME)))
                            .collect(Collectors.toList());
    }
    ...
    private void dumpIndexDiff(NodeState beforeRoot, NodeState afterRoot,
        List<String> reindexingLuceneDefPaths) {
    ...
    static class VisibleChangesDiff implements NodeStateDiff {
        ...
        public boolean childNodeDeleted(String name, NodeState before) {
            ...
            return MISSING_NODE.compareAgainstBaseState(before,
                new VisibleChangesDiff(pw, indent + INDENT));
        }
