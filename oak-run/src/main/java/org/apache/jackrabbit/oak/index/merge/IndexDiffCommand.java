    ...
    public void execute(String... args) throws Exception {
        ...
        OptionSpec<String> comparePathOption = parser
            .accepts("compare", "Path to index definition file or files (.json). " +
                "Compare index1 and index2, or all indexes there to the indexBase file")
            .withOptionalArg()
            .defaultsTo("");
        ...
        if (!customDir.isEmpty()) {
            System.out.println(
                "Listing differences between out-of-the-box and customized indexes " +
        ...
        if (!comparePath.isEmpty()) {
            ...
            if (!indexBaseFile.isEmpty()) {
                System.out.println(
                    "Comparing indexes in " + indexBaseFile + " against all indexes in " +
                        " directory \"" +
                        comparePath + "\"");
                ...
                if (Files.isRegularFile(Paths.get(comparePath))) {
                    System.out.println("Comparing indexes " + index1 + " and " + index2 +
                        ...
                        comparePath + "\"");
                    System.out.println(
                        IndexDiff.compareIndexesInFile(Paths.get(comparePath), index1, index2));
                } else {
                    System.out.println("Comparing indexes " + index1 + " and " + index2 +
                        " for directory \"" +
                        comparePath + "\"");
