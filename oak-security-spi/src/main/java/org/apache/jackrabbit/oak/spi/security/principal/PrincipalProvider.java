...
/**
 * The {@code PrincipalProvider} defines methods to provide access to sources of {@link Principal}s.
 * This allows the security framework share any external sources for authorization and
 * authentication, as may be used by a custom {@link javax.security.auth.spi.LoginModule} for
 * example.
 * <p>
 * A single {@code PrincipalProvider} implementation is expected to exposed principals from one
 * single source. In contrast to the
 * {@link org.apache.jackrabbit.api.security.principal.PrincipalManager} which will expose all known
 * and accessible principals from possibly different sources. See also
 * {@link CompositePrincipalProvider} for a mechanism to combine principals of different providers.
 */
@ProviderType
public interface PrincipalProvider {

    /**
     * Returns the principal with the specified name or {@code null} if the principal does not
     * exist.
     *
    ...
    /**
     * Returns the {@code ItemBasedPrincipal} with the specified {@code principalOakPath} or
     * {@code null} if no principal with that path exists.
     *
     ...
     * or indirect member of. Thus for any principal returned in the iterator
     * {@link org.apache.jackrabbit.api.security.principal.GroupPrincipal#isMember(Principal)} must
     * return {@code true}.
     * <p>
     * Example:<br> If Principal is member of Group A, and Group A is member of Group B, this method
     * will return Group A and Group B.
     *
    ...
    /**
     * Tries to resolve the specified {@code userID} to a valid principal and it's group membership.
     * This method returns an empty set if the specified ID cannot be resolved.
     *
     * @param userID A userID.
     * @return The set of principals associated with the specified {@code userID} or an empty set if
     ...
     * @param searchType Limit the search to certain types of principals. Valid values are any of
     ...
     */
    @NotNull
    default Iterator<? extends Principal> findPrincipals(@Nullable String nameHint,
        boolean fullText, int searchType, long offset, long limit) {
        if (offset < 0) {
