...
/**
 * Configurable provider taking care of building login contexts for the desired authentication
 * mechanism.
 * <p>
 * This provider defines a single method {@link #getLoginContext(javax.jcr.Credentials, String)}
 * that takes the {@link Credentials credentials} and the workspace name such as passed to
...
public interface LoginContextProvider {
     ...
     *                      String) login} method of the repository.
     * @param workspaceName The name of the workspace that is being accessed by the login called.
     * @return a new login context
     * @throws LoginException If an error occurs while creating a new context.
     */
    @NotNull
    LoginContext getLoginContext(Credentials credentials, String workspaceName)
