 ...
 *
 * @see {@code org.apache.jackrabbit.oak.security.user.action.AccessControlActionTest} for
...
public class AccessControlActionTest implements UserConstants {
    ...
    private final AuthorizationConfiguration authorizationConfiguration = mock(
        AuthorizationConfiguration.class);

    private void initSecurityProvider(@NotNull String adminId, @NotNull String anonymousId,
        @NotNull String... adminPrincipalNames) throws Exception {
        initSecurityProvider(null, adminId, anonymousId, adminPrincipalNames);
    }

    private void initSecurityProvider(@Nullable AccessControlManager acMgr, @NotNull String adminId,
        @NotNull String anonymousId, @NotNull String... adminPrincipalNames) throws Exception {
        ...
        when(authorizationConfiguration.getParameters()).thenReturn(
            ConfigurationParameters.of(PermissionConstants.PARAM_ADMINISTRATIVE_PRINCIPALS,
                adminPrincipalNames));
        if (acMgr != null) {
            when(authorizationConfiguration.getAccessControlManager(root,
                NamePathMapper.DEFAULT)).thenReturn(acMgr);
        }
        when(securityProvider.getConfiguration(UserConfiguration.class)).thenReturn(
            userConfiguration);
    ...
    private AccessControlManager mockAccessControlManager(boolean addEntrySuccess)
        throws Exception {
        AccessControlManager acMgr = mock(AccessControlManager.class);
        when(acMgr.getApplicablePolicies("/none")).thenReturn(
            AccessControlPolicyIteratorAdapter.EMPTY);
        AccessControlPolicy policy = mock(AccessControlPolicy.class);
        when(acMgr.getApplicablePolicies("/nonACL")).thenReturn(
            new AccessControlPolicyIteratorAdapter(ImmutableList.of(policy)));
        JackrabbitAccessControlList acl = mock(JackrabbitAccessControlList.class);
        if (addEntrySuccess) {
            when(
                acl.addAccessControlEntry(any(Principal.class), any(Privilege[].class))).thenReturn(
                true);
        }
        when(acMgr.getApplicablePolicies("/acl")).thenReturn(
            new AccessControlPolicyIteratorAdapter(ImmutableList.of(acl)));
        return acMgr;
    ...
    private AccessControlAction createAction(@NotNull String[] userPrivNames,
        @NotNull String[] groupPrivNames) {
        AccessControlAction action = new AccessControlAction();
        action.init(securityProvider, ConfigurationParameters.of(
    ...
    private static void mockAuthorizable(@NotNull Authorizable a, @NotNull String id,
        @Nullable String principalName, @Nullable String path) throws RepositoryException {
    ...
    private static User mockUser(@NotNull String id, @Nullable String principalName,
    ...
    public void testOnCreateUserEmptyPrivs() throws Exception {
        ...
        AccessControlAction action = createAction(new String[0],
    ...
    public void testOnCreateGroupEmptyPrivs() throws Exception {
        initSecurityProvider(DEFAULT_ADMIN_ID, DEFAULT_ANONYMOUS_ID);
        AccessControlAction action = createAction(new String[]{PrivilegeConstants.JCR_READ},
            new String[0]);
    ...
    public void testOnCreateNoApplicablePolicy() throws Exception {
        initSecurityProvider(mockAccessControlManager(false), DEFAULT_ADMIN_ID,
    ...
    private static void verifyInvokations(@NotNull Authorizable a, boolean hasApplicable)
