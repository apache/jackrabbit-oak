...
public class PrivilegeUtilTest {
    ...
    public void testGetPrivilegesTree() {
        Root root = when(mock(Root.class).getTree(PRIVILEGES_PATH)).thenReturn(mock(Tree.class))
                                                                   .getMock();
    ...
    public void testReadDefinitions() {
        ...
        when(defTree.getProperty(PrivilegeConstants.REP_IS_ABSTRACT)).thenReturn(
            PropertyStates.createProperty(PrivilegeConstants.REP_IS_ABSTRACT, true));
    ...
    public void testReadDefinitionsWithAggregates() {
        ...
        Tree defTree = when(mock(Tree.class).getProperty(REP_AGGREGATES)).thenReturn(
            PropertyStates.createProperty(REP_AGGREGATES, aggregateNames, Type.NAMES)).getMock();
    ...
    public void testGetPrivilegeOakNameDefaultMapper() throws Exception {
        ...
        assertEquals(Privilege.JCR_ADD_CHILD_NODES,
            PrivilegeUtil.getOakName(Privilege.JCR_ADD_CHILD_NODES, NamePathMapper.DEFAULT));
    ...
    public void testGetPrivilegeOakName() throws Exception {
        ...
        when(mapper.getOakNameOrNull(Privilege.JCR_LIFECYCLE_MANAGEMENT)).thenReturn(
            JCR_LIFECYCLE_MANAGEMENT);
        assertEquals(JCR_LIFECYCLE_MANAGEMENT,
            PrivilegeUtil.getOakName(Privilege.JCR_LIFECYCLE_MANAGEMENT, mapper));
