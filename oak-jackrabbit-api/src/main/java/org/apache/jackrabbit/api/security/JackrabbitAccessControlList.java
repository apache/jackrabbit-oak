 ...
 * {@link javax.jcr.security.AccessControlManager#setPolicy(String, AccessControlPolicy) written
 * back} and {@link javax.jcr.Session#save() saved}.
 */
@ProviderType
public interface JackrabbitAccessControlList extends JackrabbitAccessControlPolicy,
    ...
    /**
     * Returns the names of the supported restrictions or an empty array if no restrictions are
     * respected.
     *
    ...
    /**
     * Return the expected {@link javax.jcr.PropertyType property type} of the restriction with the
     * specified <code>restrictionName</code>.
     *
    ...
    /**
     * Returns <code>true</code> if the restriction is multivalued; <code>false</code> otherwise. If
     * an given implementation doesn't support multivalued restrictions, this method always returns
     * <code>false</code>.
     *
     * @param restrictionName Any of the restriction names retrieved from
     *                        {@link #getRestrictionNames()}.
     * @return <code>true</code> if the restriction is multivalued; <code>false</code>
     * if the restriction with the given name is single value or if the implementation doesn't
     * support multivalued restrictions, this method always returns <code>false</code>.
     * @throws RepositoryException If an error occurs.
    ...
    /**
     * Returns <code>true</code> if this policy does not yet define any entries.
     *
    ...
    /**
     * Same as {@link #addEntry(Principal, Privilege[], boolean, Map)} using some implementation
     ...
     * @return true if this policy has changed by incorporating the given entry; false otherwise.
     * @throws AccessControlException If any of the given parameter is invalid or cannot be handled
     ...
     * <p>
     * An <code>AccessControlException</code> is thrown if any of the specified parameters is
     * invalid or if some other access control related exception occurs.
     *
     * @param principal    the principal to add the entry for
     * @param privileges   the privileges to add
     * @param isAllow      if <code>true</code> if this is a positive (allow) entry
     * @param restrictions A map of additional restrictions used to narrow the effect of the entry
     *                     to be created. The map must map JCR names to a single
     ...
     *                       to be created. The map must map JCR names to a single
     *                       {@link javax.jcr.Value} object.
     * @param mvRestrictions A map of additional multivalued restrictions used to narrow the effect
     *                       of the entry to be created. The map must map JCR names to a
    ...
    /**
     * If the <code>AccessControlList</code> implementation supports reordering of entries the
     * specified <code>srcEntry</code> is inserted at the position of the specified
     * <code>destEntry</code>.
     * <p>
     * If <code>destEntry</code> is <code>null</code> the entry is moved to the end of the list.
     * <p>
     * If <code>srcEntry</code> and <code>destEntry</code> are the same no changes are made.
     *
