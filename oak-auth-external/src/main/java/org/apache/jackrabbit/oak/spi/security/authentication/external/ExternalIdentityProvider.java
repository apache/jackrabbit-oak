...
/**
 * {@code ExternalIdentityProvider} defines an interface to an external system that provides users
 * and groups that can be synced with local ones.
 */
public interface ExternalIdentityProvider {
    ...
    /**
     * Returns the identity for the given reference or {@code null} if it does not exist. The
     * provider should check if the {@link ExternalIdentityRef#getProviderName() provider name}
     * matches his own name or is {@code null} and should not return a foreign identity.
     *
    ...
    /**
     * Returns the user for the given (local) id. if the user does not exist {@code null} is
     * returned.
     *
    ...
    /**
     * Authenticates the user represented by the given credentials and returns it. If the user does
     * not exist in this provider, {@code null} is returned. If the authentication fails, a
     ...
     */
    @Nullable
    ExternalUser authenticate(@NotNull Credentials credentials)
        throws ExternalIdentityException, LoginException;

    /**
     * Returns the group for the given (local) group name. if the group does not exist {@code null}
     * is returned.
     *
