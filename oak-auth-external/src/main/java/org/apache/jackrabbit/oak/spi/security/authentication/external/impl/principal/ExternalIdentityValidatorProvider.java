...
/**
 * {@code ValidatorProvider} used to assure that the system maintained properties associated with
 * external identities are only written by system sessions and are consistent.
 *
 * @since Oak 1.5.3
 */
class ExternalIdentityValidatorProvider extends ValidatorProvider implements
    ExternalIdentityConstants {

    private static final Map<Integer, String> ERROR_MSGS = ImmutableMap.<Integer, String>builder()
                                                                       .put(70,
                                                                           "Attempt to create, modify or remove the system property 'rep:externalPrincipalNames'")
                                                                       .put(71,
                                                                           "Property 'rep:externalPrincipalNames' must be multi-valued of type STRING.")
                                                                       .put(72,
                                                                           "Property 'rep:externalPrincipalNames' requires 'rep:externalId' to be present on the Node.")
                                                                       .put(73,
                                                                           "Property 'rep:externalId' cannot be removed as long as 'rep:externalPrincipalNames' is present.")
                                                                       .put(74,
                                                                           "Attempt to add, modify or remove the system maintained property 'rep:externalId'.")
                                                                       .put(75,
                                                                           "Property 'rep:externalId' may only have a single value of type STRING.")
                                                                       .build();
    ...
    private final class ExternalIdentityValidator extends DefaultValidator {
        ...
        private void checkAddModifyProperties(@NotNull NodeState parent, @NotNull String name,
            @NotNull PropertyState propertyState, boolean isModify) throws CommitFailedException {
            ...
            if (REP_EXTERNAL_PRINCIPAL_NAMES.equals(name)) {
                ...
                if (!Type.STRINGS.equals(propertyState.getType()) || !propertyState.isArray()) {
                    throw new CommitFailedException(CommitFailedException.CONSTRAINT, 71,
                        ERROR_MSGS.get(71));
                }
                if (!parent.hasProperty(REP_EXTERNAL_ID)) {
                    throw new CommitFailedException(CommitFailedException.CONSTRAINT, 72,
                        ERROR_MSGS.get(72));
                }
            }
            if (REP_EXTERNAL_ID.equals(name) && protectedExternalIds) {
                if (isModify && !isSystem) {
                    throw new CommitFailedException(CommitFailedException.CONSTRAINT, 74,
                        ERROR_MSGS.get(74));
                }
                if (!Type.STRING.equals(propertyState.getType()) || propertyState.isArray()) {
                    throw new CommitFailedException(CommitFailedException.CONSTRAINT, 75,
                        ERROR_MSGS.get(75));
                }
            }
        }
        ...
        public void propertyDeleted(PropertyState before) throws CommitFailedException {
            ...
            if (REP_EXTERNAL_ID.equals(name)) {
                if (parent.hasProperty(REP_EXTERNAL_PRINCIPAL_NAMES)) {
                    throw new CommitFailedException(CommitFailedException.CONSTRAINT, 73,
                        ERROR_MSGS.get(73));
                }
        ...
        private void checkIsSystem() throws CommitFailedException {
            if (!isSystem) {
                throw new CommitFailedException(CommitFailedException.CONSTRAINT, 70,
                    ERROR_MSGS.get(70));
            }
        }
