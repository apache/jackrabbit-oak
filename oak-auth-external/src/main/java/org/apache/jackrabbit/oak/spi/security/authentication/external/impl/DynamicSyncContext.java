...
/**
 * Extension of the {@code DefaultSyncContext} that doesn't synchronize group membership of new
 * external users into the user management of the repository. Instead it will only synchronize the
 * principal names up to the configured depths. In combination with the a dedicated
 * {@code PrincipalConfiguration} this allows to benefit from the repository's authorization model
 * (which is solely based on principals) i.e. full compatibility with the default approach without
 * the complication of synchronizing user management information into the repository, when user
 * management is effectively take care of by the third party system.
 * <p>
 * With the
 * {@link org.apache.jackrabbit.oak.spi.security.authentication.external.impl.DefaultSyncHandler}
 * this feature can be turned on using
 * {@link
 * org.apache.jackrabbit.oak.spi.security.authentication.external.basic.DefaultSyncConfig.User#setDynamicMembership(boolean)}
 * <p>
 * Note: users and groups that have been synchronized before the dynamic membership feature has been
 * enabled will continue to be synchronized in the default way and this context doesn't take
 * effect.
 *
 * @since Oak 1.5.3
 */
public class DynamicSyncContext extends DefaultSyncContext {
    ...
    public boolean convertToDynamicMembership(@NotNull Authorizable authorizable)
    ...
    public SyncResult sync(@NotNull ExternalIdentity identity) throws SyncException {
        if (identity instanceof ExternalUser) {
            ...
            if (!isSameIDP(ref)) {
                ...
                return new DefaultSyncResultImpl(
                    new DefaultSyncedIdentity(identity.getId(), ref, true, -1),
                    SyncResult.Status.FOREIGN);
            }
            return sync((ExternalGroup) identity, ref);
        } else {
            throw new IllegalArgumentException(
                "identity must be user or group but was: " + identity);
        }
    }

    @NotNull
    private SyncResult sync(@NotNull ExternalGroup identity, @NotNull ExternalIdentityRef ref)
        ...
        try {
            ...
            if (group != null) {
                ...
                log.debug("ExternalGroup {}: synchronizing as dynamic group {}.", ref.getString(),
                    identity.getId());
                ...
                log.debug("ExternalGroup {}: Not synchronized as Group into the repository.",
                    ref.getString());
                return new DefaultSyncResultImpl(
                    new DefaultSyncedIdentity(identity.getId(), ref, true, -1),
    ...
    //-------------------------------------------------< DefaultSyncContext >---
    @Override
    protected void syncMembership(@NotNull ExternalIdentity external, @NotNull Authorizable auth,
        long depth) throws RepositoryException {
        if (auth.isGroup()) {
            return;
        }
        ...
        if (groupsSyncedBefore && !enforceDynamicSync()) {
            ...
            try {
                ...
                Map<ExternalIdentityRef, SyncEntry> map = collectSyncEntries(declaredGroupRefs,
                    depth);
                ...
                log.error("Failed to synchronize membership information for external identity {}",
                    external.getId(), e);
            }
        }
    }
    ...
    @Override
    protected void applyMembership(@NotNull Authorizable member, @NotNull Set<String> groups)
        throws RepositoryException {
        log.debug("Dynamic membership sync enabled => omit setting auto-membership for {} ",
            member.getID());
    }

    /**
     * Retrieve membership of the given external user (up to the configured depth) and add (or
     * replace) the rep:externalPrincipalNames property with the accurate collection of principal
     * names.
     *
    ...
    private void setExternalPrincipalNames(@NotNull Authorizable authorizable,
        @NotNull Collection<SyncEntry> syncEntries) throws RepositoryException {
        Value[] vs;
        if (syncEntries.isEmpty()) {
            vs = new Value[0];
        } else {
            Set<String> principalsNames = syncEntries.stream()
                                                     .map(syncEntry -> syncEntry.principalName)
    ...
    @NotNull
    private Map<ExternalIdentityRef, SyncEntry> collectSyncEntries(
        @NotNull Iterable<ExternalIdentityRef> declaredGroupRefs, long depth)
        throws RepositoryException, ExternalIdentityException {
        if (depth <= 0) {
    ...
    /**
     * Recursively collect the sync entries of the given declared group references up to the given
     * depth.
     * <p>
     * Note, that this method will filter out references that don't belong to the same IDP (see
     * OAK-8665).
     *
     * @param declaredGroupRefs The declared group references for a user or a group.
     * @param depth             Configured membership nesting; the recursion will be stopped once
     *                          depths is < 1.
     * @param map               The map to be filled with all group refs and the corresponding sync
     *                          entries.
     * @throws ExternalIdentityException If an error occurs while resolving the the external group
     *                                   references.
     */
    private void collectSyncEntries(@NotNull Iterable<ExternalIdentityRef> declaredGroupRefs,
        long depth, @NotNull Map<ExternalIdentityRef, SyncEntry> map)
        throws ExternalIdentityException, RepositoryException {
    ...
    /**
     * Evaluate if looking up the external group from the IDP can be omitted (i.e. no nesting and
     * IDP implements PrincipalNameResolver. Finally, the shortcut does not make sense if 'dynamic
     * group' option is enabled, as the external group is needed for the subsequent group sync.
     *
     * @param depth The configured membership nesting depth.
     * @return {@code true} if looking up the external group on IDP can be avoided; {@code false}
     * otherwise.
     */
    ...
    /**
     * Tests if the given existing user/group collides with the external group having the same
     * principall name. It is considered a conflict if the existing authorizable is a user (and not
     * a group) or if it doesn't belong to the same IDP (i.e. a local group or one defined for a
     * different IDP).
     * <p>
     * NOTE: this method does not verify if the 'rep:authorizableId' or the identifier part of
     * 'rep:externalId' match, Instead it assumes that external identities and synced authorizables
     * that are associated with the same IDP can be trusted to be consistent as long as they have
     * the same principal name.
     *
     * @param authorizable An authorizable with the given principal name or {@code null}.
     * @return {@code true} if the given user/group collides with the external group with the given
     * principal name; {@code false} otherwise.
     * @throws RepositoryException If an error occurs
     */
    private boolean isConflictingGroup(@Nullable Authorizable authorizable,
        @NotNull String principalName) throws RepositoryException {
        if (authorizable == null) {
            return false;
        } else if (!authorizable.isGroup()) {
            log.warn("Existing user '{}' collides with external group defined by IDP '{}'.",
                authorizable.getID(), idp.getName());
            return true;
            ...
            log.warn(
                "Existing group with id '{}' doesn't have matching principal name. found '{}', expected '{}', IDP '{}'.",
                authorizable.getID(), authorizable.getPrincipal().getName(), principalName,
                idp.getName());
            return true;
        } else {
    ...
    private void createDynamicGroups(@NotNull Iterable<SyncEntry> syncEntries)
        throws RepositoryException {
        for (SyncEntry syncEntry : syncEntries) {
            ...
            checkNotNull(syncEntry.externalGroup,
                "Cannot create dynamic group from null ExternalIdentity.");
    ...
    @NotNull
    private Collection<String> clearGroupMembership(@NotNull Authorizable authorizable)
        throws RepositoryException {
    ...
    private void clearGroupMembership(@NotNull Authorizable authorizable,
        @NotNull Set<String> groupPrincipalNames, @NotNull Set<Group> toRemove)
        throws RepositoryException {
        Iterator<Group> grpIter = authorizable.declaredMemberOf();
        Set<String> autoMembership = ((authorizable.isGroup()) ? config.group()
            : config.user()).getAutoMembership(authorizable);
        while (grpIter.hasNext()) {
            ...
            if (isSameIDP(grp)) {
                ...
                log.warn(
                    "Ignoring unexpected membership of '{}' in group '{}' crossing IDP boundary.",
                    authorizable.getID(), grp.getID());
            }
        }
    }
    ...
    private static boolean groupsSyncedBefore(@NotNull Authorizable authorizable)
        throws RepositoryException {
        return authorizable.hasProperty(REP_LAST_SYNCED) && !authorizable.hasProperty(
    ...
    private static boolean requiresCleanup(@NotNull Authorizable authorizable)
        throws RepositoryException {
        return authorizable.hasProperty(REP_LAST_SYNCED) && !authorizable.hasProperty(
            ExternalIdentityConstants.REP_LAST_DYNAMIC_SYNC);
    }
    ...
    /**
     * Helper object to avoid repeated lookup of principalName, {@link ExternalGroup} and
     * synchronized {@link Group} for a given {@link ExternalIdentityRef} during
     * {@link #syncMembership(ExternalIdentity, Authorizable, long)}.
     */
    private static class SyncEntry {
        ...
        private SyncEntry(@NotNull String principalName, @Nullable ExternalGroup externalGroup,
