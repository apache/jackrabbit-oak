 ...
 */
public class PrincipalSyncContext extends DefaultSyncContext {

    private static final Logger log = LoggerFactory.getLogger(PrincipalSyncContext.class);

    public PrincipalSyncContext(@Nonnull DefaultSyncConfig config,
        ...
        if (identity instanceof ExternalUser) {
            ...
            try {
                ...
                if (group != null) {
                    ...
                    log.debug("ExternalGroup " + ref.getString() + ": Not synchronized as authorizable Group into the repository.");
    ...
    private void collectPrincipalNames(@Nonnull Set<String> principalNames, @Nonnull Iterable<ExternalIdentityRef> declaredGroupIdRefs, long depth) throws ExternalIdentityException {
        for (ExternalIdentityRef ref : declaredGroupIdRefs) {
            ...
            if (extId instanceof ExternalGroup) {
                ...
                log.debug("Not an external group => ignore");
                continue;
            }
        }
    }
