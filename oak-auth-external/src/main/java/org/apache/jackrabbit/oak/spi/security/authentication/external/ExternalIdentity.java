...
public interface ExternalIdentity {
    ...
    /**
     * Returns the local id of this identity as it would be used in this repository. This usually
     * corresponds to {@link org.apache.jackrabbit.api.security.user.Authorizable#getID()}
     *
     ...
     * {@link org.apache.jackrabbit.api.security.user.Authorizable#getPrincipal()}.{@link
    ...
    /**
     * Returns the desired intermediate relative path of the authorizable to be created. For
     * example, one could map an external hierarchy into the local users and groups hierarchy. The
     * provider must escape all characters so that the path is a valid JCR path. The path is always
     * considered relative, even if it starts with a '/'.
     *
