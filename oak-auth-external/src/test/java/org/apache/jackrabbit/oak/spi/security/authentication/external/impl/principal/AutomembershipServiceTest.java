...
public class AutomembershipServiceTest extends AbstractAutoMembershipTest {
    ...
    public void testMissingDynamicMembership() {
        assertSame(DynamicMembershipProvider.EMPTY,
            service.getDynamicMembershipProvider(root, userManager, getNamePathMapper()));
    }

    @Test
    public void testDynamicMembership() {
        context.registerService(SyncHandler.class, new DefaultSyncHandler(),
            getSyncHandlerParams());
        assertTrue(scTracker.isEnabled());

        context.registerService(SyncHandlerMapping.class, new SyncHandlerMapping() {
        }, getMappingParams());
        assertTrue(service.getDynamicMembershipProvider(root, userManager,
            getNamePathMapper()) instanceof AutoMembershipProvider);
    }
