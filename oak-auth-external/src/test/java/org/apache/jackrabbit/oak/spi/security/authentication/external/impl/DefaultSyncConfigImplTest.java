...
public class DefaultSyncConfigImplTest {
    ...
    public void testDefaultUser() {
        DefaultSyncConfig.User userConfig = DefaultSyncConfigImpl.of(ConfigurationParameters.EMPTY)
                                                                 .user();
        ...
        assertEquals(PARAM_USER_ENFORCE_DYNAMIC_MEMBERSHIP_DEFAULT,
            userConfig.getEnforceDynamicMembership());
        assertEquals(PARAM_USER_MEMBERSHIP_NESTING_DEPTH_DEFAULT,
            userConfig.getMembershipNestingDepth());
        assertEquals(
            ConfigurationParameters.Milliseconds.of(PARAM_USER_MEMBERSHIP_EXPIRATION_TIME_DEFAULT),
            ConfigurationParameters.Milliseconds.of(userConfig.getMembershipExpirationTime()));

        assertEquals(ConfigurationParameters.Milliseconds.of(PARAM_USER_EXPIRATION_TIME_DEFAULT),
            ConfigurationParameters.Milliseconds.of(userConfig.getExpirationTime()));
        assertEquals(PARAM_ENABLE_RFC7613_USERCASE_MAPPED_PROFILE_DEFAULT,
            userConfig.isApplyRFC7613UsernameCaseMapped());
        assertArrayEquals(PARAM_USER_AUTO_MEMBERSHIP_DEFAULT,
            userConfig.getAutoMembership().toArray(new String[0]));
        assertEquals(getMapping(PARAM_USER_PROPERTY_MAPPING_DEFAULT),
            userConfig.getPropertyMapping());
        assertEquals(PARAM_USER_PATH_PREFIX_DEFAULT, userConfig.getPathPrefix());
    }

    @Test
    public void testUserDynamicMembership() {
        ConfigurationParameters params = ConfigurationParameters.of(PARAM_USER_DYNAMIC_MEMBERSHIP,
            true);
        ...
        params = ConfigurationParameters.of(PARAM_USER_DYNAMIC_MEMBERSHIP, true,
            PARAM_USER_ENFORCE_DYNAMIC_MEMBERSHIP, true);
        userConfig = DefaultSyncConfigImpl.of(params).user();
        assertTrue(userConfig.getDynamicMembership());
        assertTrue(userConfig.getEnforceDynamicMembership());
    }
    ...
    public void testDefaultGroup() {
        DefaultSyncConfig.Group groupConfig = DefaultSyncConfigImpl.of(
            ConfigurationParameters.EMPTY).group();
        assertEquals(ConfigurationParameters.Milliseconds.of(PARAM_GROUP_EXPIRATION_TIME_DEFAULT),
            ConfigurationParameters.Milliseconds.of(groupConfig.getExpirationTime()));
        assertEquals(PARAM_ENABLE_RFC7613_USERCASE_MAPPED_PROFILE_DEFAULT,
            groupConfig.isApplyRFC7613UsernameCaseMapped());
        assertArrayEquals(PARAM_GROUP_AUTO_MEMBERSHIP_DEFAULT,
            groupConfig.getAutoMembership().toArray(new String[0]));
        assertEquals(getMapping(PARAM_GROUP_PROPERTY_MAPPING_DEFAULT),
            groupConfig.getPropertyMapping());
    ...
    public void testGroup() {
        ConfigurationParameters params = ConfigurationParameters.of(PARAM_GROUP_DYNAMIC_GROUPS,
            true);
    ...
    public void testInvalidMapping() {
        ...
        DefaultSyncConfig.User userConfig = DefaultSyncConfigImpl.of(
            ConfigurationParameters.of(PARAM_USER_PROPERTY_MAPPING, invalidMapping)).user();
    ...
    public void testValidMapping() {
        ...
        DefaultSyncConfig.User userConfig = DefaultSyncConfigImpl.of(
            ConfigurationParameters.of(PARAM_USER_PROPERTY_MAPPING, validMapping)).user();
        assertEquals(ImmutableMap.of("valid", "mapping", "valid2", "mapping"),
            userConfig.getPropertyMapping());
    }
