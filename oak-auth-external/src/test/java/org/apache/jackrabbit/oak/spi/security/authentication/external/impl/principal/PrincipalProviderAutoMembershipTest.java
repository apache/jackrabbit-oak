...
/**
 * Extension of the {@link ExternalGroupPrincipalProviderTest} with 'automembership' configured in
 * the {@link DefaultSyncConfig}.
 */
...
public class PrincipalProviderAutoMembershipTest extends ExternalGroupPrincipalProviderTest {
    ...
    private static final String USER_AUTO_MEMBERSHIP_GROUP_PRINCIPAL_NAME =
        "p-" + USER_AUTO_MEMBERSHIP_GROUP_ID;

    private static final String GROUP_AUTO_MEMBERSHIP_GROUP_ID = "testGroup2-" + UUID.randomUUID();
    private static final String GROUP_AUTO_MEMBERSHIP_GROUP_PRINCIPAL_NAME =
        "p-" + GROUP_AUTO_MEMBERSHIP_GROUP_ID;

    private static final String CONFIG_AUTO_MEMBERSHIP_GROUP_ID = "testGroup3-" + UUID.randomUUID();
    private static final String CONFIG_AUTO_MEMBERSHIP_GROUP_PRINCIPAL_NAME =
        "p-" + CONFIG_AUTO_MEMBERSHIP_GROUP_ID;
    ...
    private final AutoMembershipConfig amc = when(
        mock(AutoMembershipConfig.class).getAutoMembership(any(Authorizable.class)))
    ...
    public PrincipalProviderAutoMembershipTest(boolean nestedAutomembership, boolean dynamicGroups,
        @NotNull String name) {
    ...
    public void before() throws Exception {
        ...
        userAutoMembershipGroup = uMgr.createGroup(USER_AUTO_MEMBERSHIP_GROUP_ID,
            new PrincipalImpl(USER_AUTO_MEMBERSHIP_GROUP_PRINCIPAL_NAME), null);
        groupAutoMembershipGroup = uMgr.createGroup(GROUP_AUTO_MEMBERSHIP_GROUP_ID,
            new PrincipalImpl(GROUP_AUTO_MEMBERSHIP_GROUP_PRINCIPAL_NAME), null);
        configAutoMembershipGroup = uMgr.createGroup(CONFIG_AUTO_MEMBERSHIP_GROUP_ID,
            new PrincipalImpl(CONFIG_AUTO_MEMBERSHIP_GROUP_PRINCIPAL_NAME), null);
    ...
    Set<Principal> getExpectedGroupPrincipals(@NotNull String userId) throws Exception {
        ImmutableSet.Builder<Principal> builder = ImmutableSet.<Principal>builder()
                                                              .addAll(
                                                                  super.getExpectedGroupPrincipals(
                                                                      userId))
                                                              .add(
    ...
    public void testGetAutoMembershipPrincipal() throws Exception {
        assertNull(
            principalProvider.getPrincipal(userAutoMembershipGroup.getPrincipal().getName()));
        assertNull(
            principalProvider.getPrincipal(groupAutoMembershipGroup.getPrincipal().getName()));
    ...
    public void testGetGroupPrincipalsIgnoresNonGroupPrincipals() throws Exception {
        ...
        Set<Principal> result = pp.getMembershipPrincipals(
            um.getAuthorizable(USER_ID).getPrincipal());
        assertTrue(result.stream().map(Principal::getName)
                         .noneMatch(USER_AUTO_MEMBERSHIP_GROUP_PRINCIPAL_NAME::equals));
    }
    ...
    public void testGetMembershipPrincipalsUnknownGroupPrincipal() throws Exception {
        GroupPrincipal gp = when(mock(GroupPrincipal.class).getName()).thenReturn(
            idp.getGroup("a").getPrincipalName()).getMock();
    ...
    public void testGetMembershipPrincipalsExternalGroupPrincipal() throws Exception {
        ...
        Set<Principal> dynamicGroupMembership = principalProvider.getMembershipPrincipals(
            externalGroupPrincipal);
        if (dynamicGroups) {
            Set<Principal> expected = ImmutableSet.of(groupAutoMembershipGroup.getPrincipal(),
                configAutoMembershipGroup.getPrincipal());
    ...
    private @Nullable Principal getExternalGroupPrincipal(@NotNull Principal extUserPrincipal) {
        Iterator<Principal> it = principalProvider.getMembershipPrincipals(extUserPrincipal)
                                                  .iterator();
    ...
    public void testFindPrincipalsByHint() throws Exception {
        ...
        for (String hint : hints) {
            Iterator<? extends Principal> res = principalProvider.findPrincipals(hint,
