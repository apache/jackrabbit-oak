...
public class SystemPrincipalConfigTest extends AbstractExternalAuthTest {
    ...
    public SystemPrincipalConfigTest(String[] systemUserNames, String name) {
        this.systemUserNames =
            (systemUserNames == null) ? null : ImmutableSet.copyOf(systemUserNames);
    }
    ...
    public void before() throws Exception {
        ...
        systemPrincipalConfig =
            (systemUserNames == null) ? new SystemPrincipalConfig(Collections.emptySet())
                : new SystemPrincipalConfig(systemUserNames);
    }
    ...
    private void assertIsSystem(@NotNull Set<Principal> principals, boolean expectedIsSystem)
        throws Exception {
        ...
        List<? extends ValidatorProvider> validatorProviders = externalPrincipalConfiguration.getValidators(
            workspaceName, principals, new MoveTracker());
    ...
    /**
     * Test regular principals with the configured system-user names => must not be recognized as
     * system subjects.
     */
    @Test
    public void testRegularPrincipalSubject() throws Exception {
        ...
        principals = ImmutableSet.of(new PrincipalImpl(SYSTEM_USER_NAME_2),
            new PrincipalImpl(SYSTEM_USER_NAME_1));
        assertIsSystem(principals, false);
    ...
    public void testSystemUserSubject() throws Exception {
        Set<Principal> principals = Collections.singleton(
            (SystemUserPrincipal) () -> SYSTEM_USER_NAME_NOT_CONFIGURED);
        assertIsSystem(principals, false);
    }
    ...
    public void testConfiguredSystemUserSubject() throws Exception {
        Set<Principal> principals = Collections.singleton(
            (SystemUserPrincipal) () -> SYSTEM_USER_NAME_1);
        assertIsSystem(principals, configContainsSystemUser(SYSTEM_USER_NAME_1));

        principals = ImmutableSet.of(EveryonePrincipal.getInstance(),
            (SystemUserPrincipal) () -> SYSTEM_USER_NAME_2);
        assertIsSystem(principals, configContainsSystemUser(SYSTEM_USER_NAME_2));

        principals = ImmutableSet.of((SystemUserPrincipal) () -> SYSTEM_USER_NAME_2,
            (SystemUserPrincipal) () -> SYSTEM_USER_NAME_1);
        assertIsSystem(principals,
