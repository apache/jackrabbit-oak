...
public class AutoMembershipPrincipalsTest extends AbstractAutoMembershipTest {
    ...
    public void before() throws Exception {
        ...
        when(amConfig.getAutoMembership(authorizable)).thenReturn(
            ImmutableSet.of(automembershipGroup3.getID()));
        when(amConfig.getAutoMembers(any(UserManager.class), any(Group.class))).thenReturn(
            Collections.emptyIterator());
        when(amConfig.getAutoMembers(userManager, automembershipGroup3)).thenReturn(
    ...
    @NotNull
    private Set<Principal> getAutoMembership(@NotNull String idpName,
    ...
    public void testGetPrincipalsMultipleGroups() throws Exception {
        ...
        Set<Principal> expected = Sets.newHashSet(automembershipGroup1.getPrincipal(),
            automembershipGroup2.getPrincipal());
    ...
    public void testGetPrincipalsMixed() throws Exception {
        ...
        assertFalse(principals.isEmpty());
        assertEquals(ImmutableSet.of(automembershipGroup1.getPrincipal()),
    ...
    public void testGetPrincipalsInherited() throws Exception {
        ...
        Set<Principal> expected = Sets.newHashSet(testGroup.getPrincipal(),
            automembershipGroup1.getPrincipal(), automembershipGroup2.getPrincipal());
        assertEquals(expected, principals);
    ...
    public void testGroupLookupByPrincipalFails() throws Exception {
        ...
        when(um.getAuthorizable(automembershipGroup1.getPrincipal())).thenThrow(
    ...
    public void testInheritedGroupLookupFails() throws Exception {
        ...
        AutoMembershipPrincipals amprincipals = new AutoMembershipPrincipals(um, MAPPING,
            Collections.emptyMap());
        Map<Principal, Group> map = amprincipals.getAutoMembership(IDP_VALID_AM, authorizable,
            true);
    ...
    public void testInheritedFromAMConfig() throws Exception {
        ...
        AutoMembershipPrincipals amprincipals = new AutoMembershipPrincipals(userManager,
            Collections.emptyMap(),
            getAutoMembershipConfigMapping());
    ...
    public void testIsInheritedMemberGroupLookupFails() throws Exception {
        AutoMembershipPrincipals amprincipals = new AutoMembershipPrincipals(userManager, MAPPING,
            Collections.emptyMap());
    ...
    public void testIsInheritedMemberConfiguredUser() throws Exception {
        Map<String, String[]> mapping = ImmutableMap.of(IDP_INVALID_AM,
            new String[]{getTestUser().getID()});
        AutoMembershipPrincipals amprincipals = new AutoMembershipPrincipals(userManager, mapping,
            Collections.emptyMap());
        assertFalse(amprincipals.isInheritedMember(IDP_INVALID_AM, getTestGroup(), authorizable));
    }
    ...
    public void testGetConfiguredIdpNames() {
        assertEquals(ImmutableSet.of(IDP_VALID_AM, IDP_MIXED_AM),
            amp.getConfiguredIdpNames(() -> AUTOMEMBERSHIP_GROUP_ID_1));
        assertEquals(ImmutableSet.of(IDP_VALID_AM),
            amp.getConfiguredIdpNames(() -> AUTOMEMBERSHIP_GROUP_ID_2));
