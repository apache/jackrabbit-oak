...
public class AutoMembershipAwareTest extends AbstractExternalAuthTest {
    ...
    public void before() throws Exception {
        ...
        properties.put(DefaultSyncConfigImpl.PARAM_GROUP_AUTO_MEMBERSHIP,
            new String[]{GROUP_AUTOMEMBERSHIP_ID});
        properties.put(DefaultSyncConfigImpl.PARAM_USER_AUTO_MEMBERSHIP,
            new String[]{USER_AUTOMEMBERSHIP_ID});
    ...
    public void testNameMatch() {
        ...
        when(amc.getAutoMembers(any(UserManager.class), any(Group.class))).thenReturn(
            Iterators.singletonIterator(authorizable));

        context.registerService(AutoMembershipConfig.class, amc,
            Collections.singletonMap(AutoMembershipConfig.PARAM_SYNC_HANDLER_NAME, sh.getName()));
    ...
    public void testMultipleMatches() {
        ...
        assertEquals(ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1),
            config.getAutoMembership(authorizable));
        assertEquals(ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1, CONFIG_AUTOMEMBERSHIP_ID_2,
            USER_AUTOMEMBERSHIP_ID), config.getAutoMembership(user));
    ...
    public void testSyncConfigIsUpdatedToCoverAutoMembershipConfig() throws Exception {
        ...
        assertEquals(ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1, USER_AUTOMEMBERSHIP_ID),
            scU.getAutoMembership(authorizable));
        assertEquals(ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1, CONFIG_AUTOMEMBERSHIP_ID_2,
            USER_AUTOMEMBERSHIP_ID), scU.getAutoMembership(user));

        assertEquals(ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1, GROUP_AUTOMEMBERSHIP_ID),
            scG.getAutoMembership(authorizable));
        assertEquals(ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1, CONFIG_AUTOMEMBERSHIP_ID_2,
            USER_AUTOMEMBERSHIP_ID, GROUP_AUTOMEMBERSHIP_ID), scG.getAutoMembership(user));
    }
    ...
    private static void injectAutoMembershipConfig(@NotNull AutoMembershipConfig amc1,
        ...
        when(amc1.getAutoMembership(any(Authorizable.class))).thenReturn(
            ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1));

        when(amc2.getName()).thenReturn(syncHandler.getName());
        when(amc2.getAutoMembership(any(User.class))).thenReturn(
            ImmutableSet.of(CONFIG_AUTOMEMBERSHIP_ID_1, CONFIG_AUTOMEMBERSHIP_ID_2,
                USER_AUTOMEMBERSHIP_ID));

        context.registerService(AutoMembershipConfig.class, amc1,
            Collections.singletonMap(AutoMembershipConfig.PARAM_SYNC_HANDLER_NAME,
                syncHandler.getName()));
        context.registerService(AutoMembershipConfig.class, amc2,
            Collections.singletonMap(AutoMembershipConfig.PARAM_SYNC_HANDLER_NAME,
                syncHandler.getName()));
    }
}
