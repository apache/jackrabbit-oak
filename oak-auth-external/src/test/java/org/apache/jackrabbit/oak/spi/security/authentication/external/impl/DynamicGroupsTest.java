...
public class DynamicGroupsTest extends DynamicSyncContextTest {
    ...
    public static Collection<Object[]> parameters() {
        return Lists.newArrayList(
            new Object[]{DefaultSyncConfigImpl.PARAM_USER_MEMBERSHIP_NESTING_DEPTH_DEFAULT, false,
                "Membership-Nesting-Depth=0"},
            new Object[]{DefaultSyncConfigImpl.PARAM_USER_MEMBERSHIP_NESTING_DEPTH_DEFAULT + 1,
                false, "Membership-Nesting-Depth=1"},
            // NOTE: shortcut for PrincipalNameResolver is ignored if dynamic-groups are enabled
            new Object[]{DefaultSyncConfigImpl.PARAM_USER_MEMBERSHIP_NESTING_DEPTH_DEFAULT + 1,
                true, "Membership-Nesting-Depth=1, IDP implements PrincipalNameResolver"},
            new Object[]{DefaultSyncConfigImpl.PARAM_USER_MEMBERSHIP_NESTING_DEPTH_DEFAULT + 2,
                false, "Membership-Nesting-Depth=2"});
    }

    private final long membershipNestingDepth;
    private final boolean isPrincipalNameResolver;

    public DynamicGroupsTest(long membershipNestingDepth, boolean isPrincipalNameResolver,
        @NotNull String name) {
    ...
    private void assertMigratedGroups(@NotNull ExternalIdentity externalIdentity,
        @NotNull Tree userTree) throws Exception {
    ...
    public void testReSyncUserMembershipExpired() throws Exception {
        ...
        try {
            ...
            Set<String> extPNames = Arrays.stream(user.getProperty(REP_EXTERNAL_PRINCIPAL_NAMES))
                                          .filter(Objects::nonNull).map(value -> {
                    try {
            ...
            Set<String> expectedGroupIds = getExpectedSyncedGroupIds(membershipNestingDepth, idp,
            ...
            for (String groupId : getExpectedSyncedGroupIds(PREVIOUS_NESTING_DEPTH, idp,
                previouslySyncedUser)) {
            ...
            syncContext.setForceGroupSync(forceSyncGroupPrevious)
                       .setForceUserSync(forceSyncUserPrevious);
    ...
    public void testGroupPrincipalLookup() throws Exception {
        ...
        for (ExternalIdentityRef ref : getExpectedSyncedGroupRefs(membershipNestingDepth, idp,
            externalUser)) {
    ...
    public void testCrossIDPMembership() throws Exception {
        ...
        List<ExternalIdentityRef> declaredGroupRefs = ImmutableList.copyOf(
            previouslySyncedUser.getDeclaredGroups());
        ...
        if (membershipNestingDepth == 0) {
            ...
            assertEquals("Found " + groupIds, (membershipNestingDepth > 1) ? 5 : 4,
                groupIds.size());
        ...
        assertEquals(membershipNestingDepth != 0,
            principalNames.contains(local.getPrincipal().getName()));
        for (ExternalIdentityRef ref : declaredGroupRefs) {
            ExternalIdentity eg = idp.getIdentity(ref);
            assertEquals(membershipNestingDepth != 0,
                principalNames.contains(eg.getPrincipalName()));
        }
