...
public class ExternalLoginModuleTest extends AbstractSecurityTest {
    ...
    private final ExternalIdentityProviderManager extIPMgr = mock(
    ...
    private final ExternalIdentityMonitor externalIdentityMonitor = mock(
    ...
    private CallbackHandler createCallbackHandler(@Nullable Whiteboard wb,
        @Nullable ContentRepository contentRepository, @Nullable SecurityProvider securityProvider,
    ...
    public void testInitializeMissingWhiteboard() throws LoginException {
        ...
        loginModule.initialize(new Subject(), cbh, Collections.emptyMap(),
            ImmutableMap.of(PARAM_IDP_NAME, "idp", PARAM_SYNC_HANDLER_NAME, "syncHandler"));
    ...
    public void testInitializEmptyIdpName() {
        ...
        loginModule.initialize(new Subject(), createCallbackHandler(wb, null, null, null),
            Collections.emptyMap(), Collections.singletonMap(PARAM_IDP_NAME, ""));
    ...
    public void testInitializEmptySyncHanderName() {
        ...
        loginModule.initialize(new Subject(), createCallbackHandler(wb, null, null, null),
            Collections.emptyMap(), Collections.singletonMap(PARAM_SYNC_HANDLER_NAME, ""));
    ...
    public void testLoginMissingSyncHandler() throws Exception {
        ...
        loginModule.initialize(new Subject(), createCallbackHandler(wb, null, null, null),
            Collections.emptyMap(), Collections.singletonMap(PARAM_IDP_NAME, "idpName"));
        assertFalse(loginModule.login());
        assertFalse(loginModule.commit());
        assertFalse(loginModule.logout());
        verifyNoInteractions(monitor);
    ...
    public void testLoginMissingUserId() throws Exception {
        ...
        when(((CredentialsSupport) idp).getCredentialClasses()).thenReturn(
            ImmutableSet.of(GuestCredentials.class));

        when(extIPMgr.getProvider(DEFAULT_IDP_NAME)).thenReturn(idp);
        when(syncManager.getSyncHandler("syncHandler")).thenReturn(
            new DefaultSyncHandler(new DefaultSyncConfigImpl().setName("syncHandler")));

        wb.register(ExternalIdentityProviderManager.class, extIPMgr, Collections.emptyMap());
        wb.register(SyncManager.class, syncManager, Collections.emptyMap());

        CallbackHandler cbh = createCallbackHandler(wb, getContentRepository(),
            getSecurityProvider(), new GuestCredentials());
    ...
    public void testLoginCommitUpdatesSubject() throws Exception {
        ...
        CallbackHandler cbh = createCallbackHandler(wb, getContentRepository(),
            getSecurityProvider(), null);

        Subject subject = new Subject();
        loginModule.initialize(subject, cbh, sharedState,
            ImmutableMap.of(PARAM_IDP_NAME, DEFAULT_IDP_NAME, PARAM_SYNC_HANDLER_NAME,
                "syncHandler"));
    ...
    public void testLoginCommitEmptyPrincipalSet() throws Exception {
        ...
        PrincipalConfiguration pc = when(
            mock(PrincipalConfiguration.class).getPrincipalProvider(any(Root.class),
                any(NamePathMapper.class))).thenReturn(EmptyPrincipalProvider.INSTANCE).getMock();
    ...
    public void testLoginCommitImpersonationCredentials() throws Exception {
        ...
        ExternalIdentityProvider idp = mock(ExternalIdentityProvider.class,
            withSettings().extraInterfaces(CredentialsSupport.class));
        when(idp.getName()).thenReturn(DEFAULT_IDP_NAME);
        when(idp.authenticate(creds)).thenReturn(
            new TestIdentityProvider.TestUser(ID_TEST_USER, DEFAULT_IDP_NAME));
        when(((CredentialsSupport) idp).getUserId(any(Credentials.class))).thenReturn(ID_TEST_USER);
        when(((CredentialsSupport) idp).getCredentialClasses()).thenReturn(
            ImmutableSet.of(ImpersonationCredentials.class));
    ...
    public void testLoginCommitReadonlySubject() throws Exception {
        ...
        loginModule.initialize(readOnly, cbh, sharedState,
            ImmutableMap.of(PARAM_IDP_NAME, DEFAULT_IDP_NAME, PARAM_SYNC_HANDLER_NAME,
                "syncHandler"));
        assertTrue(loginModule.login());
        assertTrue(loginModule.commit());
    ...
    public void testSyncUserMissingRoot() throws LoginException {
        ...
        loginModule.initialize(new Subject(), createCallbackHandler(wb, null, null, null),
            sharedState, ImmutableMap.of(PARAM_IDP_NAME, DEFAULT_IDP_NAME, PARAM_SYNC_HANDLER_NAME,
                "syncHandler"));
        try {
            loginModule.login();
    ...
    public void testSyncUserMissingUserManager() throws LoginException {
        ...
        loginModule.initialize(new Subject(), cbh, sharedState,
            ImmutableMap.of(PARAM_IDP_NAME, DEFAULT_IDP_NAME, PARAM_SYNC_HANDLER_NAME,
                "syncHandler"));
        try {
            loginModule.login();
    ...
    public void testValidateUser() throws Exception {
        ...
        u.setProperty(REP_EXTERNAL_ID,
            getValueFactory().createValue("local;" + TestIdentityProvider.DEFAULT_IDP_NAME));
        root.commit();
    ...
    public void testLoginFailed() throws Exception {
        ...
        Credentials credentials = new SimpleCredentials(ID_TEST_USER,
            "wrongpassword".toCharArray());
        CallbackHandler cbh = createCallbackHandler(wb, getContentRepository(),
            getSecurityProvider(), credentials);
    ...
    public void testSyncUser() throws Exception {
        ...
        verify(externalIdentityMonitor).doneSyncExternalIdentity(anyLong(), any(SyncResult.class),
            anyInt());
        verify(monitor).principalsCollected(anyLong(), anyInt());
    ...
    public void testGetSyncedIdentityFails() throws Exception {
        SyncHandler sh = when(
            mock(SyncHandler.class).findIdentity(any(UserManager.class), anyString())).thenThrow(
            new RepositoryException()).getMock();
        SyncManager syncManager = when(
            mock(SyncManager.class).getSyncHandler(anyString())).thenReturn(sh).getMock();
    ...
    public void testGetExternalIdentityFails() throws Exception {
        SyncManager syncManager = when(
            mock(SyncManager.class).getSyncHandler(anyString())).thenReturn(
            new DefaultSyncHandler()).getMock();
        when(extIPMgr.getProvider(anyString())).thenReturn(new TestIdentityProvider());
