...
public class SyncConfigTrackerConcurrencyTest {
    ...
    private static final long SAMPLE_MILLIS = 50;
    ...
    /**
     * Runs {@code action} directly on {@link #THREAD_COUNT} threads in a tight loop, so all of
     * them contend on the tracker concurrently from the start rather than serializing through a
     * single task submitter, and polls the {@link ThreadMXBean} of all worker threads in a batch
     * for {@link #SAMPLE_MILLIS}, returning the accumulated blocked time observed on a
