...
import static org.apache.jackrabbit.oak.segment.azure.util.AzureConfigurationParserUtils.AzureConnectionKey.SHARED_ACCESS_SIGNATURE;
...
/**
 * Utility class for parsing Oak Segment Azure configuration (e.g. connection string, container
 * name, uri, etc.) from custom encoded String or Azure standard URI.
 */
public class AzureConfigurationParserUtils {
     ...
     * <br>
     * where the first 5 lines in the string represent a standard Azure Connection String and the
     * last two lines are Oak Segment Azure specific arguments. Please note that all configuration
     * keys are semicolon separated, except for the last entry. The order of keys is not important.
     *
     ...
     */
    public static Map<String, String> parseAzureConfigurationFromCustomConnection(
        ...
        canonicalConnectionString.append(DEFAULT_ENDPOINTS_PROTOCOL.text()).append("=")
                                 .append(tempConfig.get(DEFAULT_ENDPOINTS_PROTOCOL)).append(";");
        canonicalConnectionString.append(ACCOUNT_NAME.text()).append("=")
                                 .append(tempConfig.get(ACCOUNT_NAME)).append(";");
        if (tempConfig.containsKey(ACCOUNT_KEY)) {
            canonicalConnectionString.append(ACCOUNT_KEY.text()).append("=")
                                     .append(tempConfig.get(ACCOUNT_KEY)).append(";");
        }
        if (tempConfig.containsKey(SHARED_ACCESS_SIGNATURE)) {
            canonicalConnectionString.append(SHARED_ACCESS_SIGNATURE.text()).append("=")
                                     .append(tempConfig.get(SHARED_ACCESS_SIGNATURE)).append(";");
        }
        if (tempConfig.containsKey(BLOB_ENDPOINT)) {
            canonicalConnectionString.append(BLOB_ENDPOINT.text()).append("=")
                                     .append(tempConfig.get(BLOB_ENDPOINT)).append(";");
        }
     ...
     * <b>https</b>://<b>myaccount</b>.blob.core.windows.net/<b>container</b>/<b>repo</b>?<b>sasToken</b>.
     * The <i>sasToken</i> is optional.
     *
     * @param uriStr the Azure URI as string
     * @return parsed configuration map containing <b>accountName</b>, <b>storageUri</b>,
     * <b>dir</b>, and <b>sharedAccessSignature</b>
     * (key names in bold)
     */
