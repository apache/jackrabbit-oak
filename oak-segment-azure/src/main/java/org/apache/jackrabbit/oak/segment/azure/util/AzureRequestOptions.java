...
public class AzureRequestOptions {
    ...
    public static void applyDefaultRequestOptions(BlobRequestOptions blobRequestOptions) {
        if (blobRequestOptions.getRetryPolicyFactory() == null) {
            ...
            if (retryAttempts > 0) {
                Integer retryBackoffSeconds = Integer.getInteger(RETRY_BACKOFF_PROP,
                    DEFAULT_RETRY_BACKOFF_SECONDS);
                blobRequestOptions.setRetryPolicyFactory(
                    new RetryLinearRetry((int) TimeUnit.SECONDS.toMillis(retryBackoffSeconds),
                        retryAttempts));
            }
        }
        if (blobRequestOptions.getMaximumExecutionTimeInMs() == null) {
            ...
            if (timeoutExecution > 0) {
                blobRequestOptions.setMaximumExecutionTimeInMs(
                    (int) TimeUnit.SECONDS.toMillis(timeoutExecution));
            }
        }
        if (blobRequestOptions.getTimeoutIntervalInMs() == null) {
            ...
            if (timeoutInterval > 0) {
                blobRequestOptions.setTimeoutIntervalInMs(
                    (int) TimeUnit.SECONDS.toMillis(timeoutInterval));
            }
        }
    }

    /**
     * Optimise the blob request options for write operations. This method does not change the
     * original blobRequestOptions. This method also applies the default request options if they are
     * not already set, by calling {@link #applyDefaultRequestOptions(BlobRequestOptions)}
     *
     * @param blobRequestOptions
     * @return write optimised blobRequestOptions
     */
    public static BlobRequestOptions optimiseForWriteOperations(
        BlobRequestOptions blobRequestOptions) {
        BlobRequestOptions writeOptimisedBlobRequestOptions = new BlobRequestOptions(
            blobRequestOptions);
        ...
        if (writeTimeoutExecution != null) {
            writeOptimisedBlobRequestOptions.setMaximumExecutionTimeInMs(
                (int) TimeUnit.SECONDS.toMillis(writeTimeoutExecution));
        }

        Integer writeTimeoutInterval = Integer.getInteger(WRITE_TIMEOUT_INTERVAL_PROP);
        if (writeTimeoutInterval != null) {
            writeOptimisedBlobRequestOptions.setTimeoutIntervalInMs(
