...
//                , vmOption( "-Xrunjdwp:transport=dt_socket,server=y,suspend=y,address=5005" )
    ...
    private Option jpmsOptions() {
        ...
        if (Version.parseVersion(System.getProperty("java.specification.version")).getMajor() > 1) {
            if (java.nio.file.Files.exists(
                java.nio.file.FileSystems.getFileSystem(URI.create("jrt:/"))
                                         .getPath("modules", "java.se.ee"))) {
    ...
    private Option setupTikaAndPoi() throws IOException {
        ...
        return composite(
            composite(
                ...
                , wrappedBundle(
                    mavenBundle("org.apache.poi", "poi-scratchpad", versions.get(POI_VERSION)))
                ,
                wrappedBundle(mavenBundle("org.apache.poi", "poi-ooxml", versions.get(POI_VERSION)))
                , wrappedBundle(
                    mavenBundle("org.apache.poi", "poi-ooxml-lite", versions.get(POI_VERSION)))
                ...
                , mavenBundle("org.apache.commons", "commons-collections4",
                    versions.get(COLLECTIONS4_VERSION))
                , mavenBundle("org.apache.commons", "commons-compress",
                    versions.get(COMPRESS_VERSION))
    ...
    private Option jarBundles() throws MalformedURLException {
        ...
        List<File> jarFiles = stream(jarNames).map(jarName -> new File(jarDir, jarName))
    ...
    private static Map<String, String> setupVersions() throws IOException {
        ...
        props.load(TikaExtractionOsgiIT.class.getClassLoader()
                                             .getResourceAsStream(VERSION_PROP_RESOURCE_NAME));
