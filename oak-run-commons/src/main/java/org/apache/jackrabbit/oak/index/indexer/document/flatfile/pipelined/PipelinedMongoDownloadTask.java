...
public class PipelinedMongoDownloadTask implements Callable<PipelinedMongoDownloadTask.Result> {
    ...
    private static final Logger TRAVERSAL_LOG = LoggerFactory.getLogger(
        PipelinedMongoDownloadTask.class.getName() + ".traversal");
    ...
    /**
     * Whether to retry on connection errors to MongoDB. This property affects the query that is
     * used to download the documents from MongoDB. If set to true, the query will traverse the
     * results by order of the _modified property (does an index scan), which allows it to resume
     * after a failed connection from where it left off. If set to false, it uses a potentially more
     * efficient query that does not impose any order on the results (does a simple column scan).
     */
    ...
    /**
     * Whether to do path filtering in the Mongo query instead of doing a full traversal of the
     * document store and filtering in the indexing job. This feature may significantly reduce the
     * number of documents downloaded from Mongo. The performance gains may not be proportional to
     * the reduction in the number of documents downloaded because Mongo still has to traverse all
     * the documents. This is required because the regex expression used for path filtering starts
     * with a wildcard (because the _id starts with the depth of the path, so the regex expression
     * must ignore this part). Because of the wildcard at the start, Mongo cannot use of the index
     * on _id.
     */
    ...
    /**
     * Any document with a path that matches this regex pattern will not be downloaded. This pattern
     * will be included in the Mongo query, that is, the filtering is done by server-side at Mongo,
     * which avoids downloading the documents matching this query. This is typically a _suffix_, for
     * example "/metadata.xml$|/renditions/.*.jpg$". To exclude subtrees such as /content/abc, use
     * mongoFilterPaths instead.
     */
    ...
    /**
     * Maximum number of elements in the included/excluded paths list used for regex path filtering.
     * If after merging and de-deduplication of the paths of all the path filters the number of
     * included or excluded paths exceeds this value, then disable path filtering to avoid creating
     * Mongo queries with large number of filters
     */
    ...
    /**
     * Additional Oak paths to exclude from downloading from Mongo. This is a comma-separated list
     * of paths. These paths are only filtered if the included paths computed from the indexes
     * resolve to the root tree (/), otherwise the value of this property is ignored.
     */
    ...
    /**
     * Whether to download in parallel from Mongo with two streams, one per each secondary. This
     * applies only if Mongo is a cluster with two secondaries. One thread downloads in ascending
     * order of (_modified, _id) and the other in descending order, until they cross. This feature
     * requires that the retryOnConnectionErrors property is set to true, because it relies on
     * downloading in a given order (if retryOnConnectionErrors is false, the download is done in
     * natural order, that is, it is undefined).
     */
    ...
    public PipelinedMongoDownloadTask(MongoClientURI mongoClientURI,
        ...
        Preconditions.checkArgument(retryDuringSeconds > 0,
            "Property " + OAK_INDEXER_PIPELINED_MONGO_CONNECTION_RETRY_SECONDS
                + " must be > 0. Was: " + retryDuringSeconds);
        this.reporter.addConfig(OAK_INDEXER_PIPELINED_MONGO_CONNECTION_RETRY_SECONDS,
            String.valueOf(retryDuringSeconds));
        ...
        this.reporter.addConfig(OAK_INDEXER_PIPELINED_RETRY_ON_CONNECTION_ERRORS,
            String.valueOf(retryOnConnectionErrors));
        ...
        this.reporter.addConfig(OAK_INDEXER_PIPELINED_MONGO_REGEX_PATH_FILTERING,
            String.valueOf(regexPathFiltering));
        ...
        this.reporter.addConfig(OAK_INDEXER_PIPELINED_MONGO_REGEX_PATH_FILTERING_MAX_PATHS,
            String.valueOf(regexPathFilteringMaxNumberOfPaths));
        this.regexPathFilterFactory = new MongoRegexPathFilterFactory(
            regexPathFilteringMaxNumberOfPaths);

        boolean parallelDumpConfig = ConfigHelper.getSystemPropertyAsBoolean(
        ...
        if (parallelDumpConfig && !retryOnConnectionErrors) {
            LOG.warn("Parallel dump requires " + OAK_INDEXER_PIPELINED_RETRY_ON_CONNECTION_ERRORS
                + " to be set to true, but it is false. Disabling parallel dump.");
            this.parallelDump = false;
        } else {
            this.parallelDump = parallelDumpConfig;
        }
        this.reporter.addConfig(OAK_INDEXER_PIPELINED_MONGO_PARALLEL_DUMP,
            String.valueOf(parallelDump));
        ...
        this.reporter.addConfig(OAK_INDEXER_PIPELINED_MONGO_CUSTOM_EXCLUDE_ENTRIES_REGEX,
        ...
        this.reporter.addConfig(OAK_INDEXER_PIPELINED_MONGO_CUSTOM_EXCLUDED_PATHS,
            excludePathsString);
        ...
        List<String> invalidPaths = customExcludedPaths.stream()
                                                       .filter(p -> !PathUtils.isValid(p)
                                                           || !PathUtils.isAbsolute(p)
                                                           || PathUtils.denotesRoot(p))
                                                       .collect(Collectors.toList());
        if (!invalidPaths.isEmpty()) {
            throw new IllegalArgumentException(
                "Invalid paths in " + OAK_INDEXER_PIPELINED_MONGO_CUSTOM_EXCLUDED_PATHS + " " +
                    " system property: " + invalidPaths
                    + ". Paths must be valid, must be absolute and must not be the root.");
        }
    }
    ...
    public Result call() throws Exception {
        ...
        try {
            ...
            NodeDocumentCodecProvider nodeDocumentCodecProvider = new NodeDocumentCodecProvider(
                docStore, Collection.NODES);
            ...
            try (MongoClient client = MongoClients.create(settingsBuilder.build())) {
                ...
                return new PipelinedMongoDownloadTask.Result(
                    downloadStageStatistics.getDocumentsDownloadedTotal());
            }
        } finally {
    ...
    private void downloadWithNaturalOrdering() throws InterruptedException, TimeoutException {
        ...
        Bson mongoFilter = MongoDownloaderRegexUtils.computeMongoQueryFilter(mongoFilterPaths,
            customExcludeEntriesRegex);
        ...
        if (mongoFilter == null) {
            ...
            FindIterable<NodeDocument> mongoIterable = dbCollection
                ...
                .hint(NATURAL_HINT);
            DownloadTask downloadTask = new DownloadTask(DownloadOrder.UNDEFINED,
                downloadStageStatistics);
            downloadTask.download(mongoIterable);
            downloadTask.reportFinalResults();

        } else {
            DownloadTask ancestorsDownloadTask = new DownloadTask(DownloadOrder.UNDEFINED,
                downloadStageStatistics);
    ...
    private void downloadWithRetryOnConnectionErrors()
        ...
        if (parallelDump) {
            LOG.info(
                "Downloading in parallel with two connections, one in ascending the other in descending order");
            DownloadTask ascendingDownloadTask = new DownloadTask(DownloadOrder.ASCENDING,
                downloadStageStatistics);
            DownloadTask descendingDownloadTask = new DownloadTask(DownloadOrder.DESCENDING,
                downloadStageStatistics);

            ExecutorService downloadThreadPool = Executors.newFixedThreadPool(2,
                new ThreadFactoryBuilder().setDaemon(true).build());
            Future<?> ascendingDownloadFuture = submitDownloadTask(downloadThreadPool,
                ascendingDownloadTask, mongoFilter, THREAD_NAME_PREFIX + "-ascending");
            Future<?> descendingDownloadFuture = submitDownloadTask(downloadThreadPool,
            ...
            try {
                ...
                while (!downloadFinished) {
                    ...
                    try {
                        ...
                        LOG.info(
                            "Ascending download finished. Waiting for descending download to finish.");
                        ...
                        if (secondsElapsed == 0) {
                            ...
                            double docRate =
                                ((double) downloadStageStatistics.getDocumentsDownloadedTotal())
                                    / secondsElapsed;
                            double bytesRate =
                                ((double) downloadStageStatistics.getDocumentsDownloadedTotalBytes())
                                    / secondsElapsed;
                            formattedRate = String.format(Locale.ROOT,
                                "%1.2f nodes/s, %1.2f nodes/hr, %s/s",
                                docRate, docRate * 3600,
                                IOUtils.humanReadableByteCountBin((long) bytesRate));
                        }
                        LOG.info(
                            "Total documents dumped from Mongo {} (asc: {}, desc: {}) [{}] (Elapsed {})",
                            downloadStageStatistics.getDocumentsDownloadedTotal(),
                            ascTaskDownloadTotal, descTaskDownloadTotal,
                ...
                LOG.info("Error during download. Canceling download threads. Error: {}",
                    e.toString());
    ...
    private Future<?> submitDownloadTask(ExecutorService executor, DownloadTask downloadTask,
        Bson mongoFilter, String name) {
    ...
    private MongoFilterPaths getPathsForRegexFiltering() {
        if (!regexPathFiltering) {
            ...
            LOG.info(
                "Computing included/excluded paths for Mongo regex path filtering. PathFilters: {}",
                pathFilters.stream()
                           .map(pf -> "PF{includedPaths=" + pf.getIncludedPaths()
                               + ", excludedPaths=" + pf.getExcludedPaths() + "}")
                           .collect(Collectors.joining(", "))
            );
            MongoFilterPaths mongoFilterPaths = this.regexPathFilterFactory.buildMongoFilter(
                pathFilters, customExcludedPaths);
    ...
    private void logWithRateLimit(Runnable f) {
        ...
        if (Duration.between(lastDelayedEnqueueWarningMessageLoggedTimestamp, now).toSeconds()
            > MIN_INTERVAL_BETWEEN_DELAYED_ENQUEUING_MESSAGES) {
            f.run();
    ...
    /**
     * Downloads a given range from Mongo. Instances of this class should be used for downloading a
     * single range. To download multiple ranges, create multiple instances of this class.
     */
    private class DownloadTask {
            ...
            while (!downloadCompleted) {
                try {
                    if (lastIdDownloaded == null) {
                        ...
                        DownloadRange firstRange = new DownloadRange(0, Long.MAX_VALUE, null,
                            downloadOrder.downloadInAscendingOrder());
                        downloadRange(firstRange, mongoQueryFilter, downloadOrder);
                    } else {
                        LOG.info(
                            "Recovering from broken connection, finishing downloading documents with _modified={}",
                            nextLastModified);
                        DownloadRange partialLastModifiedRange = new DownloadRange(nextLastModified,
                            nextLastModified, lastIdDownloaded,
                            downloadOrder.downloadInAscendingOrder());
                    ...
                    if (parallelDump && mongoServerSelector.atLeastOneConnectionActive()) {
                        ...
                        int retryTime = 1000;
                        LOG.info("At least one connection is active. Retrying download in {} ms",
                            retryTime);
                        ...
                        numberOfFailures++;
                        LOG.warn(
                            "Retrying download in {} ms; number of times failed: {}; current series of failures started at: {} ({} seconds ago)",
                            retryIntervalMs, numberOfFailures, failuresStartTimestamp,
                            secondsSinceStartOfFailures);
        ...
        private void downloadRange(DownloadRange range, Bson filter, DownloadOrder downloadOrder)
            throws InterruptedException, TimeoutException {
        ...
        void download(FindIterable<NodeDocument> mongoIterable)
            throws InterruptedException, TimeoutException {
                ...
                try {
                    while (cursor.hasNext()) {
                        ...
                        if (batchSize >= maxBatchSizeBytes || nextIndex == batch.length) {
                            LOG.trace("Enqueuing block with {} elements, estimated size: {} bytes",
                                nextIndex, batchSize);
                            boolean downloadCompleted = tryEnqueueCopy(batch, nextIndex);
                            if (downloadCompleted) {
                                LOG.info(
                                    "Download of range with download order {} completed, intersected with other download.",
                                    downloadOrder);
                                return;
                            }
                            nextIndex = 0;
                            ...
                            if (parallelDump && mongoServerSelector.isConnectedToPrimary()) {
                                LOG.info(
                                    "Connected to primary. Will disconnect from MongoDB to force a new connection to a secondary.");
                                throw new MongoException(
                                    "Disconnecting from MongoDB primary to force a new connection");
                    ...
                    if (nextIndex > 0) {
                        LOG.info(
                            "Connection interrupted with recoverable failure. Enqueueing partial block with {} elements, estimated size: {}",
                            nextIndex, IOUtils.humanReadableByteCountBin(batchSize));
                        tryEnqueueCopy(batch, nextIndex);
                    }
                    throw e;
        ...
        private void downloadAncestors(List<String> basePath)
            throws InterruptedException, TimeoutException {
        ...
        private boolean tryEnqueueCopy(NodeDocument[] batch, int sizeOfBatch)
            ...
            if (downloadOrder != DownloadOrder.UNDEFINED) {
                ...
                if (sizeOfRangeNotAdded != sizeOfBatch) {
                    ...
                    LOG.info("Download complete, reached already seen document: {}: {}",
                        firstAlreadySeen.getModified(), firstAlreadySeen.getId());
                }
            }
            if (effectiveSize == 0) {
                ...
                if (!mongoDocQueue.offer(copyOfBatch, MONGO_QUEUE_OFFER_TIMEOUT.toMillis(),
                    TimeUnit.MILLISECONDS)) {
                    throw new TimeoutException(
                        "Timeout trying to enqueue batch of MongoDB documents. Waited "
                            + MONGO_QUEUE_OFFER_TIMEOUT);
                }
                ...
                if (enqueueDelay > 1) {
                    logWithRateLimit(() ->
                        LOG.info(
                            "Enqueuing of Mongo document batch was delayed, took {} ms. mongoDocQueue size {}. "
                                +
                                "Consider increasing the number of Transform threads. " +
                                "(This message is logged at most once every {} seconds)",
                            enqueueDelay, mongoDocQueue.size(),
                            MIN_INTERVAL_BETWEEN_DELAYED_ENQUEUING_MESSAGES)
                    );
                }
            }
        ...
        void reportFinalResults() {
            ...
            LOG.info("Finished download task. Dumped {} documents. Rate: {}. Elapsed {}.",
                this.documentsDownloadedTotal, formattedRate,
                FormattingUtils.formatToSeconds(downloadStartWatch));
        }
        ...
        private void reportProgress(String id) {
            ...
            LOG.info("{} from NSET Traversed #{} {} [{}] (Elapsed {})",
                msgPrefix, this.documentsDownloadedTotal, id, formattedRate,
