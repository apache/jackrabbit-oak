...
import static org.apache.jackrabbit.oak.index.indexer.document.flatfile.pipelined.PipelinedStrategyNodeDocument.SENTINEL_MONGO_DOCUMENT;
 ...
 */
class PipelinedTransformTaskNodeDocument implements Callable<PipelinedTransformTask.Result> {
    private static final Logger LOG = LoggerFactory.getLogger(PipelinedTransformTaskNodeDocument.class);
    ...
    public PipelinedTransformTaskNodeDocument(MongoDocumentStore mongoStore,
    ...
    public PipelinedTransformTask.Result call() throws Exception {
        ...
        try {
            ...
            while (true) {
                LOG.info("Waiting for a batch of Mongo documents");
                Stopwatch docQueueWaitStopwatch = Stopwatch.createStarted();
                NodeDocument[] dbObjectBatch = mongoDocQueue.take();
                long batchWaitTimeMillis = docQueueWaitStopwatch.elapsed(TimeUnit.MILLISECONDS);
                totalDocumentQueueWaitTimeMillis += batchWaitTimeMillis;
                LOG.info("Received batch of {} documents. Waited: {} ms", dbObjectBatch.length, batchWaitTimeMillis);
