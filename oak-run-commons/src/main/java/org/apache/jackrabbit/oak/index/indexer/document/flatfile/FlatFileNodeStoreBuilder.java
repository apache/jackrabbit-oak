...
public class FlatFileNodeStoreBuilder {
    ...
    /**
     * System property name for sort strategy. Allowed values are the values from enum
     * {@link SortStrategyType}
     */
    ...
    /**
     * Value of this system property indicates max memory that should be used if jmx based memory
    ...
    public enum SortStrategyType {
        /**
         * System property {@link #OAK_INDEXER_SORT_STRATEGY_TYPE} if set to this value would result
         * in {@link StoreAndSortStrategy} being used.
         */
        STORE_AND_SORT,
        /**
         * System property {@link #OAK_INDEXER_SORT_STRATEGY_TYPE} if set to this value would result
         * in {@link TraverseWithSortStrategy} being used.
         */
        TRAVERSE_WITH_SORT,
        /**
         * System property {@link #OAK_INDEXER_SORT_STRATEGY_TYPE} if set to this value would result
         * in {@link PipelinedStrategy} being used.
         */
        PIPELINED
    }
    ...
    public FlatFileNodeStoreBuilder withNodeStateEntryTraverserFactory(
        NodeStateEntryTraverserFactory factory) {
    ...
    public FlatFileStore build() throws IOException, CompositeException {
        ...
        FlatFileStore store = new FlatFileStore(blobStore, indexStoreFiles.storeFiles.get(0),
            metadataFile,
    ...
    public List<FlatFileStore> buildList(IndexHelper indexHelper, IndexerSupport indexerSupport,
        ...
        if (!fileList.stream().allMatch(FlatFileSplitter.IS_SPLIT)) {
            ...
            log.info("Split flat file to result files '{}' is done, took {} ms", fileList,
                System.currentTimeMillis() - start);
        }

        List<FlatFileStore> storeList = new ArrayList<>();
        for (File flatFileItem : fileList) {
            FlatFileStore store = new FlatFileStore(blobStore, flatFileItem, metadataFile,
                new NodeStateEntryReader(blobStore),
                unmodifiableSet(preferredPathElements), algorithm);
            storeList.add(store);
        }
    ...
    /**
     * Returns the existing list of store files if it can read from system property
     * OAK_INDEXER_SORTED_FILE_PATH which defines the existing folder where the flat file store
     * files are present. Will throw an exception if it cannot read or the path in the system
     * property is not a directory. If the system property OAK_INDEXER_SORTED_FILE_PATH in
     * undefined, or it cannot read relevant files it initializes the flat file store.
     *
    ...
    private IndexStoreFiles createdSortedStoreFiles() throws IOException, CompositeException {
        ...
        if (StringUtils.isNotBlank(sortedFilePath)) {
            ...
            log.info(
                "Attempting to read from provided sorted files directory [{}] (via system property '{}')",
        ...
        if (sortedDir.exists() && sortedDir.canRead() && sortedDir.isDirectory()) {
            ...
            if (storeFiles != null && storeFiles.length != 0) {
                ...
                if (metadataFiles == null || metadataFiles.length == 0) {
                    log.error("Unable to find metadata file in directory:{}",
                        sortedDir.getAbsolutePath());
                    return new IndexStoreFiles(Arrays.asList(storeFiles), null);
                } else {
                    checkState(metadataFiles.length == 1,
                        "Multiple metadata files available at path:{}, metadataFiles:{}",
                        sortedDir.getAbsolutePath(),
            ...
            String msg = String.format("Cannot read sorted files directory at [%s]",
                sortedDir.getAbsolutePath());
    ...
    IndexStoreSortStrategy createSortStrategy(File dir) {
        switch (sortStrategyType) {
            case STORE_AND_SORT:
                ...
                log.warn(
                    "StoreAndSortStrategy is deprecated and will be removed in the near future. Use PipelinedStrategy instead.");
                return new StoreAndSortStrategy(nodeStateEntryTraverserFactory,
                    preferredPathElements, entryWriter, dir,
                    algorithm, pathPredicate, checkpoint);
            case TRAVERSE_WITH_SORT:
                log.info("Using TraverseWithSortStrategy");
                log.warn(
                    "TraverseWithSortStrategy is deprecated and will be removed in the near future. Use PipelinedStrategy instead.");
                return new TraverseWithSortStrategy(nodeStateEntryTraverserFactory,
                    preferredPathElements, entryWriter, dir,
                    algorithm, pathPredicate, checkpoint);
            case PIPELINED:
                log.info("Using PipelinedStrategy");
                List<PathFilter> pathFilters = indexDefinitions.stream()
                                                               .map(IndexDefinition::getPathFilter)
                                                               .collect(Collectors.toList());
                List<String> indexNames = indexDefinitions.stream()
                                                          .map(IndexDefinition::getIndexName)
                                                          .collect(Collectors.toList());
                indexingReporter.setIndexNames(indexNames);
                return new PipelinedStrategy(mongoClientURI, mongoDocumentStore, nodeStore,
                    rootRevision,
    ...
    File createStoreDir() throws IOException {
        flatFileStoreDir = Files.createTempDirectory(workDir.toPath(),
            FLAT_FILE_STORE_DIR_NAME_PREFIX).toFile();
        return flatFileStoreDir;
    }

    /**
     * Returns the flat file store dir. NOTE - Only works after flat file store dir has been built
     * (i.e. after a call to {@link #build()}
     *
