 ...
 */
public class PersistedLinkedListMemoryBoundBuffer implements NodeStateEntryList {

    private final static Logger LOG = LoggerFactory.getLogger(PersistedLinkedListMemoryBoundBuffer.class);
     ...
     */
    public PersistedLinkedListMemoryBoundBuffer(String fileName, NodeStateEntryWriter writer, NodeStateEntryReader reader, int maxCacheSize, int maxCacheSizeMB) {
        this.cacheSizeLimit = maxCacheSize;
        this.cacheSizeLimitBytes = ((long) maxCacheSizeMB) * 1024 * 1024;
        this.storeFileName = fileName;
        LOG.info("Max cache size: {} elements, {} ({} MB)", maxCacheSize, cacheSizeLimitBytes, maxCacheSizeMB);
    ...
    private void addEntryToCache(Long index, NodeStateEntry entry) {
        if (index % (1024 * 1000) == 0) { // Print every million entries
            LOG.info("Entries added: {}, Entries in cache: {}, cacheSizeBytes: {}, map size: {} file size: {} bytes",
                    index, size, cacheSizeEstimationBytes, map.sizeAsLong(), store.getFileStore().size());
        }
        ...
        if (cache.size() == cacheSizeLimit || newCacheSizeBytes > cacheSizeLimitBytes) {
            LOG.info("Mem cache size {}/{} or byte size {}/{} would exceed maximum. Writing to persistent map: {} = {}, entry size: {}",
            ...
            // Move the eldest entry from the cache to the persistent map
//            Long eldestEntry = cache.keySet().stream().min(Long::compareTo).get();
//            NodeStateEntry lastEntry = cache.remove(eldestEntry);
//            LOG.info("Overflowing cache, moving eldest entry {} = {} to persistent cache", eldestEntry, lastEntry.getPath());
//            String s = writer.toString(lastEntry);
//            map.put(eldestEntry, s);
        } else {
    ...
    public void close() {
        store.close();
        LOG.info("Cache hits {} misses {}, max cache size: {}, max cache size MB: {}",
                cacheHits, cacheMisses, peakCacheSize, peakCacheSizeBytes);
    }
