...
public class MongoRegexPathFilterFactory {
    ...
    public static class MongoFilterPaths {
        ...
        public static final MongoFilterPaths DOWNLOAD_ALL = new MongoFilterPaths(List.of("/"),
            List.of());
        ...
        public boolean equals(Object o) {
            ...
            return Objects.equals(included, that.included) && Objects.equals(excluded,
                that.excluded);
        }
    ...
    /**
     * @param pathFilters         Filters representing the included/excludedPaths settings of the
     *                            indexes
    ...
    public MongoFilterPaths buildMongoFilter(List<PathFilter> pathFilters,
        ...
        for (String candidateIncludedPath : sortedCandidateIncludedPaths) {
            if (finalIncludedPathsRoots.stream().noneMatch(
                ancestor -> PathUtils.isAncestor(ancestor, candidateIncludedPath))) {
        ...
        for (String candidateExcludedPath : candidateExcludedPaths) {
            boolean notNeeded = pathFilters.stream().allMatch(
                pathFilter -> pathFilter.filter(candidateExcludedPath)
        ...
        if (finalIncludedPathsRoots.size() > maxNumberOfPaths) {
            LOG.info(
                "Number of includedPaths ({}) exceed the maximum allowed ({}), downloading everything",
                finalIncludedPathsRoots.size(), maxNumberOfPaths);
            return MongoFilterPaths.DOWNLOAD_ALL;
        }

        if (customExcludedPaths.stream().anyMatch(PathUtils::denotesRoot)) {
            LOG.warn("Ignoring custom excluded paths setting, root cannot be excluded: {}",
                customExcludedPaths);
        } else if (!isRootPath(finalIncludedPathsRoots)) {
            LOG.info(
                "Ignoring custom excluded paths because included paths did not resolve to root. Mongo filters: {}",
                finalIncludedPathsRoots);
        } else {
            finalExcludedPaths = mergeIndexAndCustomExcludePaths(finalExcludedPaths,
                customExcludedPaths);
        }
        ...
        if (finalExcludedPaths.size() > maxNumberOfPaths) {
            LOG.info(
                "Number of excludedPaths ({}) exceed the maximum allowed ({}), disabling filtering of excludedPaths",
    ...
    private boolean isRootPath(List<String> includedPaths) {
        return includedPaths.isEmpty() || (includedPaths.size() == 1 && includedPaths.get(0)
                                                                                     .equals("/"));
    }
     ...
     */
    static List<String> mergeIndexAndCustomExcludePaths(List<String> indexExcludedPaths,
        List<String> customExcludedPaths) {
