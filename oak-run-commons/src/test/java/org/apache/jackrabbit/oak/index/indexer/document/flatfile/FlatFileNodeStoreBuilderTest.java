...
public class FlatFileNodeStoreBuilderTest {
    ...
    public void assertBuild(String dir) throws CompositeException, IOException {
        FlatFileNodeStoreBuilder builder = new FlatFileNodeStoreBuilder(
            folder.getRoot()).withNodeStateEntryTraverserFactory(
    ...
    public void assertBuildList(String dir, boolean split) throws CompositeException, IOException {
        ...
        List<FlatFileStore> storeList = builder.buildList(indexHelper, indexerSupport,
            mockIndexDefns());
    ...
    private static Set<IndexDefinition> mockIndexDefns() {
        ...
        IndexDefinition defn = IndexDefinition.newBuilder(store.getRoot(), defnBuilder.build(),
            "/foo").build();
        defns.add(defn);
        ...
        Mockito.when(mockNodeTypeInfoProvider.getNodeTypeInfo("dam:Asset"))
