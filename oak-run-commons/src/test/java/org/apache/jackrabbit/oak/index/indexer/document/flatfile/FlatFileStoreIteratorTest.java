...
public class FlatFileStoreIteratorTest {
    ...
    public void bufferEstimatesMemory() {
        ...
        FlatFileStoreIterator fitr = newInMemoryFlatFileStore(nseList.iterator(), ImmutableSet.of(),
            100);
    ...
    public void memUsageConfig100() {
        try {
            ...
            NodeStateEntry e1Byte = new NodeStateEntryBuilder(EmptyNodeState.EMPTY_NODE,
                "/a/b").withMemUsage(1).build();
            NodeStateEntry e1MB = new NodeStateEntryBuilder(EmptyNodeState.EMPTY_NODE,
                "/a").withMemUsage(1 * 1024 * 1024).build();
            NodeStateEntry e100MB = new NodeStateEntryBuilder(EmptyNodeState.EMPTY_NODE,
                "/a").withMemUsage(100 * 1024 * 1024).build();

            {
                // 100 MB limit
                int mb = 100;
                List<NodeStateEntry> list = Lists.newArrayList(root, e100MB, e1Byte);
                FlatFileStoreIterator fitr = newInMemoryFlatFileStore(list.iterator(),
                    ImmutableSet.of(), mb);
...

            {
                ...
                FlatFileStoreIterator fitr = newInMemoryFlatFileStore(list.iterator(),
                    ImmutableSet.of(), mb);
                NodeState rootNS = fitr.next().getNodeState();
                NodeState aNS = rootNS.getChildNode(
                    "a");//configured limit is 10 bytes, this should work
                try {
                    aNS.getChildNode("b");
...

            {
                ...
                aNS.getChildNode(
                    "b");//configure negative value - mem usage limit should be unbounded (long_max)
            }
        } finally {
            // ignore
        }
