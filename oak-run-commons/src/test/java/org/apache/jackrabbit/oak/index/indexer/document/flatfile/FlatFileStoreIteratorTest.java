...
public class FlatFileStoreIteratorTest {
    ...
    public void bufferEstimatesMemory() {
        List<NodeStateEntry> nseList = Lists.new ArrayList<>(
    ...
    public void memUsageConfig100() {
        try {
...

            {
                // 100 MB limit
                int mb = 100;
                List<NodeStateEntry> list = Lists.new ArrayList<>(root, e100MB, e1Byte);
...

            {
                ...
                List<NodeStateEntry> list = Lists.new ArrayList<>(root, e1MB, e1Byte);
                FlatFileStoreIterator fitr = newInMemoryFlatFileStore(list.iterator(), ImmutableSet.of(), mb);
                NodeState rootNS = fitr.next().getNodeState();
