...
public class CugUtilTest extends AbstractCugTest {
    ...
    public void testHasCug() throws Exception {
        ...
        for (String path : new String[]{PathUtils.ROOT_PATH, INVALID_PATH, UNSUPPORTED_PATH,
            SUPPORTED_PATH + "/subtree", SUPPORTED_PATH2, SUPPORTED_PATH3}) {
    ...
    public void testDefinesCug() throws Exception {
        assertFalse(
            CugUtil.definesCug(root.getTree(PathUtils.concat(INVALID_PATH, REP_CUG_POLICY))));
        assertTrue(
            CugUtil.definesCug(root.getTree(PathUtils.concat(SUPPORTED_PATH, REP_CUG_POLICY))));

        Tree invalid = TreeUtil.addChild(root.getTree(SUPPORTED_PATH2), REP_CUG_POLICY,
            NodeTypeConstants.NT_OAK_UNSTRUCTURED);
    ...
    public void testIsSupportedPath() {
        Set<String> configuredPaths = CUG_CONFIG.getConfigValue(PARAM_CUG_SUPPORTED_PATHS,
            ImmutableSet.of());
    ...
    public void testGetSupportedPathsDefaultMountInfoProvider() {
        Set<String> expected = CUG_CONFIG.getConfigValue(PARAM_CUG_SUPPORTED_PATHS,
            ImmutableSet.of());
        assertEquals(expected,
            CugUtil.getSupportedPaths(CUG_CONFIG, Mounts.defaultMountInfoProvider()));
    }
    ...
    public void testGetSupportedPathsWithDifferentMounts() {
        ...
        MountInfoProvider mip = Mounts.newBuilder()
                                      .mount("private", "/libs", "/apps", "/nonCugPath").build();
    ...
    public void testGetSupportedPathsMountsAtSupportedPath() {
        MountInfoProvider mip = Mounts.newBuilder().mount("private", "/libs", SUPPORTED_PATH3)
                                      .build();
    ...
    public void testGetSupportedPathsMountsBelowSupportedPath() {
        ...
        CugUtil.getSupportedPaths(
            ConfigurationParameters.of(PARAM_CUG_SUPPORTED_PATHS, new String[]{"/"}), mip);
    }

    @Test(expected = IllegalStateException.class)
    public void testGetSupportedPathsMountsBelowSupportedPath2() {
        MountInfoProvider mip = Mounts.newBuilder()
                                      .mount("private", "/libs", SUPPORTED_PATH + "/any/path/below")
                                      .build();
        CugUtil.getSupportedPaths(CUG_CONFIG, mip);
    }
