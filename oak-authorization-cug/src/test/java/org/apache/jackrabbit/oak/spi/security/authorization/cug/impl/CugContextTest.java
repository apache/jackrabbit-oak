...
public class CugContextTest extends AbstractCugTest implements NodeTypeConstants {
    ...
    public void testDefinesLocation() {
        ...
        assertTrue(CugContext.INSTANCE.definesLocation(
            TreeLocation.create(root, CUG_PATH + "/" + CugConstants.REP_PRINCIPAL_NAMES)));
        ...
        for (String path : existingNoCug) {
            ...
            assertFalse(path, CugContext.INSTANCE.definesLocation(
                TreeLocation.create(root, path + "/" + CugConstants.REP_PRINCIPAL_NAMES)));
        }
        ...
        for (String path : nonExistingCug) {
            ...
            assertTrue(path, CugContext.INSTANCE.definesLocation(
                TreeLocation.create(root, path + "/" + CugConstants.REP_PRINCIPAL_NAMES)));
            assertFalse(path, CugContext.INSTANCE.definesLocation(
                TreeLocation.create(root, path + "/" + JcrConstants.JCR_PRIMARYTYPE)));
        }
    }

    @Test
    public void testInvalidCug() throws Exception {
        PropertyState ps = PropertyStates.createProperty(CugConstants.REP_PRINCIPAL_NAMES,
        ...
        invalidCug = TreeUtil.addChild(root.getTree(UNSUPPORTED_PATH), CugConstants.REP_CUG_POLICY,
            NT_OAK_UNSTRUCTURED);
        invalidCug.setProperty(ps);
