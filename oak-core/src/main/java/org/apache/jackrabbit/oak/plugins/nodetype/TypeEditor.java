...
public class TypeEditor extends DefaultEditor {
    ...
    public static interface ConstraintViolationCallback {
         ...
         *
         * <p>Implementors may choose to throw a
         * {@link org.apache.jackrabbit.oak.api.CommitFailedException} or to handle the error
         ...
         * @throws CommitFailedException thrown when the implementation decides to stop further
         *                               processing
         * @see TypeEditor#THROW_ON_CONSTRAINT_VIOLATION
         * @see TypeEditor#WARN_ON_CONSTRAINT_VIOLATION
         */
        void onConstraintViolation(String path, List<String> nodeTypeNames, int code,
            String message) throws CommitFailedException;
    }

    public static final ConstraintViolationCallback THROW_ON_CONSTRAINT_VIOLATION = new ConstraintViolationCallback() {

        @Override
        public void onConstraintViolation(String path, List<String> nodeTypeNames, int code,
            String message) throws CommitFailedException {
    ...
    public static final ConstraintViolationCallback WARN_ON_CONSTRAINT_VIOLATION = new ConstraintViolationCallback() {
        ...
        public void onConstraintViolation(String path, List<String> nodeTypeNames, int code,
            ...
            log.warn(new CommitFailedException(CONSTRAINT, code,
                fullPath + ": " + message).getMessage());
     ...
     *                     check the results of the callback invocations if the specified callback
     *                     does not immediately propagate constraint violations as checked
     *                     exceptions.
     * @param typesToCheck the types to check for. If {@code null}, this node is checked. Otherwise
     *                     it is checked if its primary type or one of it's mixin types is contained
     *                     in this parameters
     * @param types        the {@code /jcr:system/jcr:nodeTypes} node
     * @param primary      the node's primary type
     * @param mixins       the node's mixins
     * @param builder      a builder containing the current state of the node to check. May be used
     *                     to set a default primary type if none is set
     * @return a new TypeEditor instance
     * @throws CommitFailedException when the primary type of mixin definition is incorrect
     */
    public static TypeEditor create(@NotNull ConstraintViolationCallback callback,
    ...
    private void constraintViolation(int code, String message) throws CommitFailedException {
        List<String> nodeTypeNames =
            effective != null ? effective.getDirectTypeNames() : Collections.emptyList();
            ...
            if (type.getBoolean(JCR_IS_ABSTRACT)) {
                if (parent != null && name != null && primary.equals(parent.getDefaultType(name))) {
                    ...
                    constraintViolation(2,
                        "Abstract type " + primary + " used as the primary type");
                }
            }
            list.add(type);
        }
    ...
    private void checkValueConstraints(NodeState definition, PropertyState property,
        int requiredType) throws CommitFailedException {
        if (property.count() == 0) {
            return;
        }
    ...
    private void checkNodeTypeConstraints(NodeState after) throws CommitFailedException {
        ...
        if (!properties.isEmpty()) {
            constraintViolation(21, "Mandatory property '" + properties.iterator().next()
        ...
        for (String child : effective.getMandatoryChildNodes()) {
            if (!names.remove(child)) {
                constraintViolation(25,
                    "Mandatory child node '" + child + "' not found in a new node");
            }
        }
        ...
        for (String name : names) {
            if (!NodeStateUtils.isHidden(name)) {
                ...
                if (!effective.isValidChildNode(name, editor.getEffective())) {
                    constraintViolation(25,
                        "Unexpected child node '" + name + "' of effective type '"
                            + editor.getEffective()
                            + "' found in a new node");
                }
            }
