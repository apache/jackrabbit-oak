...
/**
 * {@code ReadOnlyVersionManager} provides implementations for read-only version operations modeled
 * after the ones available in {@link javax.jcr.version.VersionManager}.
 */
public abstract class ReadOnlyVersionManager {
    ...
    /**
     * @return the read-only {@link Tree} for the jcr:versionStorage node. The returned {@code Tree}
     * instance must be up-to-date with the {@code Root} returned by {@link #getWorkspaceRoot()}.
     */
    @NotNull
    ...
    /**
     * Return a new instance of {@code ReadOnlyVersionManager} that reads version information from
     * the tree at {@link VersionConstants#VERSION_STORE_PATH}.
     *
    ...
    /**
     * Returns {@code true} if the tree is checked out; otherwise {@code false}. The root node is
     * always considered checked out.
     *
    ...
    /**
     * Returns the tree representing the version history of the given versionable tree or
     ...
     * @throws UnsupportedRepositoryOperationException if the versionable tree is not actually
     *                                                 versionable.
    ...
    /**
     * Returns the path of the version history for the given {@code uuid}. The returned path is
     * relative to the version storage tree as returned by {@link #getVersionStorage()}.
     *
    ...
    /**
     * Returns the tree representing the base version of the given versionable tree or {@code null}
     * if none exists yet. This is the case when a versionable node is created, but is not yet
     * saved.
     *
     * @param versionable the versionable tree.
     ...
     * defines as primary node type i.e. is part of the intermediate version storage structure that
     * contains the version histories and the versions.
     *
    ...
    /**
     * Tries to retrieve the tree corresponding to specified {@code versionTree} outside of the
     * version storage based on versionable path information stored with the version history. The
     ...
     *
     * @param versionTree   The tree from within the version storage for which that versionable
     *                      correspondent should be retrieved.
     * @param workspaceName The name of the workspace for which the target should be retrieved.
     * @return A existing or non-existing tree pointing to the location of the correspondent tree
     * outside of the version storage or {@code null} if the versionable path property for the
     * specified workspace is missing or if the given tree is not located within the tree structure
        ...
        while (t.exists() && !isVersionStoreTree(t) && !t.isRoot()) {
            ...
            if (VersionConstants.NT_FROZENNODE.equals(ntName)) {
                ...
                if (prop != null) {
                    ...
                    log.warn("Missing versionable path property for {} at {}", workspaceName,
                        t.getPath());
                    break;
                }
            }
    ...
    /**
     * @return an identifier manager that is able to resolve identifiers of nodes in the version
     * storage.
     */
    ...
    /**
     * Checks if the given {@code tree} is versionable and throws a
     * {@link UnsupportedRepositoryOperationException} if it is not.
     *
     * @param tree the tree to check.
     * @return the passed tree.
     * @throws UnsupportedRepositoryOperationException if the tree is not versionable.
     * @throws RepositoryException                     if an error occurs while checking the node
     *                                                 type of the tree.
     */
    @NotNull
    ...
    /**
     * Returns {@code true} if the given {@code tree} is of type {@code mix:versionable};
