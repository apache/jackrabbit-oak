...
/**
 * Default implementation of the {@code TokenProvider} interface that keeps login tokens in the
 * content repository. As a precondition the configured the user management implementation must
 * provide paths for all {@link org.apache.jackrabbit.api.security.user.User users} that refer to a
 * valid {@link Tree} in the content repository.
 * <p>
 * <h3>Backwards compatibility with Jackrabbit 2.x</h3>
 * For security reasons the nodes storing the token information now have a dedicated node type
 * (rep:Token) which has the following definition:
 * <pre>
 ...
 * </pre>
 * Consequently the hash of the token and the expiration time of tokens generated by this provider
 * can no longer be manipulated using regular JCR item modifications.<p>
 * <p>
 * Existing login tokens generated by Jackrabbit 2.x which are migrated to OAK will still be valid
 * (unless they expire) due to the fact that {@link #getTokenInfo(String)} and the implementation of
 * the {@link TokenInfo} interface will not validate the node type of the token node associated with
 * a given token.
 */
class TokenProviderImpl implements TokenProvider, TokenConstants {
    ...
    /**
     * Optional configuration parameter to define the number of token nodes that when exceeded will
     * trigger a cleanup of expired tokens upon creation.
     */
    static final String PARAM_TOKEN_CLEANUP_THRESHOLD = "tokenCleanupThreshold";

    /**
     * Default value indicating that tokens should never be cleaned up (i.e. backwards compatible
     * behavior).
     */
    ...
    TokenProviderImpl(@NotNull Root root, @NotNull ConfigurationParameters options,
    ...
    TokenProviderImpl(@NotNull Root root, @NotNull ConfigurationParameters options,
        ...
        this.cleanupThreshold = options.getConfigValue(PARAM_TOKEN_CLEANUP_THRESHOLD,
            NO_TOKEN_CLEANUP);
    }
    ...
    /**
     * Returns {@code true} if {@code SimpleCredentials} can be extracted from the specified
     * credentials object and that simple credentials object has a {@link #TOKEN_ATTRIBUTE}
     * attribute with an {@link #TOKEN_ATTRIBUTE_DO_CREATE empty value}.
     *
     * @param credentials The current credentials.
     * @return {@code true} if the specified credentials or those extracted from
     * {@link ImpersonationCredentials} are supported and if the (extracted) credentials object
     * contain a {@link #TOKEN_ATTRIBUTE} attribute with an empty value; {@code false} otherwise.
     */
    @Override
     ...
     * That token node contains the hashed token, the expiration time and additional mandatory
     * attributes that will be verified during login.
     *
     * @param credentials The current credentials.
    ...
    public TokenInfo createToken(@NotNull Credentials credentials) {
        ...
        if (uid != null) {
            ...
            if (tokenInfo != null) {
                ...
                if (!credentialsSupport.setAttributes(creds,
                    ImmutableMap.of(TOKEN_ATTRIBUTE, tokenInfo.getToken()))) {
        ...
        if (tokenParent != null) {
            try {
                ...
                try {
                    ...
                    tokenInfo = createTokenNode(tokenParent, tokenName, expTime, uuid, id,
                        attributes);
                    ...
                    root.refresh();
                    tokenInfo = createTokenNode(tokenParent, UUID.randomUUID().toString(), expTime,
                        uuid, id, attributes);
                    root.commit(CommitMarker.asCommitAttributes());
                }
                ...
                return tokenInfo;
                     ...
                     CommitFailedException | RepositoryException e) {
    ...
    /**
     * Retrieves the token information associated with the specified login token. If no accessible
     * {@code Tree} exists for the given token or if the token is not associated with a valid user
     * this method returns {@code null}.
     *
     * @param token A valid login token.
     * @return The {@code TokenInfo} associated with the specified token or {@code null} of the
     * corresponding information does not exist or is not associated with a valid user.
     */
    @Nullable
    @Override
    ...
    /**
     * Remove expired token nodes if the configured threshold (i.e. number of token nodes) is
     * matched/exceeded. By default (i.e. unless configured with a value bigger than
     * {@link #NO_TOKEN_CLEANUP}) no cleanup is performed and this method returns without looking at
     * the token nodes; this makes this addition optional and will not affect existing
     * configurations.
     *
     * @param parent      The token parent node.
     * @param currentTime The time to be used for analysing expiration of existing tokens.
     * @param token       The token info used as random data to skip cleanup.
     */
    private void cleanupExpired(@NotNull String userId, @NotNull Tree parent, long currentTime,
    ...
    /**
     * Method that determines if the cleanup should run or not based on the randomly generated
     * token's first char, this decreases the chances to 1/8.
     *
    ...
    final class TokenInfoImpl implements TokenInfo {
        ...
        /**
         * Returns {@code true} if the specified {@code attributeName} starts with or equals
         * {@link #TOKEN_ATTRIBUTE}.
         *
         * @param attributeName The attribute name.
         * @return {@code true} if the specified {@code attributeName} starts with or equals
         * {@link #TOKEN_ATTRIBUTE}.
         */
        ...
        /**
         * Returns {@code false} if the specified attribute name doesn't have a 'jcr' or 'rep'
         * namespace prefix; {@code true} otherwise. This is a lazy evaluation in order to avoid
         * testing the defining node type of the associated jcr property.
         *
         * @param attributeName The attribute name.
         * @return {@code true} if the specified property name doesn't seem to represent repository
