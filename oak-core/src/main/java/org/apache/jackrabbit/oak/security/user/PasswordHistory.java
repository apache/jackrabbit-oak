...
final class PasswordHistory implements UserConstants {
    ...
    /**
     * If password history is enabled this method validates the new password and updated the
     * history; otherwise it returns {@code false}.
     *
     * @param userTree The user tree.
     * @param password The new password to be validated.
     * @return {@code true} if the history is enabled, the new password is not included in the
     * history and the history was successfully updated; {@code false} otherwise.
     * @throws javax.jcr.nodetype.ConstraintViolationException If the feature is enabled and the new
     *                                                         password is found in the history.
     * @throws javax.jcr.AccessDeniedException                 If the rep:pwd tree cannot be
     *                                                         accessed.
     */
    boolean updatePasswordHistory(@NotNull Tree userTree, @NotNull String password)
    ...
    /**
     * Update the history property with the current pw-hash stored in rep:password and trim the list
     * of hashes in the list according to the configured maxSize.
     *
     * @param userTree The user tree.
     * @return true if the history was successfully adjusted, false otherwise
     * @throws AccessDeniedException If the editing session cannot access or create the rep:pwd
     *                               node.
     */
    private boolean shiftPasswordHistory(@NotNull Tree userTree) throws AccessDeniedException {
        ...
        if (currentPasswordHash != null) {
            ...
            List<String> historyEntries = (historyProp == null) ? new ArrayList<>()
                : Lists.newArrayList(historyProp.getValue(Type.STRINGS));
     ...
     */
    private void checkPasswordInHistory(@NotNull Tree userTree, @NotNull String newPassword)
        throws ConstraintViolationException, AccessDeniedException {
        if (PasswordUtil.isSame(TreeUtil.getString(userTree, UserConstants.REP_PASSWORD),
            newPassword)) {
        ...
        if (pwTree.exists()) {
            ...
            if (pwHistoryProperty != null) {
                for (String historyPwHash : Iterables.limit(
                    pwHistoryProperty.getValue(Type.STRINGS), maxSize)) {
    ...
    @NotNull
    private static Tree getPasswordTree(@NotNull Tree userTree, boolean doCreate)
