 ...
 * partial subtrees from the consumer of the API.
 * <br>
 * The set of visible subtrees is defined by two parameters: include paths and exclude paths, both
 * of which are sets of absolute paths.
 * <br>
 * Any paths that are equal or are descendants of one of the
 * <b>excluded paths</b> are hidden by this implementation.
 * <br>
 * For all <b>included paths</b>, the direct ancestors, the node-state at the path itself and all
 * descendants are visible. Any siblings of the defined path or its ancestors are implicitly hidden
 * (unless made visible by another include path).
 * <br>
 * The implementation delegates to the decorated node-state instance and filters out hidden
 * node-states in the following methods:
 * <ul>
 *     <li>{@link #exists()}</li>
...
public class FilteringNodeState extends AbstractDecoratedNodeState {
    ...
    /**
     * Factory method that conditionally decorates the given node-state iff the node-state is (a)
     * hidden itself or (b) has hidden descendants.
     *
     * @param path                     The path where the node-state should be assumed to be
     *                                 located.
     * @param delegate                 The node-state to decorate.
     * @param includePaths             A Set of paths that should be visible. Defaults to ["/"] if
     *                                 {@code null}.
     * @param excludePaths             A Set of paths that should be hidden. Empty if {@code null}.
     * @param fragmentPaths            A Set of paths that should support the fragments (see below).
     *                                 Empty if {@code null}.
     * @param excludedFragments        A Set of name fragments that should be hidden. Empty if
     *                                 {@code null}.
     * @param referenceableFrozenNodes Whether nt:frozenNode are referenceable on the target.
     * @return The decorated node-state if required, the original node-state if decoration is
     * unnecessary.
     */
    @NotNull
    public static NodeState wrap(
        ...
        if (!referenceableFrozenNodes || hasHiddenDescendants(path, includes, excludes,
            safeFragmentPaths, safeExcludedFragments)) {
            return new FilteringNodeState(path, delegate, includes, excludes, fragmentPaths,
                safeExcludedFragments, referenceableFrozenNodes);
        }
        return delegate;
    }
    ...
    protected NodeState decorateChild(@NotNull final String name, @NotNull final NodeState child) {
        ...
        return wrap(childPath, child, includedPaths, excludedPaths, fragmentPaths,
            excludedFragments, referenceableFrozenNodes);
    }
    ...
    /**
     * Returns {@code true} if the given property state is the {@code jcr:uuid} property of a
     * {@code nt:frozenNode}; {@code false} otherwise.
     *
     * @param propertyState the property to check.
     * @return whether the given property state is the {@code jcr:uuid} property of a
     * {@code nt:frozenNode}.
     */
    ...
    /**
     * @return {@code true} if this node state has a nt:frozenNode; {@code false} otherwise.
     */
    ...
    /**
     * Utility method to determine whether a given path should is hidden given the include paths and
     * exclude paths.
     *
    ...
    /**
     * Utility method to determine whether the path itself or any of its descendants should be
     * hidden given the include paths and exclude paths.
     *
     * @param path              Path to be checked
    ...
    /**
     * Utility method to check whether a given set of include paths cover the given {@code path}.
     * I.e. whether the path is visible or implicitly hidden due to the lack of a matching include
     * path.
     * <br>
     ...
     */
    private static boolean isIncluded(@NotNull final String path,
        @NotNull final Set<String> includePaths) {
    ...
    /**
     * Utility method to check whether a given set of exclude paths cover the given {@code path}.
     * I.e. whether the path is hidden due to the presence of a matching exclude path.
     *
     * @param path              Path to be checked
     * @param excludePaths      Exclude paths
     * @param excludedFragments Exclude fragments
     ...
     */
    private static boolean isExcluded(@NotNull final String path,
        @NotNull final Set<String> excludePaths, @NotNull final Set<String> excludedFragments) {
        return excludePaths.contains(path) || isDescendantOfAnyPath(path, excludePaths)
            || containsAnyFragment(path, excludedFragments);
    }

    /**
     * Utility method to check whether any of the provided {@code paths} is a descendant of the
     * given ancestor path.
     *
     ...
     */
    private static boolean isAncestorOfAnyPath(@NotNull final String ancestor,
        @NotNull final Set<String> paths) {
    ...
    /**
     * Utility method to check whether any of the provided {@code paths} is an ancestor of the given
     * descendant path.
     *
     ...
     */
    private static boolean isDescendantOfAnyPath(@NotNull final String descendant,
        @NotNull final Set<String> paths) {
        for (final String p : paths) {
    ...
    /**
     * Utility method to check whether the passed path contains any of the provided
     * {@code fragments}.
     *
     * @param path      Path
     ...
     */
    private static boolean containsAnyFragment(@NotNull final String path,
        @NotNull final Set<String> fragments) {
    ...
    /**
     * Utility method to return the given {@code Set} if it is not empty and a default Set
     * otherwise.
     *
