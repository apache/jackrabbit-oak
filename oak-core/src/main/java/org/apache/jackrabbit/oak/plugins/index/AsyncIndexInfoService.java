...
public interface AsyncIndexInfoService {
    ...
    /**
     * Returns all the async indexing lanes which are active in the setup based on given root
     * NodeState
     *
     * @param root root NodeState from which async index state is read
     */
    ...
    /**
     * Determines if any index lane has completed any indexing cycle between given two repository
     * states
     *
     * @param before before state of root node
     * @param after  after state of root node
     * @return true if any indexing lane has completed any indexing cycle i.e. its lastIndexTo time
     * has changed
     */
