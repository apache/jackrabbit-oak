...
public final class EveryoneFilter {
    ...
    /**
     * Inject the everyone principal in the result if the given query (defined by name hint and
     * search type) matched everyone and the search expects the complete set to be return (i.e. no
     * offset and no limit defined).
     *
     ...
     * @return The principal iterator with additionally {@link EveryonePrincipal} attached if the
     * given query matches the everyone principal name or search-type and doesn't specify a range
     * (offset or limit). If the query doesn't match everyone or if a range was defined the original
     * iterator is returned unmodified.
     */
    public static Iterator<Principal> filter(@NotNull Iterator<Principal> resultPrincipals,
        @Nullable String nameHint, int searchType, long offset, long limit) {
        boolean noRange = offset == 0 && limit == Long.MAX_VALUE;
        if (noRange && matchesEveryone(nameHint, searchType)) {
            Iterator<Principal> principals = Iterators.concat(resultPrincipals,
    ...
    /**
     * Predicate to make sure the everyone principal is only included once in the result set.
     */
