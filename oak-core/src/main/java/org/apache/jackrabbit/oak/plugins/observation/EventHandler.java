...
/**
 * Handler of content change events. Used to decouple processing of changes from the content diff
 * logic that detects them.
 * <p>
 * As the content diff recurses down the content tree, it will call the
 * {@link #getChildHandler(String, NodeState, NodeState)} method to specialize the handler instance
 * for each node under which changes are detected. The other handler methods always apply to the
 * properties and direct children of the node for which that handler instance is specialized. The
 * handler is expected to keep track of contextual information like the path or identifier of the
 * current node based on the sequence of those specialization calls.
 * <p>
 * The events belonging to this instance <em>should</em> be delivered before events to other
 * instance deeper down the tree are delivered.
 * <p>
...
public interface EventHandler {
     ...
     * method to post-process information collected during the content diff.
     *
     * @param before before state, non-existent if this node was added
     * @param after  after state, non-existent if this node was removed
     */
    ...
    /**
     * Returns a handler of events within the given child node, or {@code null} if changes within
     * that child are not to be processed.
     *
