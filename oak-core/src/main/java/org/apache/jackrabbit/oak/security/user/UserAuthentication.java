...
/**
 * Implementation of the Authentication interface that validates credentials against user
 * information stored in the repository. If no user exists with the specified userID or if the user
 * has been disabled authentication will will fail irrespective of the specified credentials.
 * Otherwise the following validation is performed:
 *
 * <ul>
...
class UserAuthentication implements Authentication, UserConstants {
        ...
        try {
            ...
            if (credentials instanceof SimpleCredentials) {
                ...
                if (loginId.equals(creds.getUserID()) && userCreds instanceof CredentialsImpl) {
                    success = PasswordUtil.isSame(((CredentialsImpl) userCreds).getPasswordHash(),
                        creds.getPassword());
                }
                ...
                success = (credentials instanceof GuestCredentials)
                    || credentials == PreAuthenticatedLogin.PRE_AUTHENTICATED;
            }
            userId = user.getID();
    ...
    public Principal getUserPrincipal() {
        if (principal == null) {
            throw new IllegalStateException(
                "Principal can only be retrieved after successful authentication.");
        }
        return principal;
    }
    ...
    //--------------------------------------------------------------------------
    @Nullable
    private static User getValidUser(@NotNull UserManager userManager, @NotNull String loginId)
        throws RepositoryException, AccountNotFoundException, AccountLockedException {
        ...
        if (user.isDisabled()) {
            throw new AccountLockedException(
                "User with ID " + loginId + " has been disabled: " + user.getDisabledReason());
        }
        return user;
    }
    ...
    private static boolean equalUserId(@NotNull ImpersonationCredentials creds,
        @NotNull String userId) {
        Credentials base = creds.getBaseCredentials();
        return (base instanceof SimpleCredentials) && userId.equals(
            ((SimpleCredentials) base).getUserID());
    }
        ...
        try {
            ...
            if (newPasswordObject != null) {
                if (newPasswordObject instanceof String) {
                    ...
                    log.warn(
                        "Aborted password change for user {}: provided new password is of incompatible type {}",
                        loginId, newPasswordObject.getClass().getName());
                }
            }
    ...
    private boolean isPasswordExpired(@NotNull User user) throws RepositoryException {
        ...
        boolean forceInitialPwChange = params.getConfigValue(PARAM_PASSWORD_INITIAL_CHANGE,
        ...
        if (maxAge > 0) {
            ...
            if (passwordLastModified == null) {
                ...
                long expiryTime =
                    passwordLastModified + TimeUnit.MILLISECONDS.convert(maxAge, TimeUnit.DAYS);
