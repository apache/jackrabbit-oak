...
public class UserImporterTest extends UserImporterBaseTest implements UserConstants {
    ...
    public void testInitGetUserManagerFails() throws Exception {
        Session s = when(mock(JackrabbitSession.class).getUserManager()).thenThrow(
    ...
    public void testInitAutosaveEnabledUserManager() throws Exception {
        ...
        UserConfiguration uc = when(
            mock(UserConfiguration.class).getUserManager(root, getNamePathMapper())).thenReturn(
            umgr).getMock();
        when(sp.getConfiguration(UserConfiguration.class)).thenReturn(uc);

        assertEquals(!isAutosave(),
            importer.init(mockJackrabbitSession(), root, getNamePathMapper(), false,
                ImportUUIDBehavior.IMPORT_UUID_COLLISION_REMOVE_EXISTING,
                new ReferenceChangeTracker(), sp));
    }
    ...
    public void testInitInvalidUserManager() throws Exception {
        ...
        assertFalse(importer.init(mockJackrabbitSession(), root, getNamePathMapper(), false,
    ...
    public void testInitAlreadyInitialized() throws Exception {
        assertTrue(init());
        importer.init(mockJackrabbitSession(), root, getNamePathMapper(), isWorkspaceImport(),
            ImportUUIDBehavior.IMPORT_UUID_COLLISION_REMOVE_EXISTING, new ReferenceChangeTracker(),
            getSecurityProvider());
    }
    ...
    public void testHandlePropInfoNotInitialized() throws Exception {
        importer.handlePropInfo(createUserTree(), mock(PropInfo.class),
    ...
    public void testHandlePropInfoParentNotAuthorizable() throws Exception {
        assertTrue(init());
        assertFalse(importer.handlePropInfo(root.getTree(PathUtils.ROOT_PATH), mock(PropInfo.class),
            mock(PropertyDefinition.class)));
    }
    ...
    public void testHandleAuthorizableId() throws Exception {
        ...
        assertEquals(userTree.getPath(),
            getUserManager(root).getAuthorizable(TEST_USER_ID).getPath());
    }
    ...
    public void testHandleAuthorizableIdMismatch() throws Exception {
        assertTrue(init());
        Tree userTree = createUserTree();
        importer.handlePropInfo(userTree, createPropInfo(REP_AUTHORIZABLE_ID, "mismatch"),
            mockPropertyDefinition(NT_REP_AUTHORIZABLE, false));
    }
    ...
    public void testHandleAuthorizableIdConflictExisting() throws Exception {
        assertTrue(init());
        Tree userTree = createUserTree();
        importer.handlePropInfo(userTree, createPropInfo(REP_AUTHORIZABLE_ID, testUser.getID()),
            mockPropertyDefinition(NT_REP_AUTHORIZABLE, false));
    }
    ...
    public void testHandlePrincipalName() throws Exception {
        ...
        Tree userTree = createUserTree();
        assertTrue(
            importer.handlePropInfo(userTree, createPropInfo(REP_PRINCIPAL_NAME, "principalName"),
                mockPropertyDefinition(NT_REP_AUTHORIZABLE, false)));
    ...
    public void testHandleEveryonePrincipalNameOnUser() throws Exception {
        assertTrue(init());
        Tree userTree = createUserTree();
        importer.handlePropInfo(userTree,
            createPropInfo(REP_PRINCIPAL_NAME, EveryonePrincipal.NAME),
            mockPropertyDefinition(NT_REP_AUTHORIZABLE, false));
    }
    ...
    public void testHandlePassword() throws Exception {
        ...
        assertTrue(importer.handlePropInfo(userTree, createPropInfo(REP_PASSWORD, pwHash),
            mockPropertyDefinition(NT_REP_USER, false)));
    ...
    public void testHandlePwNodePropertyInvalidDef() throws Exception {
        ...
        assertFalse(importer.handlePropInfo(pwHistory,
            createPropInfo(null, PasswordUtil.buildPasswordHash("pw")), pd));
    }
    ...
    public void testHandlePwNodePropertyValidDef() throws Exception {
        ...
        assertTrue(importer.handlePropInfo(pwHistory,
            createPropInfo(null, PasswordUtil.buildPasswordHash("pw")), pd));
    }
    ...
    public void testHandleImpersonatorsOnGroup() throws Exception {
        ...
        Tree groupTree = createGroupTree();
        assertFalse(
            importer.handlePropInfo(groupTree, createPropInfo(REP_IMPERSONATORS, "impersonator1"),
                mockPropertyDefinition(NT_REP_USER, true)));
    }
    ...
    public void testHandleImpersonatorsSinglePropertyDef() throws Exception {
        ...
        Tree userTree = createUserTree();
        assertFalse(
            importer.handlePropInfo(userTree, createPropInfo(REP_IMPERSONATORS, "impersonator1"),
                mockPropertyDefinition(NT_REP_USER, false)));
        assertNull(userTree.getProperty(REP_IMPERSONATORS));
    }
    ...
    public void testHandleMembers() throws Exception {
        ...
        Tree groupTree = createGroupTree();
        assertTrue(
            importer.handlePropInfo(groupTree, createPropInfo(REP_MEMBERS, "member1", "member2"),
    ...
    public void testHandleMembersOnUser() throws Exception {
        init();
        Tree userTree = createUserTree();
        assertFalse(importer.handlePropInfo(userTree, createPropInfo(REP_MEMBERS, "member1"),
            mockPropertyDefinition(NT_REP_MEMBER_REFERENCES, true)));
    }
    ...
    public void testHandleMembersSinglePropertyDef() throws Exception {
        init();
        Tree groupTree = createGroupTree();
        assertFalse(importer.handlePropInfo(groupTree, createPropInfo(REP_MEMBERS, "member1"),
    ...
    public void testHandleDisabled() throws Exception {
        init();
        Tree userTree = createUserTree();
        assertTrue(importer.handlePropInfo(userTree, createPropInfo(REP_DISABLED, "disabled"),
            mockPropertyDefinition(NT_REP_USER, false)));
    ...
    public void testHandleDisabledOnGroup() throws Exception {
        init();
        Tree groupTree = createGroupTree();
        assertFalse(importer.handlePropInfo(groupTree, createPropInfo(REP_DISABLED, "disabled"),
            mockPropertyDefinition(NT_REP_USER, false)));
    ...
    public void testHandleDisabledMvProperty() throws Exception {
        init();
        Tree userTree = createUserTree();
        importer.handlePropInfo(userTree, createPropInfo(REP_DISABLED, "disabled", "disabled"),
            mockPropertyDefinition(NT_REP_USER, false));
    }
    ...
    public void testHandleDisabledMvPropertyDef() throws Exception {
        init();
        Tree userTree = createUserTree();
        assertFalse(importer.handlePropInfo(userTree, createPropInfo(REP_DISABLED, "disabled"),
            mockPropertyDefinition(NT_REP_USER, true)));
    ...
    public void testHandleUnknownProperty() throws Exception {
        init();
        Tree userTree = createUserTree();
        assertFalse(importer.handlePropInfo(userTree, createPropInfo("unknownProperty", "value"),
            mockPropertyDefinition(NT_OAK_UNSTRUCTURED, false)));
    ...
    public void testPropertiesCompletedNewUser() throws Exception {
        ...
        verify(testAction, times(1)).onCreate(any(User.class), nullable(String.class),
    ...
    public void testPropertiesCompletedNewSystemUser() throws Exception {
        ...
        verify(testAction, never()).onCreate(any(User.class), nullable(String.class),
            any(Root.class), any(NamePathMapper.class));
    }
    ...
    public void testPropertiesCompletedNewGroup() throws Exception {
        ...
        verify(testAction, times(1)).onCreate(any(Group.class), any(Root.class),
    ...
    public void testStartRepMembersBelowRootTree() throws Exception {
        init(true);
        Tree repMembers = TreeUtil.addChild(root.getTree(PathUtils.ROOT_PATH), "memberTree",
            NT_REP_MEMBERS);
    ...
    public void testStartRepMembersBelowAnyTree() throws Exception {
        init(true);
        Tree anyTree = TreeUtil.addChild(root.getTree(PathUtils.ROOT_PATH), "anyTree",
            NT_OAK_UNSTRUCTURED);
    ...
    public void testStartChildInfoWithOtherProperty() throws Exception {
        ...
        importer.start(memberRefList);
        importer.startChildInfo(createNodeInfo("memberRef", NT_REP_MEMBER_REFERENCES),
            ImmutableList.of(createPropInfo("otherName", "member1")));
        importer.processReferences();
    ...
    public void testStartRepMembersChildInfo() throws Exception {
        ...
        importer.startChildInfo(createNodeInfo("memberTree", NT_REP_MEMBERS),
            ImmutableList.of(createPropInfo("anyProp", "memberValue")));
    }
    ...
    public void testStartOtherChildInfo() throws Exception {
        ...
        assertTrue(importer.start(memberRefList));
        importer.startChildInfo(createNodeInfo("memberRef", NT_OAK_UNSTRUCTURED),
            ImmutableList.of(createPropInfo(REP_MEMBERS, "member1")));
    }
    ...
    public void testEndChildInfoIsNoop() throws Exception {
        ...
        SecurityProvider sp = mock(SecurityProvider.class);
        UserConfiguration uc = when(
            mock(UserConfiguration.class).getUserManager(r, npmapper)).thenReturn(
            getUserManager(root)).getMock();
        when(sp.getConfiguration(UserConfiguration.class)).thenReturn(uc);

        importer.init(mockJackrabbitSession(), r, npmapper, false,
            ImportUUIDBehavior.IMPORT_UUID_COLLISION_REMOVE_EXISTING, reftracker, sp);
