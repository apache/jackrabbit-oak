...
public class PrefilteringBackgroundObserverTest {
    ...
    private CommitInfo includingCommitInfo = new CommitInfo("includingSession",
        CommitInfo.OAK_UNKNOWN);
    private CommitInfo excludingCommitInfo = new CommitInfo("excludingSession",
        CommitInfo.OAK_UNKNOWN);
    ...
    class TestPattern {
        ...
        TestPattern(boolean excluded, int numEvents, boolean flush, int expectedNumEvents,
            int expectedNumResetCalls) {
            this.flush = flush;
        ...
        TestPattern(boolean excluded, int numEvents, boolean flush, int expectedNumEvents,
            int expectedNumResetCalls, int expectedQueueSizeAtStart, int expectedQueueSizeAtEnd) {
        ...
        public String toString() {
            return "excluded=" + excluded + ", numEvents=" + numEvents + ", flush=" + flush
                + ", expectedNumEvents=" + expectedNumEvents + ", expectedNumResetCalls="
                + expectedNumResetCalls;
        }
    }
        ...
        for (TestPattern testPattern : testPatterns) {
            ...
            if (testPattern.expectedQueueSizeAtStart >= 0) {
                assertEquals("loopCnt=" + loopCnt + ", queue size mis-match at start",
                    testPattern.expectedQueueSizeAtStart,
                    filteringObserver.getBackgroundObserver().getMBean().getQueueSize());
            }
            ...
            if (testPattern.flush) {
                executeRunnables(runnableQ, testPattern.numEvents + testPattern.expectedNumEvents
                    + testPattern.expectedNumResetCalls + 10);
            }
            ...
            if (testPattern.expectedQueueSizeAtEnd >= 0) {
                assertEquals("loopCnt=" + loopCnt + ", queue size mis-match at end",
                    testPattern.expectedQueueSizeAtEnd,
                    filteringObserver.getBackgroundObserver().getMBean().getQueueSize());
            }
        }
    }
