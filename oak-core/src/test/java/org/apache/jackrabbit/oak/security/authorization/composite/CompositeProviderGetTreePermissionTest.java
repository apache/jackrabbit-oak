...
public class CompositeProviderGetTreePermissionTest extends AbstractSecurityTest {
    ...
    private CompositePermissionProvider createProvider(
        ...
        return CompositePermissionProvider.create(root, ImmutableList.copyOf(providers),
    ...
    public void testEmptyProvidersRootTree() {
        ...
        assertSame(TreePermission.EMPTY,
            composite.getTreePermission(rootTree, TreeType.DEFAULT, TreePermission.EMPTY));
    }

    @Test
    public void testEmptyProvidersCompositeParentPermission() {
        TreePermission tp = mock(TreePermission.class);
        AggregatedPermissionProvider aggr = when(
            mock(AggregatedPermissionProvider.class).getTreePermission(any(Tree.class),
        ...
        AggregatedPermissionProvider[] providers = new AggregatedPermissionProvider[]{aggr,
            new FullScopeProvider(root)};
        CompositeTreePermission parentTp = (CompositeTreePermission) CompositeTreePermission.create(
            rootTree, getTreeProvider(), new TreeTypeProvider(Context.DEFAULT), providers, OR);
    ...
    public void testEmptyProvidersMockParentPermission() {
        ...
        Tree ntTree = getRootProvider().createReadOnlyRoot(root)
                                       .getTree(NodeTypeConstants.NODE_TYPES_PATH);
    ...
    public void testSingleProvider() {
        ...
        Tree rootTree = getRootProvider().createReadOnlyRoot(root).getTree(PathUtils.ROOT_PATH);
        TreePermission rootTreePermission = composite.getTreePermission(rootTree, TreeType.HIDDEN,
        ...
        Tree systemTree = rootTree.getChild(JcrConstants.JCR_SYSTEM);
        assertSame(tp,
            composite.getTreePermission(systemTree, TreeType.INTERNAL, rootTreePermission));
    ...
    public void testTwoProvider() {
        ...
        AggregatedPermissionProvider composite = createProvider(OR, aggr,
            new FullScopeProvider(root));

        Tree rootTree = getRootProvider().createReadOnlyRoot(root).getTree(PathUtils.ROOT_PATH);
        TreePermission rootTreePermission = composite.getTreePermission(rootTree, TreeType.VERSION,
            TreePermission.EMPTY);
        ...
        Tree systemTree = rootTree.getChild(JcrConstants.JCR_SYSTEM);
        assertTrue(composite.getTreePermission(systemTree, TreeType.ACCESS_CONTROL,
            rootTreePermission) instanceof CompositeTreePermission);
