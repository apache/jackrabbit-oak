    ...
    public static void main(String[] args) throws Exception {
        ...
        RepositoryFixture[] allFixtures = new RepositoryFixture[]{
            ...
            OakRepositoryFixture.getMongoWithDS(uri,
                ...
                benchmarkOptions.isThrottlingEnabled().value(options)),
            OakRepositoryFixture.getMongoNS(uri,
                benchmarkOptions.getDropDBAfterTest().value(options),
                cacheSize * MB, benchmarkOptions.isThrottlingEnabled().value(options)),
            OakRepositoryFixture.getSegmentTar(benchmarkOptions.getBase().value(options), 256,
                cacheSize,
            ...
            OakRepositoryFixture.getSegmentTarWithDataStore(
                benchmarkOptions.getBase().value(options), 256, cacheSize,
                benchmarkOptions.getMmap().value(options),
                benchmarkOptions.getBinariesInlineThreshold().value(options),
                benchmarkOptions.getFdsCache().value(options)),
            ...
            OakRepositoryFixture.getRDB(benchmarkOptions.getRdbjdbcuri().value(options),
                ...
                benchmarkOptions.getRdbjdbcpasswd().value(options),
            ...
            OakRepositoryFixture.getRDBWithDS(benchmarkOptions.getRdbjdbcuri().value(options),
                ...
                benchmarkOptions.getFdsCache().value(options),
                benchmarkOptions.getVgcMaxAge().value(options)),
            OakRepositoryFixture.getCompositeStore(benchmarkOptions.getBase().value(options), 256,
                cacheSize,
                benchmarkOptions.getMmap().value(options),
                benchmarkOptions.getBinariesInlineThreshold().value(options)),
        ...
        addToBenchMarkList(Arrays.asList(
                ...
                GetParentNodeTest.withNodeAPIAndParentVisible(
                    benchmarkOptions.getRunAsAdmin().value(options)),
                GetParentNodeTest.withSessionAPIAndParentVisible(
                    benchmarkOptions.getRunAsAdmin().value(options)),
                GetParentNodeTest.withNodeAPIAndParentNotVisible(
                    benchmarkOptions.getRunAsAdmin().value(options)),
                GetParentNodeTest.withSessionAPIAndParentNotVisible(
                    benchmarkOptions.getRunAsAdmin().value(options)),
                ...
                new CompositeAuthorizationTest(
                    benchmarkOptions.getRunAsAdmin().value(options),
                    benchmarkOptions.getItemsToRead().value(options)),
                // NOTE: this is currently the no of configurations
                ...
                new SaveHasItemGetItemTest(
                    ...
                    benchmarkOptions.getNumberOfGroups().value(options),
                    benchmarkOptions.getReport().value(options)),
                new GetPrivilegeCollectionIncludeNamesTest(
                    benchmarkOptions.getItemsToRead().value(options),
                    benchmarkOptions.getNumberOfInitialAce().value(options),
                    benchmarkOptions.getNumberOfGroups().value(options),
                    benchmarkOptions.getReport().value(options),
                ...
                new ConcurrentEveryoneACLTest(benchmarkOptions.getRunAsAdmin().value(options),
                    benchmarkOptions.getItemsToRead().value(options)),
                new AceCreationTest(benchmarkOptions.getBatchSize().value(options),
                    benchmarkOptions.getNumberOfInitialAce().value(options),
                ...
                new FindAuthorizableWithScopeTest(benchmarkOptions.getNumberOfUsers().value(options),
                    benchmarkOptions.getNumberOfGroups().value(options),
                    benchmarkOptions.getQueryMaxCount().value(options),
                ...
                new AutoMembershipTest(benchmarkOptions.getNumberOfUsers().value(options),
                    benchmarkOptions.getNumberOfGroups().value(options),
                    benchmarkOptions.getDynamicMembership().value(options),
                    benchmarkOptions.getAutoMembership().values(options)),
        ...
        if (argset.isEmpty()) {
            ...
            if (options.has(benchmarkOptions.getCsvFile())) {
                out = new PrintStream(
                    FileUtils.openOutputStream(benchmarkOptions.getCsvFile().value(options), true));
            }
