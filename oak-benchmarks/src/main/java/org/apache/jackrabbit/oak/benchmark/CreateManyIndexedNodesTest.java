...
/**
 * Test for measuring the performance of creating many nodes which contain a property that is
 * indexed
 * <p>
 * This test does not want to replicate the many-child-nodes test, where all the child nodes are
 * created from the same parent. instead, this test spreads out the nodes over a number of parent.
 */
        ...
        public void run() {
            try {
                for (int i = 0; i < NODES_PER_THREAD; i++) {
                    ...
                    Node level1Node = session.getNode(
                        "/" + ROOT_NODE_NAME + "/testNode/level1_" + level1);
