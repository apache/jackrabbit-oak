    ...
    try {
        ...
        if (q != null && q.length() > 0) {
            ...
            if (q.startsWith("related:")) {
                ...
                stmt = "//element(*, nt:file)[rep:similar(jcr:content, '" + path
            ...
            if (total < 10 && !q.startsWith("related:")) {
                try {
                    RowIterator spellResultItr = jcrSession.getWorkspace().getQueryManager()
                                                           .createQuery(
                                                                   "/jcr:root[rep:spellcheck('" + q
                                                                           + "')]/(rep:spellcheck())",
...
<form name="gs" method="GET">
    <p>
        <input type="text" name="q" size="41" maxlength="2048"
               value="<%= Text.encodeIllegalXMLCharacters(q) %>" title="Search">
...
<p><font class="p" color="#cc0000">Did you mean:</font>
    <a href="search.jsp?q=<%= Text.encodeIllegalXMLCharacters(suggestedQuery) %>" class="p">
        <b><i><%= Text.encodeIllegalXMLCharacters(suggestedQuery) %>
        </i></b>
    </a><br>
</p>
<% } %>
<p/>Your search - <b><%= Text.encodeIllegalXMLCharacters(q) %>
...
<table border=0 cellpadding=0 cellspacing=0 width=100% class="t bt">
    <tr>
        <td><font size=-1>Results <b><%= from + 1 %>
        </b> - <b><%= to %>
        </b> of about <b><%= totalResults %>
        </b> <%= queryTerms %>. (<b><%= executedIn %>
        </b> seconds)&nbsp;</font></td>
    </tr>
</table>
<% if (suggestedQuery != null) { %>
...
<div>
    <%
        while (rows.hasNext() && rows.getPosition() < to) {
            ...
            String lastModified = df.format(
                    resource.getProperty("jcr:lastModified").getDate().getTime());
    %>
    <h6>
        <a href="<%= Text.encodeIllegalXMLCharacters(request.getContextPath() + "/repository/" + wspName + file.getPath()) %>"
           class=l><%= Text.encodeIllegalXMLCharacters(file.getName()) %>
        </a></h6>
    <table border=0 cellpadding=0 cellspacing=0>
        <tr>
            <td><font><%= r.getValue("rep:excerpt(jcr:content)").getString() %>
                <%= Text.encodeIllegalXMLCharacters(file.getPath()) %> - <%= size %>
                - <%= lastModified %> -
                <nobr><a
                        href="<%= Text.encodeIllegalXMLCharacters(request.getContextPath()) %>/search.jsp?q=related:<%= Text.encodeIllegalXMLCharacters(URLEncoder.encode(file.getPath(), "UTF-8")) %>">Similar
                    pages</a></nobr>
            </font></td>
        </tr>
    </table>
    ...
    <table border=0 cellpadding=0 cellspacing=0 align=center>
        ...
        if (currentPageIndex != (Long) indexes.get(0)) {
            %>
                <td nowrap align=right><a
                        href=search.jsp?q=<%= Text.encodeIllegalXMLCharacters(q) %>&start=<%=
        ...
        for (Iterator it = indexes.iterator(); it.hasNext(); ) {
            ...
            if (pageIdx == currentPageIndex) {
                ...
                %>
                        <td nowrap><font size=-1><a
                                href=search.jsp?q=<%= Text.encodeIllegalXMLCharacters(q) %>&start=<%=
                        pageIdx * 10 %>><%= pageIdx + 1 %>
                        </a><%
            }
        }
...
<table>
    <tr>
        <td><br>
            <form method=GET
                  action=<%= Text.encodeIllegalXMLCharacters(request.getContextPath()) %>/search.jsp>
                <font size=-1><input type=text name=q size=31 maxlength=2048
                                     value="<%= Text.encodeIllegalXMLCharacters(q) %>"
                                     title="Search"> <input type=submit value="Search">
                </font></form>
            <br><font size=-1>
                <a href="<%= Text.encodeIllegalXMLCharacters(request.getContextPath()) %>/swr.jsp?q=<%= Text.encodeIllegalXMLCharacters(q) %>&swrnum=<%= rows.getSize() %>">Search&nbsp;within&nbsp;results</a>
                | <a href="http://issues.apache.org/jira/browse/JCR" target=_blank>Dissatisfied?
                Help us improve</a></font><br>
            <br>
        </td>
    </tr>
</table>
