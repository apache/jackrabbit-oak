...
/**
 * Comparator for {@link ClusterNodeInfo} instances based on a given environment defined by a
 * {@code machineId} and {@code instanceId}. The comparator orders cluster node info with a matching
 * environment before others that do not match and then compares the clusterId, lower values first.
 */
class ClusterNodeInfoComparator implements Comparator<ClusterNodeInfo> {

    private static final Comparator<Boolean> BOOLEAN_REVERSED = Comparator.comparing(
        Boolean::booleanValue).reversed();
