...
@ObjectClassDefinition(
    ...
    description =
        "NodeStore implementation based on Document model. For configuration option refer " +
...
@interface Configuration {
    ...
    @AttributeDefinition(
        ...
        description = "Long value indicating interval (in milliseconds) with which the "
            + "journal (for external changes) is cleaned up. Default is "
            + DocumentNodeStoreService.DEFAULT_JOURNAL_GC_INTERVAL_MILLIS)
    ...
    @AttributeDefinition(
        ...
        description =
            "Version Garbage Collector (GC) logic will only consider those deleted for GC which " +
    ...
    @AttributeDefinition(
        ...
        description =
            "A Version GC run has a gap of this delay factor to reduce continuous load on system" +
    ...
    @AttributeDefinition(
        ...
        description =
            "Blob Garbage Collector (GC) logic will only consider those blobs for GC which " +
    ...
    @AttributeDefinition(
        ...
        description =
            "This is the default interval in which the snapshots of locally tracked blob ids will"
    ...
    @AttributeDefinition(
        ...
        description =
            "Value in seconds. Determines the duration beyond which it can be safely assumed " +
    ...
    @AttributeDefinition(
        ...
        description = "Minimal delay (in milliseconds) before a clusterId " +
            "can be reused after a recovery, 0 or negative for no delay. Default: "
