...
public class MongoDocumentStoreThrottlingMetricsUpdater implements Closeable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(
        MongoDocumentStoreThrottlingMetricsUpdater.class);
    ...
    public MongoDocumentStoreThrottlingMetricsUpdater(final @NotNull MongoDatabase localDb,
        final @NotNull AtomicDouble oplogWindow) {
    ...
    public void scheduleUpdateMetrics() {
        throttlingMetricsExecutor.scheduleAtFixedRate(() -> {
            ...
            if (!document.containsKey(MAX_SIZE) || !document.containsKey(SIZE)) {
                LOG.warn("Could not get stats for local.{}  collection. collstats returned: {}.",
                    OPLOG_RS, document);
                ...
                double usedSizeGb =
                    ceil(((double) usedSize / (1024 * 1024 * 1024)) * 1000000) / 1000000;
                MongoCollection<Document> localDbCollection = localDb.getCollection(OPLOG_RS);
                Document first = localDbCollection.find().sort(new Document(NATURAL, 1)).limit(1)
                                                  .first();
                Document last = localDbCollection.find().sort(new Document(NATURAL, -1)).limit(1)
                                                 .first();

                if (isNull(first) || isNull(last)) {
                    LOG.warn(
                        "Objects not found in local.oplog.rs -- is this a new and empty db instance?");
                    oplogWindow.set(MAX_VALUE);
                } else {
    ...
    // helper methods
    @VisibleForTesting
    static double updateOplogWindow(final double maxSize, final double usedSize,
