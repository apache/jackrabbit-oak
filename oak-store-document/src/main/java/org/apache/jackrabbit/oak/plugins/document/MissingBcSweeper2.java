...
/**
 * The {@code MissingBcSweeper2} is used for the so-called sweep2, which is a repository traversal
 * updating documents that have missing branch commit ("_bc") properties (details see OAK-9176).
 * This class is similar to NodeDocumentSweeper as it is based on the same principles, with a few
 * notable exceptions (like it only looks at _commitRoot and _revisions etc). And due to these
 * exceptions the class is forked rather than modified/subclasses (also to enable later refactoring
 * of the NodeDocumentSweeper itself).
 * <p>
...
final class MissingBcSweeper2 {
    ...
    /**
     * Performs a sweep2 and reports the required updates to the given sweep listener.
     *
    ...
    private UpdateOp sweepOne(NodeDocument doc) throws DocumentStoreException {
        ...
        for (String property : filter(doc.keySet(), COMMITROOT_OR_REVISIONS)) {
            ...
            for (Map.Entry<Revision, String> entry : valueMap.entrySet()) {
                ...
                if (!includedClusterIds.isEmpty() && !includedClusterIds.contains(
                    rev.getClusterId())) {
        ...
        if (lastElapsed >= LOGINTERVALMS) {
            ...
            String message = String.format(
                ...
                executingClusterId, restrictionMsg, totalCount,
                df.format(totalElapsed, TimeUnit.MILLISECONDS), totalRateMin, lastCount,
