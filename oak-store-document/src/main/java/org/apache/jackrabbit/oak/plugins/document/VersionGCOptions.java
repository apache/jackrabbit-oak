...
public class VersionGCOptions {
    ...
    /**
     * Set the limit of number of resource id+_modified strings (not length) held in memory during a
     * collection run. Any more will be stored and sorted in a temporary file.
     *
     ...
     * is attempted with different parameters, depending on other settings. Note that if the
     * inspected time interval is equal or less than {@link #precisionMs}, the collection limit will
     * be ignored.
     *
     ...
     * precision available on modification dates of documents, but can be set larger to avoid
     * querying the database too often. Note however that {@link #collectLimit} will not take effect
     * for runs that query equal or shorter than precision duration.
     *
    ...
    /**
     * Set the maximum number of iterations that shall be attempted in a single run. A value of 0
     * means that there is no limit. Since the garbage collector uses iterations to find suitable
     * time intervals and set sizes for cleanups, limiting the iterations is only recommended for
     * setups where the collector is called often.
     *
    ...
    /**
     * Set a delay factor between batched database modifications. This rate limits the writes to the
     * database by a garbage collector. 0, e.g. no delay, is the default. This is recommended when
     * garbage collection is done during a maintenance time when other system load is low.
     * <p>
     * For factory &gt; 0, the actual delay is the duration of the last batch modification times the
     * factor. Example: 0.25 would result in a 25% delay, e.g. a batch modification running 10
     * seconds would be followed by a sleep of 2.5 seconds.
     *
