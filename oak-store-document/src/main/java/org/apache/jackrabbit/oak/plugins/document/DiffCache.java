...
abstract class DiffCache {
    ...
    /**
     * Returns a jsop diff for the child nodes at the given path. The returned String may contain
     * the following changes on child nodes:
     * <ul>
    ...
    /**
     * Starts a new cache entry for the diff cache. Actual changes are added to the entry with the
     * {@link Entry#append(Path, String)} method.
     *
     * @param from  the from revision.
     * @param to    the to revision.
     * @param local true indicates that the entry results from a local change, false if it results
     ...
     * {@link #getChanges(RevisionVector, RevisionVector, Path, Loader)} and reports the changes by
     * calling the appropriate methods on {@link Diff}.
     *
     * @param jsop the jsop diff to parse.
     * @param diff the diff handler.
     * @return {@code true} it the complete jsop was processed or {@code false} if one of the
     * {@code diff} callbacks requested a stop.
    ...
    interface Entry {
        ...
        /**
         * Called when all changes have been appended and the entry is ready to be used by the
         * cache.
         *
         * @return {@code true} if the entry was successfully added to the cache, {@code false}
         * otherwise.
         */
        boolean done();
    }
