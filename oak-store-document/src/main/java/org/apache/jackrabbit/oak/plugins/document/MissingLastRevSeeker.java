...
public class MissingLastRevSeeker {
    ...
    /**
     * Get the candidates with modified time greater than or equal the specified {@code startTime}
    ...
    /**
     * Acquire a recovery lock for the given cluster node info document. This method may break a
     * lock when it determines the cluster node holding the recovery lock is no more active or its
     * lease expired.
     *
     * @param clusterId   id of the cluster that is going to be recovered
     ...
     * {@link ClusterNodeInfoDocument#isRecoveryNeeded(long)} where the passed timestamp is the
     * current time.
     *
     * @return {@code true} if any of the cluster nodes need recovery, {@code false} otherwise.
     */
     ...
     *
     * @deprecated use {@link ClusterNodeInfoDocument#isRecoveryNeeded(long)} instead.
     */
