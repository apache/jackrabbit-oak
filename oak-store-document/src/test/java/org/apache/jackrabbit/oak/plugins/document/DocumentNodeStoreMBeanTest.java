...
public class DocumentNodeStoreMBeanTest {
    ...
    private static final long REVISION_GC_MAX_AGE_MILLIS = TimeUnit.SECONDS.toMillis(
        DEFAULT_VER_GC_MAX_AGE);

    private static final long SLIGHTLY_OLDER_THAN_REVISION_GC_MAX_AGE_MILLIS =
        TimeUnit.SECONDS.toMillis(DEFAULT_VER_GC_MAX_AGE) + 5000;
    ...
    public void createCheckpointWithFutureRevision() {
        ...
        assertNotNull(r);
        String revisionString = new Revision(r.getTimestamp() + TimeUnit.HOURS.toMillis(1), 0,
            r.getClusterId()).toString();
        assertThrows(IllegalArgumentException.class, () ->
    ...
    public void createCheckpointWithRevisionTooOld() {
        Revision r = ns.getHeadRevision().getRevision(ns.getClusterId());
        assertNotNull(r);
        String revisionString = new Revision(
            r.getTimestamp() - SLIGHTLY_OLDER_THAN_REVISION_GC_MAX_AGE_MILLIS, 0,
            r.getClusterId()).toString();
        assertThrows(IllegalArgumentException.class, () ->
            bean.createCheckpoint(revisionString, ONE_HOUR, false));
    }
    ...
    public void createCheckpointBeforeExistingCheckpoint() {
        ...
        Revision r = ns.getHeadRevision().getRevision(ns.getClusterId());
        assertNotNull(r);
        String revisionString = new Revision(r.getTimestamp() - ONE_HOUR, 0,
            r.getClusterId()).toString();
